...
if ($_REQUEST["w"]=="saveCatalogueGeneralInfo"){ list($answer,$err)=$cat->saveCatalogueGeneralInfo($_REQUEST["art_id"],$_REQUEST["article_nr_displ"],$_REQUEST["inner_cross"],$_REQUEST["brand_id"],$_REQUEST["goods_group_id"],$_REQUEST["article_name"],$_REQUEST["article_info"],$_REQUEST["article_name_ukr"],$_REQUEST["unique_number"],$_REQUEST["export_status"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err);}

if ($_REQUEST["w"]=="loadArticleParams"){ $GLOBALS['_RESULT'] = array("content"=>$cat->loadArticleParams($_REQUEST["art_id"]));}
if ($_REQUEST["w"]=="saveCatalogueParams"){ list($answer,$err)=$cat->saveCatalogueParams($_REQUEST["art_id"],$_REQUEST["goods_group_id"],$_REQUEST["template_id"],$_REQUEST["params_value"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err);}
...
if ($_REQUEST["w"]=="loadArticlePricing"){ $GLOBALS['_RESULT'] = array("content"=>$cat->loadArticlePricing($_REQUEST["art_id"],$_REQUEST["brand_id"]));}
...
if ($_REQUEST["w"]=="saveArticlePriceRating"){ list($answer,$err,$user_name,$date)=$cat->saveArticlePriceRating($_REQUEST["art_id"],$_REQUEST["kol_elm"],$_REQUEST["template_id"],$_REQUEST["minMarkup"],$_REQUEST["cash_id"],$_REQUEST["prc"],$_REQUEST["prs"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err,"user_name"=>$user_name,"date"=>$date);}
...
if ($_REQUEST["w"]=="printJmovingStorageSelect"){
	...
	$GLOBALS['_RESULT'] = array("content"=>$content);
}
...
if ($_REQUEST["w"]=="saveClientConditions"){ list($answer,$err)=$cl->saveClientConditions($_REQUEST["client_id"],$_REQUEST["cash_id"],$_REQUEST["country_cash_id"],$_REQUEST["price_lvl"],$_REQUEST["margin_price_lvl"],$_REQUEST["price_suppl_lvl"],$_REQUEST["margin_price_suppl_lvl"],$_REQUEST["markup_min"],$_REQUEST["tpoint_id"],$_REQUEST["client_vat"],$_REQUEST["payment_delay"],$_REQUEST["credit_limit"],$_REQUEST["credit_cash_id"],$_REQUEST["credit_return"],$_REQUEST["doc_type_id"],$_REQUEST["rounding_price"],$_REQUEST["detail_id"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err);}
...
if ($_REQUEST["w"]=="loadSupplPricePrefix"){ $GLOBALS['_RESULT'] = array("content"=>$suppl->loadSupplPricePrefix($_REQUEST["suppl_id"]));}
if ($_REQUEST["w"]=="showSupplPricePrefix"){ $GLOBALS['_RESULT'] = array("content"=>$suppl->showSupplPricePrefix($_REQUEST["prefix_id"]));}

if ($_REQUEST["w"]=="saveSupplPricePrefix"){list($answer,$err)=$suppl->saveSupplPricePrefix($_REQUEST["prefix_id"],$_REQUEST["suppl_id"],$_REQUEST["suppl_brand"],$_REQUEST["brand_id"],$_REQUEST["prefix"],$_REQUEST["return_delay"],$_REQUEST["warranty_info"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err); }
if ($_REQUEST["w"]=="dropSupplPricePrefix"){list($answer,$err)=$suppl->dropSupplPricePrefix($_REQUEST["prefix_id"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err); }

//if ($_REQUEST["w"]=="saveCsvSupplPriceImport"){list($answer,$err)=$suppl->saveCsvSupplPriceImport($_REQUEST["suppl_id"],$_REQUEST["start_row"],$_REQUEST["kol_cols"],$_REQUEST["cash"],$_REQUEST["kours_usd"],$_REQUEST["kours_eur"],$_REQUEST["cols"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err); }
//if ($_REQUEST["w"]=="clearSupplPriceImport"){list($answer,$err)=$suppl->clearSupplPriceImport($_REQUEST["suppl_id"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err); }
//if ($_REQUEST["w"]=="loadSupplPriceTablePreview"){$GLOBALS['_RESULT'] = array("content"=>$suppl->loadTablePreview($_REQUEST["suppl_id"],$_REQUEST["brands"]));}
...
if ($_REQUEST["w"]=="saveUsersGeneralInfo"){ list($answer,$err)=$users->saveUsersGeneralInfo($_REQUEST["users_id"],$_REQUEST["name"],$_REQUEST["post"],$_REQUEST["tpoint_id"],$_REQUEST["role_id"],$_REQUEST["phone2"],$_REQUEST["login"],$_REQUEST["pass"],$_REQUEST["status"],$_REQUEST["email"],$_REQUEST["access_dp"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err);}
...
if ($_REQUEST["w"]=="clearUsersAccess"){ list($answer,$err)=$users->clearUsersAccess($_REQUEST["users_id"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err);}
...
if ($_REQUEST["w"]=="showDpCard"){ list($content,$doc_prefix_nom,$label_unknown)=$dp->showDpCard($_REQUEST["dp_id"]); $GLOBALS['_RESULT'] = array("content"=>$content,"doc_prefix_nom"=>$doc_prefix_nom,"label_unknown"=>$label_unknown);}
...
if ($_REQUEST["w"]=="roundingDpDiscount"){ list($price_end,$summ)=$dp->roundingDpDiscount($_REQUEST["dp_id"],$_REQUEST["price_end"],$_REQUEST["amount"]); $GLOBALS['_RESULT'] = array("price_end"=>$price_end,"summ"=>$summ);}
...
if ($_REQUEST["w"]=="showSupplToLocalChangeForm"){ list($content,$header)=$dp->showSupplToLocalChangeForm($_REQUEST["art_id"],$_REQUEST["dp_id"],$_REQUEST["dp_str_id"]); $GLOBALS['_RESULT'] = array("content"=>$content,"header"=>$header);}
...
if ($_REQUEST["w"]=="loadDpImport"){$GLOBALS['_RESULT'] = array("content"=>$dp->loadDpImport($_REQUEST["dp_id"]));}
if ($_REQUEST["w"]=="loadTablePreview"){$GLOBALS['_RESULT'] = array("content"=>$dp->loadTablePreview($_REQUEST["dp_id"],$_REQUEST["brands"]));}
if ($_REQUEST["w"]=="saveTablePreview"){list($answer,$err)=$dp->saveTablePreview($_REQUEST["dp_id"],$_REQUEST["brands"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err); }
...
if ($_REQUEST["w"]=="showStorageFieldsViewForm"){$GLOBALS['_RESULT'] = array("content"=>$dp->showStorageFieldsViewForm());}
if ($_REQUEST["w"]=="saveStorageFieldsViewForm"){ list($answer,$err)=$dp->saveStorageFieldsViewForm($_REQUEST["kol_fields"],$_REQUEST["fl_id"],$_REQUEST["fl_ch"],$_REQUEST["table_key"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err);}
...
if ($_REQUEST["w"]=="showDpStorselForWriteOff"){ $content=$dp->showDpStorselForWriteOff($_REQUEST["dp_id"]); $GLOBALS['_RESULT'] = array("content"=>$content); }

if ($_REQUEST["w"]=="sendDpStorselToWriteOff"){ list($answer,$err,$write_off_nom,$write_off_prefix)=$dp->sendDpStorselToWriteOff($_REQUEST["dp_id"],$_REQUEST["kol_storsel"],$_REQUEST["ar_storsel"],$_REQUEST["status_write_off"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err,"write_off_nom"=>$write_off_nom,"write_off_prefix"=>$write_off_prefix); }

//------------				SALE INVOICE			  --------------------------

if ($_REQUEST["w"]=="sendDpStorselToSaleInvoice"){ list($answer,$err,$sale_invoice_nom,$sale_invoice_prefix,$sale_invoice_doc_type_id)=$dp->sendDpStorselToSaleInvoice($_REQUEST["dp_id"],$_REQUEST["kol_storsel"],$_REQUEST["ar_storsel"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err,"sale_invoice_nom"=>$sale_invoice_nom,"sale_invoice_prefix"=>$sale_invoice_prefix,"sale_invoice_doc_type_id"=>$sale_invoice_doc_type_id); }
...
if ($_REQUEST["w"]=="showSupplOrder"){ $content=$suppl->showSupplOrder($_REQUEST["so_id"]); $GLOBALS['_RESULT'] = array("content"=>$content);}
if ($_REQUEST["w"]=="saveSupplOrder"){ list($answer,$err)=$suppl->saveSupplOrder($_REQUEST["so_id"],$_REQUEST["amount_order"],$_REQUEST["delivery_data_finish"],$_REQUEST["delivery_time_finish"],$_REQUEST["delivery_type_id"],$_REQUEST["suppl_order_status_id"],$_REQUEST["suppl_order_doc"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err); }
