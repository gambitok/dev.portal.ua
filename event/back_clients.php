...
if ($accss == "1") {
	...
	$form_htm = RD . "/tpl/back_clients.htm";
	$form = "";
	...
	if ($w == "") {
		...
		$content = str_replace("{back_clients_range}", $range_list, $content);
		$data_cur = date("Y-m-d");
    ...
    if ($w == "exportBackImportArticles") {
        $client_id = $links[2];
        $storage_id = $links[3];
        $cell_id = $links[4];
        $status = $links[5];
        $form = $back_clients->exportBackImportArtilces($client_id, $storage_id, $cell_id, $status);
    }
