...
require_once (RD."/lib/mysql_class.php");$db=new dbt;
...
if(!empty($_FILES)){
	...
	if ($user_id>0 && $art_id!="" && $art_id>0){
		...
		if(move_uploaded_file($_FILES['file']['tmp_name'],$targetFile)){session_start(); 
			$db->query("insert into T2_PHOTOS (`ART_ID`,`USER_ID`,`PHOTO_NAME`) values ('$art_id','$user_id','$real_file_name');");
		}
	}
}
?>