...
$(document).ready(function() {
    shortcut.add("Insert", function() {
		...
		if (select_id > 0) {
			if (storsel_type_id === 0) {
				addNewRowLocal();
			}
			if (storsel_type_id === 1) {
				addNewRow();
			}
		}
	});
...
function printBarcode(select_id) {
	if (select_id == "" || select_id == 0) {
		toastr["error"](errs[0]);
	}
	if (select_id > 0) {
		window.open("/Storsel/printBarcode/" + select_id, "_blank", "printWindow");
	}
}
...
function saveStorselBugForm(select_id, str_id) {
	if (select_id.length > 0 && str_id.length > 0) {
		...
		if (storage_select_bug <= 0 || storage_select_bug.length == 0) {
			er = 1; swal("Помилка!", "Оберіть тип відхилення", "error");
		}
		if (dif_amount_barcode <= 0 || dif_amount_barcode == 0) {
			er = 1; swal("Помилка!", "Не вказано кількість відхилення", "error");
		}
		if (dif_amount_barcode > amount) {
			er = 1; swal("Помилка!", "Кількість відхилення не повинна перевищувати кількісті відбору", "error");
		}
		if (err == 0) {
			swal({
...
function saveStorselNoscanForm(select_id, art_id, str_id) {
	if (select_id.length > 0 && art_id.length > 0 && str_id.length > 0) {
		...
		if (dif_amount_barcode <= 0 || dif_amount_barcode == 0) {
			er = 1; swal("Помилка!", "Не вказано кількість без сканування", "error");
		}
		if (dif_amount_barcode > amount) {
			er = 1; swal("Помилка!", "Кількість без сканування не повинна перевищувати кількості для приймання", "error");
		}
		if (err == 0) {
			swal({
