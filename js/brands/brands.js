...
function showBrandsCard(brands_id){
	...
	if (brands_id>0){
		...
		function (result, errors){ if (errors) {alert(errors);} if (result){  
			...
			$("#brands_kind").select2({placeholder: "Виберіть тип",dropdownParent: $("#BrandsCard")});
			$("#brands_country").select2({placeholder: "Виберіть країну",dropdownParent: $("#BrandsCard")});
		}}, true);
	}
}
//save all
...
//save import file info to table
