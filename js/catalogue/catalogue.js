...
function fil2Search(){
	...
	if (str_id>0){
		...
		function (result, errors){ if (errors) {alert(errors);} if (result){
			$("#catalogue_range").html(result["content"]);
			$("#catalogue_header").html(result["header"]);
			if ($("#datatable").length) $('#datatable').DataTable().destroy();
			$("#catalogue_range").html(result["content"]);
			$("#catalogue_header").html(result["header"]);
...
function saveCatalogueGeneralInfo(){
	...
	if (art_id.length>0){
		JsHttpRequest.query($rcapi,{ 'w':'saveCatalogueGeneralInfo','art_id':art_id,'article_nr_displ':article_nr_displ,'barcode':barcode,'inner_cross':inner_cross,'brand_id':brand_id,'goods_group_id':goods_group_id,'article_name':article_name, 'article_info':article_info, 'article_name_ukr':article_name_ukr, 'unique_number':unique_number},
		function (result, errors){ if (errors) {alert(errors);} if (result){  
			if (result["answer"]==1){ 
				swal("Збережено!", "Внесені Вами зміни успішно збережені.", "success");
				var art=$("#catalogue_art").val();
...
function saveCatalogueParamsTemplate(art_id){
	...
	function (isConfirm) {
		if (isConfirm) {
			...
			var template_caption=$("#tmp_template_caption").val();
			var template_descr=$("#tmp_template_descr").val();	
...
function findCatalogueDonorIndexSearch(){
	var index=$("#form_donor_search").val();
	if (index.length>1){
		JsHttpRequest.query($rcapi,{ 'w':'findCatalogueDonorIndexSearch', 'index': index},
		function (result, errors){ if (errors) {alert(errors);} if (result){  
			document.getElementById("donor_search_result").innerHTML=result["content"];
		}}, true);
	}
}
...
function saveCatalogueDonorForm(art_id){
	var search_number=$("#form_donor_search_number").val();
	var display_nr=$("#form_donor_display_nr").val();
	var art_id2=$("#form_donor_art_id2").val();
	var ch=[];for (var i=1;i<=7;i++){var che=0;if (document.getElementById("don_"+i).checked){che=1;}ch[i]=che;}
...
function loadArticlePricing(art_id){
	if (art_id<=0 || art_id==""){toastr["error"](errs[0]);}
	if (art_id>0){
		JsHttpRequest.query($rcapi,{ 'w': 'loadArticlePricing', 'art_id':art_id}, 
		function (result, errors){ if (errors) {alert(errors);} if (result){  
			document.getElementById("pricing_place").innerHTML=result["content"];
			$('#catalogue_tabs').tab();
...
function recalcPRArt(art_id,k,st){ var kol_elm=12; var prev_price=prev_percent=price=percent=oper_price=0;var ii=0; var ch=0;
...
function saveArticlePriceRating(art_id){var kol_elm=12;  var prc=[]; var prs=[];
	if (art_id.length>0){
		var minMarkup=$("#artMinMarkUp_"+art_id).val();var er=0;
		var template_id=$("#priceRatingTemplate_"+art_id+" option:selected").val();
		for (var i=0;i<=kol_elm;i++){  var i0=parseInt(i)-1;
		...
		if (er==1){ swal("Помилка!","Ти олень безрогий","error");}
		if (er==0){
			JsHttpRequest.query($rcapi,{ 'w':'saveArticlePriceRating', 'art_id':art_id, 'kol_elm':kol_elm, 'template_id':template_id, 'minMarkup':minMarkup, 'prc':prc, 'prs':prs},
			function (result, errors){ if (errors) {alert(errors);} if (result){  
				if (result["answer"]==1){
...
function changeArtId() {
	var suppl_status=$("#suppl_status").val();
	console.log(suppl_status);
}

function changeArtId() {
	var suppl_status=$("#suppl_status").prop("checked");
	if (!suppl_status) {
	...
	if (art_id>0 && art_id!="") {
		...
		function (result, errors){ if (errors) {alert(errors);} if (result){  
			if (result.content) {
				...
				toastr["warning"]("Помилка додавання індексу!");
			}
		}}, true);	
	}
}
...
function showArticleLogs(art_id) {
	if (art_id.length>0){
		...
		function (result, errors){ if (errors) {alert(errors);} if (result){  
			$("#FormModalWindow3").modal('show');
			document.getElementById("FormModalBody3").innerHTML=result["content"];
			document.getElementById("FormModalLabel3").innerHTML="Історія змін "+art_id;
		}}, true);	
	}
}
...
function saveArticleInfo(art_id,lang_id,sort,i) {
	var text=$("#info_text-"+i).val();
	var value=$("#info_value-"+i).val();
	var new_sort=$("#info_sort-"+i).val();
	JsHttpRequest.query($rcapi,{ 'w':'saveArticleInfo','art_id':art_id,'lang_id':lang_id,'text':text,'value':value,'sort':sort,'new_sort':new_sort},
	function (result, errors){ if (errors) {alert(errors);} if (result){  
		if (result["answer"]==1){ 
			swal("Збережено!", "Внесені Вами зміни успішно збережені.", "success");
...
function dropArticleInfo(art_id,lang_id,sort) {
	JsHttpRequest.query($rcapi,{ 'w':'dropArticleInfo','art_id':art_id,'lang_id':lang_id,'sort':sort},
	function (result, errors){ if (errors) {alert(errors);} if (result){  
		if (result["answer"]==1){ 
...
function addArticleInfo(art_id) {
	...
	var value=$("#info_value").val();
	var sort=$("#info_sort").val();
	var lang_id=$("#info_lang option:selected").val();
...
function saveArticleShortInfo(art_id,lang_id,sort,i) {
	var text=$("#info_text-"+i).val();
	var value=$("#info_value-"+i).val();
	var new_sort=$("#info_sort-"+i).val();
	JsHttpRequest.query($rcapi,{ 'w':'saveArticleShortInfo','art_id':art_id,'lang_id':lang_id,'text':text,'value':value,'sort':sort,'new_sort':new_sort},
	function (result, errors){ if (errors) {alert(errors);} if (result){  
		if (result["answer"]==1){ 
			swal("Збережено!", "Внесені Вами зміни успішно збережені.", "success");
...
function dropArticleShortInfo(art_id,lang_id,sort) {
	JsHttpRequest.query($rcapi,{ 'w':'dropArticleShortInfo','art_id':art_id,'lang_id':lang_id,'sort':sort},
	function (result, errors){ if (errors) {alert(errors);} if (result){  
		if (result["answer"]==1){ 
			swal("Видалено!", "Внесені Вами зміни успішно збережені.", "success");
			loadArticleShortInfo(art_id);
...
function saveArticleCross() {
	var cross=$("#cross_value").val();
	var new_cross=$("#new_cross_value").val();
