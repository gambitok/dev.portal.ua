...
function saveSupplVat(suppl_id){
	...
	function (isConfirm) {
		if (isConfirm) {
            let price_in_vat=0; if (document.getElementById("price_in_vat").checked){price_in_vat=1;}
            let show_in_vat=0; if (document.getElementById("show_in_vat").checked && document.getElementById("show_in_vat").disabled==false){show_in_vat=1;}
            let price_add_vat=0; if (document.getElementById("price_add_vat").checked && document.getElementById("price_add_vat").disabled==false){price_add_vat=1;}
...
function showNumbersList() {
    ...
    if (suppl_id==="0") {toastr["error"]("Виберіть постачальника");}
...
function filterBackSupplList() {
	let date_start = $("#date_start").val();
	let date_end = $("#date_end").val();
	let suppl_id = $("#back_suppl_select option:selected").val();
    JsHttpRequest.query($rcapi,{ 'w': 'filterBackSupplList', 'date_start':date_start, 'date_end':date_end, 'suppl_id':suppl_id},
        function (result, errors){ if (errors) {alert(errors);} if (result){
            let dt=$("#datatable");
            dt.DataTable().destroy();
