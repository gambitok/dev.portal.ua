...
class income {
    ...
    function makeIncomeCardFinish($income_id) { $db = DbSingleton::getDb(); $dbt = DbSingleton::getTokoDb();
        ...
        if ($n == 1) {
            ...
            if ($storage_id == 0 || $storage_cells_id == 0 || $client_id == 0) {
                $answer = 0; $err = "Не вказано \"Склад зберігання\" або \"Комірка зберігання\" або \"Клієнт\". Накладну не проведено!";
            }
            ...
            if ($storage_id > 0 && $storage_cells_id > 0 && $client_id > 0) {
                if ($oper_status == 30) {
                    ...
                    $answer = 0; $err = "Накладну заблоковано. Зміни вносити заборонено.";
                }
            }
        }
        return array($answer, $err);
    ...
    function showIncomeUnknownStrList($income_id) { $db = DbSingleton::getDb();
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            $art_id_comment = "";
            if ($art_id == 0) {
                $art_id_comment = "Не визначено ART_ID! Артикул відсутній у базі";
            }
            $article_nr_displ = $db->result($r,$i - 1,"article_nr_displ");
            $brand_id = $db->result($r,$i - 1,"brand_id");
            $brand_name = $cat->getBrandName($brand_id);
            ...
            if ($brand_id == 0 || $brand_name == "") {
                $brand_id_comment = "Не визначено Бренд!";
            }
            $country_id = $db->result($r,$i - 1,"country_id");
            $country_abr = $cat->getCountryAbr($country_id);
            $country_id_comment = "";
            if ($country_id == 0 || $country_abr == "") {
                $country_id_comment = "Не визначено Країну!";
            }
            $costums_id = $db->result($r,$i - 1,"costums_id");
            $costums_code = $cat->getCostumsCode($costums_id);
            $costums_id_comment = "";
            if ($costums_id == 0 || $costums_code == "") {
                $costums_id_comment = "Не визначено Митний код!";
            }
            $amount = $db->result($r,$i - 1,"amount");
            $amount_comment = "";
            if ($amount <= 0) {
                $amount_comment = "Не визначено кількість товару!";
            }
            $price_buh_cashin = $db->result($r,$i - 1,"price_buh_cashin");
            $price_comment = "";
            if ($price_buh_cashin <= 0) {
                $price_comment = "Не визначено ціну товару!";
            }
            $weight_netto = $db->result($r,$i - 1,"weight_netto");
            $weight_comment = "";
            if ($weight_netto <= 0) {
                $weight_comment = "Не визначено вагу товару!";
            }
    ...
    function exportIncomeUnStr($income_id) { $db = DbSingleton::getDb();
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            $art_id = $db->result($r,$i - 1,"art_id");
            if ($art_id == 0) {
                $comment .= "Не визначено ART_ID! Артикул відсутній у базі";
            }
            $article_nr_displ = $db->result($r,$i - 1,"article_nr_displ");
            $brand_id = $db->result($r,$i - 1,"brand_id");
            $brand_name = $cat->getBrandName($brand_id);
            if ($brand_id == 0 || $brand_name == "") {
                $comment .= "Не визначено Бренд!";
            }
            $country_id = $db->result($r,$i - 1,"country_id");
            $country_abr = $cat->getCountryAbr($country_id);
            if ($country_id == 0 || $country_abr == "") {
                $comment .= "Не визначено Країну!";
            }
            $costums_id = $db->result($r,$i - 1,"costums_id");
            $costums_code = $cat->getCostumsCode($costums_id);
            if ($costums_id == 0 || $costums_code == "") {
                $comment .= "Не визначено Митний код!";
            }
            $amount = $db->result($r,$i - 1,"amount");
            if ($amount <= 0) {
                $comment .= "Не визначено кількість товару!";
            }
            $price_buh_cashin = $db->result($r,$i - 1,"price_buh_cashin");
            if ($price_buh_cashin <= 0) {
                $comment .= "Не визначено ціну товару!";
            }
            $weight_netto = $db->result($r,$i - 1,"weight_netto");
            if ($weight_netto <= 0) {
                $comment .= "Не визначено вагу товару!";
            }
...
//    }
    ...
    function loadIncomeSpend($income_id) { $db = DbSingleton::getDb();
        ...
        for ($is = 1; $is <= $ns; $is++) {
            ...
            if ($list2 == "") {
                $list2 = "<tr><td colspan=10 align='center'>Відсутні витрати</td></tr>";
            }
...
//    }
    ...
    function showCsvPreview($income_id) { $db = DbSingleton::getDb();
        ...
        $csv_file_name="Оберіть файл";$pre_table="<h3 align='center'>Записи відсутні</h3>";
                    ...
                    while (($buffer = fgets($handle, 4096)) !== false) {
                        ...
                        if ($buffer!=""){
                            ...
                            for ($i=1;$i<=$kol_cols;$i++) {
                                ...
                                if ($ex_cols==1) {
                                    $cols_list.="<th><select id=\"clm-$i\" size='1'><option value='0'>-</option><option value='1'>Індекс</option><option value='2'>Бренд</option><option value='3'>Країна</option><option value='4'>Митний код</option><option value='5'>Кількість</option><option value='6'>Ціна</option><option value='7'>Вага</option></select></th>";
                                }
                            }
                            if ($row!="") {
...
//    }
    ...
    function setIncomeBarcodes($income_id) { $db = DbSingleton::getDb(); $dbt = DbSingleton::getTokoDb();
        ...
        if ($n > 0) {
            ...
            if ($num == 0) {
                $list = "<h3>Штрих-кодів для заповнення не знайдено!</h3>";
            }
        } else {
            $list = "<h3>Відсутні поля для заповнення!</h3>";
        }
        return $list;
    }
    ...
    function setIncomePrices($income_id) { $db = DbSingleton::getDb(); $dbt = DbSingleton::getTokoDb();
        ...
        $headers = "<th>ART_ID</th><th>OPER PRICE</th><th>Валюта</th>";
