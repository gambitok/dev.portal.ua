...
class income {
    ...
    function getIncomeDocNom($income_id){$db=DbSingleton::getDb();
        session_start();$doc_nom="-";$prefix="";
        $r=$db->query("SELECT * FROM `J_INCOME` WHERE `id`='$income_id' LIMIT 1;");$n=$db->num_rows($r);
    ...
    function get_doc_prefix($client_id,$prefix_id){ $db=DbSingleton::getDb();
        $r=$db->query("SELECT `prefix` FROM `A_CLIENTS_DOCUMENT_PREFIX` WHERE `client_id`='$client_id' AND `id`='$prefix_id' AND `status`='1' LIMIT 1;");$n=$db->num_rows($r);$prefix="ДФ";
    ...
    function get_doc_client_prefix($client_id){ $db=DbSingleton::getDb();
        ...
        $r=$db->query("SELECT `id` FROM `A_CLIENTS_DOCUMENT_PREFIX` WHERE `client_id`='$client_id' AND `doc_type_id`='$doc_type_id' AND `status`='1' LIMIT 1;");$n=$db->num_rows($r);
    ...
    function get_client_doc_nom_new($client_id){ $db=DbSingleton::getDb();
        $r=$db->query("SELECT MAX(`doc_nom`) as mid FROM `J_INCOME` WHERE `client_id`='$client_id' AND `status`='1' AND `oper_status`='31' LIMIT 1;");$doc_nom=0+$db->result($r,0,"mid")+1;
        return $doc_nom;
    }
    ...
    function newIncomeCard($type_id){$db=DbSingleton::getDb();
        ...
        $db->query("INSERT INTO `J_INCOME` (`id`,`type_id`,`prefix`,`doc_nom`,`user_id`,`data`) 
        VALUES ('$income_id','$type_id','$this->prefix_new','$doc_nom','$user_id',CURDATE());");
        return $income_id;
    }

    function getNBUKours($data,$val){$db=DbSingleton::getDb();
        if ($val==1){$val="usd";} if ($val==2){$val="usd";} if ($val==3){$val="euro";}
        $r=$db->query("SELECT `$val` FROM `kours` WHERE `data`='$data' LIMIT 1;");$n=$db->num_rows($r);$kours="";
    ...
    function show_income_list(){$db=DbSingleton::getDb();
        $gmanual=new gmanual; $data_cur=date("Y-m-d");
    ...
    function show_income_list_filter($data_start,$data_end){$db=DbSingleton::getDb();
        $gmanual=new gmanual; $data_cur=date("Y-m-d");
        if ($data_start!='' && $data_end!='') $where="AND j.data>='$data_start 00:00:00' AND j.data<='$data_end 23:59:59'";
        else $where=" AND j.data>='$data_cur 00:00:00' AND j.data<='$data_cur 23:59:59'";
        $r=$db->query("SELECT j.*, CASH.name as cash_name, c.name as client_seller_name, c2.name as client_name 
        FROM `J_INCOME` j
            LEFT OUTER JOIN CASH on CASH.id=j.cash_id
            LEFT OUTER JOIN A_CLIENTS c on c.id=j.client_seller
    ...
    function search_documents_income_list($s_nom){$db=DbSingleton::getDb();
        $slave=new slave;$gmanual=new gmanual;$s_nom=$slave->qq($s_nom);
    ...
    function getIncomeSupplDocNom($income_id){$db=DbSingleton::getDb();
        session_start();$invoice_income="";
        $r=$db->query("SELECT `invoice_income` FROM `J_INCOME` WHERE `id`='$income_id' LIMIT 1;");$n=$db->num_rows($r);
    ...
    function getKourForDate($cash_id_to,$cash_id_from,$data){$db=DbSingleton::getDb();
        ...
        if ($cash_id_from!=$cash_id_to){
            ...
            ORDER BY `id` DESC LIMIT 1;");$n=$db->num_rows($r);
    ...
    function checkInvoiceIncome($invoice_income, $seller, $date) { $db=DbSingleton::getDb();
        $r=$db->query("SELECT `prefix`, `doc_nom` FROM `J_INCOME` 
        WHERE `invoice_income`='$invoice_income' AND `client_seller`='$seller' AND `invoice_data`='$date' AND `oper_status`='31' LIMIT 1;"); $n=$db->num_rows($r);
    ...
    function showIncomeCard($income_id) { $db=DbSingleton::getDb();
        ...
        if ($n==1){ $cl=new clients;
            ...
            $usd_to_uah=$db->result($r,0,"usd_to_uah"); if ($usd_to_uah=="0.00"){$usd_to_uah=$this->getKourForDate($cash_id,2,$data);}
            $eur_to_uah=$db->result($r,0,"eur_to_uah"); if ($eur_to_uah=="0.00"){$eur_to_uah=$this->getKourForDate($cash_id,3,$data);}
            ...
            if ($user_id==1 || $user_id==7 || $user_id==8) $form=str_replace("{oper_disabled2}"," ",$form);
    ...
    function showIncomeImportStrList($income_id,$oper_status){$db=DbSingleton::getDb();
        $slave=new slave;$cat=new catalogue;$manual=new manual;$list="";$op=0;//???
    ...
    function showIncomeLocalStrList($income_id,$oper_status){$db=DbSingleton::getDb();
        $slave=new slave;$cat=new catalogue;$list="";$op=0;//???
        $r=$db->query("SELECT * FROM `J_INCOME_STR` WHERE `income_id`='$income_id' ORDER BY `id` ASC;");$n=$db->num_rows($r);$kl_rw=$n;if ($op==0){$kl_rw+=2;}
        for ($i=1;$i<=$kl_rw;$i++){
    ...
    function getIncomeInfo($income_id){$db=DbSingleton::getDb();
        $prefix=$doc_nom=$data=$client_seller=$storage_id="";$invoice_income=0;
        $r=$db->query("SELECT * FROM `J_INCOME` WHERE `id`='$income_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $prefix=$db->result($r,0,"prefix");
            $doc_nom=$db->result($r,0,"doc_nom");
    ...
    function printIncome($income_id){$db=DbSingleton::getDb();
        $cat=new catalogue; $summ_buh=0;$list="";
    ...
    function saveIncomeCard($income_id,$data,$client_seller,$invoice_income,$cash_id,$client_id,$invoice_data,$cours_to_uah,$cours_to_uah_nbu,$invoice_summ,$usd_to_uah,$eur_to_uah,$costums_pd_uah,$costums_pp_uah,$costums_summ_uah){$db=DbSingleton::getDb();
        ...
        if ($income_id>0){ $k=0;
            ...
            if ($k>0) {
                ...
                $db->query("UPDATE `J_INCOME` SET `client_id`='$client_id', `prefix`='ДФ', `data`='$data', `invoice_income`='$invoice_income', `invoice_data`='$invoice_data', 
    ...
    function makeIncomeCardFinish($income_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        ...
        $answer=0;$err="";$income_id=$slave->qq($income_id);
        $r=$db->query("SELECT `oper_status`, `storage_id`, `storage_cells_id`, `client_id`, `import_1c` FROM `J_INCOME` WHERE `id`='$income_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $oper_status=$db->result($r,0,"oper_status");
            $storage_id=$db->result($r,0,"storage_id");
            $storage_cells_id=$db->result($r,0,"storage_cells_id");
                ...
                if ($oper_status==30) {
                    ...
                    for ($i=1;$i<=$n1;$i++){
                        ...
                        */
                        ...
                        $rs=$dbt->query("SELECT `AMOUNT` as `amount` FROM `T2_ARTICLES_STRORAGE` WHERE `ART_ID`='$art_id' AND `STORAGE_ID`='$storage_id';"); $ns=$dbt->num_rows($rs);
    ...
    function checkIncomUnStr($income_id,$unknown_id,$art_id,$article_nr_displ,$brand_id,$country_id,$costums_id,$amount,$price,$weight){$db=DbSingleton::getDb();
        ...
        $r=$db->query("SELECT `type_id`, `oper_status` FROM `J_INCOME` WHERE `id`='$income_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $oper_status=$db->result($r,0,"oper_status");
            ...
            if ($oper_status==30) {$article_nr_displ=$slave->qq($article_nr_displ);
                ...
                if (($income_type_id==1 and ($art_id>0 && $brand_id>0 && $country_id>0 && $costums_id>0 && $amount>0 && $price>0 && $weight>0)) || ($income_type_id==0 and ($art_id>0 && $amount>0 && $price>0))) {
    ...
    function dropIncomUnStr($income_id,$unknown_id){$db=DbSingleton::getDb();
        $slave=new slave;$answer=0;$err="";
        $income_id=$slave->qq($income_id);$unknown_id=$slave->qq($unknown_id);
        $r=$db->query("SELECT `oper_status` FROM `J_INCOME` WHERE `id`='$income_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $oper_status=$db->result($r,0,"oper_status");
            if ($oper_status==30) {
    ...
    function dropIncomeStr($income_id,$art_id) {$db=DbSingleton::getDb();
        $slave=new slave;$answer=0;$err="Помилка видалення даних!"; $income_id=$slave->qq($income_id);
        $r=$db->query("SELECT `oper_status` FROM `J_INCOME` where `id`='$income_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $oper_status=$db->result($r,0,"oper_status");
            if ($oper_status==30) {
    ...
    function clearIncomeStr($income_id){$db=DbSingleton::getDb();
        $slave=new slave;$answer=0;$err=""; $income_id=$slave->qq($income_id);
        $r=$db->query("SELECT `oper_status` FROM `J_INCOME` WHERE `id`='$income_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $oper_status=$db->result($r,0,"oper_status");
            if ($oper_status==30) {
    ...
    function showIncomeClientList($sel_id){$db=DbSingleton::getDb();
        ...
        for ($i=1;$i<=$n;$i++){
            ...
            $id==$sel_id ? $cur="background-color:#0CF;" : $cur="";
    ...
    function showStorageSelectList($sel_id){$db=DbSingleton::getTokoDb();
        ...
        $r=$db->query("SELECT * FROM `STORAGE` WHERE `status`='1' ORDER BY `name` ASC, `id` ASC;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
            $name=$db->result($r,$i-1,"name");
...
//    }
    ...
    function getIncomeSpendSumms($income_id){$db=DbSingleton::getDb();
        $rs=$db->query("SELECT * FROM `SPEND_ITEM` WHERE `ison`='1' ORDER BY `id` ASC;");$ns=$db->num_rows($rs);$summs=array();
        for ($is=1;$is<=$ns;$is++){
    ...
    function loadIncomeSpend($income_id){$db=DbSingleton::getDb();
        $slave=new slave;$gmanual=new gmanual;$list="";
        ...
        for ($is=1;$is<=$ns;$is++){
            ...
            $r=$db->query("SELECT * FROM `J_INCOME_SPEND_STR` WHERE `income_id`='$income_id' AND `spend_item_id`='$spend_item_id' AND `status`='1' ORDER BY `data` ASC, `id` ASC;");$n=$db->num_rows($r);
    ...
    function loadIncomeCommets($income_id){$db=DbSingleton::getDb();
        ...
        WHERE cc.income_id='$income_id' ORDER BY cc.id DESC;");$n=$db->num_rows($r);$list="";
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
    ...
    function dropIncomeComment($income_id,$comment_id){$db=DbSingleton::getDb();
        ...
        if ($income_id>0 && $comment_id>0){
            $r=$db->query("SELECT * FROM `J_INCOME_COMMENTS` WHERE `income_id`='$income_id' AND `id`='$comment_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
    ...
    function labelArtUnknownCount($income_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT COUNT(`id`) as kol FROM `J_INCOME_STR_UNKNOWN` WHERE `income_id`='$income_id';");$kol=0+$db->result($r,0,"kol");$label="";
    ...
    function labelCommentsCount($income_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT COUNT(`id`) as kol FROM `J_INCOME_COMMENTS` WHERE `income_id`='$income_id';");$kol=0+$db->result($r,0,"kol");$label="";
        if ($kol>0){$label="<span class='label label-tab label-info'>$kol</span>";}
        return array($kol,$label);
    }
...
//    }
    ...
    function incomeCDNDropFile($income_id,$file_id){$db=DbSingleton::getDb();
        ...
        if ($income_id>0 && $file_id>0){
            $r=$db->query("SELECT `file_name` FROM `J_INCOME_CDN` WHERE `income_id`='$income_id' AND `id`='$file_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){ //$file_name=$db->result($r,0,"file_name");
...
//    }
    ...
    function showGoodGroupTree($sel_id){$db=DbSingleton::getDb();
        ...
        $r=$db->query("SELECT * FROM `GOODS_GROUP` WHERE `PARRENT_ID`='0' ORDER BY `NAME` ASC;");$n=$db->num_rows($r);$tree="";
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"ID");
            $name=$db->result($r,$i-1,"NAME");
    ...
    function showGoodGroupSubLevel($parrent_id,$sel_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `GOODS_GROUP` WHERE `PARRENT_ID`='$parrent_id' ORDER BY `NAME` ASC;");$n=$db->num_rows($r);$tree="";
        if ($n>0){$tree.="<ul>";
            for ($i=1;$i<=$n;$i++){
                $id=$db->result($r,$i-1,"ID");
                $name=$db->result($r,$i-1,"NAME");
    ...
    function showCsvPreview($income_id){$db=DbSingleton::getDb();
        $csv_exist=0;$csv_file_name="Оберіть файл";$pre_table="<h3 align='center'>Записи відсутні</h3>";$kol_cols=0;$fn=0;
        $r=$db->query("SELECT * FROM `J_INCOME_CSV` WHERE `income_id`='$income_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
    ...
    function getCountryId($code){$db=DbSingleton::getTokoDb();
        $slave=new slave;$id=0; $code=$slave->qq($code);
        $r=$db->query("SELECT `COUNTRY_ID` FROM `T2_COUNTRIES` WHERE `COUNTRY_NAME`='$code' OR (`ALFA2`='$code' AND `ALFA2`!='') OR (`ALFA3`='$code' AND `ALFA3`!='') LIMIT 1;");$n=$db->num_rows($r);
    ...
    function saveIncomeConditions($income_id,$cash_id,$country_cash_id,$price_lvl,$payment_delay,$credit_limit,$credit_cash_id,$credit_return){$db=DbSingleton::getDb();
        ...
        if ($income_id>0){
            $r=$db->query("SELECT * FROM `A_CLIENTS_CONDITIONS` WHERE `income_id`='$income_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==0){
    ...
    function saveIncomeDetails($income_id,$address_jur,$address_fakt,$edrpou,$svidotctvo,$vytjag,$vat,$mfo,$bank,$account,$not_resident,$nr_details){$db=DbSingleton::getDb();
        ...
        if ($income_id>0){
            $r=$db->query("SELECT * FROM `A_CLIENT_DETAILS` WHERE `income_id`='$income_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==0){
...
//    }
    ...
    function setIncomeBarcodes($income_id) {$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        $r=$db->query("SELECT * FROM `J_INCOME_STR` WHERE `income_id`='$income_id';"); $n=$db->num_rows($r);
        if ($n>0) {
            $list="<h3>Заповнені штрих-коди:</h3>
            <table class=\"table\">
                <thead><tr>
                    <th>#</th>
                    <th>ART_ID</th>
            ...
            for ($i=1;$i<=$n;$i++) {
                $art_id = $db->result($r, $i-1, "art_id");
                $r2=$dbt->query("SELECT * FROM `T2_BARCODES` WHERE `ART_ID`='$art_id' LIMIT 1;"); $n2=$dbt->num_rows($r2);
                if ($n2==0) {$num++;
                    $r3=$dbt->query("SELECT MAX(`BARCODE`) as mid FROM `T2_BARCODES`;");$barcode=0+$db->result($r3,0,"mid")+1;
            ...
            if ($num==0) $list="<h3>Штрих-кодів для заповнення не знайдено!</h3>";
        } else $list="<h3>Відсутні поля для заповнення!</h3>";
        return $list;
    }

    function setIncomePrices($income_id) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        $r=$db->query("SELECT * FROM `J_INCOME_STR` WHERE `income_id`='$income_id';"); $n=$db->num_rows($r); $arts="";
        for ($i=1;$i<=$n;$i++) {
            $art_id = $db->result($r, $i-1, "art_id");
            ...
            $cash_id=$dbt->result($r2,0,"cash_id"); $cash_name=$this->getCashAbr($cash_id); if ($cash_id=="") $cash_name="-";
        ...
        $headers="<th>ART_ID</th><th>OPER PRICE</th><th>Валюта</th>";$inputs="<th>-</th><th>-</th><th>-</th>";
        for ($i=0;$i<=11;$i++) {
            $headers.="<th>Прайс $i</th>";
        ...
        $list="<h2>Шаблон націнки</h2><hr>
        <div class='row'>
            <div class='col-lg-1'>
                <label for=\"minMarkup\" class=\"control-label\">Мін. націнка: </label>
            </div>    
            <div class='col-lg-1'>
                <input id=\"minMarkup\" class=\"form-control price_numbers\" type=\"text\" value=\"0\" style=\"text-align: right;\">
            </div>    
            <div class='col-lg-2'>
                <button class=\"btn btn-warning\" onclick=\"showPreArticlePriceRating('$income_id');\"><i class='fa fa-check-square'></i> Перевірити</button>    
            </div>    
            <div class='col-lg-1'>
                <label for=\"price_cash_id\" class=\"control-label\">Валюта </label>
            </div>  
            <div class='col-lg-2'>
                <select class='form-control' name='price_cash_id' id='price_cash_id'>
                    <option value='0'>По замовчуванні</option>
                    <option value='1'>Гривня</option>
                    <option value='2'>Долар</option>
        ...
        <div class='row'>
            <div class='col-lg-offset-10 col-lg-2 text-right'>
                <button type=\"button\" class=\"btn btn-primary\" onclick=\"saveIncomeArticlePriceRating('$income_id')\"><i class='fa fa-save'></i> Зберегти</button>
                <button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\" aria-hidden=\"true\">Закрити</button>
            </div>
        </div>";
        return $list;
    }
    ...
    function showPreArticlePriceRating($income_id, $price_cash_id, $prs) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        ...
        for ($i=1;$i<=$n;$i++) {
            ...
            $r2=$dbt->query("SELECT * FROM `T2_ARTICLES_PRICE_RATING` WHERE `art_id`='$art_id' AND `in_use`=1 LIMIT 1;"); $n2 = $db->num_rows($r2);
            $cash_id=$dbt->result($r2,0,"cash_id"); $cash_name=$this->getCashAbr($cash_id); if ($cash_id=="") $cash_name="-";
    ...
    function saveIncomeArticlePriceRating($income_id, $price_cash_id, $minMarkup, $prs) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        session_start(); $user_id=$_SESSION["media_user_id"];
        $rin=$db->query("SELECT * FROM `J_INCOME_STR` WHERE `income_id`='$income_id';"); $nin=$db->num_rows($rin);
        for ($ii=1;$ii<=$nin;$ii++) {
            ...
            if ($art_id>0){
                ...
                $r=$dbt->query("SELECT * FROM `T2_ARTICLES_PRICE_RATING` WHERE `art_id`='$art_id' AND `in_use`='1' LIMIT 1;");$n=$dbt->num_rows($r);
                for ($j=0;$j<=11;$j++) {
                    $percent=$prs[$j];
                    if ($j==0) {
                        $price=$oper_price;
                ...
                $default_cash_id = 2;
                if ($n==0 && $price_cash_id>0) $default_cash_id = $price_cash_id;
                ...
                for ($ip=0;$ip<=11;$ip++){ $ipp=$ip+1; $query.=",`price_$ipp`,`persent_$ipp`"; }
    ...
    function getSummCash($summ, $cash_id, $to_cash) {
        ...
        if ($cash_id==$to_cash) $summary=$summ;
    ...
    function recalculateMadeIncome($income_id) {$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        ...
        for ($i=1;$i<=$n;$i++) {
            ...
            $db->query("UPDATE `T2_ARTICLES_PARTITIONS` SET `oper_price`='$new_oper_price', `price`='$price_usd', `price_buh_uah`='$price_uah', `price_man_uah`='$price_pda' 
            WHERE `id`='$partition_id' LIMIT 1;");
        }
    ...
    function getGeneralStock($art_id) {$dbt=DbSingleton::getTokoDb();
        ...
        $general_stock=$dbt->result($r,0,"GENERAL_STOCK");
