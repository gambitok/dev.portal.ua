...
class clients {
    ...
    function getClientNameById($sel_id, $field = "name") { $db = DbSingleton::getDb();
        $name = "";
    ...
    function getUserNameById($sel_id, $field = "name") { $db = DbSingleton::getDb();
        $name = "";
        $r = $db->query("SELECT `$field` FROM `A_CLIENTS_USERS` WHERE `id` = '$sel_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 1) {
            $name = $db->result($r, 0, "$field");
        }
    ...
    function getClientCash($client_id) { $db = DbSingleton::getDb();
        $cash_id = 1;
    ...
    function checkEmptyClients() { $db = DbSingleton::getDb();
        $r = $db->query("SELECT `id` FROM `A_CLIENTS` WHERE `name` = '' AND `full_name` = '';");
        $n = $db->num_rows($r);
        if ($n > 0) {
            $list = "<ul>";
            for ($i = 1; $i <= $n; $i++) {
    ...
    function getUserInfo($client_id) { $db = DbSingleton::getDb();
        $list = "";
        $r = $db->query("SELECT `name`, `phone` FROM `A_CLIENTS_USERS` WHERE `client_id` = '$client_id';");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $name = $db->result($r, $i - 1, "name");
    ...
    function showClientsParrentTree($sel_id, $prnt_id) { $db = DbSingleton::getDb();
        ...
        FROM `A_CLIENTS` c 
            ...
            LEFT OUTER JOIN `T2_REGION` t2rg on t2rg.REGION_ID=c.region
            LEFT OUTER JOIN `T2_CITY` t2ct on t2ct.CITY_ID=c.city
        WHERE c.status=1 AND c.parrent_id=0;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
            $name = $db->result($r, $i - 1, "name");
            ...
            $city = $db->result($r, $i - 1, "CITY_NAME");
            $cur = "";
            $fn = "<i class='fa fa-thumb-tack' onClick='setClientParrent(\"$id\", \"$name\")'></i>";
            if ($id == $prnt_id) {
                $cur = "style='background-color:#FFFF00'";
            }
            if ($id == $sel_id) {
                $fn = "";
    ...
    function moveClientsRetailAddress($client_id, $new_client_id) { $db = DbSingleton::getDb();
        $r = $db->query("SELECT `id` FROM `A_CLIENTS_USERS_ADDRESS` WHERE `client_id`='$client_id' AND `type_id`=2;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
    ...
    function moveClientsRetail($user_retail_id, $client_retail_id) { $db = DbSingleton::getDb();
        ...
        if ($user_retail_id > 0) {
            $r = $db->query("SELECT * FROM `A_CLIENTS_USERS_RETAIL` WHERE `id` = '$user_retail_id' LIMIT 1;");
            $n = $db->num_rows($r);
            if ($n > 0) {
                $name = $db->result($r, 0, "name");
                ...
                $db->query("INSERT INTO `A_CLIENTS` (`id`, `name`, `full_name`, `phone`, `email`, `country`, `state`, `region`, `city`, `client_category`) VALUES ('$client_id','$name','$name','$phone','$email','$country','$state','$region','$city','$client_category');");
    ...
    function moveClientsConditionsRetail($client_retail_id, $client_id) { $db = DbSingleton::getDb();
        $r = $db->query("SELECT * FROM `A_CLIENTS_CONDITIONS` WHERE `client_id` = '$client_retail_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 1) {
            $cash_id = $db->result($r, 0, "cash_id");
    ...
    function showCategoryTree($sel_id = 0) { $db = DbSingleton::getDb();
        $tree = "";
        $r = $db->query("SELECT `id`, `name` FROM `A_CATEGORY` WHERE `parrent_id` = '0' ORDER BY `id` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
            $name = $db->result($r, $i - 1, "name");
    ...
    function showCategoryTreeSubLevel($parrent_id, $sel_id) { $db = DbSingleton::getDb();
        $tree = "";
        $r = $db->query("SELECT `id`, `name` FROM `A_CATEGORY` WHERE `parrent_id` = '$parrent_id' ORDER BY `id` ASC;");
        $n = $db->num_rows($r);
        if ($n > 0) {
            $tree .= "<ul>";
            for ($i = 1; $i <= $n; $i++) {
    ...
    function checkClientSubclients($sel_id) { $db = DbSingleton::getDb();
        ...
        FROM `A_CLIENTS` c 
            LEFT OUTER JOIN `A_ORG_TYPE` ot on ot.id=c.org_type 
        WHERE c.parrent_id='$sel_id' AND c.status='1' 
        ORDER BY c.id ASC;");
        $n = $db->num_rows($r);
        if ($n > 0) {
            $kol_childs = $n;
    ...
    function showClientCard($client_id) { $db = DbSingleton::getDb();
        ...
        $org_type = $parrent_id = 0;
        $full_name = $name = $email = $phone = $client_category_list = $childsTable = $clds_disabled = $chlds_hidden = $parrent_main_active = "";
        $form = ""; $form_htm = RD . "/tpl/clients_card.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT c.*, ot.full_name as ot_full_name 
        FROM `A_CLIENTS` c  
            LEFT OUTER JOIN `A_ORG_TYPE` ot on ot.id=c.org_type
        WHERE c.id='$client_id' AND c.status='1' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 0) {
            $client_id = 0;
        }
        if ($n == 1) {
            ...
            $parrent_main_active = "";
            $chlds_hidden = "hidden";
            $clds_disabled = "disabled";
            if ($hasChilds > 0) {
                $parrent_main_active = " disabled";
                $chlds_hidden = "";
                $clds_disabled = "";
            }
    ...
    function show_clients_retail_list($press = false) { $db = DbSingleton::getDb();
        $list="";
        $r = $db->query("SELECT * FROM `A_CLIENTS_USERS_RETAIL` ORDER BY `client_category` DESC, `data` DESC, `name` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id=$db->result($r, $i - 1, "id");
            $client_id=$db->result($r, $i - 1, "client_id");
            $client_id=$this->getClientNameById($client_id);
            $client_category=$db->result($r, $i - 1, "client_category");
            $client_category_name=$this->getManualName($client_category);
            $data=$db->result($r, $i - 1, "data");
            $name=$db->result($r, $i - 1, "name");
            $email=$db->result($r, $i - 1, "email");
            $phone=$db->result($r, $i - 1, "phone");
            $city_id=$db->result($r, $i - 1, "city_id");
            $city_id=$this->getCityName($city_id);
    ...
    function showClientRetailCard($user_id) { $db = DbSingleton::getDb();
        $slave=new slave;
        $form=""; $form_htm=RD."/tpl/clients_retail_card.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT * FROM `A_CLIENTS_USERS_RETAIL` WHERE `id`='$user_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 0) {
        ...
        if ($n == 1) {
            ...
            $user_id_created = $db->result($r,0,"user_id_created");
            $client_id_created = $db->result($r,0,"client_id_created");
            $move_client_retail = "";
            if ($status == 147) {
                $user_id_created_name = $this->getUserNameById($user_id_created);
                $client_id_created_name = $this->getClientNameById($client_id_created);
                $move_client_retail = "<div class='hr-line-dashed'></div>Контрагент: $client_id_created_name (ID:$client_id_created), пользователь: $user_id_created_name (ID:$user_id_created)";
            }
            $form = str_replace("{move_client_retail}",$move_client_retail,$form);
            $form = str_replace("{user_id}",$user_id,$form);
            $form = str_replace("{client_id}",$client_id,$form);
            $form = str_replace("{client_name}",$this->getClientNameById($client_id),$form);
            $form = str_replace("{user_category}",$client_category,$form);
            $form = str_replace("{category_list}",$this->showUserCategorySelectList($client_category),$form);
            ...
            $form = str_replace("{disabled_status}",($status == 146 || $status == 147) ? "disabled" : "",$form);
    ...
    function showClientRetailAddress($client_id) { $db = DbSingleton::getDb();
        $list = "";
        $r = $db->query("SELECT `id`, `address` FROM `A_CLIENTS_USERS_ADDRESS` WHERE `client_id` = '$client_id' AND `type_id` = 2;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
    ...
    function newClientRetailCard() { $db = DbSingleton::getDb();
        $client_id = 10;
        $r = $db->query("SELECT MAX(`id`) as mid FROM `A_CLIENTS_USERS_RETAIL`;");
        $user_id = 0 + $db->result($r,0,"mid") + 1;
    ...
    function showUserStatusSelectList($sel_id) { $db = DbSingleton::getDb();
        $list = "";
        $r = $db->query("SELECT `id`, `mcaption` FROM `manual` WHERE `key` = 'user_retail_status' AND `ison` = 1 ORDER BY `mcaption`, `id` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
    ...
    function showUserCategorySelectList($sel_id) { $db = DbSingleton::getDb();
        $list = "";
        $r = $db->query("SELECT `id`, `mcaption` FROM `manual` WHERE `key` = 'customers_categories' AND `ison` = 1 ORDER BY `id` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
            $name = $db->result($r, $i - 1, "mcaption");
    ...
    function loadCityOptions($city_id) { $db = DbSingleton::getDb();
        $list = "";
        $r = $db->query("SELECT `CITY_ID`, `CITY_NAME` FROM `T2_CITY` ORDER BY `CITY_NAME` ASC LIMIT 10;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "CITY_ID");
            $name = $db->result($r, $i - 1, "CITY_NAME");
            $sel = ($id == $city_id) ? " selected" : "";
            $list .= "<option value='$id' $sel>$name</option>";
        }
        return $list;
    }
    ...
    function saveClientGeneralInfo($client_id, $org_type, $name, $full_name, $phone, $email, $parrent_id, $country_id, $state_id, $region_id, $city_id, $c_category_kol, $c_category, $user_category) { $db = DbSingleton::getDb();
        ...
        if ($name != "" || $full_name != "") {
            if ($client_id == 0) {
                $r = $db->query("SELECT MAX(`id`) as mid FROM `A_CLIENTS`;");
                $new_client_id = 0 + $db->result($r,0,"mid") + 1;
            ...
            $err = "Спершу введіть назву клієнта!";
        }
    ...
    function loadClientDocumentPrefix($client_id) { $db = DbSingleton::getDb();
        $gmanual = new gmanual;
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_document_prefix_list.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT `id`, `doc_type_id`, `prefix` FROM `A_CLIENTS_DOCUMENT_PREFIX` WHERE `client_id` = '$client_id' AND `status` = '1' ORDER BY `doc_type_id` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
    ...
    function showClientDocumentPrefixForm($client_id, $prefix_id) { $db = DbSingleton::getDb();
        $gmanual = new gmanual;
        $prefix = ""; $doc_type_id = 0;
        $form = ""; $form_htm = RD . "/tpl/clients_document_prefix_form.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT `doc_type_id`, `prefix` FROM `A_CLIENTS_DOCUMENT_PREFIX` WHERE `id` = '$prefix_id' AND `client_id` = '$client_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 1) {
            $doc_type_id = $db->result($r, 0, "doc_type_id");
    ...
    function saveClientDocumentPrefixForm($client_id, $prefix_id, $doc_type_id, $prefix) { $db = DbSingleton::getDb();
        ...
        if ($client_id > 0 && $doc_type_id > 0 && $prefix != "") {
            $r = $db->query("SELECT COUNT(`id`) as kol FROM `A_CLIENTS_DOCUMENT_PREFIX` WHERE `doc_type_id`='$doc_type_id' AND `client_id`='$client_id' AND `id`<>'$prefix_id' AND `status`='1';");
            $doc_type_ex = $db->result($r,0,"kol");
            if ($doc_type_ex > 0) {
                $answer = 0; $err = "Обраний Вами тип документу вже має префікс! Відредагуйте існуючий префікс за потреби";
            }
            if ($doc_type_ex == 0) {
                if ($prefix_id == 0 || $prefix_id == "") {
                    $r = $db->query("SELECT MAX(`id`) as mid FROM `A_CLIENTS_DOCUMENT_PREFIX`;");
            ...
            $answer = 0; $err = "Не заповнені усі поля!";
        }
        return array($answer, $err);
    }
    ...
    function loadClientUsers($client_id) { $db = DbSingleton::getDb();
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_users_list.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm);}
        $r = $db->query("SELECT * FROM `A_CLIENTS_USERS` WHERE `client_id`='$client_id' AND `status`='1' ORDER BY `name` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
            $name = $db->result($r, $i - 1, "name");
        ...
        if ($n == 0) {
            $list = "<tr><td align='center' colspan=6><h3 class='text-center'>Користувачі відсутні</h3></td></tr>";
        }
    ...
    function showClientUserForm($client_id, $user_id_sel) { $db = DbSingleton::getDb();
        $form = ""; $form_htm = RD . "/tpl/clients_user_form.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }

        $r = $db->query("SELECT * FROM `A_CLIENTS_USERS` WHERE `id` = '$user_id_sel' LIMIT 1;");
        $id     = $db->result($r, 0, "id");
        $name   = $db->result($r, 0, "name");
        $email  = $db->result($r, 0, "email");
        $phone  = $db->result($r, 0, "phone");
        $pass   = $db->result($r, 0, "pass");
        $main   = $db->result($r, 0, "main");
        $price  = $db->result($r, 0, "price_status");
        $nulls  = $db->result($r, 0, "status_nulls");
        $export = $db->result($r, 0, "export_status");

        $main_ch="";if ($main==1){$main_ch=" checked='checked'";}
        $price_ch="";if ($price==1){$price_ch=" checked='checked'";}
        $user_nulls="";if ($nulls==1){$user_nulls=" checked='checked'";}
        $export_ch="";if ($export==1){$export_ch=" checked='checked'";}

        $form = str_replace("{client_id}", $client_id, $form);
        $form = str_replace("{user_id}", $id, $form);
        ...
        $form = str_replace("{user_nulls}", $user_nulls, $form);
    ...
    function saveClientUserForm($client_id, $user_id, $name, $email, $phone, $pass, $main, $price, $export, $user_nulls, $user_invoice)
    {
        ...
        $phone=$slave->qq($phone); $pass=$slave->qq($pass); $main=$slave->qq($main); $price=$slave->qq($price); $export=$slave->qq($export); $user_nulls=$slave->qq($user_nulls);
        $phone = $this->formatValidPhone($phone);

        if ($client_id > 0) {
            $r = $db->query("SELECT COUNT(`id`) as kol, `name` FROM `A_CLIENTS_USERS` WHERE `phone`='$phone' AND `id`!='$user_id' AND `status`='1';");
            $phone_ex = $db->result($r,0,"kol");
            $phone_name = $db->result($r,0,"name");

            $r = $db->query("SELECT COUNT(`id`) as kol, `name` FROM `A_CLIENTS_USERS` WHERE `email`='$email' AND `id`!='$user_id' AND `status`='1';");
            $email_ex = $db->result($r,0,"kol");
            $email_name = $db->result($r,0,"name");
            ...
            if ($email_ex == 0 && $phone_ex == 0) {
                ...
                if ($main == 1) {
                    $db->query("UPDATE `A_CLIENTS_USERS` SET `main`='0' WHERE `id`='$user_id' AND `client_id`='$client_id';");
                }
                $db->query("UPDATE `A_CLIENTS_USERS` SET `name`='$name', `email`='$email', `phone`='$phone', `pass`='$pass', `main`='$main', `price_status`='$price', `export_status`='$export', `status_nulls`='$user_nulls', `invoice_status`='$user_invoice' 
    ...
    function getClientInvoiceStatus($user_id) { $db = DbSingleton::getDb();
        $r = $db->query("SELECT `invoice_status` FROM `A_CLIENTS_USERS` WHERE `id` = '$user_id' LIMIT 1;");
    ...
    function loadClientContacts($client_id) { $db = DbSingleton::getDb();
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_contacts_list.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT `id`, `name`, `post` FROM `A_CLIENTS_CONTACTS` WHERE `client_id` = '$client_id' ORDER BY `name` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
            $name = $db->result($r, $i - 1, "name");
        ...
        if ($n == 0) {
            $list = "<tr><td align='center'><h3 class='text-center'>Контакти відсутні</h3></td></tr>";
        }
    ...
    function getContactCon($contact_id) { $db = DbSingleton::getDb();
        $manual = new manual;
        $list = "<ul>";
        $r = $db->query("SELECT `sotc_cont`, `contact_value` FROM `A_CLIENTS_CONTACTS_CON` WHERE `contact_id` = '$contact_id' ORDER BY `id` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
    ...
    function getContactConForm($contact_id) { $db = DbSingleton::getDb();
        $manual = new manual;
        $list = "";
        $r = $db->query("SELECT `id`, `sotc_cont`, `contact_value` FROM `A_CLIENTS_CONTACTS_CON` WHERE `contact_id` = '$contact_id' ORDER BY `id` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
    ...
    function showClientContactForm($client_id, $contact_id_sel) { $db = DbSingleton::getDb();
        $form = ""; $form_htm = RD . "/tpl/clients_contacts_form.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT `id`, `name`, `post` FROM `A_CLIENTS_CONTACTS` WHERE `id` = '$contact_id_sel' LIMIT 1;");
    ...
    function loadClientCommets($client_id) { $db = DbSingleton::getDb();
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_comment_block.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function dropClientComment($client_id, $comment_id) { $db = DbSingleton::getDb();
        ...
        if ($client_id > 0 && $comment_id > 0) {
            $r = $db->query("SELECT * FROM `A_CLIENTS_COMMENTS` WHERE `CLIENT_ID`='$client_id' AND `ID`='$comment_id' LIMIT 1;");
            $n = $db->num_rows($r);
            if ($n == 1) {
    ...
    function loadClientsDetailsFile($client_id, $file_type) { $db = DbSingleton::getDb();
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_details_file.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT cd.*, u.name as user_name 
         FROM `A_CLIENTS_DTLS` cd 
            LEFT OUTER JOIN `media_users` u on u.id=`cd`.USER_ID 
        WHERE cd.CLIENT_ID='$client_id' AND cd.FILE_TYPE='$file_type' AND cd.STATUS='1' ORDER BY cd.FILE_NAME ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
    ...
    function clientsDetailsDropFile($client_id, $file_type, $file_id) { $db = DbSingleton::getDb();
        ...
        if ($client_id > 0 && $file_id > 0 && $file_type != "") {
            $r = $db->query("SELECT `FILE_NAME` FROM `A_CLIENTS_DTLS` WHERE `CLIENT_ID`='$client_id' AND `FILE_TYPE`='$file_type' AND `ID`='$file_id' LIMIT 1;");
            $n = $db->num_rows($r);
            if ($n == 1) {
    ...
    function loadClientCDN($client_id) { $db = DbSingleton::getDb();
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_cdn_block.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT cc.*, u.name as user_name 
        FROM `A_CLIENTS_CDN` cc 
            LEFT OUTER JOIN `media_users` u on u.id=cc.USER_ID 
        WHERE cc.CLIENT_ID='$client_id' AND cc.STATUS='1' ORDER BY cc.FILE_NAME ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $file_id=$db->result($r, $i - 1, "ID");
            $file_name=$db->result($r, $i - 1, "FILE_NAME");
    ...
    function clientsCDNDropFile($client_id, $file_id) { $db = DbSingleton::getDb();
        ...
        if ($client_id > 0 && $file_id > 0) {
            $r = $db->query("SELECT `FILE_NAME` FROM `A_CLIENTS_CDN` WHERE `CLIENT_ID`='$client_id' AND `ID`='$file_id' LIMIT 1;");
            $n = $db->num_rows($r);
            if ($n == 1) {
                unlink(RD.'/cdn/clfiles/$client_id/$file_name');
...
//    }
    ...
    function loadCitySelectList($region_id, $sel_id) { $db = DbSingleton::getDb();
        $slave = new slave;
        $list = "";
        $r = $db->query("SELECT `CITY_ID`, `CITY_NAME` FROM `T2_CITY` WHERE `REGION_ID`='$region_id' ORDER BY `CITY_NAME` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "CITY_ID");
            $name = $db->result($r, $i - 1, "CITY_NAME");
    ...
    function showCategoryCheckList($client_id) { $db = DbSingleton::getDb();
        $list="";
        $r = $db->query("SELECT `id`, `name` FROM `A_CATEGORY` WHERE `parrent_id`=0 ORDER BY `id` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            ...
            $client_disabled = "";
            if ($i == 1) {
                $client_disabled = ($this->checkSaleInvoiceClients($client_id)) || ($this->checkJPayClients($client_id)) ? "disabled" : "";
            }
    ...
    function checkClientCategorySelect($client_id, $category_id) { $db = DbSingleton::getDb();
        $ch = 0;
        $r = $db->query("SELECT `category_id` FROM `A_CLIENTS_CATEGORY` WHERE `client_id`='$client_id' AND `category_id`='$category_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 1) {
            $ch = 1;
        }
    ...
    function showTpointListSelect($sel_id) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT `id`, `name` FROM `T_POINT` ORDER BY `name` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
            $name = $db->result($r, $i - 1, "name");
    ...
    function showPriceLvlListSelect($sel_id) {
        $list = "";
        for ($i = 0; $i <= 200; $i++) {
            $sel = ($sel_id == $i) ? "selected=\"selected\"" : "";
    ...
    function showGoodGroupTree($sel_id) { $db = DbSingleton::getDb();
        $tree = "";
        $form = ""; $form_htm = RD . "/tpl/clients_goods_group_form.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT `ID`, `NAME` FROM `GOODS_GROUP` WHERE `PARRENT_ID` = '0' ORDER BY `NAME` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "ID");
            $name = $db->result($r, $i - 1, "NAME");
    ...
    function showGoodGroupSubLevel($parrent_id, $sel_id) { $db = DbSingleton::getDb();
        $tree = "";
        $r = $db->query("SELECT `ID`, `NAME` FROM `GOODS_GROUP` WHERE `PARRENT_ID` = '$parrent_id' ORDER BY `NAME` ASC;");
        $n = $db->num_rows($r);
        if ($n > 0) {
            $tree .= "<ul>";
            for ($i = 1; $i <= $n; $i++) {
    ...
    function loadClientSupplConditions($client_id) { $db = DbSingleton::getDb();
        $gmanual = new gmanual;
        $prepayment_checked = $prepay_all_checked = $prepay_summ_disabled = $prepay_persent_readonly = "";
        $form = ""; $form_htm = RD . "/tpl/clients_suppl_conditions.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT * FROM `A_CLIENTS_SUPPL_CONDITIONS` WHERE `client_id`='$client_id' LIMIT 1;");
        $prepayment = $db->result($r,0,"prepayment");
        if ($prepayment == 1) {
            $prepayment_checked = " checked='checked'";
        }
        $prepay_all = $db->result($r,0,"prepay_all");
        if ($prepay_all == 1) {
            $prepay_all_checked = " checked='checked'";
            $prepay_summ_disabled = "readonly";
        }
        $prepay_summ = $db->result($r,0,"prepay_summ");
        $prepay_type = $db->result($r,0,"prepay_type");
        if ($prepay_type == 65) {
            $prepay_persent_readonly = " readonly";
        }
    ...
    function loadClientConditions($client_id) { $db = DbSingleton::getDb();
        $client_vat_checked = "";
        $form = ""; $form_htm = RD . "/tpl/clients_conditions.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        ...
        $client_vat=$db->result($r,0,"client_vat");
        if ($client_vat == 1) {
            $client_vat_checked = " checked='checked'";
        }
        $doc_type_id=$db->result($r,0,"doc_type_id");
        ...
        $rs = $db->query("SELECT `rounding_price` FROM `A_CLIENTS` WHERE `id`='$client_id' LIMIT 1;");
...
//        for ($i = 1; $i <= $n; $i++) {
//            $id=$db->result($r, $i- 1 ,"i d");
//            $bank=$db->result($r, $i- 1 ,"b ank");
//            $account=$db->result($r, $i- 1 ,"a ccount");
...
//    }
    ...
    function getClientDetailsList2($detail_id, $tpoint_id, $doc_type_id) { $db = DbSingleton::getDb();
        $arr = []; $list = "";
        $r = $db->query("SELECT `client_id` FROM `T_POINT_CLIENTS` WHERE `in_use`=1 AND `tpoint_id`='$tpoint_id' AND `sale_type`='$doc_type_id';");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $client_id = $db->result($r, $i - 1, "client_id");
        ...
        if ($n > 0) {
            $list = "";
            $r = $db->query("SELECT * FROM `A_CLIENT_DETAILS` WHERE `client_id` IN ($client_list);");
            $n = $db->num_rows($r);
            for ($i = 1; $i <= $n; $i++) {
                $id = $db->result($r, $i - 1, "id");
                $client_id = $db->result($r, $i - 1, "client_id");
                ...
                $ch = ($id == $detail_id) ? "selected='selected'" : "";
    ...
    function getRoundingPriceList($sel) { $db = DbSingleton::getDb();
        $list = "";
        $r = $db->query("SELECT `ID`, `CAPTION` FROM `PRICE_ROUNDING` WHERE `STATUS` = 1;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "ID");
    ...
    function setClientRounding($client_id) { $db = DbSingleton::getDb();
        ...
        if ($client_rounding == 0) {
            $r1 = $db->query("SELECT * FROM `A_CLIENTS_CATEGORY` WHERE `client_id`='$client_id' AND `category_id`=1;");
            $n1 = $db->num_rows($r1);
            $r2 = $db->query("SELECT * FROM `A_CLIENTS_CONDITIONS` WHERE `client_id`='$client_id' AND `cash_id`=1 AND `doc_type_id` IN (64,63);");
            $n2 = $db->num_rows($r2);
            if ($n1 > 0 && $n2 > 0) {
    ...
    function showClientConditionsHistory($client_id) {$db = DbSingleton::getDb();
        $list = "";
        $r = $db->query("SELECT * FROM `A_CLIENTS_CONDITIONS` WHERE `client_id`='$client_id';");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $cash_id = $db->result($r, $i - 1, "cash_id");
            $country_cash_id = $db->result($r, $i - 1, "country_cash_id");
            $credit_cash_id = $db->result($r, $i - 1, "credit_cash_id");
            ...
            $country_cash_id = $this->getCashName($country_cash_id);
            $credit_cash_id = $this->getCashName($credit_cash_id);
            $tpoint_id = $this->getTpointName($tpoint_id);
            $doc_type_id = $this->getDocTypeName($doc_type_id);
        ...
        $r = $db->query("SELECT * FROM `A_CLIENTS_CONDITIONS_HISTORY` WHERE `client_id`='$client_id' ORDER BY `data` DESC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $data = $db->result($r, $i - 1, "data");
            $user_id = $db->result($r, $i - 1, "user_id");
            ...
            $doc_type_id = $db->result($r, $i - 1, "doc_type_id");
            $detail_id = $db->result($r, $i - 1, "detail_id");
            $color = ($i == 1) ? "style='background:lightgreen;'" : "";
            $user_id = $this->getMediaUserName($user_id);
            $cash_id = $this->getCashName($cash_id);
            $country_cash_id = $this->getCashName($country_cash_id);
            $credit_cash_id = $this->getCashName($credit_cash_id);
            $tpoint_id = $this->getTpointName($tpoint_id);
            $doc_type_id = $this->getDocTypeName($doc_type_id);
        ...
        $form = ""; $form_htm = RD . "/tpl/clients_conditions_history.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function loadClientDetails($client_id){$db = DbSingleton::getDb();
        $form = ""; $form_htm = RD . "/tpl/clients_details.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function loadClientDetailsList($client_id) { $db = DbSingleton::getDb();
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_details_list.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT * FROM `A_CLIENT_DETAILS` WHERE `client_id`='$client_id';");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
    ...
    function loadClientDetailsCard($id, $client_id) { $db = DbSingleton::getDb();
        $form = ""; $form_htm = RD . "/tpl/clients_details_card.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm);}
        $r = $db->query("SELECT * FROM `A_CLIENT_DETAILS` WHERE `id` = '$id' LIMIT 1;");
        $n = $db->num_rows($r);
        ...
        $main_ch = "";
        if ($main == 1) {
            $main_ch = " checked='checked'";
        }
        if ($n == 0) {
            $r = $db->query("SELECT MAX(`id`) as mid FROM `A_CLIENT_DETAILS`;");
            $detail_id = 0 + $db->result($r, 0, "mid") + 1;
    ...
    function showCountryManual($sel_id) { $db = DbSingleton::getDb();
        $manual = new manual;
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_country_list.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function showCountryForm($id) { $db = DbSingleton::getDb();
        $manual = new manual;
        $form = ""; $form_htm = RD . "/tpl/clients_country_form.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function showCostumsManual($sel_id) { $db = DbSingleton::getDb();
        $manual=new manual;
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_costums_list.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function showCostumsForm($id) { $db = DbSingleton::getDb();
        $manual = new manual;
        $form = ""; $form_htm = RD . "/tpl/clients_costums_form.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function loadClientStorage($client_id) { $db = DbSingleton::getDb();
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_storage_list.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        ...
        WHERE cc.client_id='$client_id' AND cc.status='1';");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
            $name = $db->result($r, $i - 1, "name");
        ...
        if ($n == 0) {
            $list = "<tr><td align='center' colspan=7><h3 class='text-center'>Склади відсутні</h3></td></tr>";
        }
    ...
    function loadClientMandate($client_id) { $db = DbSingleton::getDb();
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_mandate_list.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT * FROM `A_CLIENTS_MANDATE` WHERE `client_id`='$client_id' AND `status`='1';");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
        ...
        if ($n == 0) {
            $list = "<tr><td align='center' colspan=6><h3 class='text-center'>Доручення відсутні</h3></td></tr>";
        }
    ...
    function showClientMandateForm($client_id, $mandate_id) { $db = DbSingleton::getDb();
        $form=""; $form_htm=RD."/tpl/clients_mandate_form.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function loadClientBasis($client_id) { $db = DbSingleton::getDb();
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/clients_basis_list.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT * FROM `A_CLIENTS_BASIS` WHERE `client_id`='$client_id' AND `status`='1';");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
            $number = $db->result($r, $i - 1, "number");
        ...
        if ($n == 0) {
            $list = "<tr><td align='center' colspan=6><h3 class='text-center'>Підстави відсутні</h3></td></tr>";
        }
    ...
    function showClientBasisForm($client_id, $basis_id) { $db = DbSingleton::getDb();
        $form = ""; $form_htm = RD . "/tpl/clients_basis_form.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function showClientStorageForm($client_id, $storage_id) { $db = DbSingleton::getDb();
        $slave = new slave;
        $form = ""; $form_htm = RD . "/tpl/clients_storage_form.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function showClientSupplGeneralSaldoForm($client_id) { $db = DbSingleton::getDb();
        ...
        if ($client_id > 0) {
            ...
            $data_to = date("Y-m-t");
            $form_htm = RD . "/tpl/client_general_saldo_list.htm";
            if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
            ...
            GROUP BY b.doc_type_id, b.doc_id 
            ORDER BY b.id ASC;");
            $n = $db->num_rows($r);
            if ($n > 0) {
                for ($i = 1; $i <= $n; $i++) {
                    ...
                    if ($doc_type_id == 1) {
                        $document_name = $sale_invoice->getSaleInvoiceName($doc_id);
                        $function = "showSaleInvoiceCard(\"$doc_id\");";
                    }
                    if ($doc_type_id == 2) {
                        list($jpay_doc_type_id,$document_name) = $sale_invoice->getJPayName($doc_id);
                        $function = "viewJpayMoneyPay(\"$doc_id\")";
                        if ($jpay_doc_type_id == 99) {$summ = "";}
                    }
                    if ($doc_type_id == 3) {
                        list(,$document_name) = $sale_invoice->getJPayName($doc_id);
                    }
                    if ($doc_type_id == 5) {
                        $document_name = $back_clients->getBackClientsName($doc_id);
                    }
            ...
            if ($n == 0) {
                $list = "<tr><td colspan='10' align='center'>Документи відсутні</td></tr>";
                $saldo_end = $saldo_start;
            }
            $form = str_replace("{list}", $list, $form);
            $form = str_replace("{saldo_end}", $saldo_end."".$this->getCashAbr($saldo_cash_id), $form);
            $form = str_replace("{saldo_data_end}", date("Y-m-d"), $form);
            $form = str_replace("{client_id}", $client_id, $form);
        }
    ...
    /*
     * фільтр взаєморозрахунки
     * */
    function filterClientGeneralSaldoForm($client_id, $data_from, $data_to, $saldo_articles = false) { $db = DbSingleton::getDb();
        $sale_invoice = new sale_invoice;
        $back_clients = new back_clients;
        $list = "";
        $client_saldo_start = $client_saldo_end = $client_saldo_data_start = $client_saldo_data_end = 0;
        if ($client_id > 0) {
            ...
            if ($n > 0) {
                for ($i = 1; $i <= $n; $i++) {
                    ...
                    $document_name = ""; $function = "";
                    if ($doc_type_id == 1) {
                        $invoice_name = $sale_invoice->getSaleInvoiceName($doc_id);
                        $function = "showSaleInvoiceCard(\"$doc_id\");";
                        $document_name = $invoice_name;

                        $rs = $db->query("SELECT * FROM `J_SALE_INVOICE_STR` WHERE `invoice_id`='$doc_id';");
                        $ns = $db->num_rows($rs);
                        if ($ns > 0 && $saldo_articles == true) {
                            $document_name = "<div class='center-block' style='margin: 15px'>$invoice_name</div>";
                            $document_name .= "<table class='table table-striped table-hover table-bordered'>";
                            $document_name .= "<thead><tr>
                                <th>#</th>
                                <th>Артикул</th>
                                <th>Назва</th>
                                <th>Бренд</th>
                            ...
                            for ($j = 1; $j <= $ns; $j++) {
                                $art_id = $db->result($rs, $j - 1, "art_id");
                                $article_nr_displ = $db->result($rs, $j - 1, "article_nr_displ");
                                $brand_id = $db->result($rs, $j - 1, "brand_id");
                                $amount = $db->result($rs, $j - 1, "amount");
                                $price_end = $db->result($rs, $j - 1, "price_end");
                                $summ = $db->result($rs, $j - 1, "summ");
                    ...
                    if ($doc_type_id == 5) {
                        $back_clients_name = $back_clients->getBackClientsName($doc_id);
                        $document_name = $back_clients_name;
                        $rs = $db->query("SELECT * FROM `J_BACK_CLIENTS_STR` WHERE `back_id`='$doc_id';");
                        $ns = $db->num_rows($rs);
                        if ($ns > 0 && $saldo_articles == true) {
                            $document_name = "<div class='center-block' style='margin: 15px'>$back_clients_name</div>";
                            $document_name .= "<table class='table table-striped table-hover table-bordered'>";
                            $document_name .= "<thead><tr>
                                <th>#</th>
                                <th>Артикул</th>
                            ...
                            for ($j = 1; $j <= $ns; $j++) {
                                ...
                                $brand_id = $db->result($rs, $j - 1, "brand_id");
                                $amount = $db->result($rs, $j - 1, "amount");
                                $price_end = $db->result($rs, $j - 1, "price");
                                $summ = $db->result($rs, $j - 1, "summ");
                                $name = $this->getArticleName($art_id);
                                $brand_name = $this->getBrandName($brand_id);
                                $document_name .= "
    ...
    function getSaleInvoceProlog($client_id, $date_search) { $db = DbSingleton::getDb();
        ...
        if ($date_search == "" || $date_search == 0) {
            $date_search = $today;
        }
        $r = $db->query("SELECT `prefix`, `doc_nom`, `data_create`, `data_pay` FROM `J_SALE_INVOICE` WHERE `client_conto_id` = '$client_id' AND `data_pay` <= '$date_search' AND `summ_debit` != 0;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $prefix = $db->result($r, $i - 1, "prefix");
        ...
        $form = ""; $form_htm = RD . "/tpl/sale_invoice_prolog.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        if ($n == 0) {
    ...
    function getSaleInvocePrologHistory($client_id) { $db = DbSingleton::getDb();
        $users = new users;
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/sale_invoice_prolog_history.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function checkSaleInvoceProlog($client_id, $date_start, $date_new) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT `prefix`, `doc_nom`, `data_create`, `data_pay`, `data_pay_start` FROM `J_SALE_INVOICE` WHERE `client_id` = '$client_id' AND `data_pay` <= '$date_start' AND `summ_debit` != 0;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $prefix = $db->result($r, $i - 1, "prefix");
            $doc_nom = $db->result($r, $i - 1, "doc_nom");
            ...
            if ($date_pay_start == "" || $date_pay_start == "0000-00-00") {
                $datetime1 = new DateTime($data_pay);
            } else {
                $datetime1 = new DateTime($date_pay_start);
            }
            $datetime2 = new DateTime($date_new);
            $interval = $datetime1->diff($datetime2);
            $dec = $interval->format('%a');

            $style = ($users_credit >= $dec) ? "" : "style='background:pink;";
    ...
    function editSaleInvoceProlog($client_id, $date_start, $date_new) { $db = DbSingleton::getDb();
        ...
        $users_credit = $this->getUsersAccessCredit($user_id);
        if ($client_id > 0) {
            $r = $db->query("SELECT `id`, `data_pay_start`, `data_pay` FROM `J_SALE_INVOICE` WHERE `client_id` = '$client_id' AND `data_pay` <= '$date_start' AND `summ_debit` != 0;");
            $n = $db->num_rows($r);
            for ($i = 1; $i <= $n; $i++) {
                $id = $db->result($r, $i - 1, "id");
                $date_pay_start = $db->result($r, $i - 1, "data_pay_start");
                ...
                if ($users_credit >= $dec) {
                    if ($date_pay_start == "" || $date_pay_start == "0000-00-00") {
                        $set = ",data_pay_start='$data_pay' ";
                        $date_pay_start = $data_pay;
                    } else {
                        $set = "";
                    }
    ...
    /*
     * друк взаєморозрахунків
     * */
    function printGeneralSaldoList($client_id, $data_from, $data_to, $saldo_articles = false) { $db = DbSingleton::getDb();
        $sale_invoice = new sale_invoice;
        $back_clients = new back_clients;
        $form = ""; $balans_after = 0;
        if ($client_id > 0) {
            $form_htm = RD . "/tpl/clients_print_saldo.htm";
            if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
            $saldo_end = 0;
            $client_cash_id = $this->getClientCash($client_id);
            list($saldo_start, $saldo_cash_id,) = $this->getClientBalansPeriodStart($client_id, $client_cash_id, $data_from, 0);
            ...
            if ($n > 0) {
                for ($i = 1; $i <= $n; $i++) {
                    ...
                    if ($doc_type_id == 1) {
                        ...
                        $ns = $db->num_rows($rs);
                        if ($ns > 0 && $saldo_articles == true) {
                            $document_name = "<div>$invoice_name</div>";
                            $document_name .= "<table width='100%'>";
                            $document_name .= "<thead><tr>
                                <th>#</th>
                                <th>Артикул</th>
                                <th>Назва</th>
                    ...
                    if ($doc_type_id == 5) {
                        ...
                        $ns = $db->num_rows($rs);
                        if ($ns > 0 && $saldo_articles == true) {
                            $document_name = "<div>$back_clients_name</div>";
                            $document_name .= "<table width='100%'>";
                            $document_name .= "<thead><tr>
                                <th>#</th>
                                <th>Артикул</th>
            ...
            $form = str_replace("{list}", $list, $form);
            $form = str_replace("{saldo_end}", $saldo_end . "" . $this->getCashAbr($saldo_cash_id), $form);
            $form = str_replace("{saldo_data_end}", $data_to, $form);
            $form = str_replace("{client_id}", $client_id, $form);
            $form = str_replace("{client_name}", $this->getClientName($client_id), $form);

            $dp = new dp();
            $r = $db->query("SELECT `tpoint_id`, `doc_type_id` FROM `A_CLIENTS_CONDITIONS` WHERE `client_id` = '$client_id' LIMIT 1;");
            $tpoint_id = $db->result($r, 0, "tpoint_id");
            $doc_type_id = $db->result($r, 0, "doc_type_id");
            $seller_id = $dp->getSellerId($tpoint_id, $doc_type_id);
            list($seller_name, , $edrpou, $account, $bank, $mfo, $vat) = $dp->getClientInfo($seller_id);
    ...
    function loadClientAddresses($client_id) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT `id`, `address` FROM `A_CLIENTS_USERS_ADDRESS` WHERE `client_id` = '$client_id' AND `type_id` = 1;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $address_id = $db->result($r, $i - 1, "id");
            $address = $db->result($r, $i - 1, "address");
    ...
    function addClientAddress($client_id, $address) { $db = DbSingleton::getDb();
        $r = $db->query("SELECT MAX(`id`) as mid FROM `A_CLIENTS_USERS_ADDRESS`;");
        $address_id = 0 + $db->result($r, 0, "mid") + 1;
    ...
    function show_country_list() { $db = DbSingleton::getDb();
        $manual = new manual;
        $list = "";
        $r = $db->query("SELECT * FROM `T2_COUNTRIES` WHERE 1;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "COUNTRY_ID");
            $name = $db->result($r, $i - 1, "COUNTRY_NAME");
    ...
    function showCountryCard($country_id_sel) { $db = DbSingleton::getDb();
        $slave = new slave;
        $country_id_sel = $slave->qq($country_id_sel);
        session_start();
        $user_id = $_SESSION["media_user_id"];
        $user_name = $_SESSION["user_name"];
        $form = ""; $form_htm = RD . "/tpl/country_card.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT * FROM `T2_COUNTRIES` WHERE `COUNTRY_ID` = $country_id_sel LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 0) {
            $form_htm = RD . "/tpl/access_deny.htm";
            if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function getDeliveryList($delivery_id) { $db = DbSingleton::getTokoDb();
        ...
        $r = $db->query("SELECT `ID`, `DESCRIPTION` FROM `T2_DELIVERY` WHERE `STATUS`=1;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "ID");
            $text = $db->result($r, $i - 1, "DESCRIPTION");
            $sel = ($id == $delivery_id) ? "selected" : "";
    ...
    function getPaymentList($payment_id) { $db = DbSingleton::getTokoDb();
        $list = "<option value='0'>-Не вибрано-</option>";
        $r = $db->query("SELECT `ID`, `DESCRIPTION` FROM `T2_PAYMENT` WHERE `STATUS` = 1;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "ID");
            $text = $db->result($r, $i - 1, "DESCRIPTION");
            $sel = ($id == $payment_id) ? "selected" : "";
            $list .= "<option value='$id' $sel>$text</option>";
        }
        return $list;
    }

    function getDeliveryChargeList($sel_id) { $db = DbSingleton::getDb();
        $list = "<option value='0'>-Не вибрано-</option>";
        $r = $db->query("SELECT `id`, `mcaption` FROM `manual` WHERE `key` = 'delivery_charge';");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
            $text = $db->result($r, $i - 1, "mcaption");
            $sel = ($id == $sel_id) ? "selected" : "";
            $list .= "<option value='$id' $sel>$text</option>";
        }
        return $list;
    ...
    function getExpressList($express) { $db = DbSingleton::getTokoDb();
        $list = "<option value='0'>-Не вибрано-</option>";
        $r = $db->query("SELECT `ID`, `DESCRIPTION` FROM `T2_DELIVERY_EXPRESS` WHERE `STATUS` = 1;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "ID");
            $text = $db->result($r, $i - 1, "DESCRIPTION");
            $sel = ($id == $express) ? "selected" : "";
            $list .= "<option value='$id' $sel>$text</option>";
        }
        return $list;
    }
    ...
    function getCityList($city_id)
    {
        ...
        $r = $db->query("SELECT * FROM `T2_LOCATION` WHERE `REGION_NAME` = '' OR `CITY_ID` = '$city_id';");
        $n = $db->num_rows($r);
         for ($i = 1; $i <= $n; $i++) {
             $id            = $db->result($r, $i - 1, "CITY_ID");
             ...
             $list  .= "
            <option value=\"$id\" data-foo=\"$city_name\" $sel>$city_name_foo</option>";
         }
        return $list;
    }

    function getDepartmentList($department, $department_text) {
        return "<option value='0'>-Не вибрано-</option><option value='$department' selected>$department_text</option>";
    }

    function getUserList($client_id, $user_id) { $db = DbSingleton::getDb();
        $list = "<option value='0'>-Не вибрано-</option>";
        $r = $db->query("SELECT `id`, `name` FROM `A_CLIENTS_USERS` WHERE `client_id` = $client_id;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
            $text = $db->result($r, $i - 1, "name");
            $sel = ($id == $user_id) ? "selected" : "";
            $list .= "<option value='$id' $sel>$text</option>";
        }
        return $list;
    }

    function dropClient($client_id) {
        $answer = 0; $err = "Помилка збереження даних!";
        if ($client_id > 0) {
