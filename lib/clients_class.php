...
//  $db=DbSingleton::getDb();$slave=new slave;$manual=new manual; session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	$client_id=0;
...
function showClientsParrentTree($sel_id,$prnt_id){$db=DbSingleton::getDb();
	...
	$r=$db->query("select c.*,ot.name as org_type_name, t2cn.COUNTRY_NAME, t2st.STATE_NAME, t2rg.REGION_NAME, t2ct.CITY_NAME  from A_CLIENTS c 
		...
		left outer join T2_REGION t2rg on t2rg.REGION_ID=c.region
		left outer join T2_CITY t2ct on t2ct.CITY_ID=c.city
		where c.status=1 and parrent_id=0;");$n=$db->num_rows($r);$list="";
		for ($i=1;$i<=$n;$i++){
			$id=$db->result($r,$i-1,"id");
			$name=$db->result($r,$i-1,"name");
			$org_type_name=$db->result($r,$i-1,"org_type_name");
...
function saveClientRetailGeneralInfo($user_id,$user_name,$client_id,$country_id,$state_id,$region_id,$city_id,$user_category,$user_phone,$user_email,$user_pass,$user_status,$user_data){$db=DbSingleton::getDb();
...
function saveClientGeneralInfo($client_id,$org_type,$name,$full_name,$phone,$email,$parrent_id,$country_id,$state_id,$region_id,$city_id,$c_category_kol,$c_category,$user_category){$db=DbSingleton::getDb();$slave=new slave;session_start();
    $user_id=$_SESSION["media_user_id"];$answer=0;$err="Помилка збереження даних!";$return_client=0;
...
function showClientDocumentPrefixForm($client_id,$prefix_id){$db=DbSingleton::getDb();$gmanual=new gmanual;
...
function saveClientDocumentPrefixForm($client_id,$prefix_id,$doc_type_id,$prefix){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка збереження даних!";
...
function dropClientDocumentPrefix($client_id,$prefix_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка збереження даних!";
...
function saveClientUserForm($client_id,$user_id,$name,$email,$phone,$pass,$main,$price,$export){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка збереження даних!";
...
function dropClientUser($client_id,$user_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка збереження даних!";
...
function dropClientContact($client_id,$contact_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка збереження даних!";
...
function dropClientComment($client_id,$comment_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка видалення запису!";
...
function clientsDetailsDropFile($client_id,$file_type,$file_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка видалення файлу!";
...
function clientsCDNDropFile($client_id,$file_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка видалення файлу!";
	$client_id=$slave->qq($client_id);$file_id=$slave->qq($file_id);
...
function setArticlesFotoMain($client_id,$file_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка обробки запиту!";
	$client_id=$slave->qq($client_id);$file_id=$slave->qq($file_id);
	if ($client_id>0 && $file_id>0){
...
function articlesFotoDropFile($client_id,$file_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка видалення файлу!";
	$client_id=$slave->qq($client_id);$file_id=$slave->qq($file_id);
	if ($client_id>0 && $file_id>0){
...
function loadClientSupplConditions($client_id){$db=DbSingleton::getDb();$gmanual=new gmanual;session_start();
...
function saveClientSupplConditions($client_id,$prepayment,$prepay_all,$prepay_summ,$prepay_type,$prepay_persent){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка збереження даних!";
...
function loadClientConditions($client_id){$db=DbSingleton::getDb();session_start();
...
function loadClientDetails($client_id){$db=DbSingleton::getDb();session_start();
...
function saveClientDetails($client_id,$address_jur,$address_fakt,$edrpou,$svidotctvo,$vytjag,$vat,$mfo,$bank,$account,$not_resident,$nr_details,$buh_name,$buh_edrpou){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка збереження даних!";
...
function saveclientsCountryForm($id,$name,$alfa2,$alfa3,$duty,$risk){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка збереження даних!";
...
function saveclientsCostumsForm($id,$name,$preferential_rate,$full_rate,$type_declaration,$sertification,$gos_standart){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка збереження даних!";
...
function loadArticleZED($client_id){$db=DbSingleton::getDb();session_start();
...
function saveclientsZED($client_id,$country_id,$costums_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка збереження даних!";
...
function saveClientStorageForm($client_id,$storage_id,$name,$email,$phone,$contact_person,$country,$state,$region,$city,$client_visible){$db=DbSingleton::getDb();$slave=new slave;session_start();$answer=0;$err="Помилка збереження даних!";
...
function showClientGeneralSaldoForm($client_id){$db=DbSingleton::getDb();session_start();
    $form="";$form_htm=RD."/tpl/client_general_saldo_list.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
	if ($client_id>0){
...
function getSaldoEnd($client_id){$db=DbSingleton::getDb(); session_start();
	if ($client_id>0){
...
function filterClientGeneralSaldoForm($client_id,$data_from,$data_to){$db=DbSingleton::getDb();session_start();
	if ($client_id>0){
...
function checkSaleInvoceProlog($client_id,$date_start,$date_new) { $db=DbSingleton::getDb(); $list="";
	session_start();$user_id=$_SESSION["media_user_id"];
	require_once RD.'/lib/users_class.php';$users=new users;
	$users_credit=$users->getUsersAccessCredit($user_id);
...
function editSaleInvoceProlog($client_id,$date_start,$date_new) { $db=DbSingleton::getDb();
	session_start();$user_id=$_SESSION["media_user_id"]; $answer=0;$err="Помилка збереження даних!";
	require_once RD.'/lib/users_class.php';$users=new users;
	$users_credit=$users->getUsersAccessCredit($user_id);	
	if ($client_id>0) {	
		$r=$db->query("select * from J_SALE_INVOICE where client_id='$client_id' and data_pay<='$date_start' and summ_debit!=0;"); $n=$db->num_rows($r);
...
function printGeneralSaldoList($client_id,$data_from,$data_to) {
	$db=DbSingleton::getDb();session_start();
    $form="";$form_htm=RD."/tpl/clients_print_saldo.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
