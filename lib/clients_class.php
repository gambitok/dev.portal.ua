...
//			$country=$slave->showTableField("T2_COUNTRIES","COUNTRY_NAME","COUNTRY_ID",$db->result($r,$i-1,"country"));
...
//function SaveCheckedStorage($client_id,$client_vis) {$db=DbSingleton::getDb();$answer=0;$err="������� ���������� �����!";	
//	if ($client_id>0) {
...
function showClientsParrentTree($sel_id,$prnt_id){$db=DbSingleton::getDb();$slave=new slave;
	...
	$r=$db->query("select c.*,ot.name as org_type_name, t2cn.COUNTRY_NAME, t2st.STATE_NAME, t2rg.REGION_NAME, t2ct.CITY_NAME  from A_CLIENTS c 
		...
		left outer join T2_REGION t2rg on t2rg.REGION_ID=c.region
		left outer join T2_CITY t2ct on t2ct.CITY_ID=c.city
		where c.status=1 and parrent_id=0 $where;");$n=$db->num_rows($r);$list="";
		for ($i=1;$i<=$n;$i++){
			$id=$db->result($r,$i-1,"id");
			$name=$db->result($r,$i-1,"name");
			$org_type_name=$db->result($r,$i-1,"org_type_name");
...
function moveClientsRetail($user_retail_id,$client_retail_id){$db=DbSingleton::getDb(); $answer=0;$err="������� ���������� �����!";
...
function moveClientsConditionsRetail($client_retail_id,$client_id) {$db=DbSingleton::getDb();
	...
	if ($n==1){
		...
		$db->query("insert into A_CLIENTS_CONDITIONS (`client_id`, `cash_id`, `country_cash_id`, `credit_cash_id`, `payment_delay`, `credit_limit`, `credit_return`, `price_lvl`, `margin_price_lvl`, `price_suppl_lvl`, `margin_price_suppl_lvl`, `tpoint_id`, `client_vat`, `doc_type_id`) values ('$client_id','$cash_id','$country_cash_id','$credit_cash_id','$payment_delay','$credit_limit','$credit_return','$price_lvl','$margin_price_lvl','$price_suppl_lvl','$margin_price_suppl_lvl','$tpoint_id','$client_vat','$doc_type_id');");
	}
	return;
}

function unlinkClientsParrent($client_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="������� ���������� �����!";
...
function unlinkClientsSubclient($client_id,$subclient_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="������� ���������� �����!";
...
function showClientCard($client_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	...
	if ($n==1){
		...
		list($hasChilds,$arChilds,$childsTable)=$this->checkClientSubclients($client_id);
...
function showClientRetailCard($user_id){$db=DbSingleton::getDb(); $slave=new slave;
	...
	if ($n==1){
		...
		����������: $client_id_created_name (ID:$client_id_created), ������������: $user_id_created_name (ID:$user_id_created)";
...
function saveClientRetailGeneralInfo($user_id,$user_name,$client_id,$country_id,$state_id,$region_id,$city_id,$user_category,$user_phone,$user_email,$user_pass,$user_status,$user_data){$db=DbSingleton::getDb(); $answer=0;$err="������� ���������� �����!";
	if ($user_id>0){
...
function saveClientGeneralInfo($client_id,$org_type,$name,$full_name,$phone,$email,$parrent_id,$country_id,$state_id,$region_id,$city_id,$c_category_kol,$c_category,$user_category){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function saveClientDocumentPrefixForm($client_id,$prefix_id,$doc_type_id,$prefix){$db=DbSingleton::getDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function dropClientDocumentPrefix($client_id,$prefix_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function saveClientUserForm($client_id,$user_id,$name,$email,$phone,$pass,$main,$price,$export){$db=DbSingleton::getDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function dropClientUser($client_id,$user_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function loadClientContacts($client_id){$db=DbSingleton::getDb();$slave=new slave;
	...
	$r=$db->query("select cc.* from A_CLIENTS_CONTACTS cc where cc.client_id='$client_id' order by name asc;");$n=$db->num_rows($r);$list="";
		...
		if ($n==0){$list="<tr><td align='center'><h3 class='text-center'>�������� ������</h3></td></tr>";}
...
function saveClientContactForm($client_id,$contact_id,$contact_name,$contact_post,$contact_con_kol,$con_id,$sotc_cont,$contact_value){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function dropClientContact($client_id,$contact_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function loadClientCommets($client_id){$db=DbSingleton::getDb();$slave=new slave;
	...
	$r=$db->query("select cc.*,u.name from A_CLIENTS_COMMENTS cc 
		...
		where cc.CLIENT_ID='$client_id' order by id desc;");$n=$db->num_rows($r);$list="";
		for ($i=1;$i<=$n;$i++){
			$id=$db->result($r,$i-1,"ID");
...
function saveClientComment($client_id,$comment){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	
	$client_id=$slave->qq($client_id);$comment=$slave->qq($comment);
	if ($client_id>0 && $comment!=""){
		$r=$db->query("insert into A_CLIENTS_COMMENTS (`CLIENT_ID`,`USER_ID`,`COMMENT`) values ('$client_id','$user_id','$comment');");
		$answer=1;$err="";
	}
	return array($answer,$err);
}
function dropClientComment($client_id,$comment_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ��������� ������!";
...
function clientsDetailsDropFile($client_id,$file_type,$file_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ��������� �����!";
	
	$client_id=$slave->qq($client_id);$file_id=$slave->qq($file_id);
	if ($client_id>0 && $file_id>0 && $file_type!=""){
		$r=$db->query("select FILE_NAME from A_CLIENTS_DTLS where CLIENT_ID='$client_id' and FILE_TYPE='$file_type' and ID='$file_id' limit 0,1;");$n=$db->num_rows($r);
		if ($n==1){
			$file_name=$db->result($r,0,"FILE_NAME");
			unlink(RD.'/cdn/clfiles/$client_id/$file_name');
			$r=$db->query("delete from A_CLIENTS_DTLS where CLIENT_ID='$client_id' and FILE_TYPE='$file_type' and ID='$file_id';");
			$answer=1;$err="";
		}
	}
	return array($answer,$err);
...
function clientsCDNDropFile($client_id,$file_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ��������� �����!";
	...
	if ($client_id>0 && $file_id>0){
		...
		if ($n==1){
			$file_name=$db->result($r,0,"FILE_NAME");
			unlink(RD.'/cdn/clfiles/$client_id/$file_name');
			$r=$db->query("delete from A_CLIENTS_CDN where CLIENT_ID='$client_id' and ID='$file_id';");
			$answer=1;$err="";
		}
	}
	return array($answer,$err);
...
function loadArticleFoto($client_id){$db=DbSingleton::getDb();$slave=new slave;
	...
	$r=$db->query("select t2af.*,u.name as user_name from T2_PHOTOS t2af
		...
		for ($i=1;$i<=$n;$i++){
			...
			$main_v="<a class=\"btn btn-xs btn-white\" onClick=\"setArticlesFotoMain('$client_id','$file_id')\"><i class=\"fa fa-check\"></i> ������� ����</a>";
...
function setArticlesFotoMain($client_id,$file_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ������� ������!";
	$client_id=$slave->qq($client_id);$file_id=$slave->qq($file_id);
	if ($client_id>0 && $file_id>0){
...
function articlesFotoDropFile($client_id,$file_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ��������� �����!";
	$client_id=$slave->qq($client_id);$file_id=$slave->qq($file_id);
	if ($client_id>0 && $file_id>0){
		...
		if ($n==1){
			...
			$r=$db->query("delete from T2_PHOTOS where ART_ID='$client_id' and ID='$file_id';");
			$answer=1;$err="";
		}
	}
	return array($answer,$err);
...
function getClientGeneralSaldo($sel_id){$db=DbSingleton::getDb();$saldo="0 ���";
...
//	return "<option value='NEW'>�������� ��������� �����</option>".$slave->showSelectSubListDBM("T2_CITY","REGION_ID","$region_id","CITY_ID","CITY_NAME",$sel_id);
}
...
function showTpointListSelect($sel_id){$db=DbSingleton::getDb();$list="<option value='0'>-- ������ � ������ --</option>";
...
function saveClientSupplConditions($client_id,$prepayment,$prepay_all,$prepay_summ,$prepay_type,$prepay_persent){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function showClientConditionsHistory($client_id) {$db=DbSingleton::getDb(); $list="";
	...
	$form_htm=RD."/tpl/clients_conditions_history.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}	
...
function saveClientDetails($client_id,$address_jur,$address_fakt,$edrpou,$svidotctvo,$vytjag,$vat,$mfo,$bank,$account,$not_resident,$nr_details,$buh_name,$buh_edrpou){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function saveclientsCountryForm($id,$name,$alfa2,$alfa3,$duty,$risk){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function saveclientsCostumsForm($id,$name,$preferential_rate,$full_rate,$type_declaration,$sertification,$gos_standart){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function loadArticleZED($client_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	...
	where t2z.ART_ID='$client_id' limit 0,1;");$n=$db->num_rows($r);
	if ($n==1){
...
function saveclientsZED($client_id,$country_id,$costums_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function loadClientStorage($client_id){$db=DbSingleton::getDb();$slave=new slave;
	...
	for ($i=1;$i<=$n;$i++){
		...
		if ($vis==1){$vis=" checked=''";} else $vis="";
		$list.="
		<tr>
			<td>
				<button class='btn btn-sm btn-default' onClick='showClientStorageForm(\"$client_id\",\"$id\");'><i class='fa fa-edit'></i></button>
				<button class='btn btn-sm btn-default' onClick='dropClientStorage(\"$client_id\",\"$id\",\"$city\");'><i class='fa fa-times'></i></button>
			</td>
			<td>$id</td>
			<td>$name</td>
	...
	if ($n==0){$list="<tr><td align='center' colspan=7><h3 class='text-center'>������ ������</h3></td></tr>";}
...
function loadClientBasis($client_id){$db=DbSingleton::getDb();
	...
	if ($n==0){$list="<tr><td align='center' colspan=6><h3 class='text-center'>ϳ������ ������</h3></td></tr>";}
...
function saveClientStorageForm($client_id,$storage_id,$name,$email,$phone,$contact_person,$country,$state,$region,$city,$client_visible){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function showClientGeneralSaldoForm($client_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	if ($client_id>0){  $data_from=date("Y-m-01");$data_to=date("Y-m-t"); //current_month
		...
		$saldo_start=0;$saldo_end=0; //default value
		$client_cash_id=$this->getClientCash($client_id);
		list($saldo_start,$saldo_cash_id,$saldo_data_start)=$this->getClientBalansPeriodStart($client_id,$client_cash_id,$data_from,0);
		...
		for ($i=1;$i<=$n;$i++){
			...
			$doc_type_id=$db->result($r,$i-1,"doc_type_id"); // 1-��������� (SaleInvoice); 2- ������(Pay); 3-����������(PayAuto) 
			...
			$list.="<tr align='center'>
				...
				<td><button class='btn btn-xs btn-default' title='�����������' onClick='$function'><i class='fa fa-eye'></i></button></td>
			</tr>";
		}$saldo_end=round($balans_after,2);
		}
		
		if ($n==0){$list="<tr><td colspan='8' align='center'>��������� ������</td></tr>"; $saldo_end=$saldo_start;}
		$form=str_replace("{list}",$list,$form);
...
function getSaldoEnd($client_id){$db=DbSingleton::getDb();
	...
	if ($client_id>0){ 
		...
		$saldo_cap="";
		if($saldo_end<0) $saldo_cap="<span style='color:red;'>(����)</span>";
		if($saldo_end>0) $saldo_cap="<span style='color:limegreen;'>(����������)</span>";
		
		$saldo_end=$saldo_end." ".$this->getCashAbr($saldo_cash_id)." ".$saldo_cap;
		
		$saldo_cap="";
		if($saldo_start<0) $saldo_cap="<span style='color:red;'>(����)</span>";
		if($saldo_start>0) $saldo_cap="<span style='color:limegreen;'>(����������)</span>";
...
function filterClientGeneralSaldoForm($client_id,$data_from,$data_to){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	if ($client_id>0){   //current_month
		...
		print "$saldo_start,$saldo_cash_id,$saldo_data_start";
		...
		if ($n==0){
			$list="<tr><td colspan='8' align='center'>��������� ������</td></tr>";
			$client_saldo_end=$balans_after."".$this->getCashAbr($saldo_cash_id);
			$client_saldo_start=$client_saldo_end=$saldo_start."".$this->getCashAbr($saldo_cash_id);
		}
		print "\n\n$list,$client_saldo_start,$client_saldo_end,$client_saldo_data_start,$client_saldo_data_end";
	}
...
function getClientBalansPeriodStart($client_id,$cash_id,$data_from,$recursion){$db=DbSingleton::getDb();$saldo_start=0;$saldo_data_start=$data_from;
	...
	if ($n==0){
		...
		if ($recursion<12){
			...
			list($saldo_start,$saldo_cash_id,$saldo_data_start)=$this->getClientBalansPeriodStart($client_id,$cash_id,$data_from,$recursion);
		}
...
function editSaleInvoceProlog($client_id,$date_start,$date_new) { $db=DbSingleton::getDb(); $today=date("Y-m-d"); $err="������� ���������� �����!"; 	
	session_start();$user_id=$_SESSION["media_user_id"];
	require_once RD.'/lib/users_class.php';$users=new users;
	$users_credit=$users->getUsersAccessCredit($user_id);	
	if ($client_id>0) {	
