...
function showBrandsCard($brands_id){
	...
	$r=$dbt->query("select b.* from T2_BRANDS b where b.BRAND_ID='$brands_id' limit 0,1;");
	$n=$dbt->num_rows($r);
	$r2=$dbt->query("select * from T2_BRAND_LINK where brand_id='$brands_id'");
	$n2=$dbt->num_rows($r2);
...
function saveBrandsGeneralInfo($brands_id, $brands_name, $brands_type, $brands_kind, $brands_country, $brands_visible) {
	...
	$brands_type=$slave->qq($brands_type);
	$brands_kind=$slave->qq($brands_kind);
	$brands_country=$slave->qq($brands_country);
	$brands_visible=$slave->qq($brands_visible);
	
	if ($brands_id>0){
		//$db->query("update T2_BRANDS set `BRAND_NAME`='$brands_name',`BRAND_TYPE`='$brands_type', `KIND`='$brands_kind', `COUNTRY_ID`='$brands_country', `VISIBLE`='$brands_visible' where `BRAND_ID`='$brands_id';");
...
function loadBrandsDetails($brands_id){
	...
	//$logo_name=$db->result($r,0,"logo_name");
	$link=$db->result($r,0,"link");
	
	$form=str_replace("{descr}",$descr,$form);
	//$form=str_replace("{logo_name}",$logo_name,$form);
...
function showCsvPreviewIndex(){
	$db=DbSingleton::getDb(); $slave=new slave; $csv_exist=0; $csv_file_name="������ ����"; $pre_table="<h3 align='center'>������ ������</h3>";
	...
	if ($n==1){
		...
		if (file_exists($file_path)){
			$form_htm=RD."/tpl/brands_index_str_file.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
...
function finishBrandsIndexImport($start_row,$kol_cols,$cols){
	$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();
	$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";$err2 = "���� � ���������� �������!";
	$start_row=$slave->qq($start_row);$kol_cols=$slave->qq($kol_cols);$cols=$slave->qq($cols);
		...
		if ($n==1){
			...
			if (file_exists($file_path)){
				...
				//$records_list="";
				//$bid=0;$bname=0;$btype=0;$bcountry=0;$bvisible=0;
				for ($i=1;$i<=$kol_cols;$i++){
					if ($cols[$i]==1){$bid=$i;}
					if ($cols[$i]==2){$bname=$i;}
					if ($cols[$i]==3){$btype=$i;}
