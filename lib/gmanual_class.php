...
class gmanual {
	...
	function show_key_list(){	$db=DbSingleton::getDb();$mdl=new module;$url=$mdl->get_file_url("Gmanual");session_start();
		...
		$form=str_replace("{keys_list}",$list,$form);$alert="";//???
	...
	function sendGmanualRequest1($gkey,$id,$caption){$db=DbSingleton::getDb();$slave=new slave;session_start();$label="info";//$mdl=new module;$url=$mdl->get_file_url("Gmanual");
	...
	function showGmanualList($key,$scaption){$db=DbSingleton::getDb();
		...
		for ($i=1;$i<=$n;$i++){
			...
			//$mid=$db->result($r,$i-1,"mid");
			$caption=$db->result($r,$i-1,"mcaption");
			$list.="<tr onclick='location.href=\"?$url&w=edit_gmanual&gmanualId=$id\"'>
	...
	function new_gmanual_form($key){$slave=new slave;$mdl=new module;$url=$mdl->get_file_url("Gmanual");session_start();$n=0;$alert="";//???
	...
	function show_navigation($item_id,$nav_menu){ $mdl=new module;$url=$mdl->get_file_url("gmanual");
	    $nav_menu="<a class='navigation' href='?$url'>Контрагенти</a>";
	    return $nav_menu;
	}

	function show_menu($w){$mdl=new module; $url=$mdl->get_file_url("gmanual");
		$menu="<div class='sideMenu' onclick='location.href=\"?$url\";'>Загальний реєстр</div><br />
	...
	function getManualList($filter){$db=DbSingleton::getDb();$list="";$where="";if ($filter!=""){$where=" and caption like '%$filter%' ";}$k=0;
		...
		for ($i=1;$i<=$n;$i++){$k++;
			...
			$list.="<tr bgcolor='#$tr_color' class='tRow' align='left' onclick='setCheckbox(\"m$id\");' ondblclick='setValue(\"$id\");'>
