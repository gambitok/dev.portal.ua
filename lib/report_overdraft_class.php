...
class report_overdraft {
	...
	function getTpointList() { $db=DbSingleton::getDb(); $list=""; $tpoint=$this->getTpointbyUser(); $list="<option value='0'>�� ������ �����</option>";
	...
	function showReportOverdraftList($date_cur,$client_id,$tpoint_id) { $db=DbSingleton::getDb(); $list=""; $clients=[]; $summ_uah=$summ_usd=$summ_eur=0;
		...
		foreach ($clients as $c_id=>$client) {
			...
			$list.="<tr style='cursor:pointer;background:#ddd;' align='center'>
				<td align='left'><button class='btn btn-xs' title='��������������� � ������������' onclick='showClientGeneralSaldoForm($c_id);'><i class='fa fa-th-list'></i></button> $title</td>
		...
		$summ_all="���� �� $transform_date: $summ_uah UAH / $summ_usd USD / $summ_eur EUR";
	...
	function getClientReportOverdraftList($client_id,$date_cur,$tpoint_id) { $db=DbSingleton::getDb(); $list=""; $docs=[]; $js="";
		...
		foreach ($docs as $doc) {
			...
			if ($prolog_amount>0) $js="<button class='btn btn-xs' title='����������� ���������' onclick='showDocsProlongationForm($client_id,$invoice_id);'><i class='fa fa-file'></i></button>"; else $js="";
	...
	function showDocsProlongationForm($client_id,$invoice_id) { $db=DbSingleton::getDb(); 
		...
		for ($i=1;$i<=$n;$i++){
			...
			$list.="<li>����: $date_pay_new; ����������: $user_name;</li>";
		}
		$list.="</ul>";
		return $list;
	}
