...
function show_sale_invoice_list(){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual; session_start(); $ses_tpoint_id=$_SESSION["media_tpoint_id"]; $media_user_id=$_SESSION["media_user_id"]; $media_role_id=$_SESSION["media_role_id"]; $sales=[];
	$form_htm=RD."/tpl/buh_invoice_range.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
	...
	where sv.status=1 $where and sv.doc_type_id=61 order by sv.time_stamp desc, sv.status_invoice asc, sv.data_create desc, sv.prefix asc, sv.id desc;");
	$n=$db->num_rows($r);
...
function loadSaleInvoicePartitions($invoice_id){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;$income=new income;
...
function unlockSaleInvoiceMoneyPayKours($invoice_id,$pay_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
