...
function show_sale_invoice_list(){$db=new db;$slave=new slave;$gmanual=new gmanual; session_start(); $ses_tpoint_id=$_SESSION["media_tpoint_id"]; $media_user_id=$_SESSION["media_user_id"]; $media_role_id=$_SESSION["media_role_id"]; $sales=[];
...
function loadSaleInvoicePartitions($invoice_id){$db=new db;$slave=new slave;$gmanual=new gmanual;$income=new income;
...
function unlockSaleInvoiceMoneyPayKours($invoice_id,$pay_id){$db=new db;$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="Помилка збереження даних!";
