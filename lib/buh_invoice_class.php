...
class buh_invoice {
    ...
    function show_sale_invoice_list(){$db = DbSingleton::getDb();
        ...
        $form="";$form_htm=RD."/tpl/buh_invoice_range.htm";
        ...
        for ($i=1;$i<=$n;$i++){
            ...
            if ($cash_id==1) $summ_uah+=$summ;
            if ($cash_id==2) $summ_usd+=$summ;
            if ($cash_id==3) $summ_eur+=$summ;
    ...
    function show_sale_invoice_list_filter($data_start,$data_end){$db = DbSingleton::getDb();
        ...
        FROM `J_SALE_INVOICE` sv
            ...
            LEFT OUTER JOIN `A_CLIENTS` cl on cl.id=sv.client_conto_id
            LEFT OUTER JOIN `manual` dt on dt.key='client_sale_type' AND dt.id=sv.doc_type_id
        WHERE sv.status=1 $where ORDER BY sv.time_stamp DESC, sv.status_invoice ASC, sv.data_create DESC, sv.prefix ASC, sv.id DESC;");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
    ...
    function createTaxInvoice($invoice_id){$db = DbSingleton::getDb();
        ...
        if ($n==1){
            ...
            $r1=$db->query("SELECT * FROM `J_SALE_INVOICE_STR` WHERE `invoice_id`='$invoice_id' ORDER BY `id` ASC;");
            $n1=$db->num_rows($r1);
            for ($i=1;$i<=$n1;$i++){
    ...
    function getBuhIncomeList() { $db = DbSingleton::getDb();
        ...
        $list="";
        $r=$db->query("SELECT * FROM `J_BUH_INCOME` WHERE 1;");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
            $id=$db->result($r,$i-1,"ID");
    ...
    function showBuhIncomeCard($buh_income_id) { $db = DbSingleton::getDb();
        ...
        $form_htm=RD."/tpl/buh_income_card.htm";$form="";
        if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("SELECT * FROM `J_BUH_INCOME` WHERE `ID`='$buh_income_id' LIMIT 1;");
        $n=$db->num_rows($r);
        if ($n==0) {
            $text="";
            $pay_id=0;
...
//    }
    ...
    function getPayboxList($paybox_id=0) { $db = DbSingleton::getDb();
        $list="";
        $r=$db->query("SELECT * FROM `PAY_BOX` WHERE `STATUS`=1;");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
            $id = $db->result($r, $i - 1, "id");
    ...
    function getStateIncome($state_id=0) { $db = DbSingleton::getDb();
        $list="";
        $r=$db->query("SELECT * FROM `manual` WHERE `key`='state_income';");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
            $id = $db->result($r, $i - 1, "id");
    ...
    function getBuhConvertList() { $db = DbSingleton::getDb();
        $media_users=new media_users;
        $list="";
        $r=$db->query("SELECT * FROM `J_BUH_CONVERT` WHERE 1;");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
            $id=$db->result($r,$i-1,"ID");
            $text=$db->result($r,$i-1,"TEXT");
    ...
    function showBuhConvertCard($buh_convert_id) { $db = DbSingleton::getDb();
        $media_users=new media_users;
        session_start();$media_user_id=$_SESSION["media_user_id"];
        $form_htm=RD."/tpl/buh_convert_card.htm";$form="";
        if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("SELECT * FROM `J_BUH_CONVERT` WHERE `ID`='$buh_convert_id' LIMIT 1;");
        $n=$db->num_rows($r);
        if ($n==0) {
            $text="";
            $pay_id=0;
    ...
    function saveBuhConvertCard($buh_convert_id, $text, $pay_id, $cash_id_pay, $cash_id_to, $usd, $eur, $summ, $user_id, $data, $status) { $db = DbSingleton::getDb();
        ...
        if ($buh_convert_id>0) {
            ...
            $r=$db->query("SELECT MAX(`ID`) as mid FROM `J_BUH_CONVERT`;");
            $buh_convert_id=0+$db->result($r,0,"mid")+1;
    ...
    function convertPayboxBalans($buh_convert_id, $paybox_id, $cash_id_pay, $cash_id_to, $usd, $eur, $summ, $user_id) { $db = DbSingleton::getDb();
        $r=$db->query("SELECT COUNT(`id`) as kol FROM `B_PAYBOX_BALANS` WHERE `paybox_id`='$paybox_id' AND `cash_id`='$cash_id_pay' AND `user_id`='$user_id';");
        $ex=$db->result($r,0,"kol");
        ...
        if ($ex>0) {
            ...
            $r=$db->query("SELECT * FROM `B_PAYBOX_BALANS` WHERE `paybox_id`='$paybox_id' AND `cash_id`='$cash_id_to' AND `user_id`='$user_id' LIMIT 1;");
            $n=$db->num_rows($r);
            if ($n==0) {
            ...
            $r=$db->query("SELECT * FROM `B_PAYBOX_JOURNAL` WHERE `paybox_id`='$paybox_id' AND `cash_id`='$cash_id_pay' AND `user_id`='$user_id' ORDER BY `id` DESC LIMIT 1;");
            $n=$db->num_rows($r);
            ...
            // JOURNAL TO
            $saldo_before=0;
            $r=$db->query("SELECT * FROM `B_PAYBOX_JOURNAL` WHERE `paybox_id`='$paybox_id' AND `cash_id`='$cash_id_to' AND `user_id`='$user_id' ORDER BY `id` DESC LIMIT 1;");
            $n=$db->num_rows($r);
            if ($n==1) { $saldo_before=$db->result($r,0,"saldo_after"); }
    ...
    function getPayUser($user_id, $pay_id=0) { $db = DbSingleton::getDb();
        ...
        WHERE p.`STATUS`=1 AND pb.`user_id`='$user_id' GROUP BY pb.`paybox_id`;");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
            $id = $db->result($r, $i - 1, "id");
            $name = $db->result($r, $i - 1, "name");
    ...
    function getPayCashList($pay_id, $user_id, $cash_id=0) { $db = DbSingleton::getDb();
        ...
        WHERE pb.`paybox_id`='$pay_id' AND pb.`user_id`='$user_id';");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
