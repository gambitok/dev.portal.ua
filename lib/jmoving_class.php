...
function get_jmoving_prefix($jmoving_id){ $db=DbSingleton::getDb();$prefix="��";
...
function show_jmoving_list($press){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;$income=new income; session_start(); $time=date("h:i:sa");								   
	...
	$where order by man.mid asc, j.data desc, j.doc_nom desc, j.id desc limit 0,500;");$n=$db->num_rows($r);$list="";

	for ($i=1;$i<=$n;$i++){
		$id=$db->result($r,$i-1,"id");
		$type_id=$db->result($r,$i-1,"type_id");$type_name="<i class='fa fa-inbox'></i> ������� ����������";
		if ($type_id==1){$type_name="<i class='fa fa-truck'></i> ̳� ��������";}
...
function preNewJmovingCard(){
	$form_htm=RD."/tpl/jmoving_select_type_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
	return $form;
}
...
function showJmovingCard($jmoving_id){$db=DbSingleton::getDb();$slave=new slave;$manual=new manual; $income=new income; session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	...
	if ($n==1){
		...
		if ($user_accepting!="") $data_accepting="����������: $user_accepting, $user_data_accepting";
		if ($user_accepted!="") $data_accepted="��������: $user_accepted, $user_data_accepted";
...
//			if ($this->checkStorselAllStatus($jmoving_id)==1 and $status_jmoving) $disabled48="";
			...
			$form=str_replace("{jmoving_fullprice}",number_format($this->getJMovingFullPrice($jmoving_id), 2, '.', '')." ���",$form);
...
function cancelJmoving($jmoving_id) {$db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb(); $answer=0; $err="������� ���������� �����!";
	...
	if ($n>0) {	
		...
		for ($i=1;$i<=$n;$i++){
			...
			for ($j=1;$j<=$nstr;$j++){
				...
				//1
				$amount=0;$amount_res=0;$amount_new=0;	
...
function showJmovingCardLocal($jmoving_id){$db=DbSingleton::getDb();$slave=new slave;$manual=new manual; $income=new income; session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	...
	if ($n==1){
		$id=$db->result($r,0,"id");
		$type_id=$db->result($r,0,"type_id");$type_name="������� ����������";
		$prefix=$db->result($r,0,"prefix");
		$doc_nom=$db->result($r,0,"doc_nom");
		$user_use=$db->result($r,0,"user_use");
		$data_create=$db->result($r,0,"time_stamp");
...
function closeJmovingCard($jmoving_id){session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;
...
function checkJmovingSelectAllStatus($jmoving_id,$statusJmoving){$db=DbSingleton::getDb();$ex=1;
	...
	if ($n==1){
		$status_jmoving=$db->result($r,0,"status_jmoving");
		if ($status_jmoving==44){$ex=0; $i=$n+1;}
	}
	return $ex;
}
...
function showJmovingStrList($jmoving_id,$oper_status,$storage_id_to){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$manual=new manual;$gmanual=new gmanual;$income=new income;$list="";if ($oper_status==""){$oper_status=30;}
	...
	for ($i=1;$i<=$kl_rw;$i++){
		...
		if ($i<=$n){
			...
			$amountRest="����: $stokRest | ������: $reservRest";
			$status_jmoving=$db->result($r,$i-1,"status_jmoving");
		}
		if ($oper_status==30){
...
function saveJmovingCardLocal($jmoving_id,$jmoving_op_id,$data,$storage_id_to,$comment,$kol_row,$idStr,$artIdStr,$cellIdToStr){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function setArticleToJmoving($jmoving_id,$idStr,$artIdStr,$article_nr_displStr,$brandIdStr,$storageIdFromStr,$cellIdFromStr,$amountStr){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="�������� �������. �������� ���� �������� ����� � �������� �� ���!";
	...
	if ($jmoving_id>0){
		...
		if ($amountS<=$max_moving && $rest_amount==0){$answer=0;$err="�������� �������. �������� ���� �������� ����� � �������� �� ���!";}
		if ($amountS>$max_moving && $rest_amount<=0){$answer=0;$err="�������� �������. �������� ���� �������� ����� � �������� �� ���!";}
	...
	return array($answer,$err,$idS,"����: $rr_amount | ������: $rr_reserv",$weight,$volume,$empty_kol,$label_empty);
}
function setArticleToJmovingLocal($jmoving_id,$idStr,$artIdStr,$article_nr_displStr,$brandIdStr,$storageId,$cell_from_move,$cell_to_move,$amountStr){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="�������� �������. �������� ���� �������� ����� � �������� �� ���!";
	$jmoving_id=$slave->qq($jmoving_id);
	if ($jmoving_id>0){
...
function changeArticleToJmoving($jmoving_id,$jmoving_str_id,$amount_change){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	$jmoving_id=$slave->qq($jmoving_id);
	if ($jmoving_id>0){
		...
		if ($n==1){
			...
			//print "art_id=$art_id,jmoving_id=$jmoving_id,amountS=$amountS,cell_id_from=$cell_id_from,storage_id_from=$storage_id_from\n";
			...
			if ($amountS<=$max_moving && $rest_amount==0){$answer=0;$err="ʳ������ ��� ���������� ��� ����� �� �������!";}
			if ($amountS>$max_moving && $rest_amount<=0){$answer=0;$err="ʳ������ ��� ���������� ��� ����� �� �������!";}
...
function changeArticleToJmovingLocal($jmoving_id,$jmoving_str_id,$amount_change){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	$jmoving_id=$slave->qq($jmoving_id);
	if ($jmoving_id>0){
		$jmoving_str_id=$slave->qq($jmoving_str_id);$amount_change=$slave->qq($amount_change);
...
//			print "$art_id,$jmoving_id,$amount_change,$cell_id_from,$storage_id_from\n";
...
//			print "$info,$max_moving\n";
			...
			if ($amount_change<=$max_moving && $rest_amount<0){$answer=0;$err="�������� �������. �������� ���� �������� ����� � �������� �� ���!";}
			if ($amount_change>$max_moving && $rest_amount<=0){$answer=0;$err="�������� �������. �������� ���� �������� ����� � �������� �� ���!";}
...
function dropJmovingStr($jmoving_id,$jmoving_str_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="������� �������";
	$jmoving_id=$slave->qq($jmoving_id);
...
function dropJmovingLocalStr($jmoving_id,$jmoving_str_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="������� �������";
	$jmoving_id=$slave->qq($jmoving_id);
	$r=$db->query("select oper_status,status_jmoving from J_MOVING where id='$jmoving_id' limit 0,1;");$n=$db->num_rows($r);
	if ($n==1){
		$status_jmoving=$db->result($r,0,"status_jmoving");
...
function saveJmovingLocalAutoCell($jmoving_id,$storage_id_to,$cell_id_from){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$catalogue=new catalogue;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";$kol_row=0;
		...
		for ($ic=1;$ic<=$nc;$ic++){
			...
			if ($idS>0){
				if ($art_id!="" && $art_id>0 && $article_nr_displ!=""){$kol_row+=1;
					$amountEx+=$amountS;
					$db->query("update J_MOVING_STR set `art_id`='$art_id', `article_nr_displ`='$article_nr_displ', `brand_id`='$brand_id', `amount`='$amountEx', `storage_id_from`='$storage_id_to', `cell_id_from`='$cell_id_from' where id='$idS' and jmoving_id='$jmoving_id' limit 1;");
...
function clearJmovingLocalAutoCellForm($jmoving_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="������� �������";
	$jmoving_id=$slave->qq($jmoving_id);
	$r=$db->query("select oper_status,status_jmoving from J_MOVING where id='$jmoving_id' limit 0,1;");$n=$db->num_rows($r);
	if ($n==1){
		$status_jmoving=$db->result($r,0,"status_jmoving");
...
//	print $query;
	...
	if ($list2==""){  // �������� ������� ����� ��� �������� ����� � ������� ������
		...
		for ($i=1;$i<=$n;$i++){
			...
			$amountRest="����: $stock | ������: $reserv | � ��������� �����: $jmoving_amount";
...
function setArticleToSelectAmountJmoving($art_id,$storage_id,$storage_id_to){
	$form_htm=RD."/tpl/jmoving_select_amount_article_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
...
function setArticleToSelectAmountJmovingLocal($art_id,$storage_id){
	$form_htm=RD."/tpl/jmoving_local_select_amount_article_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
...
function showArticleRestStorageSelectList($art_id,$storage_id,$storage_id_to){$db=DbSingleton::getTokoDb();$list="<option value='0'>-- ������ � ������ --</option>";
...
//	print "$where\n";
	...
	for ($i=1;$i<=$n;$i++){
		...
		if ($id==$storage_id){$sel.=" disabled"; $t="��� ����..? ";}
		if($amount!=0 || $reserv_amount!=0) {
			$list.="<option value='$id' data-max-mov='$max_moving' data-cellId-mov='0' $sel>$name $t | �������: $amount; ������: $reserv_amount; </option>";
		}
	}
	return $list;
}
...
function saveJmovingStorage($jmoving_id,$storage_id,$storage_cells_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function saveJmovingComment($jmoving_id,$comment){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	
	$jmoving_id=$slave->qq($jmoving_id);$comment=$slave->qq($comment);
	if ($jmoving_id>0 && $comment!=""){
		$r=$db->query("insert into J_MOVING_COMMENTS (`jmoving_id`,`user_id`,`comment`) values ('$jmoving_id','$user_id','$comment');");
		$answer=1;$err="";
	}
	return array($answer,$err);
}
function dropJmovingComment($jmoving_id,$comment_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ��������� ������!";
...
function jmovingCDNDropFile($jmoving_id,$file_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ��������� �����!";
	...
	if ($jmoving_id>0 && $file_id>0){
		...
		if ($n==1){
			...
			$r=$db->query("delete from J_MOVING_CDN where jmoving_id='$jmoving_id' and id='$file_id';");
			$answer=1;$err="";
		}
	}
	return array($answer,$err);
...
function newJmoving($type_id,$art_id,$storage_id_to,$status_jmoving,$oper_status,$jmov) {$db=DbSingleton::getDb(); 
	...
	//var_dump("������� JM#$jmoving_id");
	$db->query("update J_MOVING_STR set jmoving_id='$jmoving_id' where jmoving_id='$jmov' and art_id='$art_id';");													//				var_dump("������ JSTR#$jmoving_id ($jmov, $art_id)");				 
...
function startJmovingStorageSelect($jmoving_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="";
	...
	if ($n==1){
		...
		$status_jmoving=$db->result($r,0,"status_jmoving");
		$storage_id_to=$db->result($r,0,"storage_id_to");
		if ($storage_id_to==0){$answer=0;$err="�� ��������� ����� ����������.";}
		if ($status_jmoving>47 || $oper_status>30){$answer=0;$err="���������� �����������. ���� ������� ����������.";}
	...
	if ($n==1){
		...
		if ($oper_status==30 && $status_jmoving>=44 && $status_jmoving<=47 && $storage_id_to>0) {
			...
			for ($im=1;$im<=$nm;$im++){ $select_id+=1;
				...
				$this->addJuornalRecord($jmov,$select_id,$status_jmoving);
	...
	for ($i=1;$i<=$n;$i++){
		...
		$list.="<tr id='strStsRow_$i'>
			...
			if ($jmoving_status>44){
				$list.="<td style='min-width:140px;'>���-$id</td>";	
			}
...
function showJmovingSkladStorageSelectList($jmoving_id,$jmoving_status){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$manual=new manual;$gmanual=new gmanual;$list="";
	...
	if ($jmoving_status>44){
		$r=$db->query("select sel.*, s.name as storage_name, t.name as tpoint_name from J_SELECT sel
						left outer join T_POINT t on t.id=sel.tpoint_id
						left outer join STORAGE s on s.id=sel.storage_id
						where sel.status=1 and sel.parrent_doc_type_id='1' and sel.parrent_doc_id='$jmoving_id' order by sel.status_select asc, sel.data_create desc, sel.id desc;");$n=$db->num_rows($r);
		for ($i=1;$i<=$n;$i++){
			$id=$db->result($r,$i-1,"id");
...
function viewJmovingStorageSelectLocal($jmoving_id,$select_id,$jmoving_status){$db=DbSingleton::getDb();$cat=new catalogue;$slave=new slave;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	...
	return array($form,"��������� ����������� ������ � ����-$select_id");
}

function dropJmovingStorageSelect($jmoving_id,$select_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ��������� ������!";
...
function dropJmovingStorageSelectLocal($jmoving_id,$select_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ��������� ������!";
	$jmoving_id=$slave->qq($jmoving_id);$select_id=$slave->qq($select_id);
	if ($jmoving_id>0 && $select_id>0){
...
function collectJmovingStorageSelect($jmoving_id,$select_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ������� ������!";
	$jmoving_id=$slave->qq($jmoving_id);$select_id=$slave->qq($select_id);
	if ($jmoving_id>0 && $select_id>0){
...
function collectJmovingStorageSelectLocal($jmoving_id,$select_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ������� ������!";
	$jmoving_id=$slave->qq($jmoving_id);$select_id=$slave->qq($select_id);
	if ($jmoving_id>0 && $select_id>0){
		$r=$db->query("select status_jmoving from J_MOVING_SELECT where jmoving_id='$jmoving_id' and id='$select_id' limit 0,1;");$n=$db->num_rows($r);
		if ($n==1){
...
function cutJmovingStorageAll($jmoving_id,$select,$comment){ $db=DbSingleton::getDb(); $slave=new slave; $ids=[]; $comment_sent="�������� � ID: ";
	session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ������� �����!";
	$jmoving_id=$slave->qq($jmoving_id);$select_id=$slave->qq($select_id);
...
function cutJmovingStorage($jmoving_id,$select_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ������� �����!";
	$jmoving_id=$slave->qq($jmoving_id);$select_id=$slave->qq($select_id);
	if ($jmoving_id>0 && $select_id>0){
		$r=$db->query("select count(id) as kol from J_MOVING_SELECT_STR_TEMP where jmoving_id='$jmoving_id' and select_id='$select_id';");$kol=$db->result($r,0,"kol");
		if ($kol>0){
...
function getJmovingSkladStorageSelectInfo($jmoving_id,$select_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$manual=new manual;$gmanual=new gmanual;
	$r=$db->query("select ms.*, p.name as tpoint_name, s.name as storage_name, ml.mcaption as loc_type_name, mt.mcaption as status_jmoving_name, j.storage_id_to, j.comment from J_MOVING_SELECT ms 
					...
					left outer join manual ml on ml.id=ms.loc_type_id
					left outer join manual mt on mt.id=ms.status_jmoving
					where ms.jmoving_id='$jmoving_id' and ms.id='$select_id' and ms.status='1' order by ms.id asc limit 0,1;");$n=$db->num_rows($r);
	if ($n=1){
		...
		$loc_type_id=$db->result($r,0,"loc_type_id");
		$loc_type_name=$db->result($r,0,"loc_type_name");
	...
	return array("���-$select_id/$storage_name",$data,$storage_id,$storage_name,$storage_name_to,$articles_amount,$amount,$volume,$weight_netto,$weight_brutto,$comment,$datatime);
}
...
function showJmovingStorageSelectBarcodeForm($jmoving_id,$select_id){$db=DbSingleton::getDb();$cat=new catalogue;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="������� �������";
	...
	for ($i=1;$i<=$n;$i++){
		...
		$list.="<tr>
			...
			<td align='center'><button class='btn btn-xs btn-default' onclick='showJmovingStorageSelectNoscanForm(\"$jmoving_id\",\"$select_id\",\"$art_id\",\"$id\");' title='Գ������ ��� ����������' alt='Գ������ ��� ����������'><i class='fa fa-cube'></i></button></td>
			<td align='center'><button class='btn btn-xs btn-danger' onclick='showJmovingStorageSelectBugForm(\"$jmoving_id\",\"$select_id\",\"$id\");' title='���������/����/���������' alt='���������/����/���������'><i class='fa fa-bug'></i></button></td>
...
function saveJmovingStorageSelectBarcodeForm($jmoving_id,$select_id,$barcode){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="������� �������!!";
...
function finishJmovingStorageSelectBarcodeForm($jmoving_id,$select_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="������� �������!!";
	$jmoving_id=$slave->qq($jmoving_id);$select_id=$slave->qq($select_id);
	if ($jmoving_id>0 && $select_id>0){
...
function finishJmovingLocalStorageSelect($jmoving_id,$select_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="������� �������!!";
	$jmoving_id=$slave->qq($jmoving_id);$select_id=$slave->qq($select_id);
	if ($jmoving_id>0 && $select_id>0){
		$db->query("update J_MOVING_SELECT set status_jmoving='47', amount='$new_amount' where jmoving_id='$jmoving_id' and id='$select_id' limit 1;");
		$this->addJuornalRecord($jmoving_id,$select_id,47);
...
/*
function setJmovingStorageSelectSendTruck($jmoving_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="������� �������!!";
	$jmoving_id=$slave->qq($jmoving_id);
	if ($jmoving_id>0){
		...
		if ($jmoving_amount<=0){$answer=0;$err="� ��������� ���� ������.\n ���������� ��� ������ �� �������������";}
	}
	return array($answer,$err,$id,$amount_barcodes,$dif_amount_barcodes);
}
*/
function setJmovingSendTruck($jmoving_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="������� �������!!"; 
	$jmoving_id=$slave->qq($jmoving_id); 
	if ($jmoving_id>0){
		$jmoving_amount=$this->checkJmovingAmountToTruck($jmoving_id);
		if ($jmoving_amount>0){
...
function printJmovingTruckList($jmoving_id){$db=DbSingleton::getDb();$cat=new catalogue;$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"]; $storsels=[];
	...
	for ($i=1;$i<=$n;$i++){
			$id=$db->result($r,$i-1,"id");
			$art_id=$db->result($r,$i-1,"art_id");
			$select_id=$db->result($r,$i-1,"select_id");//$select_ar[$select_id]=$select_id;
//			$storsel_list.="$storsel_prefix-$select_id ";
	...
	$form=str_replace("{tpoint_address}",$tpoint_address,$form);
	$form=str_replace("{jmoving_comment}",$jmoving_comment,$form);
	$form=str_replace("{user_name}",$user_name,$form);
	$form=str_replace("{curtime}",date("Y-m-d H:i:s"),$form);
...
function showJmovingStorageSelectBugForm($jmoving_id,$select_id,$str_id){$db=DbSingleton::getDb();$cat=new catalogue;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="������� �������";
	...
	if ($n==1){
		...
		$form=str_replace("{dif_amount_barcode}",$dif_amount_barcode,$form);	
		$form=str_replace("{amount}",$amount,$form);	
...
function saveJmovingStorageSelectBugForm($jmoving_id,$select_id,$str_id,$storage_select_bug,$amount_bug){$db=DbSingleton::getDb();$slave=new slave;$manual=new manual;$cat=new catalogue;$answer=0;$err="������� �������!!";
...
//					print "amount_barcodes=$amount_barcodes\n";
...
//				print "update J_MOVING_SELECT_STR set amount_bug='$new_amount_bug', amount_barcodes='$amount_barcodes', amount_barcodes_noscan='$amount_barcodes_noscan' where id='$id' limit 1;\n";
...
function showJmovingStorageAcceptForm($jmoving_id){$db=DbSingleton::getDb();$cat=new catalogue;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="������� �������";
...
//		$bug_list=$this->getJmovingBugList($jmoving_id,$art_id);
		...
		$list.="<tr>
			...
			<td align='center' id='amrd_$id'>$dif_amount_barcodes</td>
			<td align='center' id='amrns_$id'>$amount_barcodes_noscan</td>
			<td id='amrd_$id' align='center'><button class='btn btn-xs btn-default' onclick='showJmovingAcceptNoscanForm(\"$jmoving_id\",\"$art_id\",\"$id\");' title='Գ������ ��� ����������' alt='Գ������ ��� ����������'><i class='fa fa-cube'></i></button></td>
			<td id='amrd_$id' align='center'><button class='btn btn-xs btn-danger' onclick='showJmovingAcceptBugForm(\"$jmoving_id\",\"$id\");' title='���������/����/���������' alt='���������/����/���������'><i class='fa fa-bug'></i></button></td>
			<td align='center' id='ambg_$id'>$amount_bug</td>
...
function setJmovingStorageAcceptStart($jmoving_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="������� �������!!";
	$jmoving_id=$slave->qq($jmoving_id);
	if ($jmoving_id>0){
		$r=$db->query("select status_jmoving from J_MOVING where id='$jmoving_id' limit 0,1;");$n=$db->num_rows($r);
		if ($n==1){
...
function saveJmovingAcceptBarcodeForm($jmoving_id,$barcode){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="������� �������!!";
...
function showJmovingStorageSelectNoscanForm($jmoving_id,$select_id,$art_id,$str_id){$db=DbSingleton::getDb();$cat=new catalogue;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="������� �������";
...
function saveJmovingStorageSelectNoscanForm($jmoving_id,$select_id,$art_id,$str_id,$amount_barcode_noscan){$db=DbSingleton::getDb();$slave=new slave;$manual=new manual;$cat=new catalogue;$answer=0;$err="������� �������!!";
...
function showJmovingAcceptNoscanForm($jmoving_id,$art_id,$str_id){$db=DbSingleton::getDb();$cat=new catalogue;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="������� �������";
...
function saveJmovingAcceptNoscanForm($jmoving_id,$art_id,$str_id,$amount_barcode_noscan){$db=DbSingleton::getDb();$slave=new slave;$manual=new manual;$cat=new catalogue;$answer=0;$err="������� �������!!";
...
function showJmovingAcceptBugForm($jmoving_id,$str_id){$db=DbSingleton::getDb();$cat=new catalogue;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="������� �������";
...
function saveJmovingAcceptBugForm($jmoving_id,$str_id,$storage_select_bug,$amount_bug){$db=DbSingleton::getDb();$slave=new slave;$manual=new manual;$cat=new catalogue;$answer=0;$err="������� �������!!";
...
function finishJmovingAcceptForm($jmoving_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="������� �������!!";
	$jmoving_id=$slave->qq($jmoving_id);
	if ($jmoving_id>0){
		...
		if ($nc==0){
			...
			if ($storage_id_to>0){
				$query="select ss.* from J_SELECT_STR ss left outer join J_SELECT sel on sel.id=ss.select_id
				...
				for ($is=1;$is<=$ns;$is++){
					...
					if ($er_to==1 || $er_from==1){$is+=1;$answer=0;$err="������� ��������� ������� �� �����";}
				}
			}
				...
				if ($parrent_doc_id>0 && $parrent_type_id>0){
					if ($parrent_type_id==1){ // JMOVING FOR DP, create local storsel
						...
						for ($i1=1;$i1<=$n1;$i1++){
							...
							$s_amount+=$amount_to_storsel;	$s_articles_amount+=1; //$s_volume+=$volume;$s_weight_netto+=$weight_netto;
...
function finishJmovingLocalAcceptForm($jmoving_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="������� �������!!";
	$jmoving_id=$slave->qq($jmoving_id);
	if ($jmoving_id>0){
		$r=$db->query("select * from J_MOVING where id='$jmoving_id' limit 0,1;");$n=$db->num_rows($r);
		if ($n==1){
...
//				print "$art_id,$storage_id_to,$cell_id_from2,$cell_id_to2,$amount\n";
				
				$slave->addJuornalArtDocs(2,$jmoving_id,$art_id,$amount); 
				
				$er_from=$this->updateStockFromStorageLocal($art_id,$storage_id_to,$cell_id_from2,$cell_id_to2,$amount);
//				print "er_from=$er_from\n";
			}
		}
		if ($er_from==0){
			$prefix=$this->get_jmoving_prefix($jmoving_id);$doc_nom=0;
			$db->query("update J_MOVING set status_jmoving='57',`oper_status` = '31' where id='$jmoving_id' limit 1;");
			$r=$db->query("select max(doc_nom) as doc_nom from J_MOVING where oper_status='31' and status='1' and type_id='0' and prefix='�-��' and status_jmoving='57' limit 0,1;");$doc_nom=0+$db->result($r,0,"doc_nom")+1;
...
function separateJmovingByDefect($jmoving_id) {$db=DbSingleton::getDb(); $bugs=[]; $bug1=0; $bug2=0; $bug3=0; $bug4=0; $jarr=[];
	...
	if($n>0) {											  
		...
		if ($bug1>0) {$comment="`����` ����� ���������� $prefix-$doc_nom"; $jmoving1=$this->insertIntoJmoving($type_id,$storage_id_from,$cell_id_to,$comment);}
		if ($bug2>0) {$comment="`���������` ����� ���������� $prefix-$doc_nom"; $jmoving2=$this->insertIntoJmoving($type_id,$storage_id_from,$cell_id_to,$comment);}
		if ($bug3>0) {$comment="`��������` ����� ���������� $prefix-$doc_nom"; $jmoving3=$this->insertIntoJmoving($type_id,$storage_id_from,$cell_id_to,$comment);}
		if ($bug4>0) {$comment="`³����� �볺���` ����� ���������� $prefix-$doc_nom"; $jmoving4=$this->insertIntoJmoving($type_id,$storage_id_from,$cell_id_to,$comment);}
		...
		for($i=1; $i<=$n; $i++) {
			...
			if ($bugs[$i]["storage_select_bug"]==2) {$jstr=$this->insertIntoJmovingStr($jmoving2,$bugs[$i]["art_id"],$bugs[$i]["article_nr_displ"],$bugs[$i]["brand_id"],0,$bugs[$i]["storage_id_to"],$bugs[$i]["cell_id_to"],$bugs[$i]["amount_bug"]);}
			if ($bugs[$i]["storage_select_bug"]==3) {$jstr=$this->insertIntoJmovingStr($jmoving3,$bugs[$i]["art_id"],$bugs[$i]["article_nr_displ"],$bugs[$i]["brand_id"],0,$bugs[$i]["storage_id_to"],$bugs[$i]["cell_id_to"],$bugs[$i]["amount_bug"]);}
			if ($bugs[$i]["storage_select_bug"]==4) {$jstr=$this->insertIntoJmovingStr($jmoving4,$bugs[$i]["art_id"],$bugs[$i]["article_nr_displ"],$bugs[$i]["brand_id"],0,$bugs[$i]["storage_id_to"],$bugs[$i]["cell_id_to"],$bugs[$i]["amount_bug"]);}			
		}
