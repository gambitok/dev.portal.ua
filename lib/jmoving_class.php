...
class jmoving {
    ...
    function getArtId($code,$brand_id){$db=DbSingleton::getTokoDb();
        $slave=new slave;$id=0; $code=$slave->qq($code);$code=$this->clearArticle($code);
    ...
    function get_jmoving_prefix($jmoving_id){ $db=DbSingleton::getDb();
        $r=$db->query("SELECT `type_id` FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");$n=$db->num_rows($r);$prefix="ПР";
        if ($n==1){$type_id=$db->result($r,0,"type_id"); if ($type_id==0){$prefix="В-ПР";}}
        return $prefix;
    }

    function get_df_doc_nom_new(){ $db=DbSingleton::getDb();
        $r=$db->query("SELECT MAX(`doc_nom`) as doc_nom FROM `J_MOVING` WHERE `oper_status`='30' LIMIT 1;");$doc_nom=0+$db->result($r,0,"doc_nom")+1;
        return $doc_nom;
    }
    ...
    function newJmovingCard($type_id){$db=DbSingleton::getDb();
        ...
        $db->query("INSERT INTO `J_MOVING` (`id`,`type_id`,`prefix`,`doc_nom`,`user_id`,`data`) 
        VALUES ('$jmoving_id','$type_id','$this->prefix_new','$doc_nom','$user_id',CURDATE());");
        return $jmoving_id;
    }
    ...
    function getJmovingStorage($jmoving_id) {$db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");
        $storage_id=$db->result($r,0,"storage_id_to");
        return $storage_id;
    }
    ...
    function getJmovingNote($jmoving_id) { $db=DbSingleton::getDb();
        ...
        if ($n>0) $text=$db->result($r,0,"comment"); else $text="";
        return $text;
    }

    function getJmovingStatusId($jmoving_id){$db=DbSingleton::getDb();$status_jmoving=0;
        $r=$db->query("SELECT `status_jmoving` FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");$n=$db->num_rows($r);
    ...
    function statusStorage($user_id,$storage_id,$jmoving_id) {$db=DbSingleton::getDb();
        ...
        //склад відбору
        if (!$status) {
            $r=$db->query("SELECT `storage_id_from` FROM `J_MOVING_STR` WHERE `jmoving_id`=$jmoving_id LIMIT 1;"); $storage_id_from=$db->result($r,0,"storage_id_from");
            $r2=$db->query("SELECT * FROM `media_users_storage` WHERE `user_id`='$user_id' AND `storage_id`='$storage_id_from';"); $n=$db->num_rows($r2);
            $n>0 ? $status=true : $status=false;
        }
        //автор
        $rj=$db->query("SELECT `user_id` FROM `J_MOVING` WHERE `id`=$jmoving_id LIMIT 1;"); $j_user=$db->result($rj,0,"user_id");
    ...
    function show_jmoving_list($press = null){$db=DbSingleton::getDb();
        $gmanual=new gmanual;$income=new income;session_start();$media_user_id=$_SESSION["media_user_id"];
    ...
    function filterJmovingList($name,$data_from,$data_to,$status_jmoving){$db=DbSingleton::getDb();
        ...
        session_start();$ses_tpoint_id=$_SESSION["media_tpoint_id"];$media_user_id=$_SESSION["media_user_id"]; $press=false;$where_filter="";
    ...
    function getJMovingFullPrice($jmoving_id) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        $r=$db->query("SELECT * FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id';"); $n=$db->num_rows($r); $summ=0;
        for ($i=1;$i<=$n;$i++){
            $art_id=$db->result($r,$i-1,"art_id");
            $amount=$db->result($r,$i-1,"amount");
            $r1=$dbt->query("SELECT * FROM `T2_ARTICLES_PRICE_RATING` WHERE `art_id`='$art_id' AND `in_use`=1 LIMIT 1;"); $cash_id=$dbt->result($r1,0,"cash_id");
            $koursUSD=1;
            if ($cash_id==2) $koursUSD=$this->getKoursUSD();
    ...
    function showJmovingCard($jmoving_id) { $db = DbSingleton::getDb();
        ...
        if ($n==1){
            ...
            if ($user_accepting!="" && $user_data_accepting!="0000-00-00 00:00:00") $data_accepting="Приймається: {$this->getMediaUserName($user_accepting)}, $user_data_accepting";
            if ($user_accepted!="" && $user_data_accepted!="0000-00-00 00:00:00") $data_accepted="Прийнято: {$this->getMediaUserName($user_accepted)}, $user_data_accepted";
            ...
            if ($user_id==$user_use || $user_use==0){
                ...
                $storage_to_disabled=""; if ($status_jmoving!=44){$storage_to_disabled=" disabled";} if ($kol_art_str>0){$storage_to_disabled=" disabled"; }
                ...
                $disabled48="disabled"; if ($this->checkStorselAllStatus($jmoving_id)==1 && ($status_jmoving<48 || $status_jmoving==107)){$disabled48="";} //status В дорозі
                ...
                $form=str_replace("{disabled100}",$status_jmoving==44 || $status_jmoving>=48 ? "disabled style='display:none'" : "",$form);
    ...
    function cancelJmoving($jmoving_id) {$db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        $answer=0;$err="Помилка збереження даних!";$select_id=0;
        $r=$db->query("SELECT * FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;"); $n=$db->num_rows($r);
        if ($n>0) {
            $r=$db->query("SELECT * FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id';"); $n=$db->num_rows($r);
            for ($i=1;$i<=$n;$i++){
    ...
    function checkStorselAllStatus($jmoving_id){$db=DbSingleton::getDb();$ex=1;
        $r=$db->query("SELECT `status_select` FROM `J_SELECT` WHERE `parrent_doc_type_id`='1' AND `parrent_doc_id`='$jmoving_id' AND `status`='1';");$n=$db->num_rows($r);
        if ($n==0){$ex=0;}
        for ($i=1;$i<=$n;$i++){
    ...
    function checkJmovingSelectAllStatus($jmoving_id,$statusJmoving){$db=DbSingleton::getDb();$ex=1;
        ...
        $r=$db->query("SELECT `status_jmoving` FROM `J_MOVING` WHERE `id`='$jmoving_id' AND `status`='1' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==0){$ex=0;}
        if ($n==1){
            $status_jmoving=$db->result($r,0,"status_jmoving");
    ...
    function showJmovingStrList($jmoving_id,$oper_status,$storage_id_to){$db=DbSingleton::getDb();
        $list=""; $amount_barcodes=$amount_barcodes_noscan=$amount_bug=0;$slave=new slave;
    ...
    function showJmovingStrLocalList($jmoving_id,$oper_status){$db=DbSingleton::getDb();
        $slave=new slave;$list="";if ($oper_status==""){$oper_status=30;}
    ...
    function setArticleToJmoving($jmoving_id,$idStr,$artIdStr,$article_nr_displStr,$brandIdStr,$storageIdFromStr,$cellIdFromStr,$amountStr){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        $slave=new slave;$answer=0;$err="Змінилася кількість. Закрийте вікно введення даних і повторіть ще раз!";
            ...
            if ($amountS<=$max_moving && $rest_amount>0){
                ...
                if ($idS>0){
                    if ($artIdS!="" && $artIdS>0 && $article_nr_displS!=""){
                        ...
                        $rr=$dbt->query("SELECT * FROM `T2_ARTICLES_STRORAGE` WHERE `ART_ID`='$artIdS' and `STORAGE_ID`='$storageIdFromS' LIMIT 1;");$nr=$dbt->num_rows($rr);
                        if ($nr==1){
            ...
            if ($amountS<=$max_moving && $rest_amount>0){
                ...
                if ($idS>0){
                    if ($artIdS!="" && $artIdS>0 && $article_nr_displS!=""){
                        ...
                        //CELL SET RESERV
                        $rr=$dbt->query("SELECT * FROM `T2_ARTICLES_STRORAGE_CELLS` WHERE `ART_ID`='$artIdS' AND `STORAGE_ID`='$storageIdS' AND `STORAGE_CELLS_ID`='$cell_from_moveS' LIMIT 1;");$nr=$dbt->num_rows($rr);
                        if ($nr==1){
                            $rr_amount=$dbt->result($rr,0,"AMOUNT");
                            $rr_reserv=$dbt->result($rr,0,"RESERV_AMOUNT");
                        }
    ...
    function changeArticleToJmoving($jmoving_id,$jmoving_str_id,$amount_change){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        ...
        if ($jmoving_id>0){
            ...
            $r=$db->query("SELECT `amount`, `art_id`, `storage_id_from` FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id' AND `id`='$jmoving_str_id' AND `status_jmoving`='44' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $amountEx=$db->result($r,0,"amount");
                ...
                if ($amountS<=($max_moving+$amountEx)){
                    ...
                    $rr=$dbt->query("SELECT * FROM `T2_ARTICLES_STRORAGE` WHERE `ART_ID`='$art_id' and `STORAGE_ID`='$storage_id_from' LIMIT 1;");$nr=$dbt->num_rows($rr);
                    if ($nr==1){
                        $rr_amount=$dbt->result($rr,0,"AMOUNT");
                        $rr_reserv=$dbt->result($rr,0,"RESERV_AMOUNT");
    ...
    function changeArticleToJmovingLocal($jmoving_id,$jmoving_str_id,$amount_change){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        ...
        if ($jmoving_id>0){
            $jmoving_str_id=$slave->qq($jmoving_str_id);$amount_change=$slave->qq($amount_change);
            $r=$db->query("SELECT `amount`, `art_id`, `storage_id_from`, `cell_id_from` FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id' and `id`='$jmoving_str_id' and `status_jmoving`='44' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $amountEx=$db->result($r,0,"amount");
                $art_id=$db->result($r,0,"art_id");
                $storage_id_from=$db->result($r,0,"storage_id_from");
    ...
    function dropJmovingStr($jmoving_id,$jmoving_str_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        ...
        $r=$db->query("SELECT `oper_status`, `status_jmoving` FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $status_jmoving=$db->result($r,0,"status_jmoving");
            $oper_status=$db->result($r,0,"oper_status");
            if ($oper_status==30 && ($status_jmoving==44 ||$status_jmoving==45)){
                $r1=$db->query("SELECT * FROM `J_MOVING_STR` WHERE `id`='$jmoving_str_id' LIMIT 1;");$n1=$db->num_rows($r1);
                if ($n1==1){
    ...
    function dropJmovingLocalStr($jmoving_id,$jmoving_str_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        $slave=new slave;$answer=0;$err="Помилка індексу";$jmoving_id=$slave->qq($jmoving_id);
        $r=$db->query("SELECT `oper_status`, `status_jmoving` FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $status_jmoving=$db->result($r,0,"status_jmoving");
            $oper_status=$db->result($r,0,"oper_status");
            if ($oper_status==30 && ($status_jmoving==44 ||$status_jmoving==45)){
                ...
                if ($n1==1){
                    ...
                    if ($status_jmoving_str==44){
                        ...
                        $rs=$dbt->query("SELECT * FROM `T2_ARTICLES_STRORAGE_CELLS` WHERE `ART_ID`='$art_id' and `STORAGE_ID`='$storage_id_from' and `STORAGE_CELLS_ID`='$cell_id_from' LIMIT 1;");$ns=$dbt->num_rows($rs);
                        if ($ns==1){
                            $reserv_amount_s=$dbt->result($rs,0,"RESERV_AMOUNT");
                            $amount_s=$dbt->result($rs,0,"AMOUNT");
                            $reserv_amount_s-=$amount;
                            $amount_s+=$amount;
                            $dbt->query("UPDATE `T2_ARTICLES_STRORAGE_CELLS` SET `AMOUNT`='$amount_s', `RESERV_AMOUNT`='$reserv_amount_s' 
    ...
    function updateJmovingWeightVolume($jmoving_id){$db=DbSingleton::getDb();
        $sum_weight=0;$sum_volume=0;$empty_kol=0;$art_ar=array();
    ...
    function saveJmovingLocalAutoCell($jmoving_id,$storage_id_to,$cell_id_from,$cell_id_to){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        $slave=new slave;$jmoving_id=$slave->qq($jmoving_id);$storage_id_to=$slave->qq($storage_id_to);$cell_id_from=$slave->qq($cell_id_from);$cell_id_to=$slave->qq($cell_id_to);
        ...
        if ($jmoving_id>0 && $storage_id_to>0 && $cell_id_from>0 && ($cell_id_from!=$cell_id_to)){
            ...
            for ($ic=1;$ic<=$nc;$ic++){
                ...
                list($article_nr_displ,$brand_id,)=$this->getArticleNrDisplBrand($art_id); $idS="";
                $r=$db->query("SELECT `id`, `amount` FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id' and `art_id`='$art_id' and `storage_id_from`='$storage_id_to' and `status_jmoving`='44' limit 1;");$n=$db->num_rows($r);
                if ($n==1){
                    $idS=$db->result($r,0,"id");
                    $amountEx=$db->result($r,0,"amount");
                }
                ...
                if ($idS>0){
                    if ($art_id!="" && $art_id>0 && $article_nr_displ!=""){$kol_row+=1;
                        ...
                        $rr=$dbt->query("SELECT * FROM `T2_ARTICLES_STRORAGE_CELLS` WHERE `ART_ID`='$art_id' and `STORAGE_ID`='$storage_id_to' and `STORAGE_CELLS_ID`='$cell_id_from' limit 1;");$nr=$dbt->num_rows($rr);
                        if ($nr==1){
                            $rr_amount=$dbt->result($rr,0,"AMOUNT");
                            $rr_reserv=$dbt->result($rr,0,"RESERV_AMOUNT");
                        }
                        ...
                        $rr=$dbt->query("SELECT * FROM `T2_ARTICLES_STRORAGE` WHERE `ART_ID`='$art_id' and `STORAGE_ID`='$storage_id_to' limit 1;");$nr=$dbt->num_rows($rr);
                        if ($nr==1){
                            $rr_amount=$dbt->result($rr,0,"AMOUNT");
                            $rr_reserv=$dbt->result($rr,0,"RESERV_AMOUNT");
                        }
    ...
    function clearJmovingLocalAutoCellForm($jmoving_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        $slave=new slave;$answer=0;$err="Помилка індексу";
        $jmoving_id=$slave->qq($jmoving_id);
        $r=$db->query("SELECT `oper_status`, `status_jmoving` FROM `J_MOVING` WHERE `id`='$jmoving_id' limit 1;");$n=$db->num_rows($r);
        if ($n==1){
    ...
    function showJmovingArticleSearchForm($brand_id,$article_nr_display,$jmoving_id){
        ...
        $range_list=""; $list_brand_select="";
        if ($article_nr_display!="") list($range_list,$list_brand_select)=$this->showArticlesSearchDocumentList($article_nr_display,$brand_id,0,$jmoving_id);//$storage_id_to, art_id
    ...
    function showJmovingArticleLocalSearchForm($brand_id,$article_nr_display,$jmoving_id,$storage_id_from){
        ...
        $range_list=""; $list_brand_select="";
        if ($article_nr_display!="") list($range_list,$list_brand_select)=$this->showArticlesLocalSearchDocumentList($article_nr_display,$brand_id,0,$jmoving_id,$storage_id_from);
        $form=str_replace("{article_nr_display}",$article_nr_display,$form);
        $form=str_replace("{range_list}",$range_list,$form);
        $form=str_replace("{list_brand_select}",$list_brand_select,$form);
        return $form;
    ...
    function showArticlesSearchDocumentList($art,$brand_id,$search_type,$jmoving_id){$db=DbSingleton::getTokoDb();
        $n=0;$list2="";$r="";$query="";
...
//                LEFT OUTER JOIN T2_ARTICLES_STRORAGE t2asc on t2asc.ART_ID=t2a.ART_ID
//                LEFT OUTER JOIN STORAGE s on s.id=t2asc.STORAGE_ID
//            where t2a.ARTICLE_NR_SEARCH='$art' or t2a.ARTICLE_NR_DISPL='$art' group by t2a.BRAND_ID;";
            ...
            $query="SELECT t1.ART_ID, t1.BRAND_ID, t1.ARTICLE_NR_DISPL, t1.BRAND_NAME, t1.NAME, t1.INFO, t1.BARCODE, t1.goods_group_name as goods_group_name, t1.storage_name as storage_name, t1.storage_id as storage_id, SUM(t1.stock) as stock, SUM(t1.reserv) as reserv FROM (
                ...
                FROM `T2_ARTICLES` t2a 
                    ...
                    LEFT OUTER JOIN T2_ARTICLES_STRORAGE t2asc on t2asc.ART_ID=t2a.ART_ID 
                    LEFT OUTER JOIN STORAGE s on s.id=t2asc.STORAGE_ID
                WHERE t2a.ARTICLE_NR_SEARCH='$art' or t2a.ARTICLE_NR_DISPL='$art' GROUP BY s.id
    ...
    function showArticlesLocalSearchDocumentList($art,$brand_id,$search_type,$jmoving_id,$storage_id_from){$db=DbSingleton::getTokoDb();
        $n=0;$list2="";$r="";$query="";
        if ($search_type==""){$search_type=1;}
        if ($search_type==0){
            $art=$this->clearArticle($art);
    ...
    function showJmovingArticleAmountChange($art_id,$jmoving_str_id){$db=DbSingleton::getDb();
        ...
        $r=$db->query("SELECT * FROM `J_MOVING_STR` WHERE `id`='$jmoving_str_id' AND `status_jmoving`='44' LIMIT 1;");
    ...
    function showArticleRestStorageSelectText($art_id,$cur_amount,$storage_id=null){$db=DbSingleton::getTokoDb();
        $reserv_amount=$reserv_amount_storage=0;$max_moving=$amount=0;$info="";
    ...
    function showArticleRestStorageSelectList($art_id,$storage_id){$db=DbSingleton::getTokoDb();
        ...
        for ($i=1;$i<=$n;$i++){
            ...
            $reserv_amount=$db->result($r,$i-1,"RESERV_AMOUNT");
            $max_moving=$amount;
            if ($id==$storage_id){$sel=" disabled";$t="Сам себя..? ";} else {$sel="";$t="";}
    ...
    function showStorageCellsList($storage_id,$exclude_id=null){$db=DbSingleton::getTokoDb();
        $list="<option value='0'>-- Оберіть зі списку --</option>";
...
//        $stock=0;$reserv=0;
//        if ($storage_id==""){$storage_id=0;}if ($cell_id==""){$cell_id=0;}
...
//    }
    ...
    function showStorageSelectList($sel_id,$cells_only=0){$db=DbSingleton::getTokoDb();
        $list="<option value=0>Оберіть зі списку</option>";
        $query="SELECT * FROM `STORAGE` WHERE `status`='1' ORDER BY `name` ASC, `id` ASC;";
        if ($cells_only==1){
            $query="SELECT s.* FROM `STORAGE` s 
                INNER JOIN `STORAGE_STR` ss ON (ss.storage_id=s.id)
            WHERE s.status='1' GROUP BY ss.storage_id ORDER BY s.name ASC, s.id ASC;";
        }
        $r=$db->query($query);$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
    ...
    function showStorageCellsSelectList($storage_id,$sel_id){$db=DbSingleton::getTokoDb();
        ...
        $r=$db->query("SELECT * FROM `STORAGE_CELLS` WHERE `status`='1' AND `storage_id`='$storage_id' ORDER BY `cell_value` ASC, `id` ASC;");$n=$db->num_rows($r);
        if ($n==0){$cells_show=0;}
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
            $cell_value=$db->result($r,$i-1,"cell_value");
...
//    }
    ...
    function dropJmovingComment($jmoving_id,$comment_id){$db=DbSingleton::getDb();
        ...
        if ($jmoving_id>0 && $comment_id>0){
            $r=$db->query("SELECT * FROM `J_MOVING_COMMENTS` WHERE `jmoving_id`='$jmoving_id' AND `id`='$comment_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
    ...
    function jmovingCDNDropFile($jmoving_id,$file_id){$db=DbSingleton::getDb();
        ...
        if ($jmoving_id>0 && $file_id>0){
            $r=$db->query("SELECT `file_name` FROM `J_MOVING_CDN` WHERE `jmoving_id`='$jmoving_id' AND `id`='$file_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
...
//    function showCategoryCheckList($jmoving_id){$db=DbSingleton::getDb();
...
//        $r=$db->query("SELECT `category_id` FROM `A_CLIENTS_CATEGORY`
//        WHERE `jmoving_id`='$jmoving_id' AND `category_id`='$category_id' LIMIT 1;");$n=$db->num_rows($r);
//        $n==1 ? $ch=1 : $ch=0;
//        return $ch;
//    }

//    function showMovingOpListSelect($sel_id){$db=DbSingleton::getDb();
//        $r=$db->query("SELECT * FROM `J_MOVING_OP` WHERE `in_show`='1' ORDER BY `id` ASC;");$n=$db->num_rows($r);$list="";
//        for ($i=1;$i<=$n;$i++){
//            $id=$db->result($r,$i-1,"id");
//            $name=$db->result($r,$i-1,"name");
//            $sel="";if ($sel_id==$id){$sel="selected='selected'";}
...
//    }
    ...
    function showJmovingDocumentList($jmoving_id,$jmoving_op_id){
        $income=new income; $form=""; $document_list="";
        if ($jmoving_op_id==1){
    ...
    function labelCommentsCount($jmoving_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT COUNT(`id`) as kol FROM `J_MOVING_COMMENTS` WHERE `jmoving_id`='$jmoving_id';");$kol=0+$db->result($r,0,"kol");$label="";
        if ($kol>0){$label="<span class='label label-tab label-info'>$kol</span>";}
        return $label;
    }
    ...
    function showJmovingUnknownStrList($jmoving_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id' GROUP BY `art_id` ORDER BY `id` ASC;");$n=$db->num_rows($r);$empty_kol=0;$list="";
        for ($i=1;$i<=$n;$i++){
            $art_id=$db->result($r,$i-1,"art_id");
            $article_nr_displ=$db->result($r,$i-1,"article_nr_displ");
            $brand_id=$db->result($r,$i-1,"brand_id");$brand_name=$this->getBrandName($brand_id);
    ...
    function checkJmovingUnStr($jmoving_id,$art_id,$volume,$weight,$weight2){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        ...
        $r=$db->query("SELECT `oper_status` FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1) {
            $oper_status=$db->result($r,0,"oper_status");
            if ($oper_status==30) {
                $volume=$slave->qq($volume);$weight=$slave->qq($weight);$weight2=$slave->qq($weight2);
                if ($art_id>0 && $volume>0 && $weight>0 && $weight2>0) {
                    $rs=$db->query("SELECT `art_id` FROM `T2_PACKAGING` WHERE `art_id`='$art_id' LIMIT 1;");$ns=$dbt->num_rows($rs);
    ...
    function startJmovingStorageSelect($jmoving_id) {$db=DbSingleton::getDb();
        $slave=new slave;$answer=0;$err="";
        $jmoving_id=$slave->qq($jmoving_id);
        $r=$db->query("SELECT `oper_status`, `status_jmoving`, `storage_id_to` FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $oper_status=$db->result($r,0,"oper_status");
            $status_jmoving=$db->result($r,0,"status_jmoving");
            $storage_id_to=$db->result($r,0,"storage_id_to");
            ...
            if ($oper_status==30 && $status_jmoving>=44 && $status_jmoving<=47 && $storage_id_to>0) {
                ...
                $r1=$db->query("SELECT `storage_id_from` FROM `J_MOVING_STR` 
    ...
    function startJmovingStorageSelectLocal($jmoving_id) {$db=DbSingleton::getDb();
        $slave=new slave;$answer=0;$err="";
        $jmoving_id=$slave->qq($jmoving_id);
        $r=$db->query("SELECT `oper_status`, `status_jmoving`, `storage_id_to` FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            ...
            if ($oper_status==30 && $status_jmoving>=44 && $status_jmoving<=47 && $storage_id_to>0) {
                ...
                WHERE `jmoving_id`='$jmoving_id' AND `status_jmoving`='44' GROUP BY `cell_id_from` ORDER BY `cell_id_from` ASC;");$n1=$db->num_rows($r1);
                if ($n1==0){$answer=0;$err="Відсутній товар для створення відбору";}
                if($n1>0){
                    for ($i=1;$i<=$n1;$i++){
                        $storage_id_from=$db->result($r1,$i-1,"storage_id_from");
    ...
    function makesJmovingStorageSelect($jmoving_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        $slave=new slave;$answer=0;$err="";session_start();$user_id=$_SESSION["media_user_id"]; $jmoving_id=$slave->qq($jmoving_id);$storage_id_from=0;
        $r=$db->query("SELECT `oper_status`, `status_jmoving`, `storage_id_to` FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $oper_status=$db->result($r,0,"oper_status");
            $status_jmoving=$db->result($r,0,"status_jmoving");
...
//                        $parrent_doc_id=$dp_str_id; // J_DP_STR ID
    ...
    function loadJmovingStorageCount($jmoving_id) {$db=DbSingleton::getDb();
        ...
        if ($n==0) {
            ...
            WHERE ms.jmoving_id='$jmoving_id' and ms.status_jmoving='44' and ms.status='1' ORDER BY ms.id ASC;"); $n=$db->num_rows($r);
        }
        $n!=0 ?: $n="";
        return $n;
    }
    ...
    function showJmovingSkladStorageSelectListLocal($jmoving_id,$jmoving_status){$db=DbSingleton::getDb();
        ...
        $query="SELECT ms.*, p.name as tpoint_name, s.name as storage_name, mt.mcaption as status_jmoving_name 
        FROM $tmp ms 
            LEFT OUTER JOIN `T_POINT` p on p.id=ms.tpoint_id
            LEFT OUTER JOIN `STORAGE` s on s.id=ms.storage_id
            LEFT OUTER JOIN `manual` mt on mt.id=ms.status_jmoving
    ...
    function viewJmovingStorageSelect($jmoving_id,$select_id,$jmoving_status){$db=DbSingleton::getDb();
        $gmanual=new gmanual;$select_status=0; $form=""; $list="";
        if ($jmoving_status==44){
        ...
        if ($jmoving_status>44){
            $storsel=new storsel;$select_status=$jmoving_status;$gmanual=new gmanual;
    ...
    function collectJmovingStorageSelect($jmoving_id,$select_id){$db=DbSingleton::getDb();
        $slave=new slave;$answer=0;$err="Помилка обробки запису!";
        $jmoving_id=$slave->qq($jmoving_id);$select_id=$slave->qq($select_id);
        if ($jmoving_id>0 && $select_id>0){
            $r=$db->query("SELECT `status_jmoving` FROM `J_MOVING_SELECT` WHERE `jmoving_id`='$jmoving_id' AND `id`='$select_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $status_jmoving=$db->result($r,0,"status_jmoving");
    ...
    function cutJmovingStorageAll($jmoving_id,$select,$comment){ $db=DbSingleton::getDb();
        $slave=new slave; $ids=[]; $comment_sent="Передано в ID: ";
        ...
        foreach ($select as $select_id){
            ...
            if ($kol>0){
                ...
                $rm=$db->query("SELECT * FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");$nm=$db->num_rows($rm);
                if($nm==1){
    ...
    function cutJmovingStorage($jmoving_id,$select_id){$db=DbSingleton::getDb();
        $slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="Помилка обробки даних!";
        $jmoving_id=$slave->qq($jmoving_id);$select_id=$slave->qq($select_id);
        if ($jmoving_id>0 && $select_id>0){
            $r=$db->query("SELECT COUNT(`id`) as kol FROM `J_MOVING_SELECT_STR_TEMP` WHERE `jmoving_id`='$jmoving_id' AND `select_id`='$select_id';");$kol=$db->result($r,0,"kol");
            if ($kol>0){
    ...
    function getJmovingSkladStorageSelectInfo($jmoving_id,$select_id){$db=DbSingleton::getDb();
        ...
        FROM `J_MOVING_SELECT` ms 
            ...
            LEFT OUTER JOIN `manual` ml on ml.id=ms.loc_type_id
            LEFT OUTER JOIN `manual` mt on mt.id=ms.status_jmoving
        WHERE ms.jmoving_id='$jmoving_id' AND ms.id='$select_id' AND ms.status='1' ORDER BY ms.id ASC LIMIT 1;");
        $data=$db->result($r,0,"data");
    ...
    function printJmovingStorageSelect($jmoving_id,$select_id){$db=DbSingleton::getDb();
        $slave=new slave;session_start();$user_name=$_SESSION["user_name"];
    ...
    function addJuornalRecord($jmoving_id,$select_id,$status_jmoving){$db=DbSingleton::getDb();session_start();$user_id=$_SESSION["media_user_id"];
        $db->query("INSERT INTO `J_MOVING_SELECT_JOURNAL` (`jmoving_id`,`select_id`,`user_id`,`status_jmoving`,`status_moving`) 
        VALUES ('$jmoving_id','$select_id','$user_id','$jmoving_id','$status_jmoving');"); return;
    }
    ...
    function saveJmovingStorageSelectBarcodeForm($jmoving_id,$select_id,$barcode){$db=DbSingleton::getDb();
        ...
        if ($jmoving_id>0 && $select_id>0 && $barcode!=""){
            ...
            $r=$db->query("SELECT * FROM `J_MOVING_SELECT_STR` WHERE `jmoving_id`='$jmoving_id' AND `select_id`='$select_id' AND `art_id`='$art_id' AND amount>amount_barcodes LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $id=$db->result($r,0,"id");
                $amount=$db->result($r,0,"amount");
    ...
    function finishJmovingStorageSelectBarcodeForm($jmoving_id,$select_id){$db=DbSingleton::getDb();
        ...
        if ($jmoving_id>0 && $select_id>0){
            ...
            if ($n==0){
                ...
                FROM `J_MOVING_SELECT_STR` WHERE `jmoving_id`='$jmoving_id' AND `select_id`='$select_id';");$n=$db->num_rows($r);
                if ($n==1){
                    ...
                    FROM `J_MOVING_SELECT_STR` WHERE `jmoving_id`='$jmoving_id' AND `select_id`='$select_id';");$n1=$db->num_rows($r1);
                    for ($i=1;$i<=$n1;$i++){
    ...
    function finishJmovingLocalStorageSelect($jmoving_id,$select_id){$db=DbSingleton::getDb();
        $slave=new slave;$jmoving_id=$slave->qq($jmoving_id);$select_id=$slave->qq($select_id);$id=0;$new_amount=0;
        if ($jmoving_id>0 && $select_id>0){
            $db->query("UPDATE `J_MOVING_SELECT` SET `status_jmoving`='47', `amount`='$new_amount' WHERE `jmoving_id`='$jmoving_id' AND `id`='$select_id' LIMIT 1;");
            $this->addJuornalRecord($jmoving_id,$select_id,47);
            $answer=1;$err="";
    ...
    function setJmovingSendTruck($jmoving_id){$db=DbSingleton::getDb();
        ...
        if ($jmoving_id>0){
            ...
            if ($jmoving_amount>0){
                ...
                $r=$db->query("SELECT MAX(`doc_nom`) as doc_nom FROM `J_MOVING` WHERE `oper_status`='31' AND `status`='1' AND `type_id`='1' LIMIT 1;");$doc_nom=0+$db->result($r,0,"doc_nom")+1;
    ...
    function printJmovingTruckList($jmoving_id){$db=DbSingleton::getDb();
        $slave=new slave;session_start();$user_name=$_SESSION["user_name"]; $storsels=[];$list="";
        ...
        $r=$db->query("SELECT * FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id' ORDER BY `id` ASC;");$n=$db->num_rows($r);
        $storsel=new storsel;$storsel_prefix=$storsel->storsel_prefix; $dp=new dp;
        for ($i=1;$i<=$n;$i++){
            $art_id=$db->result($r,$i-1,"art_id");
            $select_id=$db->result($r,$i-1,"select_id");
    ...
    function printJmovingStorageSelectTruckList($jmoving_id,$select_id){$db=DbSingleton::getDb();
        $slave=new slave;session_start();$user_name=$_SESSION["user_name"];$list="";
    ...
    function showJmovingStorageSelectBugForm($jmoving_id,$select_id,$str_id){$db=DbSingleton::getDb();
        $manual=new manual;$answer=0;$err="Помилка індексу";
        $form="";$form_htm=RD."/tpl/jmoving_storage_select_bug_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("SELECT * FROM `J_MOVING_SELECT_STR` WHERE `jmoving_id`='$jmoving_id' AND `select_id`='$select_id' AND `id`='$str_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $art_id=$db->result($r,0,"art_id");
    ...
    function getStorageSelectBugList($jmoving_id,$select_id,$art_id,$str_id){$db=DbSingleton::getDb();
        ...
        //if ($n==0){$r=$db->query("select * from J_MOVING_SELECT_STR_BUG where jmoving_id='$jmoving_id' and select_id='$select_id' and art_id='$art_id' order by id asc;");$n=$db->num_rows($r);}
        for ($i=1;$i<=$n;$i++){
    ...
    function addStatusJmoving($jmoving_id,$status) {$db=DbSingleton::getDb();
        $user_id=$_SESSION["media_user_id"];$data=date("Y-m-d H:i:sa");
        if ($jmoving_id>0) {
            $r=$db->query("SELECT * FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");
            $user_accepting=$db->result($r,0,"user_accepting");
            $user_accepted=$db->result($r,0,"user_accepted");
    ...
    function showJmovingStorageAcceptForm($jmoving_id) {$db=DbSingleton::getDb();
        $list="";session_start();$user_name=$_SESSION["user_name"];
    ...
    function setJmovingStorageAcceptStart($jmoving_id){$db=DbSingleton::getDb();
        $slave=new slave;$jmoving_id=$slave->qq($jmoving_id);
        if ($jmoving_id>0){
            $r=$db->query("SELECT `status_jmoving` FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $status_jmoving=$db->result($r,0,"status_jmoving");
    ...
    function saveJmovingAcceptBarcodeForm($jmoving_id,$barcode){$db=DbSingleton::getDb();
        ...
        if ($jmoving_id>0 && $barcode!=""){
            $art_id=$this->getArtIdByBarcode($barcode);
            $r=$db->query("SELECT * FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id' AND `art_id`='$art_id' AND `amount`>`amount_barcodes` LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $id=$db->result($r,0,"id");
                $amount=$db->result($r,0,"amount");
                $amount_barcodes=$db->result($r,0,"amount_barcodes");
    ...
    function saveJmovingStorageSelectNoscanForm($jmoving_id,$select_id,$art_id,$str_id,$amount_barcode_noscan){$db=DbSingleton::getDb();
        ...
        if ($jmoving_id>0 && $select_id>0 && $art_id>0 && $str_id>0 && $amount_barcode_noscan>0){
            $r=$db->query("SELECT * FROM `J_MOVING_SELECT_STR` WHERE `jmoving_id`='$jmoving_id' AND `art_id`='$art_id' AND `id`='$str_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $id=$db->result($r,0,"id");
                $amount=$db->result($r,0,"amount");
    ...
    function showJmovingAcceptNoscanForm($jmoving_id,$str_id){$db=DbSingleton::getDb();
        ...
        $r=$db->query("SELECT * FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id' AND `id`='$str_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $art_id=$db->result($r,0,"art_id");
            $article_nr_displ=$db->result($r,0,"article_nr_displ"); $article_name=$this->getArticleName($art_id);
            $brand_id=$db->result($r,0,"brand_id");$brand_name=$this->getBrandName($brand_id);
    ...
    function saveJmovingAcceptNoscanForm($jmoving_id,$art_id,$str_id,$amount_barcode_noscan){$db=DbSingleton::getDb();
        ...
        if ($jmoving_id>0 && $art_id>0 && $str_id>0 && $amount_barcode_noscan>0){
            $r=$db->query("SELECT * FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id' AND `art_id`='$art_id' AND `id`='$str_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $id=$db->result($r,0,"id");
                $amount=$db->result($r,0,"amount");
                $amountBarcodes=$db->result($r,0,"amount_barcodes");
    ...
    function checkJmovingBugs($jmoving_id) {$db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id';"); $n=$db->num_rows($r); $kol=0;
        for ($i=1;$i<=$n;$i++){
            $amount_bug=$db->result($r,$i-1,"amount_bug");
    ...
    function finishJmovingAcceptForm($jmoving_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        $slave=new slave;$dp=new dp;$answer=0;$err="Помилка індексу!!";
                ...
                if ($er_to==0 && $er_from==0){
                    ...
                    if ($parrent_doc_id>0 && $parrent_type_id>0){
                        if ($parrent_type_id==1){ // JMOVING FOR DP, create local storsel
                            $dp_id=$parrent_doc_id; $tpoint_id=$dp->getDpTpoint($dp_id);
    ...
    function finishJmovingLocalAcceptForm($jmoving_id){$db=DbSingleton::getDb();
        $slave=new slave;$answer=0;$err="Помилка індексу!!";$id=0;
        ...
        if ($n==1){
            ...
            if ($amount<=$t2s_reserv_amount){
                ...
                if ($cell_use==1){
                    ...
                    WHERE `ART_ID`='$art_id' AND `STORAGE_ID`='$storage_id_from' AND `STORAGE_CELLS_ID`='$cell_id_from' LIMIT 1;");$n1=$dbt->num_rows($r1);
                    if ($n1==1){
    ...
    function showSelectStatusList() {$db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `manual` WHERE `key`='status_jmoving' ORDER BY `mid` ASC;"); $n=$db->num_rows($r); $list="";
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
    ...
    function separateJmovingByDefect($jmoving_id) {$db=DbSingleton::getDb();
        $bugs=[]; $bug1=0; $bug2=0; $bug3=0; $bug4=0; $storage_id_from=0; $jmoving1=$jmoving2=$jmoving3=$jmoving4=0;
    ...
    function insertIntoJmovingStr($jmoving_id,$art_id,$article_nr_displ,$brand_id,$storage_id_to,$cell_id_from,$amount_bug) { $db=DbSingleton::getDb();
        ...
        $nstr=$db->num_rows($rstr); $old_amount_bug=$db->result($rstr,0,"amount_bug"); $id=0;
        if ($nstr>0) {
    ...
    function cellsJmovingDefect($art_id,$storage_to_id,$storage_from_id,$cell_to_id,$cell_from_id,$reserv) { $dbt=DbSingleton::getTokoDb();
        ...
        if ($n==0){
            $dbt->query("INSERT INTO `T2_ARTICLES_STRORAGE_CELLS` (ART_ID,AMOUNT,RESERV_AMOUNT,STORAGE_ID,STORAGE_CELLS_ID) 
            VALUES ('$art_id','$amount','0','$storage_to_id','$cell_to_id');");
        } else {
    ...
    function clearArticle($art){
        $art=str_replace(" ","",$art);$art=str_replace("_","",$art);$art=str_replace("-","",$art);$art=str_replace(".","",$art);$art=str_replace("+","",$art);$art=str_replace("'","",$art);
        $art=str_replace("/","",$art);$art=str_replace('"',"",$art);$art=preg_replace ("/[^a-zA-ZА-Яа-я0-9\s]/","",$art);$art=strtolower($art);
        return $art;
    }

    function showCatalogueBrandSelectDocumentList($r){$db=DbSingleton::getTokoDb();
        $list="";$n=$db->num_rows($r);$tkey=time();$form="";
    ...
    function loadJmovingImport($jmoving_id) {
        $form="";$form_htm=RD."/tpl/jmoving_import_str_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $header="";$header_htm=RD."/tpl/jmoving_import_str_header.htm";if (file_exists("$header_htm")){ $header = file_get_contents($header_htm);}
    ...
    function getCsvIndexBrands($jmoving_id) {$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        $brands=[]; $list="";
        $r=$db->query("SELECT * FROM `J_MOVING_IMPORT` WHERE `jmoving_id`='$jmoving_id' AND `status`=0;"); $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
            $article_nr_displ = $db->result($r, $i - 1, "article_nr_displ");
    ...
    function showTablePreview($jmoving_id){
        $form="";$form_htm=RD."/tpl/csv_str_jmoving_import.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $form=str_replace("{jmoving_id}",$jmoving_id,$form);
    ...
    function loadTablePreview($jmoving_id,$brands=0) {$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        ...
        $r=$db->query("SELECT * FROM `J_MOVING_IMPORT` WHERE `jmoving_id`='$jmoving_id' $where_arts ORDER BY `status` ASC, `id` ASC;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
            $id = $db->result($r, $i - 1, "id");
            $article_nr_displ = $db->result($r, $i - 1, "article_nr_displ");
            $amount = $db->result($r, $i - 1, "amount");
        ...
        foreach ($mas as $val) {$i++;
            ...
            if ($brands!=0) $db->query("UPDATE `J_MOVING_IMPORT` SET `selected`=1 WHERE `id`='$id';");
        }
        return $table;
    }
    ...
    function saveTablePreview($jmoving_id,$brands=0) {$db=DbSingleton::getDb();
        ...
        if ($jmoving_id>0 && $brands!=0) {
            $r=$db->query("SELECT * FROM `J_MOVING_IMPORT` WHERE `jmoving_id`='$jmoving_id' AND `status`=0 AND `selected`=1;");$n=$db->num_rows($r);
            for ($i=1;$i<=$n;$i++) {
                $id = $db->result($r, $i - 1, "id");
                $article_nr_displ = $db->result($r, $i - 1, "article_nr_displ");
                $brand_id=$brands;
                $art_id=$this->getArtId($article_nr_displ,$brand_id);
                if ($art_id!=0 && $brand_id!=0) $db->query("UPDATE `J_MOVING_IMPORT` SET `brand_id`='$brand_id', `art_id`='$art_id', `status`=1 WHERE `id`='$id';");
            }
            $answer=1;$err="";
        }
        return array($answer,$err);
    ...
    function saveCsvJmovingImport($jmoving_id,$start_row,$kol_cols,$cols){$db=DbSingleton::getDb();
        ...
        $db->query("DELETE FROM `J_MOVING_IMPORT` WHERE `jmoving_id`='$jmoving_id';");
        if ($jmoving_id>0){
            $r=$db->query("SELECT * FROM `J_MOVING_CSV` WHERE `jmoving_id`='$jmoving_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $file_name=$db->result($r,0,"file_name");
                ...
                if (file_exists($file_path)){
                    ...
                    if ($handle) {
                        ...
                        while (($buffer = fgets($handle, 4096)) !== false) {$fn+=1;
                            ...
                            if ($buffer!=""){
                                if ($fn>=$start_row){
                                    ...
                                    $r2=$db->query("SELECT MAX(`id`) as mid FROM `J_MOVING_IMPORT`;");$import_id=0+$db->result($r2,0,"mid")+1;
                                    $rs=$db->query("SELECT * FROM `J_MOVING_IMPORT` WHERE `jmoving_id`='$jmoving_id' AND `art_id`='$art_id' LIMIT 1;"); $ns=$db->num_rows($rs);
                                    if ($ns>0 && $art_id>0) {
                        ...
                        fclose($handle);
                        if (file_exists(RD."/cdn/jmoving_files/csv/$jmoving_id/$file_name")){unlink(RD."/cdn/jmoving_files/csv/$jmoving_id/$file_name");}
    ...
    function getJmovingStockInfo($art_id,$jmoving_id) { $db=DbSingleton::getTokoDb();
        ...
        if ($art_id>0) {
            ...
            for ($i=1;$i<=$n;$i++) {
                ...
                if (in_array($storage_id,$this->getUserActiveStorages($jmoving_id))) $list.="<span style='font-weight: bold;'>$storage_name</span>($amount); ";
                $full_amount+=$amount;
            }
        }
        $list=trim($list,"; ");
    ...
    function getUserActiveStorages($jmoving_id) { $db=DbSingleton::getDb();
        ...
        $r=$db->query("SELECT * FROM `CFN_USERS_STORAGE_CONFIG` WHERE `user_id`='$user_id' AND `field_active`=1 AND `storage_id`!='$storage_id_to';"); $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
            $storage_id = $db->result($r, $i - 1, "storage_id");
    ...
    function moveArticlesStorage($jmoving_id,$art_id,$article_nr_displ,$brand_id,$amount,$storages) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        ...
        if ($summ<$amount && $summ>0) {
            ...
            $amount=$summ;
            $r2=$db->query("SELECT MAX(`id`) as mid FROM `J_MOVING_STR_UNKNOWN_ARTS`;");$unknown_id=0+$db->result($r2,0,"mid")+1;
    ...
    function showJmovingStorageFieldsViewForm($jmoving_id) {$db=DbSingleton::getDb();
        session_start();$user_id=$_SESSION["media_user_id"];
        $form="";$form_htm=RD."/tpl/jmoving_storage_fields_view_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("SELECT * FROM `CFN_USERS_STORAGE_CONFIG` WHERE `user_id`='$user_id';"); $n=$db->num_rows($r); if ($n==0) $user_id=0;
        $storage_id_to=$this->getJmovingStorage($jmoving_id);
        ...
        for ($i=1;$i<=$n;$i++){
            ...
            if ($storage_id_to==$storage_id) {$checked="";$disabled="disabled";$cap=" (Склад призначення!)";} else {$disabled="";$cap="";}
    ...
    function showCsvPreview($jmoving_id){$db=DbSingleton::getDb();
        ...
        if ($n==1){
            ...
            $file_path=RD."/cdn/jmoving_files/csv/$jmoving_id/$file_name";
            if (file_exists($file_path)){
                $form="";$form_htm=RD."/tpl/csv_str_jmoving_file.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
    ...
    function finishJmovingImport($jmoving_id){$db=DbSingleton::getDb();
        $slave=new slave;$answer=0;$err="Помилка збереження даних!";$jmoving_id=$slave->qq($jmoving_id);
        if ($jmoving_id>0){
            $db->query("DELETE FROM `J_MOVING_STR_UNKNOWN_ARTS` WHERE `jmoving_id`='$jmoving_id';");
            $r=$db->query("SELECT * FROM `J_MOVING_IMPORT` WHERE `jmoving_id`='$jmoving_id';");$n=$db->num_rows($r);
            for ($i=1;$i<=$n;$i++) {
                $id = $db->result($r, $i - 1, "id");
                $article_nr_displ = $db->result($r, $i - 1, "article_nr_displ");
                $amount = $db->result($r, $i - 1, "amount");
    ...
    function getUserStorages($jmoving_id) {$db=DbSingleton::getDb();
        ...
        $r=$db->query("SELECT * FROM `CFN_USERS_STORAGE_CONFIG` WHERE `user_id`=$user_id;"); $n=$db->num_rows($r); if ($n==0) $user_id=0;
        $storage_id_to=$this->getJmovingStorage($jmoving_id);
        $r=$db->query("SELECT * FROM `CFN_USERS_STORAGE_CONFIG` WHERE `user_id`=$user_id AND `field_active`=1 AND `storage_id`!='$storage_id_to' ORDER BY `field_pos` ASC;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $storage_id=$db->result($r,$i-1,"storage_id");
    ...
    function loadJmovingUnknown($jmoving_id) {$db=DbSingleton::getDb();
        $form="";$form_htm=RD."/tpl/jmoving_unknown_list.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("SELECT * FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==0){$form_htm=RD."/tpl/access_deny.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);} }
        if ($n==1){
    ...
    function showJmovingUnknownList($jmoving_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `J_MOVING_STR_UNKNOWN_ARTS` WHERE `jmoving_id`='$jmoving_id' ORDER BY `id` ASC;");$n=$db->num_rows($r);$empty_kol=0;$list="";
        for ($i=1;$i<=$n;$i++){
            $art_id=$db->result($r,$i-1,"art_id");
            $article_nr_displ=$db->result($r,$i-1,"article_nr_displ");
            $brand_id=$db->result($r,$i-1,"brand_id");$brand_name=$this->getBrandName($brand_id);
