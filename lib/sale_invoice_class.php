...
class sale_invoice {
    ...
    function show_sale_invoice_list()
    {
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            $summ_cap = ($summ_debit == 0) ? "" : "$summ_debit $cash_abr";
    ...
    function show_sale_invoice_list_filter($data_start, $data_end, $prefix_sel, $doc_nom_sel)
    {
        $db = DbSingleton::getDb();
        session_start();
        $dp = new dp;
    ...
    function checkTaxExist($invoice_id)
    {
        $db = DbSingleton::getDb();
        $tax_id = 0;
    ...
    function showSaleInvoiceCard($invoice_id)
    {
        ...
        if ($n == 1) {
            ...
            $form = str_replace("{cb_check_visible_add}", ($this->checkCBCheck($client_id, $user_id) && ($summ == $sum_pay && $summ_debit == 0) && $nn == 0) ? "" : "none", $form);
            $form = str_replace("{cb_check_visible_show}", ($this->checkCBCheck($client_id, $user_id) && ($summ == $sum_pay && $summ_debit == 0) && $nn > 0) ? "" : "none", $form);
    ...
    public function checkCBCheck($client_id, $user_id)
    {
        $db = DbSingleton::getDb();
    ...
    public function checkCBCheckStr($invoice_id, $doc_type_id = 1)
    {
        $status = 0;
        $no_names = [];
        $db = DbSingleton::getDb();
    ...
    public function getCbCheckStr($invoice_id, $doc_type_id = 1, $cashier_name = "", $department = "", $payment_id = 1, $email = "")
    {
        $db = DbSingleton::getDb();
        $answer = 0; $err = "";
        $receipt = [];
    ...
    public function showCbCheckForm($invoice_id, $doc_type_id)
    {
        ...
        if ($doc_type_id == 2) {
            ...
            $n = $db->num_rows($r);
            if ($n > 0) {
                $payment_id_sel = $db->result($r, 0, "PAY_TYPE_ID");
        ...
        $form = str_replace("{radio_email_range}", $list, $form);
        $form = str_replace("{radio_payment_range}", $list_p, $form);

        return $form;
    }

    public function addCbCheck($invoice_id, $doc_type_id = 1, $payment_id = 1, $email = "")
    {
        session_start();
        $user_id = $_SESSION["media_user_id"];
        $db = DbSingleton::getDb();
    ...
    public function showCbCheck($invoice_id, $doc_type_id = 1)
    {
        $check_id = 0;
        $answer = 0;
    ...
    function savePartitionsInvoiceAmount($partition_id, $invoice_amount)
    {
        $db = DbSingleton::getDb();
        if ($partition_id > 0) {
    ...
    function showCashListSelect($sel_id, $ns = "")
    {
        $db = DbSingleton::getDb();
        if ($ns == "") {
            $ns = 1;
    ...
    function showTpointPayBoxSelectList($client_id, $tpoint_id)
    {
        $db = DbSingleton::getDb();
        $list = "";
    ...
    function saveSaleInvoiceMoneyPay($invoice_id, $pay_id, $kredit, $pay_type_id, $paybox_id, $doc_cash_id, $cash_id, $cash_kours)
    {
        $db = DbSingleton::getDb();
        $slave = new slave;
        session_start();
    ...
    /*TEST*/
    function getSellerDetails($client_id, $seller_id)
    {
        $db = DbSingleton::getDb();
    ...
    function printSaleInvoice($invoice_id, $type)
    {
        if ($type == "" || $type == 0) {
            $type = 1;
        }
        ...
        if ($n == 1) {
            ...
            list($edrpou, $account, $bank, $mfo, $vat) = $this->getSellerDetails($client_cont_id, $seller_id);
...
//            if ($n > $max_row && $doc_type_id == 64) {$form_htm=RD."/tpl/dp_sale_invoice_print_64_a4.htm";}
            ...
            $mandate_data   = ($mandate_data != 0) ? date("d.m.Y", strtotime($mandate_data)) : "";
...
//            if ($n > $max_row && $doc_type_id == 64) { $mp->print_document($form,"A4"); }
    ...
    function getMandateData($client_id, $data)
    {
        $db = DbSingleton::getDb();
    ...
    function getBasisData($client_id, $data)
    {
        $db = DbSingleton::getDb();
        $number = $data_from = "";
    ...
    function printSaleInvoiceBuh($invoice_id)
    {
        $db = DbSingleton::getDb();
        $cat = new catalogue;
        $slave = new slave;
    ...
    function exportSaleInvoiceExcel($invoice_id, $separator = "")
    {
        $db = DbSingleton::getDb();
        $cat = new catalogue;
        ...
        if ($n == 1) {
            ...
            for ($i = 1; $i <= $n; $i++) {
               ...
               // $invoice_summ += $summ;
                ...
                if ($doc_type_id == 61) {
                    array_push($list, "$i;$article_nr_displ;$brand_name;$zed;$article_name;$amount;$price_format;$summ_format\n");
                } else {
    ...
    function getClientInvoiceCron($invoice_id)
    {
        $db = DbSingleton::getDb();
        $list = "";
    ...
    function createSaleInvoiceExcel($invoice_id, $separator, $name, $email, $mail_id)
    {
        $db = DbSingleton::getDb();
    ...
    function sendMail($receiver, $user_name, $filename, $doc_name, $data_doc, $mail_id)
    {
        $db = DbSingleton::getDb();
