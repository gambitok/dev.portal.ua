...
class report_clients {
    ...
    function getCashList() { $db = DbSingleton::getDb();
        $list="";
        $r=$db->query("SELECT * FROM `CASH`;");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r, $i - 1, "id");
            $name=$db->result($r, $i - 1, "name");
            if ($id==1) $selected="selected"; else $selected="";
    ...
    function getClientsList($clients=NULL) { $db = DbSingleton::getDb();
        $list="";
        $r=$db->query("SELECT * FROM `A_CLIENTS` WHERE `status`='1';");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            ...
            if ($clients!=null) {
                ...
                if (in_array($client_id, $arr_clients)) {
                    $sel="selected";
                }
            }
    ...
    function getTpointList() { $db = DbSingleton::getDb();
        $list="";
        $r=$db->query("SELECT * FROM `T_POINT` WHERE `status`='1';");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r, $i - 1, "id");
            $name=$db->result($r, $i - 1, "name");
    ...
    function getClientCategoryList() { $db = DbSingleton::getDb();
        $list = "";
        $r = $db->query("SELECT * FROM `manual` WHERE `key`='customers_categories' ORDER BY mid ASC;");
        $n = $db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id = $db->result($r, $i - 1, "id");
    ...
    function getGoodsGroupList() { $db = DbSingleton::getTokoDb();
        $list="";
        $r=$db->query("SELECT * FROM `GOODS_GROUP`;");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r, $i - 1, "ID");
            $parrent_id=$db->result($r, $i - 1, "PARRENT_ID");
            $parrent_id ? $pr="" : $pr="$";
            $name=$db->result($r, $i - 1, "NAME");
	...
	function getClientLocation($client_id) { $db = DbSingleton::getDb();
		...
		$tpoint=$this->getTpointName($tpoint_id);
	...
	function getSummReportsSales($date_start, $date_end, $cash_id, $client_id) { $db = DbSingleton::getDb();
		...
		WHERE `time_stamp`>='$date_start 00:00:00' AND `time_stamp`<='$date_end 23:59:59' AND `client_conto_id`='$client_id';");
	    $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
	...
	function getSummReportsBacks($date_start, $date_end, $cash_id, $client_id) { $db = DbSingleton::getDb();
		...
		WHERE j.time_stamp>='$date_start 00:00:00' AND j.time_stamp<='$date_end 23:59:59' AND js.client_conto_id='$client_id' AND j.status_back=103;");
		$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
			$usd_to_uah=$db->result($r, $i - 1, "usd_to_uah");
			$eur_to_uah=$db->result($r, $i - 1, "eur_to_uah");
    ...
    function getClientJpay($client_id, $cash_id, $date_start, $date_end) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT j.* FROM `J_PAY` j WHERE j.`client_id`='$client_id' $where_date");
        $n = $db->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
    ...
    function getClientSumInvoice($client_id, $cash_id) { $db = DbSingleton::getDb();
        $income = new income;
        $summary = 0;
        $r = $db->query("SELECT `invoice_id` FROM `J_SALE_INVOICE_PROLONGATION` WHERE `client_id`='$client_id';");
        $n = $db->num_rows($r);
        if ($n>0) {
            for ($i=1; $i<=$n; $i++) {
	...
	function showReportClients($date_start, $date_end, $clients, $cash_id, $tpoint_id, $client_category) { $db = DbSingleton::getDb();
		...
		$form = ""; $form_htm=RD."/tpl/report_clients_table.htm";
    ...
    function showAnalyticsClients($date_start, $date_end, $clients, $cash_id, $tpoint_id, $price_id, $margin_status, $states, $regions, $citys) { $db = DbSingleton::getDb();
        ...
        $summ_list=0; $list=""; $sales=$backs=[];
        $form = "";$form_htm=RD."/tpl/analytics_clients_table.htm";
        if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        if ($clients=="") $where=""; else $where="j.client_conto_id IN ($clients) AND ";
        if ($tpoint_id=="0") $where_tpoint=""; else $where_tpoint="AND cc.tpoint_id=$tpoint_id";
    ...
    function getClientsSales($client_id, $month, $cash_id) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT * FROM `J_CLIENTS_SALE` WHERE `client_id`='$client_id' AND `month`='$month' AND `status`=1;");
        $n = $db->num_rows($r);
        if ($n>0) {
    ...
    function addClientSales($client_id, $month, $s_uah, $s_usd, $s_eur) { $db = DbSingleton::getDb();
        $r = $db->query("SELECT * FROM `J_CLIENTS_SALE` WHERE `client_id`='$client_id' AND `month`='$month' AND `status`=1 LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n==0) {
            $db->query("INSERT INTO `J_CLIENTS_SALE` (`client_id`, `month`, `s_uah`, `s_usd`, `s_eur`, `status`) VALUES ('$client_id', '$month', '$s_uah', '$s_usd', '$s_eur', '1');");
        }
        return true;
    }
    ...
    function getClientPrice($client_id, $date_start, $date_end) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT * FROM `A_CLIENTS_CONDITIONS_HISTORY` WHERE `client_id`='$client_id' AND `data`>='$date_start 00:00:00' AND `data`<='$date_end 23:59:59';");
        $n = $db->num_rows($r);
        if ($n>0) {
            $tn = 0;
            for ($i=1; $i<=$n; $i++) {
    ...
    function getLocationList($key) { $db = DbSingleton::getDb();
        $list = "";
        $r = $db->query("SELECT * FROM `T2_$key` ORDER BY `".$key."_NAME` ASC;");
        $n = $db->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
    ...
    function getLocationClientsList($states, $regions, $citys) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT * FROM `A_CLIENTS` WHERE `status`=1 $where;");
        $n = $db->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
            $client_id = $db->result($r, $i - 1, "id");
    ...
    function getPriceClientsList($price_id) { $db = DbSingleton::getDb();
        ...
        if ($price_id==0) {
            ...
            $price_id = $price_id - 1;
            $r = $db->query("SELECT * FROM `A_CLIENTS_CONDITIONS` WHERE `price_lvl`='$price_id';");
            $n = $db->num_rows($r);
            for ($i=1; $i<=$n; $i++) {
                $client_id = $db->result($r, $i - 1, "client_id");
                array_push($clients, $client_id);
    ...
    function updateCitysRange($text, $citys_selected) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT *  FROM `T2_CITY` WHERE `CITY_NAME` LIKE '%$text%' $where;");
        $n = $db->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
    ...
    function getParrentGoodsGroup($goods_group) { $db=DbSingleton::getTokoDb();
        $parrent_ids = [];
        $r = $db->query("SELECT * FROM `GOODS_GROUP` WHERE `PARRENT_ID` IN ($goods_group);");
        $n = $db->num_rows($r);
        if ($n>0) {
            for ($i=1;$i<=$n;$i++) {
                $id = $db->result($r, $i - 1, "ID");
    ...
    function getArticlesStorageArray($brands, $goods_group) { $db = DbSingleton::getTokoDb();
        ...
        $r = $db->query("SELECT `ART_ID`, (`AMOUNT`+`RESERV_AMOUNT`) as total FROM `T2_ARTICLES_STRORAGE` HAVING total > 0;");
        $n = $db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
            $art_id = $db->result($r, $i - 1, "ART_ID");
            list($brand_id, $goods_group_id) = $this->getArticleIdInfo($art_id);
    ...
    function showReportSalesArticles($date_start, $date_end, $brands, $goods_group, $client_ids, $params) {
        $list=""; $form = ""; $form_htm=RD."/tpl/report_sales_articles_table.htm";
        if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        ...
        foreach ($arts as $art_id) {
            ...
            if ($order<0) $style_order = "style='background:pink;'"; else $style_order = "";
    ...
    function getIncomeArticleInfo($art_id) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT * FROM `J_ART_DOCS` WHERE `art_id`='$art_id' AND `doc_type`=1 ORDER BY `id` DESC LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n>0) {
    ...
    function getAmountSalesArticle($data_start, $data_end, $art_id, $clients) { $db = DbSingleton::getDb();
        ...
        $n=$db->num_rows($r);
        $amount_sales = ($n > 0) ? $db->result($r, 0, "amount_sales") : 0;
        ...
        $n=$db->num_rows($r);
        $amount_backs = ($n > 0) ? $db->result($r, 0, "amount_backs") : 0;

        return ($amount_sales - $amount_backs);
    }
    ...
    function getSummPartitionSalesArticle($date_start, $date_end, $art_id) { $db = DbSingleton::getDb();
		...
		WHERE js.time_stamp>='$date_start 00:00:00' AND js.time_stamp<='$date_end 23:59:59' AND jsp.art_id='$art_id' GROUP BY jsp.art_id LIMIT 1;");
        $n=$db->num_rows($r);
        if ($n>0) {
