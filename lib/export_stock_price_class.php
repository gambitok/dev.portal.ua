...
class export_stock_price {
	...
	function exportPrices($price_select) {
        header('Content-Type: text/csv; charset=windows-1251');
		header('Content-Disposition: attachment; filename=export_prices.csv');
		$output = fopen('php://output', 'w');
		fputcsv($output, array("ART_ID","ARTICLE_NR_DISPL","ARTICLE_NR_SEARCH","BRAND_ID","BRAND_NAME","PRICE","CASH","OPER_PRICE"), $delimiter = ';');
	...
	function getArticlesStockData() { $db = DbSingleton::getTokoDb();
		...
		ORDER BY t2b.`BRAND_NAME` ASC, t2a.`ARTICLE_NR_DISPL` ASC");
		$n = $db->num_rows($r);
		for ($i = 1; $i <= $n; $i++) {
	...
	function getArticlesPriceData($price_select) { $db = DbSingleton::getTokoDb();
	    $array = [];
	    $price_val = "price_$price_select";
	    $cash_to = 2; // usd
		$r = $db->query("SELECT t2a.*, t2b.`BRAND_NAME`, t2pr.`$price_val`, t2pr.`cash_id`, t2ps.OPER_PRICE 
		FROM `T2_ARTICLES_PRICE_RATING` t2pr
			LEFT JOIN `T2_ARTICLES` t2a ON t2pr.`art_id`=t2a.`art_id` 
			LEFT JOIN `T2_ARTICLES_PRICE_STOCK` t2ps ON t2ps.`ART_ID`=t2a.`art_id`
			LEFT JOIN `T2_BRANDS` t2b ON t2a.`BRAND_ID`=t2b.`BRAND_ID`
		WHERE t2pr.`in_use`=1;");
		$n = $db->num_rows($r);
		for ($i = 1; $i <= $n; $i++) {
			...
			$array[$i] = array($art_id, $art_displ, $art_search, $brand_id, $brand, $price, $cash_name, $oper_price);
		}
		return $array;
	}
    ...
    function getSupplList($art_id) { $dbt = DbSingleton::getTokoDb();
        $suppl_list = "";
        $r = $dbt->query("SELECT * FROM `T2_SUPPL_IMPORT` WHERE `art_id`='$art_id';");
        $n = $dbt->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
    ...
    function getArticlesClientsData() { $db = DbSingleton::getDb(); $dbt = DbSingleton::getTokoDb();
        $client_id = 930;
        $array = [];
        $r = $db->query("SELECT `tpoint_id` FROM `A_CLIENTS_CONDITIONS` WHERE `client_id`='$client_id' LIMIT 1;");
        $tpoint_id = $db->result($r,0,"tpoint_id");

        $storage_str = "";
        $r = $dbt->query("SELECT `storage_id` FROM `T_POINT_STORAGE` WHERE `tpoint_id`='$tpoint_id';");
        $n = $dbt->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
	    ...
	    FROM `T2_ARTICLES_STRORAGE` t2s
			...
			LEFT JOIN `T2_BRANDS` t2b ON t2a.`BRAND_ID`=t2b.`BRAND_ID`
		WHERE t2n.`LANG_ID`=16 GROUP BY t2a.`ART_ID`;");
        $n = $dbt->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            ...
            $title=$this->remove_utf8_bom($title);
            $descr = mb_convert_encoding($descr, "utf-8", "windows-1251");
            $descr = $this->remove_utf8_bom($descr);

            $r2 = $dbt->query("SELECT SUM(`AMOUNT`) as storage_amount FROM `T2_ARTICLES_STRORAGE` 
            WHERE `ART_ID`='$art_id' AND `STORAGE_ID` IN ($storage_str) GROUP BY `ART_ID`;");
            $n2 = $dbt->num_rows($r2);
            if ($n2 > 0) {
                ...
                WHERE `ART_ID`='$art_id' AND `STORAGE_ID` IN ($storage_none_str) GROUP BY `ART_ID`;");
                $n3 = $dbt->num_rows($r3);
                if ($n3 > 0) {
            ...
            if ($amount > 0 && $price > 0) {
                $array[$i] = array("$article_nr_displ", "$brand_name", "$amount", "$delivery", "$price", "$title", "$descr", "$images");
            }
        }
        return $array;
    }
    ...
    function getSupplArticlesClientsData() { $db = DbSingleton::getDb(); $dbt = DbSingleton::getTokoDb();
        ...
        $storage_str = "";
        $r = $dbt->query("SELECT * FROM `T_POINT_STORAGE` WHERE `tpoint_id`='$tpoint_id';");
        $n = $dbt->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $storage_id = $dbt->result($r,$i-1,"storage_id");
            $storage_str .= "$storage_id";
    ...
    function getDpClientPriceLevels($client) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT * FROM `A_CLIENTS_CONDITIONS` WHERE `client_id`='$client' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 1) {
    ...
    function getMinDeliveryTime($tpoint_id, $storage_str) { $db = DbSingleton::getTokoDb();
        ...
        $n = $db->num_rows($r);
        if ($n == 1) {
            $delivery_days = $db->result($r,0,"min_delivery");
        } else {
            $delivery_days = 0;
        }
        return $delivery_days;
    ...
    function exportPriceRating($brand_select) {
        header('Content-Type: text/csv; charset=windows-1251');
        header('Content-Disposition: attachment; filename=export_prices.csv');
...
//        ob_clean();
        ...
        $array = $this->getArticlesPriceRating($brand_select);
        foreach ($array as $fields) {
            fputcsv($output, $fields, $delimiter = ';');
        }
        exit(0);
    ...
    function getArticlesPriceRating($brand_select) { $db = DbSingleton::getTokoDb();
        $where = ($brand_select != 0) ? "AND t2a.`BRAND_ID`='$brand_select'" : "";
        $r = $db->query("SELECT t2a.ARTICLE_NR_DISPL, t2b.BRAND_NAME, t2ps.OPER_PRICE, t2ps.GENERAL_STOCK, t2pr.* 
        FROM `T2_ARTICLES_PRICE_RATING` t2pr
            LEFT JOIN `T2_ARTICLES` t2a ON t2a.ART_ID = t2pr.art_id
            LEFT JOIN `T2_BRANDS` t2b ON t2b.BRAND_ID = t2a.BRAND_ID
            LEFT JOIN `T2_ARTICLES_PRICE_STOCK` t2ps ON t2ps.ART_ID = t2pr.art_id
        WHERE t2pr.in_use = 1 $where;");
        $n = $db->num_rows($r);
        $array = [];
        for ($i = 1; $i <= $n; $i++) {
            $art_id = $db->result($r, $i - 1, "art_id");
            ...
            $min_markup = $db->result($r, $i - 1, "minMarkup");
            $sales = $this->getLineArticleSales($art_id);
            $array[$i] = [$art_id, $article_nr_displ, $brand_name, $oper_price, $min_markup, $general_stock, $sales];
            for ($j = 1; $j <= 50; $j++) {
                $array[$i][] = $db->result($r, $i - 1, "persent_$j");
                $array[$i][] = $db->result($r, $i - 1, "price_$j");
            }
        }
        return $array;
    }
    ...
    function getBrandList() { $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT `BRAND_ID`, `BRAND_NAME` FROM `T2_BRANDS` WHERE `VISIBLE`=1 ORDER BY `BRAND_NAME` ASC;");
        $n = $db->num_rows($r);
        $list = "";
        for ($i = 1; $i <= $n; $i++) {
            $brand_id = $db->result($r, $i - 1, "BRAND_ID");
