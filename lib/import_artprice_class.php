...
function showCsvPreview(){$db=DbSingleton::getDb();$slave=new slave; $csv_exist=0;$csv_file_name="������ ����";$pre_table="<h3 align='center'>������ �� �����������</h3>"; session_start();$user_id=$_SESSION["media_user_id"];
				...
				while (($buffer = fgets($handle, 4096)) !== false) {$fn+=1;
					...
					if ($buffer!=""){
						...
						for ($i=1;$i<=$kol_cols;$i++){
							...
							if ($ex_cols==1){$cols_list.="<th><select id=\"clm-$i\" size='1'><option value='0'>-</option><option value='1'>ART_ID</option><option value='2'>��, $</option><option value='3'>Min, %</option>
							<option value='4'>0 �����</option><option value='5'>1 �����</option><option value='6'>2 �����</option>
							<option value='7'>3 �����</option><option value='8'>4 �����</option><option value='9'>5 �����</option>
							<option value='10'>6 ����� </option><option value='11'>7 �����</option><option value='12'>8 �����</option>
							<option value='13'>9 �����</option><option value='14'>10 �����</option><option value='15'>11 �����</option>
...
function finishArtpriceCsvImport($start_row,$kol_cols,$cols){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	...
	if ($n==1){
		...
		if (file_exists($file_path)){
			...
			$art_id=0;$oc_price=0;$min_price=0;$price[0]=0;$price[1]=0;$price[2]=0;$price[3]=0;$price[4]=0;$price[5]=0;$price[6]=0;
			$price[7]=0;$price[8]=0;$price[9]=0;$price[10]=0;$price[11]=0;$perc[0]=0;$perc[1]=0;$perc[2]=0;$perc[3]=0;$perc[4]=0;
			$perc[5]=0;$perc[6]=0;$perc[7]=0;$perc[8]=0;$perc[9]=0;$perc[10]=0;$perc[11]=0;
			//print "kol_cols=$kol_cols";
			for ($i=1;$i<=$kol_cols;$i++){
			...
			$handle = @fopen($file_path, "r");
			if ($handle) { //$db->query("delete from catalogue_price where provider='$provider';");
				set_time_limit(0);$max_cols=0;$kol_elem=11;
				while (($buffer = fgets($handle, 4096)) !== false) {$fn+=1;
					if ($buffer!=""){
						$buf=explode(";",$buffer);
						if ($fn>=$start_row){
