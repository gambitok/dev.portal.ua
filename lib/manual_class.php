...
class manual {
	...
	function addNewCity($region_id,$name){$db=DbSingleton::getDb();$slave=new slave;
		...
		$db->query("insert into T2_CITY (`CITY_ID`,`CITY_NAME`,`REGION_ID`,`ALFA2`,`ALFA3`) 
		value ('$id','$name','$region_id','".strtoupper(substr($name,0,2))."','".strtoupper(substr($name,0,3))."');");
		return $id;
	}
...
class manualD {
	...
	function loadManualDData($key,$manValue,$manText){$db=DbSingleton::getDb();$list="";$k=0;
		...
		for ($i=1;$i<=$n;$i++){$k+=1;
			...
			$list.="<tr bgcolor='#$tr_color' class='tRow' align='center'>
	...
	function showManualDList($key,$SCaption){$db=DbSingleton::getDb();$where="";$form="<table width='100%' border='0'>{list}</table>";$list="";$k=0;
		if ($SCaption!=""){$where=" and mcaption LIKE '%$SCaption%'";}
		$r=$db->query("SELECT *	FROM manual_desc where `key`='$key' $where order by mcaption,id asc limit 0,100;");	$n=$db->num_rows($r);
		for ($i=1;$i<=$n;$i++){$k+=1;
			$id=$db->result($r,$i-1,"id");
			$mid=$db->result($r,$i-1,"mid");
			$mdesc=strip_tags($db->result($r,$i-1,"mdesc"));$mdesc=substr($mdesc,0,200);
...
class manualK {
	...
	function loadManualKData($key,$manValue,$text,$filter){$db=DbSingleton::getDb();$list="";$k=0;
...
class manualP {
	...
	function loadManualPData($key,$manValue,$manText,$parrKey,$parrKeyId){$db=DbSingleton::getDb();$list="";$k=0;if ($parrKeyId==""){$parrKeyId=0;}
		...
		for ($i=1;$i<=$n;$i++){$k+=1;
			...
			$mcaption=$db->result($r,$i-1,"mcaption");
			$tr_color="ffffff";if ($k==2){$tr_color="fafafa";$k=0;}
			$list.="<tr bgcolor='#$tr_color' class='tRow' align='center' onclick='setValue(\"$mid\");'>
	...
	function showManualPList($key,$SCaption,$parrKey,$parrKeyId){$db=DbSingleton::getDb();$where="";$form="<table width='100%' border='0'>{list}</table>";$list="";$k=0;
		if ($SCaption!=""){$where=" and mcaption LIKE '%$SCaption%'";}if ($parrKeyId==""){$parrKeyId=0;}
		$r=$db->query("SELECT *	FROM manual where `key`='$key' and parrentKey='$parrKey' and parrentKeyId='$parrKeyId' $where order by mcaption,id asc limit 0,100;");	$n=$db->num_rows($r);
		for ($i=1;$i<=$n;$i++){$k+=1;
			$mid=$db->result($r,$i-1,"mid");
			$mvalue=$db->result($r,$i-1,"mvalue");
			$mcaption=$db->result($r,$i-1,"mcaption");
...
class manualManager {
	...
	function loadManualManagerData($manValue,$manText,$bankOc){$db=DbSingleton::getDb();$list="";$k=0;if ($bankOc==""){$bankOc=0;}
		...
		for ($i=1;$i<=$n;$i++){$k+=1;
			...
			$tr_color="ffffff";if ($k==2){$tr_color="fafafa";$k=0;}
			$list.="<tr bgcolor='#$tr_color' class='tRow' align='center' onclick='setManualManager(\"$id\");'>
                <td width='2%'>$i</td>
	...
	function showManualManagerList($SCaption,$bankOc){$db=DbSingleton::getDb();$where="";$form="<table width='100%' border='0'>{list}</table>";
