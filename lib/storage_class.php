...
function loadStorageUsers($storage_id) {$db=DbSingleton::getDb(); 
	...
	if ($list=="") $list="�����"; if ($list_tp=="") $list_tp="�����";									
...
function saveStorageGeneralInfo($storage_id,$name,$full_name,$address,$storekeeper,$country_id,$state_id,$region_id,$city_id,$order_by){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function showStorageParamTypeSelectList($storage_id,$sel_id){$db=DbSingleton::getTokoDb();$list="";;
	...
	for ($i=1;$i<=$n;$i++){
		...
		$list.="<option value='$param_id' $sel>$param_name $param_type | $code_length(�������)</option>";
	}
	return $list;
}


function saveStorageDetailsForm($storage_id,$storage_str_id,$param_id){ $db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	...
	}else{$answer=0;$err="�� ������ ��� ��������� �� �����!$storage_id,$storage_str_id,$param_id;";}
	return array($answer,$err);
}
function dropStorageDetails($storage_id,$storage_str_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function loadStorageCells($storage_id){$db=DbSingleton::getTokoDb();$slave=new slave;$gmanual=new gmanual;
	...
	for ($i=1;$i<=$n;$i++){
		$id=$db->result($r,$i-1,"id");
		$default=$db->result($r,$i-1,"default");$default_cap="-"; if ($default==1){$default_cap="�� ������������";}
...
function saveStorageCellsForm($storage_id,$cells_id,$str_kol,$cell_str_ids,$cell_param_ids,$cell_vls,$def_ch){ $db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function dropStorageCells($storage_id,$cells_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
