...
function updateStorselStatus($select_id) {$db=DbSingleton::getDb(); $answer=0;$err="������� ���������� �����!";
	$db->query("update J_SELECT set status_select=128 where id='$select_id';");
    $answer=1; $err="";
    return array($answer,$err);
}

function showStorselCard($select_id){$db=DbSingleton::getDb();$slave=new slave;$manual=new manual;$gmanual=new gmanual;$jmoving=new jmoving; $dp=new dp;    
	session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	$form_htm=RD."/tpl/storsel_card.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
	...
	if ($n==1){
		...
		if ($user_id!=$user_use && $user_use>0){
			...
			$admin_unlock="";
			if ($user_id==1 || $user_id==2){$admin_unlock="<button class='btn btn-sm btn-warning' onClick='unlockStorselCard(\"$id\");'><i class='fa fa-unlock'></i> ������������</button>";}
...
function unlockStorselCard($select_id){session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;
...
function closeStorselCard($select_id){session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;
...
function showStorselStrList($select_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$manual=new manual;$gmanual=new gmanual;$list="";
...
function loadStorselCommets($select_id){$db=DbSingleton::getDb();$slave=new slave;
	$form_htm=RD."/tpl/storsel_comment_block.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
...
function saveStorselComment($select_id,$comment){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function dropStorselComment($select_id,$comment_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ��������� ������!";
...
function labelCommentsCount($select_id){$db=DbSingleton::getDb();$slave=new slave;$kol=0;$label="";
...
function startJmovingStorageSelect($jmoving_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="";
	...
	if ($n==1){
		...
		if ($status_select>47 || $oper_status>30){$answer=0;$err="���������� �����������. ���� ������� ����������.";}
...
function showJmovingSkladStorageSelectList($jmoving_id,$jmoving_status){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$manual=new manual;$gmanual=new gmanual;$list="";
	...
	$r=$db->query("select ms.*, p.name as tpoint_name, s.name as storage_name, ml.mcaption as loc_type_name, mt.mcaption as status_select_name from J_MOVING_SELECT$tmp ms 
...
function viewJmovingStorageSelect($jmoving_id,$select_id,$jmoving_status){$db=DbSingleton::getDb();$cat=new catalogue;$slave=new slave;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	$form_htm=RD."/tpl/jmoving_storage_select_view.htm";
...
function collectStorsel($select_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ������� ������!"; $date=date("Y-m-d H:i:s");
...
function getStorselInfo($select_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$manual=new manual;$gmanual=new gmanual;
	$r=$db->query("select ms.*, p.name as tpoint_name, s.name as storage_name, mt.mcaption as status_select_name from J_SELECT ms 
					left outer join T_POINT p on p.id=ms.tpoint_id
					left outer join STORAGE s on s.id=ms.storage_id
					left outer join manual mt on mt.id=ms.status_select
					where ms.id='$select_id' and ms.status='1' limit 0,1;");$n=$db->num_rows($r);
	if ($n=1){
		$tpoint_id=$db->result($r,0,"tpoint_id");
	...
	return array("���-$select_id/$storage_name",$data_create,$data_start,$data_collect,$storage_id,$storage_name,$articles_amount,$amount,$volume,$weight_netto,$weight_brutto,$tpoint_id,$tpoint_name,$parrent_doc_type_id,$parrent_doc_id);
}
...
function printStorselView($select_id){$db=DbSingleton::getDb();$cat=new catalogue;$dp=new dp; $slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	$form_htm=RD."/tpl/storsel_select_print.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}								  
									  
	$rstr=$db->query("select storage_id from J_SELECT where id='$select_id';");
	$storage=$db->result($rstr,0,"storage_id");
...
function printStorselView2($select_id){$db=DbSingleton::getDb();$cat=new catalogue;$dp=new dp; $slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"]; $mas=[];
	$form_htm=RD."/tpl/storsel_select_print.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}								  
	...
	order by cll.cell_value desc, j.article_nr_displ asc;"); $n=$db->num_rows($r);
...
//		</tr>";
	...
	foreach ($mas as $key=>$value) {$i++;
		...
		$amount=$value["amount"];
		$storage_id_from=$value["storage_id_from"];
		$storage_name_from=$value["storage_name_from"];
		$cell_id_from=$value["cell_id_from"];
...
function showStorselBarcodeForm($select_id){$db=DbSingleton::getDb();$cat=new catalogue;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="������� �������";
...
//		����� - ������ ������, ��� ��� �������
		...
		$list.="<tr $style>
			...
			<td align='center'><button class='btn btn-xs btn-default' onclick='showStorselNoscanForm(\"$select_id\",\"$art_id\",\"$id\");' title='Գ������ ��� ����������' alt='Գ������ ��� ����������'><i class='fa fa-cube'></i></button></td>
			<td align='center'><button class='btn btn-xs btn-danger' onclick='showStorselBugForm(\"$select_id\",\"$id\");' title='���������/����/���������' alt='���������/����/���������'><i class='fa fa-bug'></i></button></td>
...
function saveStorselBarcodeForm($select_id,$barcode){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="������� �������!!";
...
function finishStorselBarcodeForm($select_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="������� �������!!";$dp=new dp;
				...
				for ($i=1;$i<=$n1;$i++){
					...
					if ($na>0){
						...
						if ($parrent_doc_type_id==1){ // �����������
							...
							if ($parrent_doc_type_id2==1 && $parrent_doc_id2>0){$all_pd_summ=0; //����������� � ������������
						...
						if ($parrent_doc_type_id==2){ $all_pd_summ=0; // ����������� ��� �����������
							...
							if ($n2==1){
								...
								if ($new_amount_select>0){
									$db->queryP("update J_DP_STR set amount_collect='$new_amount_select', amount_bug='$amount_dp_bug', summ='$summ_dp', status_dps='96' where id='$dp_str_id' limit 1;");
								}
								if ($new_amount_select==0){
									$db->queryP("update J_DP_STR set amount_collect='$new_amount_select', amount_bug='$amount_dp_bug', summ='$summ_dp', status_dps='170' where id='$dp_str_id' limit 1;");
								}
...
function showStorselBugForm($select_id,$str_id){$db=DbSingleton::getDb();$cat=new catalogue;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="������� �������";
...
function saveStorselBugForm($select_id,$str_id,$storage_select_bug,$amount_bug){$db=DbSingleton::getDb();$slave=new slave;$manual=new manual;$cat=new catalogue;$answer=0;$err="������� �������!!";
...
function showStorselNoscanForm($select_id,$art_id,$str_id){$db=DbSingleton::getDb();$cat=new catalogue;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="������� �������";
...
function saveStorselNoscanForm($select_id,$art_id,$str_id,$amount_barcode_noscan){$db=DbSingleton::getDb();$slave=new slave;$manual=new manual;$cat=new catalogue;$answer=0;$err="������� �������!!";
