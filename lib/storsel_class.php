...
class storsel {
    ...
    function getSaleInvoiceName($dp_id, $select_id) { $db = DbSingleton::getDb();
        ...
        WHERE sis.dp_id = '$dp_id' AND sis.select_id = '$select_id' AND sis.`status` = 1 LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 1) {
    ...
    function statusStorage($user_id, $storage_id) { $db = DbSingleton::getDb();
        ...
        $n = $db->num_rows($r);
        return (($super_user) || ($n > 0));
    }

    function show_storsel_list($status = 0)
    {
        ...
        $count = 0;
        $parrent_doc_status = 0;
        $list = "";
        $super_user = $users->getSuperUser($media_user_id);
        $where = " AND sel.tpoint_id = '$ses_tpoint_id'";
        if (($media_user_id == 1) || ($media_user_id == 7) || ($super_user)) {
            $where = "";
        }
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            $parrent_doc_id = $db->result($r, $i - 1, "parrent_doc_id") + 0;
            ...
            if ($parrent_doc_type_id == 2) {
                $r2 = $db->query("SELECT `order_info_id` FROM `J_DP` WHERE `id` =  $parrent_doc_id LIMIT 1;");
                $order_info_id = $db->result($r2, 0, "order_info_id") + 0;
                if ($order_info_id > 0) {
            ...
            // JBACKSUPPL
            if ($parrent_doc_type_id == 3) {
                $doc_parrent = $suppl->getBackSupplName($parrent_doc_id);
                $parrent_doc_status = $suppl->getBackSupplStatusId($parrent_doc_id);
            }
    ...
    function showStorselCard($select_id) { $db = DbSingleton::getDb();
        ...
        $id = 0;
        $form = ""; $form_htm = RD . "/tpl/storsel_card.htm";
        if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}

        $r = $db->query("SELECT * FROM `J_SELECT` WHERE `id` = '$select_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 0) {
        ...
        if ($n == 1) {
            ...
            if ($parrent_doc_type_id == 2) {
                $doc_parrent = $dp->getDpName($parrent_doc_id);
            }
            if ($parrent_doc_type_id == 3) {
                $doc_parrent = $suppl->getBackSupplName($parrent_doc_id);
            }
            ...
            if ($user_id != $user_use && $user_use > 0) {
                $form_htm = RD ."/tpl/storsel_use_deny.htm";
                if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function closeStorselCard($select_id) {
        session_start();
        $user_id = $_SESSION["media_user_id"];
        $this->unsetStorselCardUserAccess($select_id, $user_id);
        return 1;
    }
    ...
    function showStorselStrList($select_id) { $db = DbSingleton::getDb();
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            if ($cell_id_from > 0) {
                $storage_name_from .= " " . $this->getStorageCellName($cell_id_from);
            }
            $list .= "<tr align='right'>
                <td align='left'>$i</td>
    ...
    function loadStorselCommets($select_id) { $db = DbSingleton::getDb();
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/storsel_comment_block.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT cc.*, u.name 
        FROM `J_SELECT_COMMENTS` cc 
            LEFT OUTER JOIN `media_users` u ON (u.id = cc.USER_ID) 
        WHERE cc.select_id = '$select_id' 
        ORDER BY `id` DESC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
    ...
    function dropStorselComment($select_id, $comment_id) { $db = DbSingleton::getDb();
        ...
        if ($select_id > 0 && $comment_id > 0) {
            $r = $db->query("SELECT * FROM `J_SELECT_COMMENTS` WHERE `select_id` = '$select_id' AND `id` = '$comment_id' LIMIT 1;");
            $n = $db->num_rows($r);
            if ($n == 1) {
    ...
    function labelCommentsCount($select_id) { $db = DbSingleton::getDb();
        $label = "";
        $r = $db->query("SELECT COUNT(`id`) as kol FROM `J_SELECT_COMMENTS` WHERE `select_id`='$select_id';");
    ...
    function startJmovingStorageSelect($jmoving_id) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT `oper_status`, `status_select`, `storage_id_to` FROM `J_MOVING` WHERE `id` = $jmoving_id LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 1) {
            ...
            if ($status_select > 47 || $oper_status > 30) {
            ...
            if ($oper_status == 30 && $status_select >= 44 && $status_select <= 47 && $storage_id_to > 0) {
                ...
                $r1 = $db->query("SELECT `storage_id_from` 
                FROM `J_MOVING_STR` 
                WHERE `jmoving_id`='$jmoving_id' AND `status_select`='44' 
                GROUP BY `storage_id_from`, `cell_id_from` ORDER BY `storage_id_from` ASC;");
                $n1 = $db->num_rows($r1);
                if ($n1 == 0) {
                ...
                if ($n1 > 0) {
                    for ($i = 1; $i <= $n1; $i++) {
                        ...
                        $ra = $db->query("SELECT * FROM `J_MOVING_STR` WHERE `jmoving_id`='$jmoving_id' AND `storage_id_from`='$storage_id_from' AND `status_select`='44';");
                        $na = $db->num_rows($ra);
                        for ($a = 1; $a <= $na; $a++) {
    ...
    function makesJmovingStorageSelect($jmoving_id) { $db = DbSingleton::getDb(); $dbt = DbSingleton::getTokoDb();
        ...
        if ($n == 1) {
            ...
            if ($oper_status == 30 && $status_select >= 44 && $status_select <= 47 && $storage_id_to > 0) {
                ...
                $rm=$db->query("SELECT * FROM `J_MOVING_SELECT_TEMP` WHERE `jmoving_id`='$jmoving_id' AND `status_select`='44';");
                $nm=$db->num_rows($rm);
                for ($im=1;$im<=$nm;$im++) {
                    $select_id+=1;
                    ...
                    for ($im2=1;$im2<=$nm2;$im2++) {
                        ...
                        if ($nsc==0) {
                            $rsc2=$dbt->query("SELECT * FROM `T2_ARTICLES_STRORAGE` WHERE `ART_ID`='$art_id' AND `STORAGE_ID`='$storage_id_from' LIMIT 1;");
    ...
    function makesJmovingStorageSelectLocal($jmoving_id) { $db = DbSingleton::getDb();
        ...
        $answer = 0; $err = "";
        $jmoving_id=$slave->qq($jmoving_id);
        $storage_id_from = $cell_id_from = 0;
        $r = $db->query("SELECT `oper_status`, `status_select`, `storage_id_to` FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 1) {
            $oper_status=$db->result($r, 0, "oper_status");
            ...
            if ($oper_status==30 && $status_select>=44 && $status_select<=47 && $storage_id_to>0) {
                ...
                $rm=$db->query("SELECT * FROM `J_MOVING_LOCAL_SELECT_TEMP` WHERE `jmoving_id`='$jmoving_id' AND `status_select`='44';");
                $nm=$db->num_rows($rm);
                for ($im=1;$im<=$nm;$im++) {
                    $select_id+=1;
                    $select_id_t=$db->result($rm,$im-1,"id");
    ...
    function loadJmovingStorageSelect($jmoving_id, $jmoving_status) { $db = DbSingleton::getDb();
        ...
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm);}
        $r = $db->query("SELECT `status_select` FROM `J_MOVING` WHERE `id`='$jmoving_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 0) {
            $form_htm = RD . "/tpl/access_deny.htm";
            if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
    ...
    function showJmovingSkladStorageSelectList($jmoving_id, $jmoving_status) { $db = DbSingleton::getDb();
        $list = "";
        $tmp = ""; $where_status = "AND ms.status_select='$jmoving_status'";
        if ($jmoving_status == 44) {
            $tmp ="_TEMP";
        }
        if ($jmoving_status > 44) {
            $where_status = "AND ms.status_select IN (45,46,47,48)";
        }
        ...
        WHERE ms.jmoving_id='$jmoving_id' $where_status AND ms.status='1' 
        ORDER BY ms.id ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $id = $db->result($r, $i - 1, "id");
    ...
    function showJmovingSkladStorageSelectListLocal($jmoving_id, $jmoving_status) { $db = DbSingleton::getDb();
        $list = "";
        $tmp = "J_MOVING_SELECT"; $where_status = "AND ms.status_select='$jmoving_status'";
    ...
    function viewJmovingStorageSelect($jmoving_id, $select_id, $jmoving_status) { $db = DbSingleton::getDb();
        $jmoving = new jmoving; $cat = new catalogue;
        $list = "";
    ...
    function viewJmovingStorageSelectLocal($jmoving_id, $select_id, $jmoving_status) { $db = DbSingleton::getDb();
        ...
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT * FROM $tmp WHERE `jmoving_id`='$jmoving_id' AND `select_id`='$select_id' ORDER BY `id` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $article_nr_displ = $db->result($r, $i - 1, "article_nr_displ");
            $brand_id = $db->result($r, $i - 1, "brand_id");
    ...
    function collectStorsel($select_id) { $db = DbSingleton::getDb();
        ...
        $select_id = $slave->qq($select_id);
        if ($select_id > 0) {
            $r = $db->query("SELECT `status_select`, `parrent_doc_id` FROM `J_SELECT` WHERE `id`='$select_id' LIMIT 1;");
            $n = $db->num_rows($r);
            if ($n == 1) {
                $status_select = $db->result($r, 0, "status_select");
    ...
    function getStorageOrder($storage_id) { $db = DbSingleton::getTokoDb();
        ...
        $order_cap = "";
        if ($order_by == 0) {
            $order_cap = "ORDER BY `article_nr_displ` ASC";
        }
        if ($order_by == 1) {
            $order_cap = "ORDER BY `cell_id_from` DESC, `article_nr_displ` ASC";
        }
        return $order_cap;
    }
    ...
    function printStorselView($select_id) { $db = DbSingleton::getDb();
        $cat = new catalogue; $dp = new dp; $jmoving = new jmoving; $slave = new slave;
        session_start();
        $user_id = $_SESSION["media_user_id"];
        $dp_name = $jmoving_name = $client_name = "";
        $form = ""; $form_htm = RD . "/tpl/storsel_select_print.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $list = "";
        ...
        $r = $db->query("SELECT * FROM `J_SELECT_STR` WHERE `select_id`='$select_id' $order_by;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $art_id = $db->result($r, $i - 1, "art_id");
            $article_nr_displ = $db->result($r, $i - 1, "article_nr_displ");
        ...
        if ($parrent_doc_type_id == 1) {
            ...
            $r = $db->query("SELECT `parrent_doc_id` FROM `J_MOVING` WHERE `id` = $parrent_doc_id LIMIT 1;");
            $dp_id = $db->result($r, 0, "parrent_doc_id");
        ...
        $form = str_replace("{weight_netto}",$weight_netto,$form);
        $form = str_replace("{weight_brutto}",$weight_brutto,$form);
        $form = str_replace("{user_name}",$this->getMediaUserName($user_id),$form);
        $form = str_replace("{curtime}",date("Y-m-d H:i:s"),$form);
        $form = str_replace("{client_name}",$client_name,$form);
        $form = str_replace("{tpoint_address}",$this->getTpointAddress($tpoint_id),$form);
        ...
        if ($parrent_doc_type_id == 1) {
            $r = $db->query("SELECT `parrent_doc_id` FROM `J_MOVING` WHERE `id` = $parrent_doc_id LIMIT 1;");
            $dp_id = $db->result($r, 0, "parrent_doc_id");
            $dp_note1 = $dp->getDpNote($dp_id);
            $dp_note2 = $dp->getDpUserDelivery($dp_id);
        }
        if ($parrent_doc_type_id == 2) {
            $dp_note1 = $dp->getDpNote($parrent_doc_id);
            $dp_note2 = $dp->getDpUserDelivery($parrent_doc_id);
        }
        $dp_note = "$dp_note1<br>$dp_note2";
    ...
    function printStorselView2($select_id) { $db = DbSingleton::getDb();
        $cat = new catalogue; $dp = new dp; $slave = new slave;
        session_start();
        $user_id = $_SESSION["media_user_id"];
        $mas = [];
        $client_name = $jmoving_name = $dp_name = $list = "";
        $form = ""; $form_htm = RD . "/tpl/storsel_select_print.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT j.*, cll.cell_value 
        FROM `J_SELECT_STR` j
            LEFT OUTER JOIN `STORAGE_CELLS` cll on cll.id=j.cell_id_from
        WHERE j.select_id='$select_id' AND cll.status=1 
        ORDER BY cll.cell_value DESC, j.article_nr_displ ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $art_id = $db->result($r, $i - 1, "art_id");
            $article_nr_displ = $db->result($r, $i - 1, "article_nr_displ");
    ...
    function addJournalRecord($select_id, $status_select) { $db = DbSingleton::getDb();
        session_start();
        $user_id = $_SESSION["media_user_id"];
        $db->query("INSERT INTO `J_SELECT_JOURNAL` (`select_id`,`user_id`,`status_select`) VALUES ('$select_id','$user_id','$status_select');");
        return true;
    }

    function getStorselJournalRecords($select_id) { $db = DbSingleton::getDb();
        $data = array();
        $r = $db->query("SELECT * FROM `J_SELECT_JOURNAL` WHERE `select_id`='$select_id' ORDER BY `id` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $status_select = $db->result($r, $i - 1, "status_select");
    ...
    function showStorselBarcodeForm($select_id) { $db = DbSingleton::getDb();
        ...
        $cat = new catalogue;
        $list = "";
        $form = ""; $form_htm = RD . "/tpl/storsel_barcode_list.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT * FROM `J_SELECT_STR` WHERE `select_id`='$select_id' ORDER BY `id` ASC;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
        ...
        $form = str_replace("{select_nom}",$select_nom,$form);
        $form = str_replace("{select_data}","",$form);
        $form = str_replace("{user_name}",$this->getMediaUserName($user_id),$form);
        $form = str_replace("{curtime}",date("Y-m-d H:i:s"),$form);
    ...
    function getBarcodeInfo($barcode) { $db = DbSingleton::getTokoDb();
        $info = "";
        $r = $db->query("SELECT `ART_ID` FROM `T2_BARCODES` WHERE `BARCODE`='$barcode' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n > 0) {
            $art_id = $db->result($r, 0, "ART_ID");
            $r = $db->query("SELECT t2a.`ARTICLE_NR_DISPL`, t2b.`BRAND_NAME` 
            ...
            $info = "$article_nr_displ / $brand_name";
        }
        return $info;
    }
    ...
    function saveStorselBarcodeForm($select_id, $barcode) { $db = DbSingleton::getDb();
        $slave = new slave;
        $barcode_info = ($barcode == "") ? "" : $this->getBarcodeInfo($barcode);
        ...
        if ($select_id > 0 && $barcode != "") {
            ...
            $r = $db->query("SELECT * FROM `J_SELECT_STR` WHERE `select_id`='$select_id' AND `art_id`='$art_id' AND amount>amount_barcodes ORDER BY `id` ASC;");
            $n = $db->num_rows($r);
            for ($i = 1; $i <= $n; $i++) {
                $id = $db->result($r, $i - 1, "id");
                $amount = $db->result($r, $i - 1, "amount");
    ...
    function cancelStorselScan($select_id) { $db = DbSingleton::getDb();
        ...
        if ($n > 0) {
            ...
            if ($parrent_doc_type_id == 1) {
                ...
                $r = $db->query("SELECT * FROM `J_MOVING_STR` WHERE `jmoving_id`='$parrent_doc_id';");
                $n = $db->num_rows($r);
                for ($i = 1; $i <= $n; $i++) {
                    $id = $db->result($r, $i-1, "id");
                    $amount_barcodes = $db->result($r, $i-1, "amount_barcodes");
                ...
                if ($parrent_doc_type_id2 == 1 && $parrent_doc_id2 > 0) {
                    $full_summ = 0;
                    $r = $db->query("SELECT * FROM `J_DP_STR` WHERE `dp_id`='$parrent_doc_id2';");
                    $n = $db->num_rows($r);
                    for ($i = 1; $i <= $n; $i++) {
                        $id = $db->result($r, $i-1, "id");
            ...
            if ($parrent_doc_type_id == 2) {
                $full_summ = 0;
                $r = $db->query("SELECT * FROM `J_DP_STR` WHERE `dp_id`='$parrent_doc_id';");
                $n = $db->num_rows($r);
                for ($i = 1; $i <= $n; $i++) {
                    $id = $db->result($r, $i-1, "id");
                    $price_dp = $db->result($r, $i - 1, "price_end");
    ...
    function finishStorselBarcodeForm($select_id) { $db = DbSingleton::getDb();
        $slave = new slave; $dp = new dp; $suppl = new suppl;
        $answer = 0; $err = "Помилка індексу!";
        $id = 0; $parrent_doc_type_id = 0;
        $select_id = $slave->qq($select_id);
        $cur_date = date("Y-m-d H:i:s");
        session_start();
        $user_id = $_SESSION["media_user_id"];
        if ($select_id > 0) {
            $r = $db->query("SELECT * FROM `J_SELECT_STR` WHERE `select_id`='$select_id' AND amount>(amount_barcodes+amount_barcodes_noscan+amount_bug);");
            $n = $db->num_rows($r);
            if ($n > 0) {
                $answer = 0; $err = "Не завершено перевірку по штрих-кодам";
            }
            if ($n == 0) {
                $r = $db->query("SELECT (SUM(amount_barcodes)+SUM(amount_barcodes_noscan)) as new_amount FROM `J_SELECT_STR` WHERE `select_id`='$select_id';");
                $n = $db->num_rows($r);
                if ($n == 1) {
                    ...
                    $r = $db->query("SELECT `parrent_doc_id`, `parrent_doc_type_id` FROM `J_SELECT` WHERE `id`='$select_id' LIMIT 1;");
                    $n = $db->num_rows($r);
                    $parrent_doc_id = $db->result($r, 0, "parrent_doc_id");
                    $j_parrent_doc_type_id = $db->result($r, 0, "parrent_doc_type_id");
                    if ($n > 0 && $j_parrent_doc_type_id == 1) {
                        $db->query("UPDATE `J_MOVING` SET `status_jmoving`='107' WHERE `id`='$parrent_doc_id' LIMIT 1;");
                    }

                    $r1 = $db->query("SELECT * FROM `J_SELECT_STR` WHERE `select_id`='$select_id';");
                    $n1 = $db->num_rows($r1);
                    for ($i = 1; $i <= $n1; $i++) {
                        ...
                        WHERE `select_id`='$select_id' AND `art_id`='$art_id'");
                        $na = $db->num_rows($ra);
                        ...
                        if ($na > 0) {
                            ...
                            if ($parrent_doc_type_id == 1) {
                                ...
                                if ($parrent_doc_type_id2 == 1 && $parrent_doc_id2 > 0) {
                                    // предпродажа с перемещением
                                    $all_pd_summ = 0;
                                    $r2 = $db->query("SELECT * FROM `J_DP_STR` WHERE `dp_id`='$parrent_doc_id2' AND `art_id`='$art_id' AND `storage_id_from`='$select_storage_id_from' LIMIT 1;");
                                    $n2 = $db->num_rows($r2);
                                    if ($n2 == 1) {
                                        $dp_str_id = $db->result($r2, 0, "id");
                                        $amount_dp = $db->result($r2, 0, "amount");
                                        $amount_dp_bug = 0;
                                        $amount_dp_collect = $db->result($r2, 0, "amount_collect");
                                        $ers = 0;
                                        if ($amount_dp_collect > 0) {
                                            $amount_dp_bug = $amount_dp - $amount_dp_collect;
                                            $ers = 1;
                                        }
                                        if ($storsel_amount_bug > 0 && $ers == 0) {
                                            $amount_dp_bug = $storsel_amount_bug;
                                        }
                                        $price_dp = $db->result($r2, 0, "price_end");
                                        $summ_dp = $db->result($r2, 0, "summ");
                                        if ($amount_dp != $new_amount_select) {
                                            $summ_dp = round($price_dp * $new_amount_select, 2);
                                            $all_pd_summ = 1;
                                        }
                                        ...
                                        if ($all_pd_summ == 1) {
                                            $dp->updateDpSumm($parrent_doc_id2);
                                        }
                                    }
                                }
                            }
                            ...
                            if ($parrent_doc_type_id == 2) {
                                $all_pd_summ = 0;
                                $r2 = $db->query("SELECT * FROM `J_DP_STR` WHERE `dp_id`='$parrent_doc_id' AND `art_id`='$art_id' AND `cur_select_str_id`='$str_id' LIMIT 1;");
                                $n2 = $db->num_rows($r2);
                                if ($n2 == 1) {
                                    $dp_str_id = $db->result($r2, 0, "id");
                                    $amount_dp = $db->result($r2, 0, "amount");
                                    ...
                                    if ($all_pd_summ == 1) {
                                        $dp->updateDpSumm($parrent_doc_id);
                                    }
                                }
                            }
                            ...
                            if ($parrent_doc_type_id == 3) {
                                $all_pd_summ = 0;
                                $r2 = $db->query("SELECT * FROM `J_BACK_SUPPL_STR` WHERE `back_suppl_id`='$parrent_doc_id' AND `art_id`='$art_id' AND `cur_select_str_id`='$str_id' LIMIT 1;");
                                $n2 = $db->num_rows($r2);
                                if ($n2 == 1) {
                                    $back_suppl_str_id = $db->result($r2, 0, "id");
                                    $amount_back_suppl = $db->result($r2, 0, "amount");
                                    $amount_back_suppl_bug = 0;
                                    $amount_back_suppl_collect = $db->result($r2, 0, "amount_collect");
                                    $ers = 0;
                                    if ($amount_back_suppl_collect > 0) {
                                        $amount_back_suppl_bug = $amount_back_suppl - $amount_back_suppl_collect;
                                        $ers = 1;
                                    }
                                    if ($storsel_amount_bug > 0 && $ers == 0) {
                                        $amount_back_suppl_bug = $storsel_amount_bug;
                                    }
                                    // $price_back_suppl = $db->result($r2, 0," price_end");
                                    // $summ_back_suppl = $db->result($r2, 0," summ");
                                    if ($amount_back_suppl != $new_amount_select) {
                                        // $summ_back_suppl = round($price_back_suppl * $new_amount_select, 2);
                                        $all_pd_summ = 1;
                                    }
                                    if ($new_amount_select > 0) {
                                        // `summ`='$summ_back_suppl',
                                        $db->query("UPDATE `J_BACK_SUPPL_STR` SET `amount_collect`='$new_amount_select', `amount_bug`='$amount_back_suppl_bug' WHERE `id`='$back_suppl_str_id' LIMIT 1;");
                                        $db->query("UPDATE `J_BACK_SUPPL` SET `status_back_suppl`=195 WHERE `id`='$parrent_doc_id' LIMIT 1;");
                                    }
                                    if ($new_amount_select == 0) {
                                        // `summ`='$summ_back_suppl',
                                        $db->query("UPDATE `J_BACK_SUPPL_STR` SET `amount_collect`='$new_amount_select', `amount_bug`='$amount_back_suppl_bug' WHERE `id`='$back_suppl_str_id' LIMIT 1;");
                                    ...
                                    // якщо відібрано не всі позиції, перерахувати суму документу
                                    if ($all_pd_summ == 1) {
                                        $suppl->updateBackSupplSumm($parrent_doc_id);
                                    }
                                }
                    ...
                    if ($parrent_doc_type_id == 1) {
                        $koef = 0;
                        $rch = $db->query("SELECT * FROM `J_DP_STR` WHERE `dp_id`='$parrent_doc_id';");
                        $nch = $db->num_rows($rch);
                        for ($i = 1; $i <= $nch; $i++) {
                        ...
                        if ($koef == $nch) {
                            // Виконано
                            $db->query("UPDATE `J_DP` SET `status_dp`=81 WHERE `id`='$parrent_doc_id';");
                        }
                    }
    ...
    function showStorselBugForm($select_id, $str_id) { $db = DbSingleton::getDb();
        $cat = new catalogue; $manual = new manual;
        session_start();
        $answer = 0; $err = "Помилка індексу";
        $form = ""; $form_htm = RD . "/tpl/storsel_bug_form.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT * FROM `J_SELECT_STR` WHERE `select_id`='$select_id' AND `id`='$str_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 1) {
            $art_id = $db->result($r, 0, "art_id");
            $article_nr_displ = $db->result($r, 0, "article_nr_displ");
    ...
    function getStorselParrentData($select_id) { $db = DbSingleton::getDb();
        ...
        if ($parrent_doc_type_id == 1) {
            $r = $db->query("SELECT * FROM `J_MOVING` WHERE `id`='$parrent_doc_id' LIMIT 1;");
            $n = $db->num_rows($r);
            if ($n == 1) {
    ...
    function showStorselNoscanForm($select_id, $str_id) { $db = DbSingleton::getDb();
        $cat = new catalogue;
        $answer = 0; $err = "Помилка індексу";
        $form = ""; $form_htm = RD . "/tpl/storsel_noscan_form.htm";
        if (file_exists("$form_htm")) { $form = file_get_contents($form_htm); }
        $r = $db->query("SELECT * FROM `J_SELECT_STR` WHERE `select_id`='$select_id' AND `id`='$str_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n == 1) {
    ...
    function saveStorselNoscanForm($select_id, $art_id, $str_id, $amount_barcode_noscan) { $db = DbSingleton::getDb();
        ...
        if ($select_id > 0 && $art_id > 0 && $str_id > 0 && $amount_barcode_noscan > 0) {
            $r = $db->query("SELECT * FROM `J_SELECT_STR` WHERE `select_id`='$select_id' AND `art_id`='$art_id' AND `id`='$str_id' LIMIT 1;");
            $n = $db->num_rows($r);
            if ($n == 1) {
                $id = $db->result($r, 0, "id");
                $amount = $db->result($r, 0, "amount");
    ...
    function updateStockStorageBug($art_id, $storage_id_from, $cell_id_from, $cell_use, $amount) { $dbt = DbSingleton::getTokoDb();
        ...
        if ($n == 1) {
            ...
            if ($amount <= $t2s_reserv_amount) {
                ...
                if ($cell_use == 1) {
                    $r1 = $dbt->query("SELECT `AMOUNT`, `RESERV_AMOUNT` FROM `T2_ARTICLES_STRORAGE_CELLS` WHERE `ART_ID`='$art_id' AND `STORAGE_ID`='$storage_id_from' AND `STORAGE_CELLS_ID`='$cell_id_from' LIMIT 1;");
                    $n1 = $dbt->num_rows($r1);
                    if ($n1 == 1) {
    ...
    function calculateStorselParams($select_id) { $db = DbSingleton::getDb();
        ...
        $r = $db->query("SELECT * FROM `J_SELECT_STR` WHERE `select_id`='$select_id';");
        $n = $db->num_rows($r);
        if ($n > 0) {
            for ($i = 1; $i <= $n; $i++) {
                $art_id = $db->result($r, $i - 1, "art_id");
