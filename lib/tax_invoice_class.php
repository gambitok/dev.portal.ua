...
class tax_invoice {
    ...
    function show_tax_invoice_list(){$db=DbSingleton::getDb();$gmanual=new gmanual;
        ...
        for ($i=1;$i<=$n;$i++){
            ...
            $function="showTaxInvoiceCard(\"$id\");";$prefix="НН";
            if ($tax_type_id==161){$function="showTaxInvoiceBackCard";$prefix="КНН";}
    ...
    function getSellerSelectList($sel_id){$db=DbSingleton::getDb();$list="";
        ...
        where c.status='1' and cc.category_id='3' and c.org_type=2 order by c.id asc;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
            $name=$db->result($r,$i-1,"name");
    ...
    function showTaxInvoiceCard($tax_id){$db=DbSingleton::getDb();$gmanual=new gmanual;session_start();$user_id=$_SESSION["media_user_id"];
        $doc_nom=$client_id=$seller_id=$tax_type_id=$tpoint_id=$status_tax=$doc_xml_nom=$summ=0; $client_name=$data_send=$data_create="";
        $form="";$form_htm=RD."/tpl/tax_invoice_card.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        ...
        if ($n==0){$summ=0;$doc_xml_nom="";$status_tax=155;$tax_type_id=160;$data_create=date("Y-m-d");$doc_nom=0;}
        if ($n==1){
    ...
    function showTaxInvoiceBackCard($tax_id){$db=DbSingleton::getDb();$gmanual=new gmanual;session_start();$user_id=$_SESSION["media_user_id"];
        ...
        $form="";$form_htm=RD."/tpl/tax_invoice_back_card.htm";	if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $cash_id=1;$cash_name=$this->getCashName($cash_id);
        $r=$db->query("select * from J_TAX_INVOICE where status=1 and id='$tax_id' limit 0,1;");$n=$db->num_rows($r);
        if ($n==0){$summ=0;$doc_xml_nom="";$status_tax=155;$tax_type_id=161;$data_create=date("Y-m-d");$doc_nom=0;}
        if ($n==1){
            //$id=$tax_id;
            $tax_type_id=$db->result($r,0,"tax_type_id");
            $doc_xml_nom=$db->result($r,0,"doc_xml_nom");
    ...
    function getTaxInvoceHeader($tax_id){$db=DbSingleton::getDb();
        $doc_nom=$seller_id=$tax_type_id=$cash_id=$summ=0;$seller_name=$cash_name=$data_create="";
        $r=$db->query("select * from J_TAX_INVOICE where status=1 and id='$tax_id' limit 0,1;");$n=$db->num_rows($r);
        if ($n==1){
            $tax_type_id=$db->result($r,0,"tax_type_id");
            $doc_nom=$db->result($r,0,"doc_nom");
    ...
    function showTaxStrList($tax_id,$status_tax){$db=DbSingleton::getDb();$list="";
        ...
        if ($status_tax==155){
            $list="<tr id='taxStrNewRow' class='hidden'>
                ...
                <td style='min-width:90px;'><input class='form-control input-xs numberOnly' type='text' id='zedStr_0' value='' placeholder='код'></td>
                <td style='min-width:160px;'><input class='form-control input-xs' type='text' id='goods_nameStr_0' value='' placeholder='Товар'></td>
    ...
    function showTaxStrBackList($tax_id,$status_tax){$db=DbSingleton::getDb();$list="";
        if ($status_tax==155){$opr="";}else{$opr=" readonly disabled";}
        $r=$db->query("select * from J_TAX_INVOICE_STR where status=1 and tax_id='$tax_id' order by id asc;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $str_id=$db->result($r,$i-1,"id");
        ...
        if ($status_tax==155){
            $list="<tr id='taxStrNewRow' class='hidden'>
                ...
                <td style='min-width:40px;'>
                    <input class='form-control input-xs' type='text numberOnly' id='nomStr_0'  value='' placeholder='рядок'>
    ...
    function saveTaxCard($tax_id,$data_create,$data_send,$cash_id,$tax_summ,$tax_type_id,$tpoint_id,$seller_id,$client_id,$status_tax,$doc_xml_nom){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="Помилка збереження даних!";
    ...
    function saveTaxCardData($tax_id,$frm,$tto,$idStr,$zedStr,$goods_nameStr,$amountStr,$priceStr,$summStr){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
        ...
        if ($tax_id>0){
            ...
            for($i=$frm;$i<=$tto;$i++){
                ...
                if ($goods_nameS!="" && $goods_nameS!="undefined"){
                    if ($idS=="" || $idS==0){
                        $r=$db->query("select max(id) as mid from J_TAX_INVOICE_STR;");$idS=0+$db->result($r,0,"mid")+1;
    ...
    function saveTaxBackCard($tax_id,$tax_to_back_id,$data_create,$data_send,$cash_id,$tax_summ,$tax_type_id,$tpoint_id,$seller_id,$client_id,$status_tax,$doc_xml_nom){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="Помилка збереження даних!";
    ...
    function saveTaxBackCardData($tax_id,$frm,$tto,$idStr,$tsidStr,$nomStr,$zedStr,$goods_nameStr,$amountStr,$priceStr,$summStr){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
        $tax_id=$slave->qq($tax_id);$frm=$slave->qq($frm);$tto=$slave->qq($tto);
        if ($tax_id>0){
    ...
    function exportTaxInvoiceXML($tax_id){$db=DbSingleton::getDb();$invoice_summ=0;
        $r=$db->query("select ti.*, sl.full_name as seller_name, sld.edrpou as seller_edrpou, sld.vytjag as seller_vytjag, sld.buh_name, sld.buh_edrpou, cl.full_name as client_name, cld.edrpou as client_edrpou, cld.vytjag as client_vytjag, ch.abr2 as cash_abr 
        from J_TAX_INVOICE ti
            left outer join CASH ch on ch.id=ti.cash_id
            left outer join A_CLIENTS sl on sl.id=ti.seller_id
    ...
    function exportTaxBackInvoiceXML($tax_id){$db=DbSingleton::getDb();$invoice_summ=0;
        $r=$db->query("select ti.*, sl.full_name as seller_name, sld.edrpou as seller_edrpou, sld.vytjag as seller_vytjag, sld.buh_name, sld.buh_edrpou, cl.full_name as client_name, cld.edrpou as client_edrpou, cld.vytjag as client_vytjag, ch.abr2 as cash_abr 
        from J_TAX_INVOICE ti
            left outer join CASH ch on ch.id=ti.cash_id
            left outer join A_CLIENTS sl on sl.id=ti.seller_id
    ...
    function dropTaxStr($tax_id,$tax_str_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка індексу";
    ...
    //				$tax_summ=$this->updateTaxSumm($tax_id);
            ...
            }else {$answer=0;$err="Видалення заблоковано. Накладну передано в ДПІ.";}
        }
        return array($answer,$err,$tax_summ);
    }

    function loadTaxInvoiceCDN($tax_id){$db=DbSingleton::getDb();
        $form="";$form_htm=RD."/tpl/tax_invoice_cdn_block.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
    ...
    function showTaxSelectList($sel_id){$db=DbSingleton::getDb();
        $form="";$form_htm=RD."/tpl/tax_select_tree.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("select ti.*, si.prefix as si_prefix, si.doc_nom as si_nom, t.name as tpoint_name, sl.name as seller_name, cl.name as client_name, ch.abr2 as cash_abr 
        from J_TAX_INVOICE ti
            left outer join J_SALE_INVOICE si on si.id=ti.sale_invoice_id
            left outer join CASH ch on ch.id=ti.cash_id
    ...
    function unlinkTaxBack($tax_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function loadTaxBackSellerClient($tax_id){$db=DbSingleton::getDb(); $answer=0;$err="Помилка";
        $client_id=$seller_id=0;$client_name=$seller_name="";
        $r=$db->query("select * from J_TAX_INVOICE where status=1 and id='$tax_id' limit 0,1;");$n=$db->num_rows($r);
        if ($n==1){
            $client_id=$db->result($r,0,"client_id");$client_name=$this->getClientName($client_id);
            $seller_id=$db->result($r,0,"seller_id");$seller_name=$this->getClientName($seller_id);
