...
function makesJmovingStorageSelect($jmoving_id){$db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();$slave=new slave; $answer=0;$err="";
	...
	$rm=$db->query("select * from J_MOVING_SELECT_TEMP where jmoving_id='$jmov' and status_jmoving='44';");$nm=$db->num_rows($rm);
			...
			for ($im=1;$im<=$nm;$im++){ $select_id+=1;
				...
				$db->query("delete from J_MOVING_SELECT_TEMP where jmoving_id='$jmov' and id='$select_id_t';");

				$this->addJuornalRecord($jmov,$select_id,$status_jmoving);
				
				$rm2=$db->query("select * from J_MOVING_SELECT_STR_TEMP where jmoving_id='$jmov' and select_id='$select_id_t';");$nm2=$db->num_rows($rm2);
				for ($im2=1;$im2<=$nm2;$im2++){
					...
					$db->query("delete from J_MOVING_SELECT_STR_TEMP  where jmoving_id='$jmov' and id='$id2';");
				}
