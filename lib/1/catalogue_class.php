...
//		$slave->print_debug($query."n=$n<br>");
	for ($i=1;$i<=$n;$i++){
		...
		$list.="
		<tr style='cursor:pointer;' onClick='location.href=\"/Catalogue/$display_nr2/$brand_id/$art_id/$display_nr-$brand_name-".$slave->translit_file($name)."\"'>
			<td>$display_nr</td>
			<td>$brand_name</td>
			<td>$name</td>
		</tr>";
...
//				print "$query\n";
	...
	if ($query_2!="" || $list2==""){  // �������� ������� ����� ��� �������� ����� � ������� ������
		...
		if ($lst_kr[1]!=""){$lst_kr[1]=str_replace("{kind_name}","��������� �������",$lst_kr[1]);$list.=$lst_kr[1];}
		if ($lst_kr[2]!=""){$lst_kr[2]=str_replace("{kind_name}","������",$lst_kr[2]);$list.=$lst_kr[2];}
		if ($lst_kr[3]!=""){$lst_kr[3]=str_replace("{kind_name}","������� ������� �",$lst_kr[3]);$list.=$lst_kr[3];}
		if ($lst_kr[4]!=""){$lst_kr[4]=str_replace("{kind_name}","������� ������ � ����",$lst_kr[4]);$list.=$lst_kr[4];}
		if ($lst_kr[5]!=""){$lst_kr[5]=str_replace("{kind_name}","����",$lst_kr[5]);$list.=$lst_kr[5];}
...
function getArticleNrDisplBrand($art_id){$db=DbSingleton::getTokoDb();$article_nr_displ="";$brand_id=0;$brand_name="";
	...
	$r=$db->query("select t2a.ART_ID,t2a.BRAND_ID,t2a.ARTICLE_NR_DISPL, t2b.BRAND_NAME from T2_ARTICLES t2a  left outer join T2_BRANDS t2b on t2b.BRAND_ID=t2a.BRAND_ID  where t2a.ART_ID='$art_id' limit 0,1;");$n=$db->num_rows($r);
	if ($n==1){
		$brand_id=$db->result($r,0,"BRAND_ID");
		$article_nr_displ=$db->result($r,0,"ARTICLE_NR_DISPL");
		$brand_name=$db->result($r,0,"BRAND_NAME");
	}
	return array($article_nr_displ,$brand_id,$brand_name);
}
...
function saveCatalogueGeneralInfo($art_id,$article_nr_displ,$barcode,$inner_cross,$brand_id,$goods_group_id,$article_name,$article_info){$db=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function showArtilceGallery($art_id,$disp_nomber){$db=DbSingleton::getTokoDb();$slave=new slave;
	...
	$r=$db->query("select t2af.* from T2_PHOTOS t2af where t2af.ART_ID='$art_id' order by t2af.PHOTO_NAME asc;");$n=$db->num_rows($r);$list="";$slide_list="";
		for ($i=1;$i<=$n;$i++){
			$file_id=$db->result($r,$i-1,"ID");
			$user_id=$db->result($r,$i-1,"USER_ID");
...
function loadArticleFoto($art_id){$db=DbSingleton::getTokoDb();$slave=new slave;
	...
	$r=$db->query("select t2af.* from T2_PHOTOS t2af where t2af.ART_ID='$art_id' order by t2af.PHOTO_NAME asc;");$n=$db->num_rows($r);$list="";
	for ($i=1;$i<=$n;$i++){
		$file_id=$db->result($r,$i-1,"ID");
		$user_id=$db->result($r,$i-1,"USER_ID");
		$file_name=$db->result($r,$i-1,"PHOTO_NAME"); $file_name=trim(preg_replace('/\s\s+/', ' ', $file_name));
...
function loadCatalogueGoodGroupTemplateParams($art_id,$template_id){session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
...
function saveCatalogueParamsTemplate($art_id,$goods_group_id,$template_id,$template_name,$cn,$params_id,$fields_type,$params_name){$db=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	...
	if ($art_id>0){
		...
		if ($template_id==0){
			...
			$db->query("insert into GOODS_GROUP_TEMPLATE (`ID`,`GOODS_GROUP_ID`,`NAME`) values ('$template_id','$goods_group_id','$template_name');");
		}
		if ($template_id>0){
			$db->query("update GOODS_GROUP_TEMPLATE set `NAME`='$template_name' where `ID`='$template_id' and `GOODS_GROUP_ID`='$goods_group_id';");
			for ($i=1;$i<=$cn;$i++){
				$param_id=$params_id[$i];
				$param_name=$params_name[$i];
				$field_type=$fields_type[$i];
				if ($param_id==0 && $param_name!=""){
					$db->query("insert into GOODS_GROUP_TEMPLATE_PARAMS (`PARAM_ID`,`TEMPLATE_ID`,`NAME`,`FIELD_TYPE`) values ('','$template_id','$param_name','$field_type');");
				}
				if ($param_id>0){
					$db->query("update GOODS_GROUP_TEMPLATE_PARAMS set `NAME`='$param_name', `FIELD_TYPE`='$field_type' where `PARAM_ID`='$param_id' and `TEMPLATE_ID`='$template_id';");
				}
			}
			$answer=1;$err="";
		}
	...
	if ($art_id>0){
		...
		foreach($typ_array as $typ_id){
			if ($typ_id>0 && $typ_id!=""){
				...
				$r2=$db->query("select max(LA_ID) as mid from link_notes;");$la_id_new=0+$db->result($r2,0,"mid")+1; print"la_id_new=$la_id_new\n";
				$db->query("insert into link_notes (`LA_ID`,`LANG_ID`,`SORT`,`TEXT_NAME`,`TYPE`,`TEXT`,`DISPLAY`) values ('$la_id_new','16','1','','K','$comment','1');");
...
function showDocumentCostumsManual($sel_id,$pos){$db=DbSingleton::getTokoDb();$manual=new manual;$list="";
	...
	for ($i=1;$i<=$n;$i++){
		...
		$sel="";if ($sel_id==$id){$sel=" style='background-color:#d5fdf5'";}
		$list.="<tr class='pointer' onClick='selectCostumsDocument(\"$id\",\"$code\")' $sel>
			<td>$code</td>
			<td>$name</td>
			<td align='right'>$preferential_rate</td>
			<td align='right'>$full_rate</td>
...
function loadArticlePricing($art_id){$db=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	...
	$r=$db->query("select t2c.ART_ID,t2c.KIND,t2c.RELATION from T2_CROSS t2c left outer join T2_BRANDS t2b on t2b.BRAND_ID=t2c.BRAND_ID  where t2c.SEARCH_NUMBER='$article_nr_displ' order by t2c.KIND asc;");$n=$db->num_rows($r);$analog_str="0";$ak=array();$rk=array(); $art_id_str="";
	for ($i=1;$i<=$n;$i++){
		$ART_ID=$db->result($r,$i-1,"ART_ID");
		$KIND=$db->result($r,$i-1,"KIND");
		$RELATION=$db->result($r,$i-1,"RELATION");
	...
	$r=$db->query("select t2a.ART_ID,t2a.BRAND_ID,t2a.ARTICLE_NR_DISPL, t2b.BRAND_NAME from T2_ARTICLES t2a 
				left outer join T2_BRANDS t2b on t2b.BRAND_ID=t2a.BRAND_ID 
				left outer join T2_BARCODES t2bc on t2bc.ART_ID=t2a.ART_ID 
				where t2a.ART_ID in ($art_id_str) and t2bc.BARCODE!='';");$n=$db->num_rows($r);$ak0="";$ak1="";$ak2="";$ak3="";$ak4="";$articles_list="";
	for ($i=1;$i<=$n;$i++){
...
function getLineArticleSales($art_id){$db=DbSingleton::getTokoDb();$slave=new slave;$list="";
	if ($art_id!=""){ 
		$r=$db->query("select sum(AMOUNT) as sum_amount from T2_ARTICLES_SALES where art_id='$art_id' group by `MONTH` order by `MONTH` desc;");$n=$db->num_rows($r);
		for ($i=1;$i<=$n;$i++){
...
function showArticleSales($art_id){$db=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� �������";
	...
	if ($art_id!=""){ list($article_displ_nomber,$brand_id)=$this->getArticleNrDisplBrand($art_id);
		...
		$r=$db->query("select * from T2_ARTICLES_SALES where art_id='$art_id' order by TPOINT_ID asc, `MONTH` desc;");$n=$db->num_rows($r);$list="";$prev_tpoint=0;$kol_month=0;
		for ($i=1;$i<=$n;$i++){
			$month=substr($db->result($r,$i-1,"MONTH"),5,2)."<br>".substr($db->result($r,$i-1,"MONTH"),0,4);
			$amount=$db->result($r,$i-1,"AMOUNT");
			$tpoint_id=$db->result($r,$i-1,"TPOINT_ID"); if ($tpoint_id==0){$tpoint_id=1;}
			$tpoint_name=$this->getTpointName($tpoint_id);
			
			if ($prev_tpoint!=$tpoint_id){$kol_month=0; $prev_tpoint=$tpoint_id;
				$list=str_replace("{month_list}",$month_list,$list);
				$list=str_replace("{sale_list}",$sale_list,$list);
				$list=str_replace("{kol_elm}",$kol_month,$list);
				$list.=$block;
				$list=str_replace("{tpoint_name}",$tpoint_name,$list);
				$month_list="";$sale_list="";
			}$kol_month+=1;
			$month_list.="<th style='text-align:center'>$month</th>";
			$sale_list.="<td style='text-align:center'>$amount</td>";
	...
	if ($list2==""){  // �������� ������� ����� ��� �������� ����� � ������� ������
		...
		for ($i=1;$i<=$n;$i++){
			...
			$list.="<tr style='cursor:pointer'>
				<td class='text-center'><button class='btn btn-sm btn-default' onclick='setArticleToDoc(\"$art_id\",\"$article_nr_displ\",\"$brand_id\",\"$brand_name\",\"$costums_id\",\"$costums_code\",\"$country_id\",\"$country_name\")'><i class='fa fa-plus'></i></button></td>
...
function showArticleStorageCellsRestForm($art_id){$db=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	if ($art_id>0){
		...
		where t2s.ART_ID='$art_id';");$n=$db->num_rows($r);$list="";
		for ($i=1;$i<=$n;$i++){
			$amount=$db->result($r,$i-1,"amount");
...
function viewArticleReservDocs($art_id,$storage_id){ $db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="�������";
	if ($art_id>0){
		...
		$r=$db->query("select j.id,j.prefix,j.doc_nom,j.type_id,j.data,SUM(js.amount) as amount,j.user_id from J_MOVING_STR js left outer join J_MOVING j on (j.id=js.jmoving_id) where js.art_id='$art_id' and js.status_jmoving in (44,45) and j.status='1' and js.amount>0 and js.storage_id_from='$storage_id' and (j.oper_status='30' or (j.oper_status='31' and j.status_jmoving='49' or j.status_jmoving='48')) group by j.id;");$n=$db->num_rows($r);
		for ($i=1;$i<=$n;$i++){
		...
		$r=$db->query("select dp.id,dp.prefix,dp.doc_nom,dp.data,SUM(dps.amount) as amount,dp.user_id,dp.client_id from J_DP_STR dps left outer join J_DP dp on (dp.id=dps.dp_id) where dps.art_id='$art_id' and dps.status_dp in (44,45) and dp.status='1' and dps.amount>0 and dps.storage_id_from='$storage_id' and (dp.oper_status='30' or dp.oper_status='31') group by dp.id;");$n=$db->num_rows($r);
		for ($i=1;$i<=$n;$i++){
...
function viewArticleCellsRest($art_id,$storage_id){ $db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="�������";
	if ($art_id>0){
		...
		$r=$dbt->query("select t2sc.AMOUNT,t2sc.RESERV_AMOUNT,sc.cell_value from T2_ARTICLES_STRORAGE_CELLS t2sc left outer join STORAGE_CELLS sc on (sc.id=t2sc.STORAGE_CELLS_ID) where t2sc.storage_id='$storage_id' and t2sc.ART_ID='$art_id' and (t2sc.AMOUNT>0 or t2sc.RESERV_AMOUNT>0);");$n=$db->num_rows($r);
		for ($i=1;$i<=$n;$i++){
			$amount=$db->result($r,$i-1,"AMOUNT");
			$reserv_amount=$db->result($r,$i-1,"RESERV_AMOUNT");
...
function showArticlesSearchListDoc($art,$brand_id,$query_2,$search_type,$doc_type,$doc_id){$db=DbSingleton::getTokoDb();$dbp=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	if ($query_2=="" && $search_type==0){$art_search=$art;
		$art=$this->clearArticle($art); if ($brand_id==0){$brand_id="";}
		
		$doc_type=$slave->qq($doc_type);$doc_id=$slave->qq($doc_id);
		print "$art,$brand_id,$query_2,$search_type,$doc_type,$doc_id";
		if ($doc_type=="dp"){
			require_once RD.'/lib/dp_class.php';$dp=new dp;
			list($price_lvl,$margin_price_lvl,$price_suppl_lvl,$margin_price_suppl_lvl,$client_vat)=$dp->getDpClientPriceLevels($doc_id);
		...
		if (($n>1 && $brand_id!="") || $one_result==1){$ak=array();$rk=array();
			...
			/*$query="select t2a.ART_ID, t2a.BRAND_ID, t2a.ARTICLE_NR_DISPL, t2b.BRAND_NAME, t2n.NAME, t2n.INFO, t2bc.BARCODE, gg.NAME as goods_group_name, `t2asc`.`AMOUNT` as stock, `t2asc`.`RESERV_AMOUNT` as reserv, s.name as storage_name, s.id as storage_id, t2apr.price_".$price_lvl.", t2sai.suppl_id, t2sai.return_delay, t2sai.warranty_info , t2si.price_usd, t2si.client_storage_id, t2si.stock_suppl
				from T2_ARTICLES t2a 
				left outer join T2_BRANDS t2b on t2b.BRAND_ID=t2a.BRAND_ID 
				left outer join T2_NAMES t2n on t2n.ART_ID=t2a.ART_ID 
				left outer join T2_BARCODES t2bc on t2bc.ART_ID=t2a.ART_ID 
	...
	if ($query_2=="" && $search_type==2){
		$query="select t2a.ART_ID, t2a.BRAND_ID, t2a.ARTICLE_NR_DISPL, t2b.BRAND_NAME, t2n.NAME, t2n.INFO, t2bc.BARCODE, gg.NAME as goods_group_name, s.name as storage_name, s.id as storage_id, t2apr.price_".$price_lvl.", t2sai.suppl_id, t2sai.return_delay, t2sai.warranty_info , t2si.price_usd, t2si.client_storage_id, t2si.stock_suppl
				from T2_ARTICLES t2a 
				left outer join T2_BRANDS t2b on t2b.BRAND_ID=t2a.BRAND_ID 
				left outer join T2_NAMES t2n on t2n.ART_ID=t2a.ART_ID 
				left outer join T2_BARCODES t2bc on t2bc.ART_ID=t2a.ART_ID 
	...
	if ($query_2=="" && $search_type==3){
		$query="select t2a.ART_ID, t2a.BRAND_ID, t2a.ARTICLE_NR_DISPL, t2b.BRAND_NAME, t2n.NAME, t2n.INFO, t2bc.BARCODE, gg.NAME as goods_group_name, s.name as storage_name, s.id as storage_id, t2apr.price_".$price_lvl.", t2sai.suppl_id, t2sai.return_delay, t2sai.warranty_info , t2si.price_usd, t2si.client_storage_id, t2si.stock_suppl
				...
				where t2a.ART_ID='$art' and t2b.`VISIBLE`='1';";
	}
	if ($query_2!=""){$query=$query_2;}
	$r=$db->query($query);$n=$db->num_rows($r);$list="";$header_list="";
	if ($query_2!="" || $list2==""){  // �������� ������� ����� ��� �������� ����� � ������� ������
		...
		for ($i=1;$i<=$n;$i++){
			...
			if ($doc_type=="dp"){
				...
				if ($suppl_id==0){
					...
					list($tpoint_stock,$tpoint_reserv)=$dp->getArticleRestTpoint($art_id,$tpoint_id); $amountRestTpoint="$tpoint_stock/$tpoint_reserv";
