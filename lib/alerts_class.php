...
class alerts {
	...
	function get_max_alerts_id(){$db=DbSingleton::getDb();  $r=$db->query("select max(id) as mid from media_alerts;");return $db->result($r,0,"mid")+1; }

	function list_alerts_navigation(){$db=DbSingleton::getDb();session_start(); $media_org_id=$_SESSION["media_org_id"];$media_user_id=$_SESSION["media_user_id"];//$url=$mdl->get_file_url("alerts");
		$form_htm=RD."/tpl/alerts_navigation_list.htm";$form="";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);} $kp=20;$list="";$k=0;
	...
	function get_alert_label($label_id){$db=DbSingleton::getDb();$label="default";
		$r=$db->query("SELECT label FROM alerts_label where id='$label_id' limit 0,1;"); $n=$db->num_rows($r);
		if ($n==1){$label=$db->result($r,0,"label");}
		return $label;
	}

	function getUserName($user_id){$db=DbSingleton::getDb();$name="";
