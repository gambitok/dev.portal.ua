...
function show_money_move_list(){$db=new db;$slave=new slave;$gmanual=new gmanual;$dp=new dp; $where="";$limit ="limit 0,300"; if ($where!=""){$limit="";}
...
function loadMoneyMoveCashBoxList($client_id,$doc_type_id,$seller_id){$db=new db;$list="";
	$list=$this->showPayBoxSelectList(0,$doc_type_id,$seller_id);
	return $list;
}
function showMoneyMoveForm(){$db=new db;$cat=new catalogue;$slave=new slave;$manual=new manual;$gmanual=new gmanual; session_start(); $user_id=$_SESSION["media_user_id"]; $user_name=$_SESSION["user_name"];  
...
function viewMoneyMove($move_id){$db=new db;$cat=new catalogue;$slave=new slave;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"]; $storsel=new storsel; $gmanual=new gmanual; 
...
function saveMoneyMove($paybox_id_from,$paybox_id_to,$user_id_to,$balans_id_from,$summ){$db=new db;$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="Помилка збереження даних!";
...
function acceptMoneyMove($move_id){$db=new db;$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="Прийняти переказ може лише адресат!";
...
function updatePayboxBalans($paybox_id,$deb_kre,$cash_id,$summ,$user_id,$money_move_id){$db=new db;
