...
function show_money_move_list(){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;$dp=new dp; $where="";$limit ="limit 0,300"; if ($where!=""){$limit="";}
	$r=$db->query("select j.*, CASH.name as cash_name, pf.name as name_from, pf.full_name as full_name_from, muf.name,
	pt.name as name_to, pt.full_name as full_name_to, mut.name, msm.mcaption as status_move_name from J_MONEY_MOVE j
	...
	for ($i=1;$i<=$n;$i++){
		...
		$paybox_id_to=$db->result($r,$i-1,"paybox_id_to");
		...
		if ($data!=null) {
		$data = strtotime($data);
		$date_convert=date('d-m-Y H:i:s', $data); } else $date_convert=$data;
		
		if ($data_accept!=null) {
		$data_accept = strtotime($data_accept);
		$date_accept_convert=date('d-m-Y H:i:s', $data_accept); } else $date_accept_convert=$data_accept;
...
function loadMoneyMoveCashBoxList($client_id,$doc_type_id,$seller_id){$db=DbSingleton::getDb();$list="";
	$list=$this->showPayBoxSelectList(0,$doc_type_id,$seller_id);
	return $list;
}
function showMoneyMoveForm(){$db=DbSingleton::getDb();$cat=new catalogue;$slave=new slave;$manual=new manual;$gmanual=new gmanual; session_start(); $user_id=$_SESSION["media_user_id"]; $user_name=$_SESSION["user_name"];  
	$form_htm=RD."/tpl/money_move_form.htm";	if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
...
function viewMoneyMove($move_id){$db=DbSingleton::getDb();$cat=new catalogue;$slave=new slave;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"]; $storsel=new storsel; $gmanual=new gmanual; 
	$form_htm=RD."/tpl/money_move_accept_view.htm";	if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
	$r=$db->query("select j.*, CASH.name as cash_name, pf.name as name_from, pf.full_name as full_name_from, muf.name,
	...
	left outer join PAY_BOX pt on pt.id=j.paybox_id_to
	left outer join media_users mut on mut.id=j.user_id_to
	where j.id='$move_id' limit 0,1;");$n=$db->num_rows($r);
	if ($n==1){
		$id=$db->result($r,0,"id");
...
function getPayboxUserCashSaldoList($paybox_id,$user_id){$db=DbSingleton::getDb(); $list="<option value=\"0\">--������ � ������--</option>";
...
function getPayboxResiverList($paybox_id,$balans_id_from, $user_id){$db=DbSingleton::getDb(); $list="<option value=\"0\">--������ � ������--</option>";
	$paybox_type_id=$this->getPayBoxType($paybox_id);
	$r=$db->query("select pb.* from PAY_BOX pb left outer join PAY_BOX_WORKERS pbw on pbw.paybox_id=pb.id where  pb.doc_type_id='$paybox_type_id' and pbw.status=1 and pb.status=1 and pb.in_use=1 group by pb.id order by pb.name asc;");
	$n=$db->num_rows($r);
	for ($i=1;$i<=$n;$i++){
		$id=$db->result($r,$i-1,"id");
		$name=$db->result($r,$i-1,"name");
...
function getPayboxManagerList($paybox_id,$balans_id_from){$db=DbSingleton::getDb(); $list="<option value=\"0\">--������ � ������--</option>";
	$paybox_type_id=$this->getPayBoxType($paybox_id);
	$r=$db->query("select pbw.* from PAY_BOX pb left outer join PAY_BOX_WORKERS pbw on pbw.paybox_id=pb.id where pb.id='$paybox_id' and pb.doc_type_id='$paybox_type_id' and pbw.status=1 and pb.status=1 and pb.in_use=1 group by pbw.worker_id order by pb.name asc;");
	$n=$db->num_rows($r);
	for ($i=1;$i<=$n;$i++){
...
function saveMoneyMove($paybox_id_from,$paybox_id_to,$user_id_to,$balans_id_from,$summ){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	$paybox_id_from=$slave->qq($paybox_id_from);$paybox_id_to=$slave->qq($paybox_id_to);$user_id_to=$slave->qq($user_id_to);
	...
	if ($paybox_id_from>0 && $paybox_id_to>0 && $user_id_to>0 && $balans_id_from>0 && $summ>=0){
		...
		if ($summ>$current_balans_summ){ $answer=0;$err="���� ���������� ��� ����� �� ������ � ���!"; }
...
function acceptMoneyMove($move_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="�������� ������� ���� ���� �������!";
