...
class tpoint {
    ...
    function show_tpoint_list(){$db=DbSingleton::getTokoDb();$where="";
    ...
    function deleteTpoint($tpoint_id) {$db=DbSingleton::getTokoDb(); $answer=0;$err="Помилка збереження даних!";
        if($tpoint_id>0) {
    ...
    function saveTpointGeneralInfo($tpoint_id,$name,$full_name,$address,$chief,$country_id,$state_id,$region_id,$city_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function showTpointStorageForm($tpoint_id,$s_id){$db=DbSingleton::getTokoDb();$gmanual=new gmanual;
        ...
        $r=$db->query("select * from T_POINT_STORAGE where id='$s_id' and tpoint_id='$tpoint_id' limit 0,1;");
    ...
    function showTpointSupplStorageForm($tpoint_id,$s_id){$db=DbSingleton::getDb();
        ...
        $r=$db->query("select * from T_POINT_SUPPL_STORAGE where id='$s_id' and tpoint_id='$tpoint_id' limit 0,1;");
    ...
    function dropTpointSupplStorageForm($tpoint_id,$s_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function saveTpointSupplStorageForm($tpoint_id,$s_id,$storage_id,$suppl_id){
        $db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function saveTpointStorageForm($tpoint_id,$s_id,$storage_id,$local,$default){ $db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function dropTpointStorage($tpoint_id,$s_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
        $tpoint_id=$slave->qq($tpoint_id);$s_id=$slave->qq($s_id);
        if ($tpoint_id>0 && $s_id>0){
    ...
    function loadTpointClients($tpoint_id){$db=DbSingleton::getTokoDb();$gmanual=new gmanual;
    ...
    function showTpointClientsForm($tpoint_id,$s_id){$db=DbSingleton::getTokoDb();$gmanual=new gmanual;
        $form="";$form_htm=RD."/tpl/tpoint_clients_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("select pc.* from T_POINT_CLIENTS pc where pc.id='$s_id' and pc.tpoint_id='$tpoint_id' limit 0,1;");
    ...
    function saveTpointClientsForm($tpoint_id,$s_id,$client_id,$sale_type,$tax_credit,$tax_inform,$in_use){ $db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function dropTpointClients($tpoint_id,$s_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
        $tpoint_id=$slave->qq($tpoint_id);$s_id=$slave->qq($s_id);
        if ($tpoint_id>0 && $s_id>0){
    ...
    function showTpointWorkersForm($tpoint_id,$s_id){$db=DbSingleton::getTokoDb();
        ...
        $r=$db->query("select * from T_POINT_WORKERS where id='$s_id' and tpoint_id='$tpoint_id' limit 0,1;");
    ...
    function saveTpointWorkersForm($tpoint_id,$s_id,$worker_id){ $db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function dropTpointWorkers($tpoint_id,$s_id){$db=DbSingleton::getTokoDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
        $tpoint_id=$slave->qq($tpoint_id);$s_id=$slave->qq($s_id);
        if ($tpoint_id>0 && $s_id>0){
    ...
    function showTpointClientList($sel_id){$db=DbSingleton::getDb();$slave=new slave;
    ...
    function showTpointSupplSelectList($sel_id){$db=DbSingleton::getDb();$list="";
        $r=$db->query("select c.* from A_CLIENTS c left outer join A_CLIENTS_CATEGORY cc on cc.client_id=c.id where c.status='1' and cc.category_id='2';");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
            $name=$db->result($r,$i-1,"name");
            $sel="";if ($id==$sel_id){$sel=" selected";}
    ...
    function getTpointNameById($sel_id, $field="name"){$db=DbSingleton::getTokoDb();$name="";
    ...
    function loadTpointDeliveryTime($tpoint_id){$db=DbSingleton::getTokoDb();$slave=new slave;
        ...
        for ($i=1;$i<=$n;$i++){
            ...
            // $dd_word="через ".$delivery_days." дн. ";if ($delivery_days==0){$dd_word="сьогодні";}if ($delivery_days==1){$dd_word="завтра";}
    ...
    function showTpointDeliveryForm($tpoint_id,$s_id){$db=DbSingleton::getTokoDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];
        $form="";$form_htm=RD."/tpl/tpoint_delivery_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("select * from T_POINT_DELIVERY_TIME where id='$s_id' and tpoint_id='$tpoint_id' limit 0,1;");
        $storage_id=$db->result($r,0,"storage_id");
    ...
    function saveTpointDeliveryForm($tpoint_id,$s_id,$storage_id,$week_day,$time_from,$time_to,$delivery_days,$giveout_time,$time_from_del,$time_to_del){ $db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function dropTpointDelivery($tpoint_id,$s_id){$db=DbSingleton::getTokoDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
        $tpoint_id=$slave->qq($tpoint_id);$s_id=$slave->qq($s_id);
        if ($tpoint_id>0 && $s_id>0){
    ...
    function showTpointSupplDeliveryForm($tpoint_id,$s_id){$db=DbSingleton::getTokoDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];
        $form="";$form_htm=RD."/tpl/tpoint_suppl_delivery_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("select * from T_POINT_SUPPL_DELIVERY_TIME where id='$s_id' and tpoint_id='$tpoint_id' limit 0,1;");
        $suppl_id=$db->result($r,0,"suppl_id");
    ...
    function saveTpointSupplDeliveryForm($tpoint_id,$s_id,$suppl_id,$suppl_storage_id,$week_day,$time_from,$time_to,$delivery_days,$giveout_time,$time_from_del,$time_to_del){ $db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function dropTpointSupplDelivery($tpoint_id,$s_id){$db=DbSingleton::getTokoDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
        $tpoint_id=$slave->qq($tpoint_id);$s_id=$slave->qq($s_id);
        if ($tpoint_id>0 && $s_id>0){
    ...
    function showTpointSupplFmForm($tpoint_id,$s_id){$db=DbSingleton::getTokoDb();session_start();$media_user_id=$_SESSION["media_user_id"];
        $form="";$form_htm=RD."/tpl/tpoint_suppl_fm_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("select * from T_POINT_SUPPL_FM where id='$s_id' and tpoint_id='$tpoint_id' limit 0,1;");
        $suppl_id=$db->result($r,0,"suppl_id");
        $suppl_storage_id=$db->result($r,0,"suppl_storage_id");
    ...
    function saveTpointSupplFmForm($tpoint_id,$s_id,$suppl_id,$suppl_storage_id,$price_rating_id,$price_from,$price_to,$margin,$delivery,$margin2){ $db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function dropTpointSupplFm($tpoint_id,$s_id){$db=DbSingleton::getTokoDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
        $tpoint_id=$slave->qq($tpoint_id);$s_id=$slave->qq($s_id);
        if ($tpoint_id>0 && $s_id>0){
    ...
    function loadTpointPayBox($tpoint_id){$db=DbSingleton::getDb();$list="";
        $form="";$form_htm=RD."/tpl/tpoint_pay_box_list.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("select pb.* from T_POINT_PAY_BOX pb where pb.tpoint_id='$tpoint_id' and pb.status='1' order by pb.id asc;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
            $client_id=$db->result($r,$i-1,"client_id");
            $client_name=$this->getAClientName($client_id);
    ...
    function showTpointPayBoxForm($tpoint_id,$s_id){$db=DbSingleton::getDb();
        ...
        $r=$db->query("select pb.* from T_POINT_PAY_BOX pb where pb.id='$s_id' and pb.tpoint_id='$tpoint_id' limit 0,1;");
        $client_id=$db->result($r,0,"client_id");
        $client_name=$this->getAClientName($client_id);
        $name=$db->result($r,0,"name");
        $in_use=$db->result($r,0,"in_use");
    ...
    function saveTpointPayBoxForm($tpoint_id,$s_id,$client_id,$name,$in_use){ $db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function dropTpointPayBox($tpoint_id,$s_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
        $tpoint_id=$slave->qq($tpoint_id);$s_id=$slave->qq($s_id);
        if ($tpoint_id>0 && $s_id>0){
