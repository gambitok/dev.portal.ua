...
function show_kours_list(){$db=DbSingleton::getDb();$slave=new slave;$where="";
	$r=$db->query("select k.*, c.name, c.symbol from J_KOURS k
		...
		for ($i=1;$i<=$n;$i++){
			...
			$data_to=$db->result($r,$i-1,"data_to");if ($data_to=="0000-00-00 00:00:00"){$data_to="�������� ����";}
			$in_use=$db->result($r,$i-1,"in_use");$inuse="";if ($in_use==1){$inuse="�����";}
			$list.="<tr>
				<td>$inuse</td>
				<td>$name ($symbol)</td>
				<td>$kours_value</td>
...
function saveKoursForm($kours_id,$kours_value,$cash_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
