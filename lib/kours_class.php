...
class kours {
    ...
    function show_kours_list(){$db=DbSingleton::getDb();
        $r=$db->query("SELECT k.*, c.name, c.symbol 
        FROM `J_KOURS` k
            LEFT OUTER JOIN `CASH` c on c.id=k.cash_id 
        WHERE k.in_use IN (0,1) ORDER BY k.in_use DESC, k.id ASC;");$n=$db->num_rows($r);$list="";
        for ($i=1;$i<=$n;$i++){
    ...
    function showKoursCard($kours_id){$db=DbSingleton::getDb();
        ...
        $r=$db->query("SELECT * FROM `J_KOURS` WHERE `id`='$kours_id' LIMIT 1;");$n=$db->num_rows($r);
    ...
    function showCashSelectList($sel_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `CASH` ORDER BY `name`, `id` ASC;");$n=$db->num_rows($r);$list="";
        for ($i=1;$i<=$n;$i++){
