...
class CatalogParts
{
    ...
    public function getCatalogPartsForm()
    {
        $form_htm=RD."/tpl/catalog_parts/form.htm";$form="";
        if (file_exists($form_htm)){ $form = file_get_contents($form_htm);}
        $form = str_replace(array("{catalog_range}", "{select_category_list}", "{select_brand_list}"), array("", $this->showCatalogCategoryList(), $this->showCatalogBrandList()), $form);
        return $form;
    }

    /*==== CATALOG TREE ====*/
    public function showCatalogTree(): string
    {
        ...
        for ($i=1; $i<=$n; $i++) {
            ...
            if ($str_id_parrent===""){$str_id_parrent=0;}
    ...
    public function getTecGroupTreeChilds($str_id): int
    {
        $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT COUNT(`STR_ID`) as kol FROM `T2_GROUP_TREE` WHERE `STR_ID_PARENT`='$str_id';");
        return (int)$db->result($r, 0, "kol");
    }

    /*==== /CATALOG TREE ====*/

    public function checkAutoLink($art_id)
    {
        $dbt=DbSingleton::getTokoDb();
        $r=$dbt->query("SELECT COUNT(`TYP_ID`) as count_types FROM `T2_LINKS` WHERE `ART_ID`='$art_id';");
        $n=$dbt->num_rows($r);
    ...
    public function saveCatalogParts($group_id, $arts): string
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function saveCatalogParts2($group_id, $str_id=0, $brand_id=[], $type_id=0, $text="", $name="", $name_exist="", $name_select=[], $name_exist_select=[], $checked_auto=1): string
    {
        $db = DbSingleton::getTokoDb();
        session_start();
        ...
        $where_str="";
        if ($str_id>0) {
            $where_str = "AND t2t.`STR_ID`='$str_id'";
        }
        $where_text="";
        if ($text!=="") {
            $where_text = "AND t2a.`ARTICLE_NR_DISPL` LIKE '%$text%'";
        }
        $where_name="";
        if ($name!=="") {
            $where_name = "AND t2n.`NAME` LIKE '%$name%'";
        }
        $where_name_exist="";
        if ($name_exist!=="") {
            $where_name_exist = "AND t2n.`NAME_EXIST` LIKE '%$name_exist%'";
        }
        ...
        if (!empty($brand_id)) {
            ...
            $brand_id_str = implode(",", $brand_id);
            $where_brand="AND t2a.`BRAND_ID` IN ($brand_id_str)";
        }
        ...
        if (!empty($name_select)) {
            ...
            $name_select_str = implode(",", $name_select);
            $where_name_select="AND t2n.`NAME` IN ($name_select_str)";
        }
        ...
        if (!empty($name_exist_select)) {
            ...
            $name_exist_select_str = implode(",", $name_exist_select);
            $where_name_exist_select="AND t2n.`NAME_EXIST` IN ($name_exist_select_str)";
        }
        ...
        if ($str_id > 0) {
            ...
            for ($i = 1; $i <= $n; $i++) {
                ...
                if ($type_id > 0) {
                    if (($type_id === 1) && !$this->checkCatalogTreeArts($art_id)) {
                        $arts[] = $art_id;
                    }
                    if (($type_id === 2) && $this->checkCatalogTreeArts($art_id)) {
                        $arts[] = $art_id;
                    }
                } else {
                    $arts[] = $art_id;
                ...
                if (!$checked_auto && $this->checkAutoLink($art_id)) {
                    array_pop($arts);
                }
            }
        }
    ...
    public function checkCatalogTreeArts($art_id): int
    {
        $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT * FROM `T2_TREE_ARTS` WHERE `ART_ID`='$art_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n === 0) {
            return 0;
        }
    ...
    public function showCatalogList($str_id=0, $brand_id=[], $type_id=0, $text="", $name="", $name_exist="", $name_select=[], $name_exist_select=[], $checked_auto=1): array
    {
        $dbt = DbSingleton::getTokoDb();
        $list = "";
        $arr = [];
    ...
    public function showCatalogPartsCard($art_ids_str)
    {
        $art_ids_str = trim($art_ids_str, ",");
        $art_ids = explode(",", $art_ids_str);
        $form_htm=RD."/tpl/catalog_parts/card.htm";$form="";
        if (file_exists($form_htm)){ $form = file_get_contents($form_htm);}
        $form = str_replace(array("{art_ids_range}", "{art_ids_count}", "{selected_category}", "{selected_category_list}"), array(implode(",", $art_ids), count($art_ids), "НЕ ВИБРАНО!", $this->showCatalogPartsHeader()), $form);

        return $form;
    }

    public function showCatalogPartsCard2()
    {
        $form_htm=RD."/tpl/catalog_parts/card2.htm";$form="";
        if (file_exists($form_htm)){ $form = file_get_contents($form_htm);}
        $form= str_replace(array("{selected_category}", "{selected_category_list}"), array("НЕ ВИБРАНО!", $this->showCatalogPartsHeader()), $form);
        return $form;
    }

    public function showCatalogCategoryList(): string
    {
        $dbt = DbSingleton::getTokoDb();
        $list = "";
        $r = $dbt->query("SELECT * FROM `T2_GROUP_TREE` WHERE `LNG_ID`=16 ORDER BY `TEX_TEXT` ASC;");
        $n = $dbt->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
            $str_id = (int)$dbt->result($r, $i - 1, "STR_ID");
    ...
    public function getCountArticleT2Tree($str_id)
    {
        $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT COUNT(`ART_ID`) as count_arts FROM `T2_TREE_NEW` WHERE `STR_ID`='$str_id';");
        $n = $db->num_rows($r);
    ...
    public function getStrText($str_id)
    {
        $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT * FROM `T2_GROUP_TREE` WHERE `STR_ID`='$str_id' AND `LNG_ID`=16 LIMIT 1;");
        return $db->result($r, 0, "DISP_TEXT");
    }

    public function showCatalogPartsHeader(): string
    {
        $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT * FROM `T2_TREE_HEAD`;");
        $n = $db->num_rows($r);
    ...
    public function showCatalogPartsCategory($head_id): string
    {
        $db = DbSingleton::getTokoDb();
        $arr=[];
        $list="";

		$r=$db->query("SELECT * FROM `T2_TREE_HCG` WHERE `HEAD_ID`='$head_id' ORDER BY `CAT_ID`;");
		$n=(int)$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
        ...
        foreach ($arr as $cat_id=>$group_ids) {
            ...
            usort($group_arr, static function($a, $b) {
    ...
    public function getCatName($cat_id) {$db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT `TEX_RU` FROM `T2_TREE_CAT` WHERE `CAT_ID`='$cat_id' LIMIT 1;");
        return $db->result($r, 0, "TEX_RU");
    }

    public function getGroupName($group_id) {$db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT `TEX_RU` FROM `T2_TREE_GROUP` WHERE `GROUP_ID`='$group_id' LIMIT 1;");
        return $db->result($r, 0, "TEX_RU");
    }

    public function getGroupReviewed($group_id)
    {
        $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT `REVIEWED` FROM `T2_TREE_GROUP` WHERE `GROUP_ID`='$group_id' LIMIT 1;");
        return $db->result($r, 0, "REVIEWED");
    }

    public function showCatalogBrandList(): string
    {
        $dbt = DbSingleton::getTokoDb();
        $list="";
        $r=$dbt->query("SELECT * FROM `T2_BRANDS` WHERE 1 ORDER BY `BRAND_NAME` ASC;");
        $n=$dbt->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
    ...
    public function getArticlePhoto($art_id): string
    {
        $db= DbSingleton::getTokoDb();
        $r = $db->query("SELECT * FROM `T2_PHOTOS` WHERE `ART_ID`='$art_id' ORDER BY `MAIN` DESC LIMIT 1;");
        $image_src = $db->result($r, 0, "PHOTO_NAME");
        return "<img width='100' height='100' src=\"https://toko.ua/uploads/images/catalogue/$image_src\" style=\"display: block; margin: 0 auto;\" alt=\"\">";
    }

    // NOVA _DETAIL
    public function getGroupTreeName($art_id)
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function checkArts($art_id): int
    {
        $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT COUNT(`ART_ID`) as count_arts FROM `T2_TREE_ARTS` WHERE `ART_ID`='$art_id' LIMIT 1;");
        return (int)$db->result($r, 0, "count_arts");
    }

    public function setCatalogPartsBrands($str_id, $type_id): array
    {
        $dbt = DbSingleton::getTokoDb();
        ...
        for ($i=1; $i<=$n; $i++) {
            ...
            if ($type_id === 0) {
                $arr_brand[] = $brand_id;
                $arr_name[] = $name;
                $arr_name_exist[] = $name_exist;
            }

            // НЕ ПРИВЯЗАНІ
            if (($type_id === 1) && $this->checkArts($art_id) === 0) {
                $arr_brand[] = $brand_id;
                $arr_name[] = $name;
                $arr_name_exist[] = $name_exist;
            }
            ...
            // ПРИВЯЗАНІ
            if (($type_id === 2) && $this->checkArts($art_id) > 0) {
                $arr_brand[] = $brand_id;
                $arr_name[] = $name;
                $arr_name_exist[] = $name_exist;
            }
        ...
        if (!empty($arr_brand)) {
            $list_brand .= "<option value='0'>-Показати всі-</option>";
        }
        ...
        unset($arr_name[0], $arr_name_exist[0]);

        return array($list_brand, $list_name, $arr_name, $list_name_exist, $arr_name_exist);
    }

    public function getArticleNameCount($name_select)
    {
        $db = DbSingleton::getTokoDb();
        $count = 0;
        if (!empty($name_select)) {
    ...
    public function setCatalogPartsBrandsName($str_id, $brand_id, $type_id): array
    {
        $dbt=DbSingleton::getTokoDb();
        $type_id = (int)$type_id;
        ...
        sort($arr_name);
        $arr_name_exist=array_unique($arr_name_exist);
        sort($arr_name_exist);
    ...
    public function showCatalogPartsEditCard()
    {
        $form_htm=RD."/tpl/catalog_parts/edit.htm";$form="";
        if (file_exists($form_htm)){ $form = file_get_contents($form_htm);}
    ...
    /*==== EDIT ====*/
    public function showCatalogPartsHeaderEdit(): string
    {
        $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT * FROM `T2_TREE_HEAD`;");
        $n = $db->num_rows($r);
    ...
    public function showCatalogPartsCategoryEdit($head_id): string
    {
        $db = DbSingleton::getTokoDb();
        $arr = []; $list = "";
        ...
        foreach ($arr as $cat_id=>$group_ids) {
            ...
            foreach ($group_arr as $key=>$value) {
                ...
                $history = "";
                if ($group_reviewed>0) {
                    $reviewed = "<i class='fa fa-check'></i>";
                }
                if ($count_arts>0) {
                    $history = "<a onclick=\"showCatalogLogs('$group_id');\"><i class='fa fa-history'></i> Історія ($count_arts шт.)</a>";
                }
                $list.="<li>
                    $reviewed
    ...
    public function untieCatalogGroup($head_id, $cat_id, $group_id): bool
    {
        $db = DbSingleton::getTokoDb();
    ...
    /*==== HISTORY ====*/
    public function showCatalogLogs($group_id)
    {
        $db = DbSingleton::getTokoDb();
        $form_htm=RD."/tpl/catalog_parts/history.htm";$form="";
        if (file_exists($form_htm)){ $form = file_get_contents($form_htm);}
        $list = "";
        $r = $db->query("SELECT COUNT(`ART_ID`) as count_arts, `DATE`, `USER_ID` FROM `T2_TREE_ARTS` WHERE `GROUP_ID`='$group_id' GROUP BY `DATE`, `USER_ID`;");
        $n=(int)$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
        ...
        $form = str_replace(array("{group_id}", "{history_title}", "{history_count}", "{history_range}"), array($group_id, $title, $n, $list), $form);

        return $form;
    }

    public function showCatalogLogsCard($group_id, $date, $user_id)
    {
        $db = DbSingleton::getTokoDb();
        $form_htm=RD."/tpl/catalog_parts/history_card.htm";$form="";
        if (file_exists($form_htm)){ $form = file_get_contents($form_htm);}
        $list="";
        $r = $db->query("SELECT * FROM `T2_TREE_ARTS` WHERE `GROUP_ID`='$group_id' AND `DATE`='$date' AND `USER_ID`='$user_id';");
        $n = $db->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
            $art_id = $db->result($r, $i - 1, "ART_ID");
            $date = $db->result($r, $i - 1, "DATE");
        ...
        $form = str_replace(array("{history_title}", "{history_count}", "{history_range}"), array($title, $n, $list), $form);

        return $form;
    }

    public function showCatalogLogsArt($group_id, $art_id): array
    {
        $db = DbSingleton::getTokoDb();
        $status = 0; $date = ""; $user_id = 0;
        $r = $db->query("SELECT * FROM `T2_TREE_ARTS` WHERE `GROUP_ID`='$group_id' AND `ART_ID`='$art_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n > 0) {
            $status = 1;
    ...
    public function getCatalogLogsCount($group_id)
    {
        $db = DbSingleton::getTokoDb();
        $r=$db->query("SELECT COUNT(`ART_ID`) as count_arts FROM `T2_TREE_ARTS` WHERE `GROUP_ID`='$group_id';");
        $n=(int)$db->num_rows($r);
    ...
    public function dropCatalogPartsArtsGroup($group_id, $date, $user_id)
    {
        $db = DbSingleton::getTokoDb();
        $r=$db->query("SELECT COUNT(`ART_ID`) as count_arts FROM `T2_TREE_ARTS` WHERE `GROUP_ID`='$group_id' AND `DATE`='$date' AND `USER_ID`='$user_id';");
        $n=(int)$db->num_rows($r);
    ...
    public function dropCatalogPartsArts($group_id, $art_id): bool
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function showCatalogPartsAddCard($head_id, $cat_id, $group_id)
    {
        $form_htm=RD."/tpl/catalog_parts/add.htm";$form="";
        if (file_exists($form_htm)){ $form = file_get_contents($form_htm);}
        $form = str_replace(array("{select_tree_head}", "{select_tree_cat}", "{select_tree_group}"), array($this->getCatalogHeadList($head_id), $this->getCatalogCatList($cat_id), $this->getCatalogGroupList($group_id)), $form);
        return $form;
    }

    public function getCatalogHeadList($head_id_sel = 0): string
    {
        $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT * FROM `T2_TREE_HEAD` ORDER BY `TEX_RU`;");
        $n = $db->num_rows($r);
        $list = "";
        for ($i=1; $i<=$n; $i++) {
            $head_id = (int)$db->result($r, $i-1, "HEAD_ID");
            $text = $db->result($r, $i-1, "TEX_RU");
            if ($head_id === (int)$head_id_sel) {
                $sel = "selected";
            } else {
                $sel = "";
            }
    ...
    public function getCatalogCatList($cat_id_sel = 0): string
    {
        $db = DbSingleton::getTokoDb();
        $list = "";
        $r = $db->query("SELECT t2c.CAT_ID, t2c.TEX_RU FROM `T2_TREE_CAT` t2c
        GROUP BY t2c.CAT_ID ORDER BY t2c.TEX_RU;");
        $n = $db->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
            $cat_id = (int)$db->result($r, $i-1, "CAT_ID");
            $text = $db->result($r, $i-1, "TEX_RU");
            if ($cat_id===(int)$cat_id_sel) {
                $sel = "selected";
            } else {
                $sel = "";
            }
    ...
    public function getCatalogGroupList($group_id_sel = 0): string
    {
        $db = DbSingleton::getTokoDb();
        $list = "";
        $r = $db->query("SELECT t2g.GROUP_ID, t2g.TEX_RU FROM `T2_TREE_GROUP` t2g
        GROUP BY t2g.GROUP_ID ORDER BY t2g.TEX_RU;");
        $n = $db->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
            $group_id = (int)$db->result($r, $i-1, "GROUP_ID");
            $text = $db->result($r, $i-1, "TEX_RU");
            if ($group_id===(int)$group_id_sel) {
                $sel = "selected";
            } else {
                $sel = "";
            }
    ...
    public function saveCatalogPartsAddCard($head_id, $cat_id, $group_id)
    {
        $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT * FROM `T2_TREE_HCG` WHERE `HEAD_ID`='$head_id' AND `CAT_ID`='$cat_id' AND `GROUP_ID`='$group_id' LIMIT 1;");
        $n = $db->num_rows($r);
        if ($n === 0) {
    ...
    public function showCatalogItem($type, $item_id): array
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function addCatalogItem($type, $text_ru, $text_ua, $text_en, $text_link, $status, $status_auto, $reviewed, $description_ru, $description_ua, $description_en): bool
    {
        $db = DbSingleton::getTokoDb();
        $table=""; $table_id=0;
        if ($type==="head")  {$table="T2_TREE_HEAD"; $table_id="HEAD_ID";}
        ...
        if ($type==="group") {$table="T2_TREE_GROUP"; $table_id="GROUP_ID";}
        if ($table!=="") {
            $r=$db->query("SELECT MAX(`$table_id`) as mid FROM `$table`;");
            $item_id=0+$db->result($r,0,"mid")+1;
    ...
    public function editCatalogItem($type, $item_id, $text_ru, $text_ua, $text_en, $text_link, $status, $status_auto, $reviewed, $description_ru, $description_ua, $description_en): bool
    {
        $db = DbSingleton::getTokoDb();
        $table=""; $table_id=0;
        if ($type==="head")  { $table = "T2_TREE_HEAD"; $table_id = "HEAD_ID"; }
    ...
    public function dropCatalogItem($type, $item_id)
    {
        $db = DbSingleton::getTokoDb();
        $table = ""; $table_id = 0;
        if ($type==="head")  { $table = "T2_TREE_HEAD"; $table_id = "HEAD_ID"; }
