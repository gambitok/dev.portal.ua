...
function showUndistribCellsCard($storage_cells_id,$income_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue; session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	...
	for ($i=1;$i<=$n;$i++){
		...
		$list.="<tr align='center'>
			...
			<td>$brand_name</td>
			<td align='right'>$amount</td>
			<td><button class='btn btn-sm btn-warning' onclick='showStorageCellSelectForm(\"$art_id\",\"$storage_id\",\"$income_id\",\"$amount\");'><i class='fa fa-delicious' title='���������� � ������'></i></button></td>
		</tr>";
	}
...
function saveUndistribCellsStorageCellForm($art_id,$income_id,$storage_id,$storage_cells_id,$amount){$db=DbSingleton::getDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	...
	if ($art_id>0 && $income_id>0 && $storage_id>0 && $storage_cells_id>0 && $amount>0){
		...
		if ($n==1){
			...
			if ($doc_storage_cells_id!=$storage_cells_id){
				...
				if ($new_amount<0){ $answer=0;$err="ʳ������ ����������� ������ �������� �������� ������!";}
				if ($op==1){
...
function saveundistribcellsCountryForm($id,$name,$alfa2,$alfa3,$duty,$risk){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function saveundistribcellsCostumsForm($id,$name,$preferential_rate,$full_rate,$type_declaration,$sertification,$gos_standart){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function loadArticleZED($undistribcells_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	...
	where t2z.ART_ID='$undistribcells_id' limit 0,1;");$n=$db->num_rows($r);
	if ($n==1){
...
function saveundistribcellsZED($undistribcells_id,$country_id,$costums_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
