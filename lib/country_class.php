...
function showCountryCard($country_id){
	...
	$r=$db->query("select t2cnt.* from T2_COUNTRIES t2cnt where t2cnt.COUNTRY_ID='$country_id' limit 0,1;");
	$n=$db->num_rows($r);
...
function saveCountryGeneralInfo($country_id, $country_name, $country_alfa2, $country_alfa3, $country_duty, $country_risk) {
	...
	$country_alfa2=$slave->qq($country_alfa2);
	$country_alfa3=$slave->qq($country_alfa3);
	$country_duty=$slave->qq($country_duty);
	$country_risk=$slave->qq($country_risk);
...
function DeleteCountry($country_id) {
	...
	if ($country_id>0){
		...
		//$dbt->query("delete from T2_COUNTRIES where `COUNTRY_ID`='$country_id';");
		$answer=1;$err="";
	}
	return array($answer,$err);
}
