...
class group_tree
{
    ...
    public function showGroupTreeHeaders(): string
    {
    ...
    public function addGroupTreeHeadStr($head_id)
    {
        $form = "";
    ...
    public function showGroupTreeHeadStr($id)
    {
        $db = DbSingleton::getTokoDb();
        $form = "";
        $form_htm = RD . "/tpl/group_tree_head_str_card.htm";
    ...
    public function showGroupTreeHeadCategory($cat_id)
    {
        $db = DbSingleton::getTokoDb();
        $form = "";
    ...
    public function saveGroupTreeHeadCategoryCard($cat_id, $head_id, $position, $disp_text_ru, $disp_text_ua, $disp_text_en): array
    {
        $db = DbSingleton::getTokoDb();
        if ($cat_id == "0") {
    ...
    public function dropGroupTreeHeadCategory($cat_id): array
    {
        $db = DbSingleton::getTokoDb();
        $answer = 0;
    ...
    public function saveGroupTreeHeadStrCard($id, $head_id, $str_id, $position, $category, $disp_text_ru, $disp_text_ua, $disp_text_en, $disp_text_link): array
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function dropGroupTreeHeadStr($id): array
    {
        $db = DbSingleton::getTokoDb();
        $answer = 0;
        $err = "Помилка видалення даних!";
    ...
    public function getTreeHeadersList($ch = "0"): string
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function getTreeGroupList($ch = "0"): string
    {
        $db = DbSingleton::getTokoDb();
        $list = "";
    ...
    public function getCategoryList($HEAD_ID, $ch = "0"): string
    {
        $db = DbSingleton::getTokoDb();
        $list = "<option value='0'>-Не выбрано-</option>";
    ...
    public function getPositionList($ch = "0"): string
    {
        $list = "<option value='0'>-Не выбрано-</option>";
        $n = 100;
        for ($i = 1; $i <= $n; $i++) {
    ...
    public function getGroupTreeStr($head_id): string
    {
        $db = DbSingleton::getTokoDb();
        $arr = [];
        $list = "";
    ...
    public function getCatPos($cat_id)
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function showGroupTreeHead($head_id)
    {
        $db = DbSingleton::getTokoDb();
        $form = "";
    ...
    public function saveGroupTreeHead($head_id, $disp_text_ru, $disp_text_ua, $disp_text_en, $head_status): array
    {
        $db = DbSingleton::getTokoDb();
        $answer = 0;
    ...
    public function dropGroupTreeHead($head_id): array
    {
        $db = DbSingleton::getTokoDb();
        $answer = 0;
        $err = "Помилка видалення даних!";
    ...
    public function showGroupTree()
    {
        ...
        $form = str_replace(array("{group_tree_range}", "{group_tree_head}"), array($menu_det, $this->showGroupTreeHeaders()), $form);

        return $form;
    }

    public function showGroupTreeCard($STR_ID)
    {
        $db = DbSingleton::getTokoDb();
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            $LNG_ID = (int)$db->result($r, $i - 1, "LNG_ID");
    ...
    public function saveGroupTreeCard($str_id, $position, $disp_text_ru, $disp_text_ua, $disp_text_en)
    {
        $db = DbSingleton::getTokoDb();
        $answer = 0;
        $err = "Помилка збереження даних!";
    ...
    public function showParrentPositions($str_id, $parent_id): string
    {
        $db = DbSingleton::getTokoDb();
        $list = "";
    ...
    public function dropUploadPhotoForm($type_id, $id): array
    {
        $db = DbSingleton::getTokoDb();
        $answer = 0;
        $err = "Помилка збереження даних!";
    ...
    public function showUploadDropzone($type_id, $str_id)
    {
        $form = "";
        ...
        $form = str_replace(array("{group_type_id}", "{group_str_id}"), array($type_id, $str_id), $form);
    ...
    public function showTreeConsForm()
    {
        $form = "";
    ...
    public function getSelectHeaders($headers = []): string
    {
        $db = DbSingleton::getTokoDb();
        ...
        for ($i = 1; $i <= $n; $i++) {
            $head_id = $db->result($r, $i - 1, "HEAD_ID");
            $text = $db->result($r, $i - 1, "TEX_RU");
            if (!in_array($head_id, $headers, true)) {
    ...
    public function getMaxPosition($head_id): array
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function loadTreeConsView()
    {
        $db = DbSingleton::getTokoDb();
        $form = "";
        ...
        $form = str_replace(array("{view_range}", "{view_hide_range}"), array($list, $this->loadTreeConsViewList()), $form);

        return $form;
    }

    public function loadTreeConsViewList($head_id = 1): string
    {
        $db = DbSingleton::getTokoDb();
        $list = "";
        $arr = [];
    ...
    public function loadTreeConsHeader()
    {
        $db = DbSingleton::getTokoDb();
        $headers = [];
    ...
    public function checkHeadPopular($head_id): bool
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function addHeadPopular($head_id): bool
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function checkHeadCatPopular($head_id): bool
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function loadTreeCons(): string
    {
        $db = DbSingleton::getTokoDb();
        $list = "";
        $r = $db->query("SELECT * FROM `T2_TREE_CONSTRUCTOR` WHERE 1 ORDER BY `POSITION` ASC;");
    ...
    public function getTreeConsCat($head_id): string
    {
        $db = DbSingleton::getTokoDb();
        $list = "";
        $r = $db->query("SELECT * FROM `T2_TREE_CONSTRUCTOR_STR` WHERE `HEAD_ID`='$head_id' ORDER BY `COL` ASC, `ROW` ASC;");
    ...
    public function getGroupRowStatus($group_id): array
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function addTreeConsColumn($head_id): array
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function dropTreeConsColumn($head_id): array
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function moveTreeConsColumn($head_id, $status): bool
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function addTreeConsCatForm($head_id)
    {
        $form = "";
        ...
        $form = str_replace(array("{select_cat}", "{head_id}"), array($this->getTreeCatRowList($head_id), $head_id), $form);

        return $form;
    }

    public function getTreeCatRowList($head_id): string
    {
        $db = DbSingleton::getTokoDb();
        $list = "";
        $cats = [];
    ...
    public function addTreeConsCat($head_id, $cat_id, $cat_col, $cat_row): array
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function dropTreeConsCat($head_id, $cat_id): array
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function saveTreeConsCatPos($head_id, $cat_id, $cat_col, $cat_row): array
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function moveTreeConsCat($head_id, $cat_id, $status): string
    {
        $db = DbSingleton::getTokoDb();
    ...
    public function showPartsForm($status = 0)
    {
        ...
        $form = str_replace(array("{parts_name}", "{parts_list}"), array("{spare_parts_catalog_cap}", $list), $form);
     ...
     * get TREE HCG LIST
     * */
    public function getGroupExistList($status): array
    {
        $db = DbSingleton::getTokoDb();
        $arr = [];
     ...
     * show TREE HCG LIST
     * */
    public function showGroupExistList($status): string
    {
        $list = "";
        ...
        foreach ($arr as $head_id => $cats) {
            ...
            foreach ($cats as $cat_id => $groups) {
                ...
                foreach ($groups as $group_id) {
                    ...
                    [$status_hidden, $status_auto] = $this->getGroupRowStatus($group_id);
    ...
    public function saveGroupStatus($type, $group_id, $status): int
    {
        $db = DbSingleton::getTokoDb();
        $status = (int)$status;
        $type_col = "STATUS";
    ...
    public function getGroupReviewsList($group_id): string
    {
        $db = DbSingleton::getTokoDb();
        $reviews = [];
        $r = $db->query("SELECT `REVIEW_ID` FROM `T2_GROUP_REVIEW` WHERE `GROUP_ID` = '$group_id';");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            if (in_array($review_id, $reviews, true)) {
    ...
    public function saveGroupExistCard($group_id, $text_ru, $text_ua, $text_en, $one_ru, $one_ua, $one_en, $h1_ru, $h1_ua, $h1_en, $desc_ru, $descr_ua, $descr_en, $text_link, $status, $status_auto, $reviews): array
    {
        $db = DbSingleton::getTokoDb();
