...
function exportDocs($client_id, $date_start, $date_end)
{
	...
	header('Content-Disposition: attachment; filename=export_documents2.csv');
...
function getBackClientsStrSumm($back_id)
{
    $db = DbSingleton::getDb();
    $summ = $col = 0;
	$r = $db->query("SELECT `price_buh_uah`, `partition_amount` FROM `J_BACK_CLIENTS_PARTITION_STR` WHERE `back_id`='$back_id';");
	$n = $db->num_rows($r);
	for ($i = 1; $i <= $n; $i++) {
...
function getTaxInvo($tax_id)
{
    $db = DbSingleton::getDb();
	$r = $db->query("SELECT `doc_nom` FROM `J_TAX_INVOICE` WHERE `id`='$tax_id';");
    return $db->result($r, 0, "doc_nom");
}

function getTaxInvoData($tax_id)
{
    $db = DbSingleton::getDb();
	$r = $db->query("SELECT `data_create` FROM `J_TAX_INVOICE` WHERE `id`='$tax_id';");
    return $db->result($r, 0, "data_create");
}

function exportBackClients($client_id, $date_start, $date_end)
{
    $db = DbSingleton::getDb();
    $list=[];
	$where = " AND j.seller_id='$client_id' AND j.data>='$date_start' AND j.data<='$date_end' ";
	...
	WHERE j.status=1 AND j.doc_type_id=61 AND tx.status=1 $where;");
	$n = $db->num_rows($r);
	for ($i = 1; $i <= $n; $i++) {
		$id=$db->result($r, $i - 1, "id");
		$prefix=$db->result($r, $i - 1, "prefix");
		$doc_nom=(int)$db->result($r, $i - 1, "doc_nom");
		if ($doc_nom===0){$doc_nom="-";}
		$name="$prefix-$doc_nom";
		...
		[$summ, $amount] = getBackClientsStrSumm($id);
...
function getSaleInvoiceStrSumm($invoice_id)
{
    $db = DbSingleton::getDb();
    $summ = $col = 0;
	$r = $db->query("SELECT `price_buh_uah`, `invoice_amount` FROM `J_SALE_INVOICE_PARTITION_STR` WHERE `invoice_id`='$invoice_id';");
	$n = $db->num_rows($r);
	for ($i = 1; $i <= $n; $i++) {
		$price = $db->result($r, $i - 1, "price_buh_uah");
...
function exportSaleInvoice($client_id, $date_start, $date_end)
{
    $db = DbSingleton::getDb();
    $list=[];
	...
	WHERE j.status=1 $where AND tx.back_id=0;");
	$n=$db->num_rows($r);
	for ($i = 1; $i <= $n; $i++) {
		$id=$db->result($r, $i - 1, "id");
		$prefix=$db->result($r, $i - 1, "prefix");
		...
		$tax_doc=$db->result($r, $i - 1, "tax_doc");
		$summ_pdv=$db->result($r, $i - 1, "summ");
		[$summ, $amount] = getSaleInvoiceStrSumm($id);
	    $summ = number_format($summ, 2, '.', '');
...
function getIncomeStrSumm($income_id)
{
    $db = DbSingleton::getDb();
    $summ = $col = 0;
	$r = $db->query("SELECT `price_buh_uah`, `amount` FROM `J_INCOME_STR` WHERE `income_id`='$income_id';");
	$n = $db->num_rows($r);
	for ($i = 1; $i <= $n; $i++) {
		$price = $db->result($r, $i - 1, "price_buh_uah");
...
function exportIncome($client_id, $date_start, $date_end)
{
    $db = DbSingleton::getDb();
    $list=[];
	...
	for ($i = 1; $i <= $n; $i++) {
		...
		$client_edrpou=$db->result($r, $i - 1, "edrpou");
		$client_vat=$db->result($r, $i - 1, "vytjag");
		[$invoice_summ, $amount] = getIncomeStrSumm($id);
...
function showSelectList()
{
	...
	WHERE cc.category_id=3 ORDER BY c.id ASC;");
	$n = $db->num_rows($r);
	for ($i = 1; $i <= $n; $i++) {
		$id = $db->result($r, $i - 1, "id");
...
{
	$form_htm = RD . "/tpl/export_doc.htm"; $form = "";
	if (file_exists($form_htm)) { $form = file_get_contents($form_htm);}
    $form = str_replace(array("{date}", "{client_list}"), array(date("Y-m-d"), showSelectList()), $form);
...
function showBrandsSelect()
{
    $db = DbSingleton::getTokoDb();
    $list = "";
	$r = $db->query("SELECT * FROM `T2_BRANDS` ORDER BY `BRAND_NAME` ASC;");
	$n = $db->num_rows($r);
	for ($i = 1; $i <= $n; $i++) {
