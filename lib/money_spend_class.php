...
class money_spend {
    ...
    function loadMoneySpendCashBoxList($client_id,$doc_type_id,$seller_id){
        $list=$this->showPayBoxSelectList(0,$doc_type_id,$seller_id);
        return $list;
    }

    function showMoneySpendForm(){$gmanual=new gmanual;session_start();$user_id=$_SESSION["media_user_id"];
        $form="";$form_htm=RD."/tpl/money_spend_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
    ...
    function viewMoneySpend($spend_id){$db=DbSingleton::getDb();session_start();$user_id=$_SESSION["media_user_id"];$slave=new slave;$data_time=date("Y-m-d H:i:s");
        ...
        from `J_MONEY_SPEND` j
            ...
            left outer join media_users muf on muf.id=j.user_id_from
            left outer join manual m on (m.`key`='spend_type_id' and m.id=j.spend_type_id)
        where j.status=1 and j.id='$spend_id' limit 1;");
    ...
    function getPayboxResiverList($paybox_id,$balans_id_from,$user_id){$db=DbSingleton::getDb(); $list="<option value=\"0\">--Оберіть зі списку--</option>";
        ...
        from `PAY_BOX` pb 
            left outer join `PAY_BOX_WORKERS` pbw on pbw.paybox_id=pb.id 
        where pbw.worker_id!='$user_id' and pb.doc_type_id='$paybox_type_id' and pbw.status=1 and pb.status=1 and pb.in_use=1 group by pb.id order by pb.name asc;");
        $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
            $name=$db->result($r,$i-1,"name");
    ...
    function saveMoneySpend($paybox_id_from,$balans_id_from,$spend_type_id,$summ,$comment){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
        $paybox_id_from=$slave->qq($paybox_id_from);$spend_type_id=$slave->qq($spend_type_id);$comment=$slave->qq($comment);$spend_id=0;
        ...
        if ($paybox_id_from>0 && $spend_type_id>0 && $balans_id_from>0 && $summ>=0){
            ...
            if ($summ<=$current_balans_summ){
                ...
                $db->query("insert into `J_MONEY_SPEND` (`id`,`paybox_id_from`,`user_id_from`,`spend_type_id`,`cash_id`,`summ`,`comment`) 
                values ('$spend_id','$paybox_id_from','$user_id_from','$spend_type_id','$cash_id','$summ','$comment');");
    ...
    function loadMoneySpendCDN($spend_id){$db=DbSingleton::getDb();
        $form="";$form_htm=RD."/tpl/money_spend_cdn_block.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
    ...
    function moneySpendCDNDropFile($spend_id,$file_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка видалення файлу!";
        $spend_id=$slave->qq($spend_id);$file_id=$slave->qq($file_id);
        if ($spend_id>0 && $file_id>0){
            $r=$db->query("select `FILE_NAME` from `MONEY_SPEND_CDN` where `spend_id`='$spend_id' and `id`='$file_id' limit 1;");$n=$db->num_rows($r);
            if ($n==1){
                unlink(RD.'/cdn/money_spend_files/$spend_id/$file_name');
                $db->query("delete from MONEY_SPEND_CDN where spend_id='$spend_id' and id='$file_id';");
                $answer=1;$err="";
            }
        }
