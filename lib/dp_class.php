...
function setDpNote($dp_id,$text) { $db=DbSingleton::getDb(); session_start();	$user=$_SESSION["media_user_id"]; $err="������� ���������� �����!";
...
function dropDpNote($dp_id) { $db=DbSingleton::getDb(); $err="������� ��������� �����!";
...
function get_doc_prefix($client_id,$prefix_id){ $db=DbSingleton::getDb();$prefix="��-";
...
function get_dp_prefix($dp_id){ $db=DbSingleton::getDb();$prefix="��";
	$r=$db->query("select type_id from J_DP where id='$dp_id' limit 0,1;");$n=$db->num_rows($r);
	if ($n==1){$type_id=$db->result($r,0,"type_id"); if ($type_id==0){$prefix="�-��";}}
	return $prefix; 
}
...
function show_dp_list_filter($status,$filstatus,$filauthor,$filtpoint){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;$income=new income;  session_start(); $ses_tpoint_id=$_SESSION["media_tpoint_id"]; $media_user_id=$_SESSION["media_user_id"];$media_role_id=$_SESSION["media_role_id"]; 
...
function showDpCard($dp_id){$db=DbSingleton::getDb();$slave=new slave;$manual=new manual; $gmanual=new gmanual; session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	...
	if ($n==1){ $this->updateDpSumm($dp_id);
		...
		if ($user_id!=$user_use && $user_use>0){
			...
			$admin_unlock="";
			if ($user_id==1 || $user_id==2){$admin_unlock="<button class='btn btn-sm btn-warning' onClick='unlockDpCard(\"$dp_id\");'><i class='fa fa-unlock'></i> ������������</button>";}
		...
		if ($user_id==$user_use || $user_use==0){
			...
			$data_pay=$db->result($r,0,"data_pay");	
			$type_id=$db->result($r,0,"type_id");if ($type_id==1){$type_name="̳� ��������";}
...
//			if($eur_to_uah!=$eur_to_uah_new){$eur_to_uah=$eur_to_uah_new;}				
			...
			$balans_conto="<b>������ �������� �����: </b>".$client->getClientGeneralSaldo($client_conto_id);
			else $balans_conto="";
...
function closeDpCard($dp_id){session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;
...
function setDpClient($dp_id,$client_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="������� ���������� �����!";
...
function getDpTpointInfo($client_id) {$db=DbSingleton::getDb();
	$r=$db->query("select c.*, acc.tpoint_id, tp.name as tpoint_name from A_CLIENTS c 
		left outer join A_CLIENTS_CONDITIONS acc on acc.client_id=c.id 
		left outer join T_POINT tp on tp.id=acc.tpoint_id 
	where c.id='$client_id' group by c.id limit 0,1;");$n=$db->num_rows($r);
...
function filterDpClientsList($sel_id,$client_id,$client_name,$phone,$email,$state_id){$db=DbSingleton::getDb();$slave=new slave;$where="";
...
function unlinkDpClient($dp_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="������� ���������� �����!";
	$dp_id=$slave->qq($dp_id);
	if ($dp_id>0){
...
function unlinkDpTpoint($dp_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="������� ���������� �����!";
	$dp_id=$slave->qq($dp_id);
	if ($dp_id>0){
...
function getIncomeStatusDpsName($art_id,$amount,$suppl_id){$db=DbSingleton::getDb();
	...
	where inc.status=1 and inc.oper_status=31 and inc.client_seller='$suppl_id' and instr.art_id='$art_id' order by inc.id desc limit 0,3;
	");$ns=$db->num_rows($rs);$inc_status_name="�������� � ���������� �������������";
	if ($ns>0){$inc_status_name="������� �� ������������� ";
		for ($is=1;$is<=$ns;$is++){
			...
			$inc_status_name.=" $inc_amount"."��. $inc_data �� ���.".$this->getStorageName($storage_id)." ".$this->getStorageCellName($storage_cells_id);
		}
	}
...
function getDpStrStatus($dp_id){$db=DbSingleton::getDb();$status_name="";
	$r=$db->query("select j.*, dps.mcaption as status_dps_name from J_DP_STR j left outer join manual dps on (dps.id=j.status_dps and dps.key='status_dps') 
	...
	for ($i=1;$i<=$kl_rw;$i++){
		...
		if ($status_dp==79){
			...
			$list.="<tr id='strRow_$i'>
				...
				<td alt='$amount_bug_info' title='$amount_bug_info'>$amount_bug</td>
				<td>
		...
		$list="
			<tr id='dpStrNewRow' class='hidden'>
				...
				<td style='min-width:140px;'><input type='hidden' id='artIdStr_0' value=''>
					<div class='input-group'>
						<input class='form-control input-xs' type='text' readonly id='article_nr_displStr_0' value='' placeholder='������ ������'>
						<span class='input-group-btn'> 
				...
				<td style='min-width:120px;'><input type='hidden' id='brandIdStr_0' value=''>
					<input class='form-control input-xs' type='text' readonly id='brandNameStr_0' value='' placeholder='�����'>
				</td>
				<td>
...
function saveDpCard($dp_id,$data_pay,$cash_id,$dp_summ,$doc_type_id,$tpoint_id,$client_id,$client_conto_id,$delivery_type_id,$carrier_id,$delivery_address){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function saveDpCardData($dp_id,$cash_id,$frm,$tto,$idStr,$artIdStr,$article_nr_displStr,$brandIdStr,$amountStr,$priceStr,$priceEndStr,$discountStr,$summStr){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function updateDpStrPrice($dp_id,$str_id,$discount_new,$cash_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function setArticleToDp($dp_id,$tpoint_id,$artIdStr,$article_nr_displStr,$brandIdStr,$storageIdStr,$amountStr){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	...
	if ($dp_id>0){
		...
		if ($amountS>$max_moving && $rest_amount<=0){$answer=0;$err="ʳ������ ��� ���������� ��� ����� �� �������! (�����������: $max_moving)";}
		if ($amountS<=$max_moving){
			...
			if ($idS=="" || $idS==0){
				...
				$no_unreserv=0;$rr_reserv=0;$amountEx=0; $can_drop=1;
			}
			if ($idS>0){
				if ($artIdS!="" && $artIdS>0 && $article_nr_displS!=""){
...
function setArticleSupplToDp($dp_id,$tpoint_id,$artIdStr,$article_nr_displStr,$brandIdStr,$supplIdStr,$supplStorageIdStr,$amountStr){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	$dp_id=$slave->qq($dp_id);$tpoint_id=$slave->qq($tpoint_id);
	if ($dp_id>0){
...
function getDpClientContoCash($client_id){$db=DbSingleton::getDb();$cash_id=1;$answer=0;$err="�������";
...
function getDpClientDocType($client_id){$db=DbSingleton::getDb();$doc_type_id=64;$answer=0;$err="�������";
...
function getClientPaymentDelay($client_id){$db=DbSingleton::getDb();$data_pay=date("Y-m-d");$answer=0;$err="�������";
...
function changeDpCash($dp_id,$cash_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
		...
		if ($n==1){
			...
			if ($oper_status==30) {
				...
				if ($client_cash_id==$cash_id || $org_type==0 || $org_type==1){
					...
					$answer=1;$err="";
				}else{$answer=0;$err="������ ���������� �볺��� ".$this->getCashName($client_cash_id).". ����� �������� �������� �� ���� ���� ��������� ���������� � ����� ".$this->getCashName($cash_id);}
			} else {$answer=0;$err="�������� �����������. ���� ������� ����������.";}
		}
...
function changeArticleToDp($dp_id,$dp_str_id,$amount_change){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
	$dp_id=$slave->qq($dp_id);
	if ($dp_id>0){
		$dp_str_id=$slave->qq($dp_str_id);$amount_change=$slave->qq($amount_change);
		$r=$db->query("select amount,art_id,storage_id_from from J_DP_STR where dp_id='$dp_id' and id='$dp_str_id' and status_dps='93' limit 0,1;");$n=$db->num_rows($r);
		if ($n==1){
			$amountEx=$db->result($r,0,"amount");
			$art_id=$db->result($r,0,"art_id");
...
function dropDpStr($dp_id,$dp_str_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="������� �������";
	$dp_id=$slave->qq($dp_id);
		...
		if ($oper_status==30 && ($status==1 ||$status==1)){
			...
			if ($n1==1){
				...
				if ($status_dps_str==93){
					...
					$suppl_storage_id=$db->result($r1,0,"suppl_storage_id");
					if ($art_id==0){
						$db->query("delete from J_DP_STR where id='$dp_str_id' and dp_id='$dp_id' limit 1;");
						$this->updateDpWeightVolume($dp_id);
						$dp_summ=$this->updateDpSumm($dp_id);
...
function updateDpSumm($dp_id){$db=DbSingleton::getDb();$slave=new slave;$sum=0;
	...
	for ($i=1;$i<=$n;$i++){
		...
		if ($cash_id==1){ $price_cash=round($price_cash*$usd_to_uah,2);}if ($cash_id==3){ $price_cash=round($price_cash*$usd_to_uah/$eur_to_uah,2); }
...
function makeDpCardFinish($dp_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$answer=0;$err="";
	...
	if ($n==1){
		...
		if ($storage_id==0 || $storage_cells_id==0){$answer=0;$err="�� ������� \"����� ���������\" ��� \"������ ���������\". �������� �� ���������!";}
...
function saveDpLocalAutoCell($dp_id,$storage_id_to,$cell_id_from){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$catalogue=new catalogue;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";$kol_row=0;
	...
	if ($dp_id>0 && $storage_id_to>0 && $cell_id_from>0){
		...
		for ($ic=1;$ic<=$nc;$ic++){
			...
			if ($idS=="" || $idS==0){
				$r=$db->query("select max(id) as mid from J_DP_STR;");$idS=0+$db->result($r,0,"mid")+1;
				$db->query("insert into J_DP_STR (`id`,`dp_id`) values ('$idS','$dp_id');");
				$no_unreserv=0;$rr_reserv=0;$amountEx=0;
			}
			if ($idS>0){
...
function clearDpLocalAutoCellForm($dp_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="������� �������";
	$dp_id=$slave->qq($dp_id);
...
function setArticleToSelectAmountDp($art_id,$dp_id){
	$form_htm=RD."/tpl/dp_select_amount_article_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
...
function showArticleRestStorageSelectText($art_id,$storage_id,$input_amount,$amountEx){$db=DbSingleton::getTokoDb();$info="";
	...
	for ($i=1;$i<=$n;$i++){
		...
		$info="�������: $amount | ������: $reserv_amount<br>� ��������� �����: $cur_amount";
	}
...
function getArticleTpointDeliveryInfo($tpoint_id,$art_id){$db=DbSingleton::getTokoDb();$slave=new slave; $info="�� �������"; $week_day=date("N");$cur_time=date("H:i:s");
	...
	if ($n==1){
		...
		$info="$delivery_days - ��. $date_del ($week_day_short)<br>� $time_from_del �� $time_to_del";
	}
	return $info;
}

function getArticleTpointSupplDeliveryInfo($tpoint_id,$art_id){$db=DbSingleton::getTokoDb(); $slave=new slave; $info="�� �������"; $week_day=date("N");$cur_time=date("H:i:s");
...
function getArticleTpointSupplPriceUsdDeliveryInfo($tpoint_id,$art_id){$db=DbSingleton::getTokoDb(); $price=0; $info="�� �������"; $week_day=date("N");$cur_time=date("H:i:s");
...
function getTpointDeliveryInfo($tpoint_id,$storage_id){$db=DbSingleton::getDb(); $info="�� �������"; $week_day=date("N");$cur_time=date("H:i:s");$slave=new slave; 
...
function getTpointSupplDeliveryInfo($tpoint_id,$suppl_id,$suppl_storage_id){$db=DbSingleton::getDb();$slave=new slave; $info="�� �������"; $week_day=date("N");$cur_time=date("H:i:s");
	...
	if ($n==1){
		...
		$week_day_short=$slave->get_weekday_abr($week);
    	$date_del=date('d.m', strtotime(' + '.$delivery_days.' days'));
		$info="$delivery_days - ��. $date_del ($week_day_short) � $time_from_del �� $time_to_del";
	}
	return $info;
}
...
function showArticleRestStorageCellsList($art_id,$storage_id){$db=DbSingleton::getTokoDb();$list="<option value='0'>-- ������ � ������ --</option>";
	...
	for ($i=1;$i<=$n;$i++){
		...
		if ($reserv_amount!=0 || $amount!=0){
			$list.="<option value='$id' data-max-mov='$max_moving' data-cellId-mov='0'>$name | �������: $amount; ������: $reserv_amount; </option>";
		}
	}
	return $list;
}

function showStorageCellsList($storage_id,$exclude_id){$db=DbSingleton::getTokoDb();$list="<option value='0'>-- ������ � ������ --</option>";
...
function getArticleRestStorageCell($art_id,$storage_id,$cell_id){$db=DbSingleton::getTokoDb();$slave=new slave; 
...
function showStorageSelectList($sel_id,$cells_only=0){$db=DbSingleton::getTokoDb();$list="<option value=0>������ � ������</option>";
...
function showStorageCellsSelectList($storage_id,$sel_id){$db=DbSingleton::getTokoDb();$list="<option value=0>������ � ������</option>";$cells_show=1;
...
function getCashAbr($sel_id){$db=DbSingleton::getDb();$name="���";
...
function getCarrierSelectList($sel_id){$db=DbSingleton::getDb();$list="";$cells_show=1;
...
function saveDpStorage($dp_id,$storage_id,$storage_cells_id){$db=DbSingleton::getDb();
	$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function saveDpComment($dp_id,$comment){$db=DbSingleton::getDb();
	$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function dropDpComment($dp_id,$comment_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ��������� ������!";
...
function dpCDNDropFile($dp_id,$file_id){$db=DbSingleton::getDb();
	...
	if ($dp_id>0 && $file_id>0){
		...
		if ($n==1){
			...
			$r=$db->query("delete from J_DP_CDN where dp_id='$dp_id' and id='$file_id';");
			$answer=1;$err="";
		}
	}
	return array($answer,$err);
...
function showdpDocumentList($dp_id,$dp_op_id,$document_id){$db=DbSingleton::getDb();
	$slave=new slave;$income=new income;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
	if ($dp_op_id==1){
...
function finddpDocumentsSearch($dp_id,$dp_op_id,$s_nom){$db=DbSingleton::getDb();
	$slave=new slave;$income=new income;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
...
function showdpUnknownStrList($dp_id){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$manual=new manual;$gmanual=new gmanual;$list="";
	...
	for ($i=1;$i<=$n;$i++){
		$id=$db->result($r,$i-1,"id");
		$art_id=$db->result($r,$i-1,"art_id");$art_id_comment="";if ($art_id==0){$art_id_comment="�� ��������� ART_ID! ������� ������� � ���";}
		$article_nr_displ=$db->result($r,$i-1,"article_nr_displ");
		$brand_id=$db->result($r,$i-1,"brand_id");$brand_name=$cat->getBrandName($brand_id);
...
function startDpExecute($dp_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="������� �������!";$suppl_ex=0;
	$dp_id=$slave->qq($dp_id);
	if ($dp_id>0){
...
function makeDpJmovingStorselPreorder($dp_id,$local){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$jmoving=new jmoving;$answer=0;$err="������� �������!";session_start();$user_id=$_SESSION["media_user_id"];
	...
	for ($i=1;$i<=$n;$i++){
		...
		if ($storage_id_from>0){
			...
			if ($storage_local==42 && $local==42){
				$r11=$db->query("select * from J_DP_STR where dp_id='$dp_id';");$n11=$db->num_rows($r11);
				...
				if ($n1>0){
					...
					$storage_dp_id=$this->newDpFromDp($dp_id,$storage_tpoint_id,$storage_id_from);$s_volume=0;$s_weight_netto=0;
		...
		if($storage_id_from==0){ $dp_cash_id=$this->getDpCashId($dp_id);
			...
			if ($n1>0){
				...
				for ($i1=1;$i1<=$n1;$i1++){
					...
					$db->query("insert into J_DP_SUPPL_ORDER (`dp_id`,`dp_str_id`,`art_id`,`article_nr_displ`,`SUPPL_ART_ID`,`brand_id`,`brand_name`,`amount`,`price`,`cash_id`,`suppl_id`,`suppl_storage_id`,`tpoint_id`,`media_user_id`) values ('$dp_id','$id','$art_id','$article_nr_displ','$suppl_art_id','$brand_id','$brand_name','$amount','$price_end','$dp_cash_id','$suppl_id','$suppl_storage_id','$tpoint_id','$user_id');"); // ��������� ������ ������ ��� �� 
...
function loadDpJmoving($dp_id){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;
	...
	for ($i=1;$i<=$n;$i++){
		$id=$db->result($r,$i-1,"id");
		$type_id=$db->result($r,$i-1,"type_id");$type_name="<i class='fa fa-inbox'></i> ������� ����������";if ($type_id==1){$type_name="<i class='fa fa-truck'></i> ̳� ��������";}
		$prefix=$db->result($r,$i-1,"prefix");
		$doc_nom=$db->result($r,$i-1,"doc_nom");
	...
	if ($n==0){$list="<tr><td colspan=6>³������ ���������� ������</td></tr>";}
...
function viewDpStorageSelect($dp_id,$select_id,$select_status){$db=DbSingleton::getDb();$cat=new catalogue;$slave=new slave;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"]; $storsel=new storsel; $gmanual=new gmanual; 
	...
	return array($form,"��������� ����������� ������ � ���-$select_id; ������ ������: ".$gmanual->get_gmanual_caption($select_status));
}
...
function getStorageSelectBugList($dp_id,$select_id,$art_id,$str_id){$db=DbSingleton::getDb();$manual=new manual;$list="";
	...
	for ($i=1;$i<=$n;$i++){
		...
		$list.="$amount_bug"."��. - $storage_select_bug_name";if ($i<$n){$list.="<br>";}
	}
	return $list;
}
...
function loadDpSaleInvoice($dp_id){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;
	...
	if ($n==0){$list="<tr><td colspan=11 align='center'>������� ������</td></tr>";}
...
function showDpStorselForSaleInvoice($dp_id){$db=DbSingleton::getDb();$cat=new catalogue;$slave=new slave;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"]; $storsel=new storsel; $gmanual=new gmanual; 
	...
	if ($n==0){$list="<tr><td colspan=13 align=center>³����� ������</td></tr>";}
	$form=str_replace("{storsel_list}",$list,$form);
	$form=str_replace("{dp_id}",$dp_id,$form);
	$r=$db->query("select sel.*, s.name as storage_name, t.name as tpoint_name from J_SELECT sel
		left outer join T_POINT t on t.id=sel.tpoint_id
...
function checkClientCreditLimitBeforeSaleInvoice($dp_id,$kol_storsel,$ar_storsel){ $db=DbSingleton::getDb();$client_saldo=0;$credit_limit=0;$summ_all=0;
	...
	if ($n==1){
		...
		$r=$db->query("select max(id) as mid from J_SALE_INVOICE;");$invoice_id=0+$db->result($r,0,"mid")+1; $sale_invoice_nom=$invoice_id; //��������
		$ai=0;$select_str="0";
...
function viewDpDatapayLimitSaleInvoice($dp_id){$db=DbSingleton::getDb();$cat=new catalogue;$slave=new slave;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"]; $storsel=new storsel; $gmanual=new gmanual; 
	$form_htm=RD."/tpl/dp_sale_invoice_list_data_pay.htm";	if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
	...
	if ($n==0){$list="<tr><td colspan=13 align=center>������� ������</td></tr>";}
	$form=str_replace("{list}",$list,$form);
	return array($form,"������ �������������� ���������: $client_name");
}
	
//function fixInvoiceDiscount() {$db=DbSingleton::getDb();
//	$r=$db->query("select ss.id as sale_str_id, ss.art_id as sale_art_id, ss.discount as sale_discount, ds.dp_id as dp_str_id, ds.art_id as dp_art_id, ds.discount as dp_discount from J_SALE_INVOICE si
//	left outer join J_SALE_INVOICE_STR ss on ss.invoice_id=si.id
//	left outer join J_DP_STR ds on (ds.dp_id=si.dp_id and ds.art_id=ss.art_id)
//	where ss.discount != ds.discount;"); $n=$db->num_rows($r);
//	for ($i=1;$i<=$n;$i++){
//		$sale_str_id=$db->result($r,$i-1,"sale_str_id");
//		$dp_discount=$db->result($r,$i-1,"dp_discount");
//		$db->query("update J_SALE_INVOICE_STR set `discount`='$dp_discount' where `id`='$sale_str_id';");
//	}
//	return $n;
//}
...
function sendDpStorselToSaleInvoice($dp_id,$kol_storsel,$ar_storsel){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
	...
	$slave=new slave;$answer=0;$err="������� �������!";$sale_invoice_nom=0; session_start();$user_id=$_SESSION["media_user_id"];$cat=new catalogue; $dp_id=$slave->qq($dp_id);
...
//								$discount2_cash=round($discount2*$usd_to_uah/$eur_to_uah,2);
				...
				$answer=0;$err="˳�� �������: $client_credit_limit$cash_name; ����/������: ".abs($client_saldo)."$cash_name. ³����������� �� �����: $sale_storsell_summ$cash_name �� �������, ��� ��������� ������������ ������ � ���� �� ����� $cl_pp$cash_name, ��� �������� �� ������� ���������";
			}
			}
		}
	}
...
function viewDpSaleInvoice($dp_id,$invoice_id){$db=DbSingleton::getDb();$cat=new catalogue;$slave=new slave;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"]; $storsel=new storsel; $gmanual=new gmanual; 
	...
	return array($form,"� $prefix-$doc_nom; ���:$doc_type_name; ������: ".$status_invoice_cap);
}
...
function printDpJournal($dp_id) {$db=DbSingleton::getDb(); $cat=new catalogue; $money=new toMoney; $slave=new slave; 
	...
	where dp.id='$dp_id' limit 0,1;"); $n=$db->num_rows($r);
...
function printDpSaleInvoice($dp_id) {$db=DbSingleton::getDb(); $cat=new catalogue; $money=new toMoney; $slave=new slave; 
	$form=""; $form_htm=RD."/tpl/dp_sale_invoice_print_non_cash.htm"; 
	if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
	
	$r=$db->query("select dp.*, cl.full_name as client_name, sl.full_name as seller_name, sl.phone, sld.edrpou, sld.account, sld.bank, sld.mfo, sld.vytjag, sld.address_fakt as seller_address, cld.address_fakt as client_address from J_SALE_INVOICE dp
...
function printSaleInvoice($invoice_id){$db=DbSingleton::getDb();$cat=new catalogue;$slave=new slave;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"]; $gmanual=new gmanual; $money=new toMoney;$invoice_summ=0;
	...
	if ($n==1){
		...
		for ($i=1;$i<=$n;$i++){
			$id=$db->result($r,$i-1,"id");
			$art_id=$db->result($r,$i-1,"art_id");$article_name=$cat->getArticleName($art_id);
			$article_nr_displ=$db->result($r,$i-1,"article_nr_displ");
			$brand_id=$db->result($r,$i-1,"brand_id");$brand_name=$cat->getBrandName($brand_id);
			$amount=$db->result($r,$i-1,"amount");
			$price=$db->result($r,$i-1,"price");
...
function loadDpMoneyPay($dp_id){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;
	...
	where pay.status=1 and pst.parrent_doc_id in ($invoice_str) and pst.parrent_doc_type_id!=0 group by pay.id order by pay.data_time desc, pay.id desc;");
	$n=$db->num_rows($r);
	...
	for ($i=1;$i<=$n;$i++){
		...
		$cash_id=$db->result($r,$i-1,"cash_id");$cash_name=$db->result($r,$i-1,"abr");
		$doc_cash_id=$db->result($r,$i-1,"doc_cash_id");$doc_cash_name=$db->result($r,$i-1,"d_abr");
		$user_name=$this->getMediaUserName($db->result($r,$i-1,"user_id"));
		$pay_cash_kours=$db->result($r,$i-1,"pay_cash_kours");
//		$status_pay=$db->result($r,$i-1,"status_pay");
//		$status_select_cap=$gmanual->get_gmanual_caption($status_select);
	...
	if ($n==0){$list="<tr><td colspan=9 align='center'><h3>��������� ������ ������</h3></td></tr>";}
...
function countOrdersSite() { $db=DbSingleton::getDb();
	$user_id=$_SESSION["media_user_id"]; $user_name=$_SESSION["user_name"]; $ses_tpoint_id=$_SESSION["media_tpoint_id"]; 
...
function showOrdersSite() {$db=DbSingleton::getDb();$dbp=DbSingleton::getTokoDb();
	$form_htm=RD."/tpl/dp_orders_site_list.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
	$data=date("Y-m-d");
...
function showOrderSiteRange($press,$data_start,$data_end) {$db=DbSingleton::getDb();$dbp=DbSingleton::getTokoDb();
    $slave=new slave;$gmanual=new gmanual; session_start(); $user_id=$_SESSION["media_user_id"]; 	$user_name=$_SESSION["user_name"];$ses_tpoint_id=$_SESSION["media_tpoint_id"];  $data_cur=date("Y-m-d");
	$where=" and o.tpoint_id='$ses_tpoint_id'";if ($user_id==1 || $user_id==2 || $user_id==7){$where="";}
	...
	$r=$db->query("select o.*, t.name as tpoint_name, c.name as client_name, cr.name as retail_name, cit.CITY_NAME as region_name, csh.abr, dt.mcaption as delivery_type_caption from orders_new o
	...
	for ($i=1;$i<=$n;$i++){
		...
		$client_user_id=$db->result($r,$i-1,"client_user_id");//$retail_name=$db->result($r,$i-1,"retail_name");
		$retail_name=$this->getClientUserName($client_id,$client_user_id);
		$tpoint_id=$db->result($r,$i-1,"tpoint_id");$tpoint_name=$db->result($r,$i-1,"tpoint_name");
		$cash_id=$db->result($r,$i-1,"cash_id");$cash_abr=$db->result($r,$i-1,"csh.abr");
		$name=$db->result($r,$i-1,"name");
		$email=$db->result($r,$i-1,"email");
		$phone=$db->result($r,$i-1,"phone");
		$region=$db->result($r,$i-1,"region");$region_name=$db->result($r,$i-1,"region_name");
		$address=$db->result($r,$i-1,"address");
		$delivery=$db->result($r,$i-1,"delivery");$delivery_type=$db->result($r,$i-1,"delivery_type_caption");
		...
		$list.="<tr id='strStsRow_$i' onClick='showOrdersSiteCard(\"$id\");' style='cursor:pointer; $color'>
			...
			<td align='right' style='min-width:120px;'>$delivery_type $delivery_address</td>
...
function showOrdersSiteCard($order_id){$db=DbSingleton::getDb();$dbp=DbSingleton::getTokoDb();$slave=new slave;$gmanual=new gmanual; session_start(); 	$user_id=$_SESSION["media_user_id"]; $user_name=$_SESSION["user_name"];$ses_tpoint_id=$_SESSION["media_tpoint_id"]; $order_id=$slave->qq($order_id);
	$form_htm=RD."/tpl/dp_orders_site_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
	...
	if ($n==0){$form="<h2 align='center'>���������� ������ ��� �������� � ������</h2>";}
	if ($n==1){
		$id=$db->result($r,0,"id");
		$data=$db->result($r,0,"data");
		...
		$retail_name=$this->getClientUserName($client_id,$client_user_id);
		$tpoint_id=$db->result($r,0,"tpoint_id");$tpoint_name=$db->result($r,0,"tpoint_name");
		$cash_id=$db->result($r,0,"cash_id");$cash_abr=$db->result($r,0,"csh.abr");
		$name=$db->result($r,0,"name");
		$email=$db->result($r,0,"email");
		$phone=$db->result($r,0,"phone");
		$region=$db->result($r,0,"region");$region_name=$db->result($r,0,"region_name");
		$address=$db->result($r,0,"address");
		$delivery=$db->result($r,0,"delivery");$delivery_type=$db->result($r,0,"delivery_type_caption");
	...
	return array($form,"���������� � ����� � $order_id �� ".$slave->data_word($data));
}
	
function deleteOrderSite($order_id) {$db=DbSingleton::getDb();$answer=0;$err="�������!";
	if ($order_id>0){
...
function createDpFromOrder($order_id){$db=DbSingleton::getDb();$slave=new slave; session_start(); $answer=0;$err="�������!";$dp_id=0;
	...
	if ($order_id>0){
		...
		if ($n==1){
			...
			$data=$db->result($r,0,"data");
			$client_id=$db->result($r,0,"client_id");$client_name=$db->result($r,0,"client_name");
			$client_user_id=$db->result($r,0,"client_user_id");$retail_name=$db->result($r,0,"retail_name");
			$tpoint_id=$db->result($r,0,"tpoint_id");$tpoint_name=$db->result($r,0,"tpoint_name");
			$cash_id=$db->result($r,0,"cash_id");$cash_abr=$db->result($r,0,"csh.abr");
			$name=$db->result($r,0,"name");
			$email=$db->result($r,0,"email");
...
function loadDpSiteOrder($dp_id){$db=DbSingleton::getDb();$slave=new slave; $gmanual=new gmanual; session_start(); $answer=0;$err="�������!";
	$dp_id=$slave->qq($dp_id);
	if ($dp==0){$form="<h2 align='center'>���������� ������ ��� �� �� �������� � ������</h2>";}
	if ($dp_id>0){
...
function showSupplToLocalChangeForm($art_id,$brand_id,$dp_id,$dp_str_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave; session_start(); $content="";$header="";
	$art_id=$slave->qq($art_id);$brand_id=$slave->qq($brand_id);$dp_id=$slave->qq($dp_id);
	$form_htm=RD."/tpl/dp_suppl_to_local_change_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
	if ($dp_id>0 && $art_id>0){
		...
		if ($n1==0){$form="<h3 align='center'>����� �볺��� �� ��������.</h3>";}
	}
	return array($form,"³��� ������� � ������� �� �������������");	
}
	
function saveDpSupplToLocalChangeForm($dp_id,$dp_str_id,$art_id,$amount,$price,$storage_id,$cell_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave; session_start(); $answer=0; $err="�������!";
	$dp_id=$slave->qq($dp_id);$dp_str_id=$slave->qq($dp_str_id);$art_id=$slave->qq($art_id);$amount=$slave->qq($amount);$price=$slave->qq($price);$storage_id=$slave->qq($storage_id);$cell_id=$slave->qq($cell_id);
	//print "11111111=$dp_id,$dp_str_id,$art_id,$amount,$price,$storage_id,$cell_id\n";
