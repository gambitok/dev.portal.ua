...
class dp
{
    ...
    {
        $db = DbSingleton::getDb();
        $access_dp_pause = intval(!$access_dp_pause);
    ...
    function showDpStrList($dp_id, $status_dp, $client_id, $cash_id, $usd_to_uah, $euro_to_uah) { $db = DbSingleton::getDb();
        ...
        for ($i = 1; $i <= $kl_rw; $i++) {
            ...
            $amount_collect         = intval($amount_collect);
            $amount_bug             = intval($amount_bug);
    ...
    function setArticleToDp($dp_id, $tpoint_id, $artIdStr, $article_nr_displStr, $brandIdStr, $storageIdStr, $amountStr, $status_action, $order_price, $discount, $return_delay) { $db = DbSingleton::getDb(); $dbt = DbSingleton::getTokoDb();
        ...
        if ($dp_id > 0) {
            ...
            $dp_id      = intval($dp_id);
            $artIdS     = intval($artIdS);
            $storageIdS = intval($storageIdS);
    ...
    function setArticleSupplToDp($dp_id, $artIdStr, $article_nr_displStr, $brandIdStr, $supplIdStr, $supplStorageIdStr, $amountStr, $status_action, $order_price, $discount, $return_delay) { $db = DbSingleton::getDb();
        ...
        if ($dp_id > 0) {
            ...
            $dp_id              = intval($dp_id);
            $artIdS             = intval($artIdS);
            $supplIdS           = intval($supplIdS);
            $supplStorageIdS    = intval($supplStorageIdS);
    ...
    function showArticleRestStorageSelectList($art_id, $dp_id) { $db = DbSingleton::getTokoDb();
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            $rs_amount_rest = intval($rs_amount_rest);
    ...
    function showDPDocErrorForm($dp_id) {
        list(, $arr) = $this->checkDpStr($dp_id);
        $list = "";
        if (!empty($arr)) {
            ...
            if ($n == 1) {
                ...
                if ($status_dp == 79) {
                    ...
                    if ($doc_type_id == 61) {
                        ...
                        list($doc_err) = $this->checkDpStr($dp_id);
                    }
                    list($usd_to_uah, $eur_to_uah) = $this->getKoursData();
            ...
            if ($n == 1) {
                ...
                if ($dp_id > 0) {
                    ...
                    for ($i = 1; $i <= $n1; $i++) {
                        ...
                        if ($suppl_id == 0) {
                            list($ans, $er, $dp_str_id) = $this->setArticleToDp($dp_id, $tpoint_id, $art_id, $art_nr_ds, $brand_id, $storage_id, $amount, $status_action, $order_price, $discount, 0);
                        }

                        if ($suppl_id > 0) {
                            list($ans, $er, $dp_str_id) = $this->setArticleSupplToDp($dp_id, $art_id, $art_nr_ds, $brand_id, $suppl_id, $storage_id, $amount, $status_action, $order_price, $discount, 0);
                        }
    ...
    */
    ...
    function showCombineDpForm()
    {
        ...
        $form = str_replace("{select_main_dp}", $list, $form);
        $form = str_replace("{select_cross_dp}", "", $form);
