...
class dp {
    ...
    function getArtId($code, $brand_id) { $db=DbSingleton::getTokoDb();
        ...
        $r=$db->query("SELECT `ART_ID` FROM `T2_ARTICLES` WHERE `ARTICLE_NR_SEARCH`='$code' AND `BRAND_ID`='$brand_id' LIMIT 1;");$n=$db->num_rows($r);$id=0;
        if ($n==1){	$id=$db->result($r,0,"ART_ID");	}
        return $id;
    }

    function getBrandId($code) { $db=DbSingleton::getTokoDb();
        $slave=new slave;$code=$slave->qq($code);
        $r=$db->query("SELECT `BRAND_ID` FROM `T2_BRANDS` WHERE `BRAND_NAME`='$code' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){	$id=$db->result($r,0,"BRAND_ID");} else $id=0;
        return $id;
    }

    function getBrandName($id) { $db=DbSingleton::getTokoDb();
        $r=$db->query("SELECT `BRAND_NAME` FROM `T2_BRANDS` WHERE `BRAND_ID`='$id' LIMIT 1;");$n=$db->num_rows($r);$name="";
        if ($n==1){	$name=$db->result($r,0,"BRAND_NAME"); }
        return $name;
    }

    function getTpointName($id) { $db=DbSingleton::getDb();
        $r=$db->query("SELECT `name` FROM `T_POINT` WHERE `id`='$id' LIMIT 1;");$n=$db->num_rows($r);$name="";
    ...
    function getDpNote($dp_id) { $db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `J_DP_NOTE` WHERE `dp_id`='$dp_id' LIMIT 1;"); $n=$db->num_rows($r);
        if ($n>0) $text=$db->result($r,0,"text"); else $text="";
        return $text;
    }

    function setDpNote($dp_id, $text) { $db=DbSingleton::getDb();
        session_start();$user=$_SESSION["media_user_id"];$err="Помилка збереження даних!";
        $r=$db->query("SELECT `text` FROM `J_DP_NOTE` WHERE `dp_id`='$dp_id' LIMIT 1;"); $n=$db->num_rows($r);
        if ($n>0) {
    ...
    function dropDpNote($dp_id) { $db=DbSingleton::getDb();
    ...
    function getDpClientName($dp_id) { $db=DbSingleton::getDb();
        $r=$db->query("SELECT `client_id` FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;");$n=$db->num_rows($r);$client_name="";
        if ($n==1) {
    ...
    function getDpClientId($dp_id) { $db=DbSingleton::getDb();
        $r=$db->query("SELECT `client_id` FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;");$n=$db->num_rows($r);$client_id=0;
        if ($n==1){
            $client_id=$db->result($r,0,"client_id");
        }
        return $client_id;
    ...
    function getDpListFilter($key) { $db=DbSingleton::getDb();
        $list="";
        if ($key=="tpoint") {
    ...
    function newDpCard() { $db=DbSingleton::getDb();
    ...
    function newDpFromDp($from_dp_id, $tpoint_id) { $db=DbSingleton::getDb();
        $dp_id=0;
        $r=$db->query("SELECT * FROM `J_DP` WHERE `id`='$from_dp_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            ...
            VALUES ('$dp_id', '$this->prefix_new', '$doc_nom', '$dp_user_id', CURDATE(), '$doc_type_id', '$tpoint_id', '$client_id', '$client_conto_id', '$data_pay', '$cash_id', '$usd_to_uah', '$eur_to_uah', '$vat_use', '$delivery_type_id', '$carrier_id', '$delivery_address');");

            $r1=$db->query("SELECT `id` FROM `orders_new` WHERE `dp_id`='$from_dp_id' LIMIT 1;");$n1=$db->num_rows($r1);
            if ($n1==1){
    ...
    function getStatusDP($dp_id) { $db=DbSingleton::getDb();
        $color="lightyellow"; $k=0;
    ...
    function getDpAccessStatus($media_user_id) { $db = DbSingleton::getDb();
        $r = $db->query("SELECT `access_dp` FROM `media_users` WHERE `id`='$media_user_id' LIMIT 1;");
        $access_dp = $db->result($r, 0, "access_dp");
        return $access_dp;
    }

    function show_dp_list($status=null) { $db = DbSingleton::getDb();
        $gmanual = new gmanual; $list = "";
        session_start(); $ses_tpoint_id=$_SESSION["media_tpoint_id"]; $media_user_id=$_SESSION["media_user_id"]; $media_role_id=$_SESSION["media_role_id"];

        $where = " AND (j.tpoint_id='$ses_tpoint_id' OR j.user_id='$media_user_id') AND ((j.status_dp!=0 AND j.summ>0) OR (j.status_dp=81)) "; $limit = "LIMIT 0,500";
        (!$status) ? $where_status = " AND j.status_dp!=81 " : $where_status = "";
        if ($media_user_id==1) {$where = " AND j.status_dp!=0"; $limit = "";}
        if ($media_role_id==1) {$where = " AND ((j.status_dp!=0 and j.summ>0) OR (j.status_dp=81)) "; $limit = "";}
        ...
        WHERE j.status=1 $where $where_status ORDER BY j.id DESC $limit;"); $n = $db->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
            $id=$db->result($r,$i-1,"id");
            $order_info_id=$db->result($r,$i-1,"order_info_id"); $del_info = $this->getOrderDeliveryInfo($order_info_id);
            $doc_type_name=$db->result($r,$i-1,"doc_type_name");
            $dp_note=$this->getDpNote($id); $dp_note=="" ? $dp_note_cap = "<i class='fa fa-minus'></i>" : $dp_note_cap = "<i class='fa fa-plus' title='$dp_note'></i>";
            if ($this->getDpAccessStatus($media_user_id)) $dp_note_cap = "$dp_note";
...
//            </tr>";
    ...
    function show_dp_list_filter($status, $filstatus, $filauthor, $filtpoint) { $db = DbSingleton::getDb();
        $gmanual=new gmanual; $list="";
        session_start(); $ses_tpoint_id=$_SESSION["media_tpoint_id"]; $media_user_id=$_SESSION["media_user_id"]; $media_role_id=$_SESSION["media_role_id"];

        $where=" AND (j.tpoint_id='$ses_tpoint_id' OR j.user_id='$media_user_id') AND j.status_dp!=0 AND j.summ>0 ";
        (!$status) ? $where_status=" AND j.status_dp!=81 " : $where_status="";
        if ($media_user_id==1){$where=" AND j.status_dp!=0";}
        if ($media_role_id==1){$where=" AND j.status_dp!=0 AND j.summ>0 ";}

        $where_filter="";
        if ($filstatus!="0") if ($filstatus>"81") $where_filter.=" AND jstr.status_dps='$filstatus'"; else $where_filter.=" AND j.status_dp='$filstatus'";
        ...
        FROM `J_DP` j
            ...
            LEFT OUTER JOIN `CASH` on CASH.id=j.cash_id
            LEFT OUTER JOIN `manual` dlv on (dlv.key='delivery_type' and dlv.id=j.delivery_type_id)
        WHERE j.status=1 $where $where_status $where_filter GROUP BY j.id ORDER BY j.id DESC LIMIT 0,500;"); $n = $db->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
            $id=$db->result($r,$i-1,"id");
            $order_info_id=$db->result($r,$i-1,"order_info_id"); $del_info = $this->getOrderDeliveryInfo($order_info_id);
            $doc_type_name=$db->result($r,$i-1,"doc_type_name");
...
//            </tr>";
    ...
    function showDpCard($dp_id) { $db = DbSingleton::getDb();
        $client = new clients; $gmanual = new gmanual;
        $prefix = ""; $comment = ""; $cells_show = ""; $label_art_unknown = ""; $doc_nom = 0;
        session_start(); $user_id = $_SESSION["media_user_id"]; $user_name = $_SESSION["user_name"];
        $form = ""; $form_htm = RD."/tpl/dp_card.htm"; if (file_exists("$form_htm")) { $form = file_get_contents($form_htm);}

        $r = $db->query("SELECT * FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;"); $n = $db->num_rows($r);
        ...
        if ($n==1) {
            ...
            if ($user_id!=$user_use && $user_use>0) {
                ...
                $admin_unlock = "";
                if ($user_id==1 || $user_id==2 || $user_id==7) { $admin_unlock = "<button class='btn btn-sm btn-warning' onClick='unlockDpCard(\"$dp_id\");'><i class='fa fa-unlock'></i> Розблокувати</button>"; }
            ...
            if ($user_id==$user_use || $user_use==0) {
                ...
                if ($status_dp==80) {
                    ...
                    if ($media_role_id==1 || $media_role_id==7) { $form = str_replace("{oper_disabled3}"," ",$form); }
                }
                if ($status_dp<82) {
                ...
                list($balans_client,$cash_abr) = $client->getClientGeneralSaldo($client_id);
                if ($balans_client>=0) {$balans_style = "info";} else {$balans_style = "danger";}

                if ($client_id!=$client_conto_id && $client_conto_id!=0) {
                    $balans_display = "";
                    list($balans_conto,$cash_conto_abr) = $client->getClientGeneralSaldo($client_conto_id);
                    if ($balans_conto>=0) {$balans_conto_style = "info";} else {$balans_conto_style = "danger";}
                } else {
                    $balans_display = "hide"; $balans_conto = ""; $cash_conto_abr = ""; $balans_conto_style = "";
                }

                $form = str_replace("{balans_client}","$balans_client $cash_abr",$form);
                $form = str_replace("{balans_style}",$balans_style,$form);
    ...
    function labelUnknownsCount($dp_id) { $db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `J_DP_STR_UNKNOWN` WHERE `dp_id`='$dp_id';");$amount=$db->num_rows($r);
        if ($amount==0) $amount="";
        return $amount;
    }

    function getDpStorageList() { $db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `STORAGE`;");$n=$db->num_rows($r);$list="";
        for ($i=1; $i<=$n; $i++){
            $name=$db->result($r,$i-1,"name");
    ...
    function closeDpCard($dp_id) {
        session_start();$user_id=$_SESSION["media_user_id"];
        $this->unsetDpCardUserAccess($dp_id,$user_id);$answer=1;
        return $answer;
    }
    ...
    function clearDpStr($dp_id) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        $slave=new slave;$answer=0;$err="";$dp_id=$slave->qq($dp_id);
        $r=$db->query("SELECT `oper_status` FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            ...
            if ($oper_status==30) {
                ...
                for ($i1=1; $i1<=$n1; $i1++){
                    ...
                    if ($status_dps_str==93){
                        ...
                        $rs=$dbt->query("SELECT * FROM `T2_ARTICLES_STRORAGE` WHERE `ART_ID`='$art_id' AND `STORAGE_ID`='$storage_id_from' LIMIT 1;");$ns=$dbt->num_rows($rs);
                        if ($ns==1){
    ...
    function getDpTpointInfo($client_id) { $db=DbSingleton::getDb();
        ...
        FROM `A_CLIENTS` c 
            LEFT OUTER JOIN `A_CLIENTS_CONDITIONS` acc ON (acc.client_id=c.id)
            LEFT OUTER JOIN `T_POINT` tp ON (tp.id=acc.tpoint_id)
        WHERE c.id='$client_id' GROUP BY c.id LIMIT 1;");
    ...
    function unlinkDpClient($dp_id) { $db=DbSingleton::getDb();
        $slave=new slave;
        $answer=0;$err="Помилка збереження даних!";$dp_id=$slave->qq($dp_id);
        if ($dp_id>0){
    ...
    function getClientArticleMaxDiscount($art_id,$price) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        session_start();$max_manager_discount=$_SESSION["user_discount"];
        $max_discount_persent=0;$max_discount_price=0;
        ...
        WHERE t2aps.ART_ID='$art_id' AND t2apr.in_use='1' LIMIT 1;");$n=$dbt->num_rows($r);
        if ($n==1){
    ...
    function getIncomeStatusDpsName($art_id, $suppl_id) { $db=DbSingleton::getDb();
        ...
        $rs=$db->query("SELECT instr.art_id, instr.amount, inc.storage_id, inc.storage_cells_id, inc.time_stamp 
        FROM `J_INCOME_STR` instr 
    ...
    function getDpStrStatus($dp_id) { $db=DbSingleton::getDb();
        ...
        WHERE j.dp_id='$dp_id' GROUP BY j.status_dps ORDER BY j.status_dps DESC;"); $n=$db->num_rows($r); $status_name="";
        for ($i=1;$i<=$n;$i++){
    ...
    function showDpStrList($dp_id, $status_dp, $client_id, $cash_id, $usd_to_uah, $euro_to_uah) { $db = DbSingleton::getDb();
        $slave = new slave;
        $list=""; $function_amount_change=""; $suppl_card="";
        $amount_bug_info=""; $delivery_info=""; $sum_weight=0; $sum_volume=0; $summ_dp=0;
        if ($status_dp==""){$status_dp=79;}
        if ($client_id==""){$client_id=$this->getDpClient($dp_id);}
        if ($cash_id==""){$cash_id=$this->getDpCashId($dp_id);}
        if ($usd_to_uah=="" || $euro_to_uah==""){list($usd_to_uah,$euro_to_uah)=$this->getKoursData();}
        ...
        FROM `J_DP_STR` j 
            ...
            LEFT OUTER JOIN `STORAGE` s2 ON (s2.id=j.location_storage_id) 
        WHERE j.dp_id='$dp_id' ORDER BY j.id ASC;"); $n=$db->num_rows($r); $kl_rw=$n;
        for ($i=1;$i<=$kl_rw;$i++) {
            ...
            if ($location_storage_id==$storage_id_from) $location_storage_name=""; else $location_storage_name="=> $location_storage_name";
            ...
            $amount=$db->result($r,$i-1,"amount"); $amount_dp=$amount; $amount=intval($amount);
            ...
            $amount_collect=intval($amount_collect); $amount_bug=intval($amount_bug);
            $price=$slave->to_money($db->result($r,$i-1,"price")); $rating_price=$db->result($r,$i-1,"price");
            ...
            $inc_status_name=""; if ($status_dps==139){ $inc_status_name=$this->getIncomeStatusDpsName($art_id,$suppl_id); }
            $status_dps_name=$db->result($r,$i-1,"status_dps_name").$inc_status_name;

            $suppl_card="";
            if ($suppl_id>0) {
                $suppl_card="<button class='btn btn-xs btn-default btn-danger' onclick='showDpSupplInfo($suppl_id, $suppl_storage_id);'><i class='fa fa-info'></i> Постачальник</button>";
            }
            ...
            list($max_discount_persent, $max_discount_price) = $this->getClientArticleMaxDiscount($art_id,$rating_price);
            ...
            if ($status_dp==79) {
                ...
                if (!($action_clients->checkActionStr($art_id,$client_id))) {
                    ...
                    list($action_amount,$action_price)=$action_clients->checkActionStr($art_id,$client_id);
            ...
            if ($status_dp>79) {
                if ($article_nr_displ!="") {
                    ...
                    if ($status_dps==139 && $inc_status_name!="") {
                        $link_btn=" <button type='button' class='btn btn-xs btn-danger' onClick=\"showSupplToLocalChangeForm('$i','$art_id','$article_nr_displ','$dp_id','$dp_str_id');\"><i class=\"fa fa-link\"></i></button>";
                    }
                    $list.="<tr>
        ...
        if ($status_dp==79) {
            $list="<tr id='dpStrNewRow' class='hidden'>
    ...
    function saveDpCard($dp_id, $data_pay, $cash_id, $dp_summ, $doc_type_id, $tpoint_id, $client_id, $client_conto_id, $delivery_type_id, $carrier_id, $delivery_address) { $db=DbSingleton::getDb();
        $slave=new slave;
        $answer=0; $err="Помилка збереження даних!";
        $dp_id=$slave->qq($dp_id);$data_pay=$slave->qq($data_pay);$cash_id=$slave->qq($cash_id);$dp_summ=$slave->qq($dp_summ);$doc_type_id=$slave->qq($doc_type_id);$tpoint_id=$slave->qq($tpoint_id);
        $client_id=$slave->qq($client_id);$client_conto_id=$slave->qq($client_conto_id);$delivery_type_id=$slave->qq($delivery_type_id);$carrier_id=$slave->qq($carrier_id);$delivery_address=$slave->qq($delivery_address);
        if ($dp_id>0){
    ...
    function roundingDpDiscount($dp_id, $price_end, $amount) {
        ...
        if ($cash_id==1) $price=$this->getClientPriceRounding($client_id, $price_end); else $price=$price_end;
        $summ=$price*$amount;
        return array($price, $summ);
    }

    function updateDpStrPrice($dp_id, $str_id, $discount_new) { $db=DbSingleton::getDb();
        $slave=new slave;
        $answer=0; $err="Помилка збереження даних!";
        ...
        if ($dp_id>0 && $str_id>0 && $discount_new!=""){
            ...
            $r=$db->query("SELECT * FROM `J_DP_STR` WHERE `dp_id`='$dp_id' AND `id`='$str_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
    ...
    function setArticleToDp($dp_id, $tpoint_id, $artIdStr, $article_nr_displStr, $brandIdStr, $storageIdStr, $amountStr, $status_action, $order_price, $return_delay) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        $slave=new slave;
        $answer=0;$err="Помилка збереження даних!";
        $weight=$volume=$empty_kol=$label_empty=$dp_summ="";
        $idS=0;$discountEx=0;$rr_amount=0;$rr_reserv=0;$amountEx=0;
        $dp_id=$slave->qq($dp_id);$tpoint_id=$slave->qq($tpoint_id);

        if ($dp_id>0){
            $artIdS=$slave->qq($artIdStr);$article_nr_displS=$slave->qq($article_nr_displStr);$brandIdS=$slave->qq($brandIdStr);
            $amountS=$slave->qq($amountStr);$storageIdS=$slave->qq($storageIdStr);

            $r=$db->query("SELECT `id`, `amount`, `discount` FROM `J_DP_STR` 
            WHERE `dp_id`='$dp_id' AND `art_id`='$artIdS' AND `storage_id_from`='$storageIdS' AND `status_dps`='93' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
            ...
            if ($amountS<=$max_moving){
                ...
                if ($idS>0){
                    if ($artIdS!="" && $artIdS>0 && $article_nr_displS!=""){
                        ...
                        //price in USD!!!!!
                        if ($status_action>0) $article_price=$order_price; else {
                            $article_price=$this->getArticlePrice($artIdS,$dp_id);
                        }

                        if ($article_price>0){
                            if ($status_action>0) $price_end=$order_price; else {
                                $return_delay=0;
                            ...
                            $db->query("UPDATE `J_DP_STR` SET `art_id`='$artIdS', `article_nr_displ`='$article_nr_displS', `brand_id`='$brandIdS', `amount`='$amountS', `storage_id_from`='$storageIdS', 
                            ...
                            `return_delay`='$return_delay' WHERE `id`='$idS' AND `dp_id`='$dp_id';");

                            list($weight,$volume,$empty_kol)=$this->updateDpWeightVolume($dp_id);
                            $dp_summ=$this->updateDpSumm($dp_id);

                            $rr=$dbt->query("SELECT * FROM `T2_ARTICLES_STRORAGE` WHERE `ART_ID`='$artIdS' AND `STORAGE_ID`='$storageIdS' LIMIT 1;");$nr=$dbt->num_rows($rr);
                            if ($nr==1){
    ...
    function setArticleSupplToDp($dp_id, $artIdStr, $article_nr_displStr, $brandIdStr, $supplIdStr, $supplStorageIdStr, $amountStr, $status_action, $order_price, $return_delay) { $db=DbSingleton::getDb();
        $slave=new slave;
        $answer=0;$err="Помилка збереження даних!";
        ...
        if ($dp_id>0){
            $artIdS=$slave->qq($artIdStr);$article_nr_displS=$slave->qq($article_nr_displStr);$brandIdS=$slave->qq($brandIdStr);
            $amountS=$slave->qq($amountStr);$supplIdS=$slave->qq($supplIdStr);$supplStorageIdS=$slave->qq($supplStorageIdStr);
            $amountEx=0;
            $r=$db->query("SELECT `id`, `amount`, `discount` FROM `J_DP_STR` 
            WHERE `dp_id`='$dp_id' AND `art_id`='$artIdS' AND `suppl_id`='$supplIdS' AND `suppl_storage_id`='$supplStorageIdS' AND `status_dps`='93' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $idS=$db->result($r,0,"id");
                $amountEx=$db->result($r,0,"amount");
                $discountEx=$db->result($r,0,"discount");
            ...
            if ($idS>0){
                if ($artIdS!="" && $supplIdS>0 && $article_nr_displS!="") {
                    ...
                    if ($article_price>0){
                        ...
                        `suppl_storage_id`='$supplStorageIdS', `price`='$article_price', `price_end`='$price_end', `summ`='$summ', `reserv_type_id`='$reserv_type_id', `return_delay`='$return_delay' 
    ...
    function getClientOrgType($client_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT `org_type` FROM `A_CLIENTS` WHERE `id`='$client_id' LIMIT 1;");$n=$db->num_rows($r);$org_type=0;
    ...
    function getDpClientContoCash($client_id){$db=DbSingleton::getDb();
        $cash_id=1;$answer=0;$err="Помилка";
    ...
    function getDpClientDocType($client_id){$db=DbSingleton::getDb();
        $doc_type_id=64;$answer=0;$err="Помилка";
    ...
    function changeDpCash($dp_id,$cash_id){$db=DbSingleton::getDb();
        ...
        if ($dp_id>0){
            $r=$db->query("SELECT `oper_status`, `client_conto_id` FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $oper_status=$db->result($r,0,"oper_status");
                if ($oper_status==30){
                    $client_conto_id=$db->result($r,0,"client_conto_id");
    ...
    function updateDpPriceCash($dp_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;");$n=$db->num_rows($r);$answer=0;
        if ($n==1){
            $oper_status=$db->result($r,0,"oper_status");
            if ($oper_status==30) {
    ...
    function getArticlePrice($art_id, $dp_id) { $dbt=DbSingleton::getTokoDb();
        list($usd_to_uah, $eur_to_uah) = $this->getKoursData(); $price = 0;
        if ($dp_id>0 && $art_id!="") {
    ...
    function dropDpStr($dp_id, $dp_str_id) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        $slave=new slave;
        $answer=0;$err="Помилка індексу";$dp_id=$slave->qq($dp_id);$dp_summ="";
        $r=$db->query("SELECT `oper_status`, `status` FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $status=$db->result($r,0,"status");
            $oper_status=$db->result($r,0,"oper_status");
            if ($oper_status==30 && ($status==1 ||$status==1)){
                $r1=$db->query("SELECT * FROM `J_DP_STR` WHERE `id`='$dp_str_id' LIMIT 1;");$n1=$db->num_rows($r1);
                if ($n1==1){
    ...
    function updateDpSumm($dp_id){$db=DbSingleton::getDb();
        ...
        for ($i=1;$i<=$n;$i++){
            ...
            if ($cash_id==1) $price_cash = $this->getClientPriceRounding($this->getDpClient($dp_id),$price_cash);
    ...
    function getDpCashId($dp_id){$db=DbSingleton::getDb();
        $cash_id=2;
        $r=$db->query("SELECT `cash_id` FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;"); $n=$db->num_rows($r);
    ...
    function getDpClient($dp_id){$db=DbSingleton::getDb();
        $client_conto_id=0;
        $r=$db->query("SELECT `client_id`, `client_conto_id` FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;"); $n=$db->num_rows($r);
        if ($n==1){
            $client_id=$db->result($r,0,"client_id");
            $client_conto_id=$db->result($r,0,"client_conto_id");
    ...
    function getDpClientPriceLevels($dp_id){$db=DbSingleton::getDb();
        ...
        if ($n==1){
            ...
            if ($client_conto_id>0){
                $r1=$db->query("SELECT * FROM `A_CLIENTS_CONDITIONS` WHERE `client_id`='$client_conto_id' LIMIT 1;"); $n1=$db->num_rows($r1);
                if ($n1==1){
    ...
    function showArticlesSearchDocumentList($art,$brand_id,$search_type,$dp_id,$tpoint_id) { $db=DbSingleton::getTokoDb();
        $cat=new catalogue;
        $r="";$query="";
        ...
        if ($search_type==0){
            ...
            if (($n>1 && $brand_id!="") || $one_result==1){$ak=array();$rk=array();
                ...
                SUM(`t2asc`.`RESERV_AMOUNT`) as reserv, s.name as storage_name, s.id as storage_id, t2apr.price_".$price_lvl.", t2sai.suppl_id, t2sai.return_delay, t2sai.warranty_info, 
        ...
        if ($search_type==1){
            $query="SELECT t2a.ART_ID, t2a.BRAND_ID, t2a.ARTICLE_NR_DISPL, t2b.BRAND_NAME, t2n.NAME, t2n.INFO, t2bc.BARCODE, gg.NAME as goods_group_name, t2apr.price_".$price_lvl.", t2sai.suppl_id, 
            t2sai.return_delay, t2sai.warranty_info, t2si.price_usd, t2si.client_storage_id, t2si.stock_suppl
            FROM `T2_ARTICLES` t2a 
                LEFT OUTER JOIN T2_BRANDS t2b on t2b.BRAND_ID=t2a.BRAND_ID 
                LEFT OUTER JOIN T2_NAMES t2n on t2n.ART_ID=t2a.ART_ID 
                LEFT OUTER JOIN T2_BARCODES t2bc on t2bc.ART_ID=t2a.ART_ID 
        ...
        if ($search_type==2){
            $query="SELECT t2a.ART_ID, t2a.BRAND_ID, t2a.ARTICLE_NR_DISPL, t2b.BRAND_NAME, t2n.NAME, t2n.INFO, t2bc.BARCODE, gg.NAME as goods_group_name, SUM(`t2asc`.`AMOUNT`) as stock, 
            SUM(`t2asc`.`RESERV_AMOUNT`) as reserv, s.name as storage_name, s.id as storage_id, t2apr.price_".$price_lvl.", t2sai.suppl_id, t2sai.return_delay, t2sai.warranty_info, t2si.price_usd, 
        ...
        if ($list2==""){
            ...
            for ($i=1;$i<=$n;$i++){$amountRestNotTpoint="";$amountRestTpoint="";$price=0;$suppl_storage_code=0;
                ...
                if ($suppl_id==0){
                    ...
                    list($tpoint_stock,$tpoint_reserv)=$this->getArticleRestTpoint($art_id,$tpoint_id);
                    $amountRestTpoint="$tpoint_stock/$tpoint_reserv";
                }
                if ($suppl_id>0){
    ...
    function getDpTpoint($dp_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT `tpoint_id` FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1");$n=$db->num_rows($r);$tpoint_id=0;
    ...
    function getArticleTpointDeliveryInfo($tpoint_id,$art_id){$db=DbSingleton::getTokoDb();
        $slave=new slave;
        $info="Не вказано"; $week_day=date("N");$cur_time=date("H:i:s");
    ...
    function getTpointDeliveryInfo($tpoint_id,$storage_id){$db=DbSingleton::getDb();
        $info="Не вказано";$week_day=date("N");$cur_time=date("H:i:s");$slave=new slave;
        ...
        ORDER BY `delivery_days` ASC LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $delivery_days=$db->result($r,0,"delivery_days");
            $time_from_del=substr($db->result($r,0,"time_from_del"),0,-3);
            $time_to_del=substr($db->result($r,0,"time_to_del"),0,-3);
    ...
    function getArticleStorageAmountDp($art_id,$dp_id,$storage_id){$db=DbSingleton::getDb();
        ...
        WHERE `dp_id`='$dp_id' AND `status_dps`='93' AND `storage_id_from`='$storage_id' AND `art_id`='$art_id' LIMIT 1;");$n=$db->num_rows($r);
    ...
    function getArticleSupplStorageAmountDp($art_id,$dp_id,$suppl_id,$suppl_storage_id){$db=DbSingleton::getDb();
        $amount=0;
        $r=$db->query("SELECT `amount` FROM `J_DP_STR` 
        WHERE `dp_id`='$dp_id' AND `status_dps`='93' AND `suppl_id`='$suppl_id' AND `suppl_storage_id`='$suppl_storage_id' AND `art_id`='$art_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){ $amount=$db->result($r,0,"amount"); }
        return $amount;
    }

    function showArticleRestStorageSelectList($art_id,$dp_id){$db=DbSingleton::getTokoDb();
        $tpoint_id=$this->getDpTpoint($dp_id);$list="";
        ...
        for ($i=1;$i<=$n;$i++){
            ...
            $cur_amount>0 ? $style="style='background:pink;'" : $style="";
            $delivery_info=$this->getTpointDeliveryInfo($tpoint_id,$id);
            $amount=intval($amount); $cur_amount=intval($cur_amount); $reserv_amount_rest=intval($reserv_amount_rest);
    ...
    function showDpAmountInputWindow($art_id,$dp_id,$storage_id){
        ...
        $form=str_replace("{art_id}",$art_id,$form);
        $amount=intval($this->getArticleStorageAmountDp($art_id,$dp_id,$storage_id));
    ...
    function getArticleName($art_id){$db=DbSingleton::getTokoDb();
        $r=$db->query("SELECT * FROM `T2_NAMES` WHERE `ART_ID`='$art_id' AND `LANG_ID`='41' LIMIT 1;");$n=$db->num_rows($r);$name="";
        if ($n==1){
            $name=$db->result($r,0,"NAME");
        }
        if ($n==0){
    ...
    function getClientContoSelectList($client_id,$sel_id){$db=DbSingleton::getDb();
        $list="";
        if ($client_id>0){
            $r=$db->query("SELECT `id`, `name` FROM `A_CLIENTS` WHERE `status`='1' AND (`parrent_id`='$client_id' OR `id`='$client_id' OR `id`='$sel_id') 
            ORDER BY `name`, `id` ASC;");$n=$db->num_rows($r);
            for ($i=1;$i<=$n;$i++){
                $id=$db->result($r,$i-1,"id");
                $name=$db->result($r,$i-1,"name");
                $sel="";if ($sel_id==$id){$sel="selected='selected'";}
...
//    function getRateTypeDeclarationdocumentPos($costums_id,$country_id){$db=DbSingleton::getDb();
//        $manual=new manual;$rate=0;$type_declaration="";$type_declaration_id=0;$duty=0;
//        $r=$db->query("SELECT `DUTY` FROM `T2_COUNTRIES` WHERE `country_id`='$country_id' LIMIT 1;");$n=$db->num_rows($r);
//        if ($n==1){
//            $duty=$db->result($r,0,"DUTY");
//        }
//        $r=$db->query("SELECT `PREFERENTIAL_RATE`, `FULL_RATE`, `TYPE_DECLARATION` FROM `T2_COSTUMS` WHERE `costums_id`='$costums_id' LIMIT 1;");$n=$db->num_rows($r);
//        if ($n==1){
...
//    }
    ...
    function loadDpCommets($dp_id){$db=DbSingleton::getDb();
        ...
        WHERE cc.dp_id='$dp_id' ORDER BY cc.id DESC;");$n=$db->num_rows($r);$list="";
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
    ...
    function dropDpComment($dp_id,$comment_id){$db=DbSingleton::getDb();
        ...
        if ($dp_id>0 && $comment_id>0){
            $r=$db->query("SELECT * FROM `J_DP_COMMENTS` WHERE `dp_id`='$dp_id' AND `id`='$comment_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
    ...
    function dpCDNDropFile($dp_id,$file_id){$db=DbSingleton::getDb();
        ...
        if ($dp_id>0 && $file_id>0){
            $r=$db->query("SELECT `FILE_NAME` FROM `J_DP_CDN` WHERE `dp_id`='$dp_id' AND `id`='$file_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
...
//    function showCategoryCheckList($dp_id){$db=DbSingleton::getDb();
...
//    }
    ...
    function checkdpCategorySelect($dp_id,$category_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT `category_id` FROM `A_CLIENTS_CATEGORY` WHERE `dp_id`='$dp_id' AND `category_id`='$category_id' LIMIT 1;");$n=$db->num_rows($r);$ch=0;
        if ($n==1){$ch=1;}
        return $ch;
    }

    function labelArtEmptyCount($dp_id,$kol){$label="";
        if ($kol==0 || $kol==""){ list(,,$kol)=$this->updateDpWeightVolume($dp_id); }
    ...
    function labelCommentsCount($dp_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT COUNT(`id`) as kol FROM `J_DP_COMMENTS` WHERE `dp_id`='$dp_id';");$kol=0+$db->result($r,0,"kol");$label="";
        if ($kol>0){$label="<span class='label label-tab label-info'>$kol</span>";}
        return array($kol, $label);
    }
    ...
    function startDpExecute($dp_id){$db=DbSingleton::getDb();
        $slave=new slave;
        $answer=0;$err="Помилка індексу!";$suppl_ex=0;$dp_id=$slave->qq($dp_id);
        if ($dp_id>0){
            $r=$db->query("SELECT `tpoint_id`, `status_dp` FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;");$n=$db->num_rows($r);
            if ($n==1){
                $status_dp=$db->result($r,0,"status_dp");
                $tpoint_id=$db->result($r,0,"tpoint_id");
                if ($status_dp==80){
    ...
    function checkRemoteStorage($dp_id,$tpoint_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT COUNT(ds.id) as kol 
        FROM `J_DP_STR` ds 
            LEFT OUTER JOIN `T_POINT_STORAGE` ps ON (ps.storage_id=ds.storage_id_from) 
        WHERE ds.dp_id='$dp_id' AND ((ps.tpoint_id='$tpoint_id' AND ps.`local`='42') OR (ps.tpoint_id!='$tpoint_id'));");$suppl_ex=0;
        $kol_rem_str=$db->result($r,0,"kol");
        if ($kol_rem_str>0){$suppl_ex=1;}
        return $suppl_ex;
    }

    function getStorageToTpointLocal($tpoint_id,$storage_id){$db=DbSingleton::getDb();
        $r=$db->query("SELECT `local` FROM `T_POINT_STORAGE` WHERE `tpoint_id`='$tpoint_id' AND `storage_id`='$storage_id' AND `status`='1' LIMIT 1;");$n=$db->num_rows($r);$local=42;
        if ($n==1){$local=$db->result($r,0,"local");}
        return $local;
    }

    function createStorsel($dp_id,$tpoint_id,$storage_id){$db=DbSingleton::getDb();
        session_start();$user_id=$_SESSION["media_user_id"];$cur_date=date("Y-m-d H:i:s");
        ...
        VALUES ('$select_id','2','$dp_id','$cur_date','$tpoint_id','$storage_id','$user_id');");
        return $select_id;
    }

    function makeDpJmovingStorselPreorder($dp_id,$local){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();
        $jmoving=new jmoving;
        session_start();$user_id=$_SESSION["media_user_id"];
        $tpoint_id=$this->getDpTpoint($dp_id);$cell_use=0;
        $r=$db->query("SELECT `storage_id_from` FROM `J_DP_STR` WHERE `dp_id`='$dp_id' AND `status_dps`='93' GROUP BY `storage_id_from`;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $storage_id_from=$db->result($r,$i-1,"storage_id_from");
            if ($storage_id_from>0){
                $storage_local=$this->getStorageToTpointLocal($tpoint_id,$storage_id_from);
                if ($storage_local==41){
                    $r1=$db->query("SELECT * FROM `J_DP_STR` WHERE `dp_id`='$dp_id' AND `status_dps`='93' AND `storage_id_from`='$storage_id_from';");$n1=$db->num_rows($r1);
                    if ($n1>0){
                        //create STORSEL
                ...
                if ($storage_local==42 && $local==41){
                    ...
                    if ($n1>0){
                        ...
                        for ($i1=1;$i1<=$n1;$i1++){
                            ...
                            $db->query("INSERT INTO `J_MOVING_STR` (`jmoving_id`,`art_id`,`article_nr_displ`,`brand_id`,`amount`,`storage_id_from`) 
                            VALUES ('$jmoving_id','$art_id','$article_nr_displ','$brand_id','$amount','$storage_id_from');");
                            // обновляем статус записи что бы понимать на каком этапе находится артикул;
                ...
                if ($storage_local==42 && $local==42){
                    ...
                    if ($n1>0){
                        ...
                        $r2=$db->query("SELECT * FROM `J_DP_STR` WHERE `dp_id`='$storage_dp_id' AND `status_dps`='93' AND `storage_id_from`='$storage_id_from';");$n2=$db->num_rows($r2);
                        if ($n2>0){
            ...
            if($storage_id_from==0){
                ...
                $r1=$db->query("SELECT * FROM `J_DP_STR` WHERE `dp_id`='$dp_id' AND `status_dps`='93' AND `storage_id_from`='$storage_id_from' AND `suppl_id`>0;");$n1=$db->num_rows($r1);
                if ($n1>0){
                    for ($i1=1;$i1<=$n1;$i1++){
                        $id=$db->result($r1,$i1-1,"id");
                        ...
                        VALUES ('$dp_id','$id','$art_id','$article_nr_displ','$suppl_art_id','$brand_id','$brand_name','$amount','$price_end','$dp_cash_id','$suppl_id','$suppl_storage_id','$tpoint_id','$user_id');"); // обновляем статус записи что бы
                        // обновляем статус записи что бы понимать на каком этапе находится артикул;
    ...
    function getTpointStorageLocal($tpoint_id){$db=DbSingleton::getDb();
        $storage_id=0;
        $r=$db->query("SELECT `storage_id` FROM `T_POINT_STORAGE` WHERE `tpoint_id`='$tpoint_id' AND `local`='41' AND `default`='1' AND `status`='1' LIMIT 1");$n=$db->num_rows($r);
    ...
    function getStorageCellsData($storage_id) { $db=DbSingleton::getDb();
        $cell_use=0;$cell_id=0;
        $r=$db->query("SELECT `id` FROM `STORAGE_CELLS` WHERE `storage_id`='$storage_id' AND `default`='1' AND `status`='1' LIMIT 1");$n=$db->num_rows($r);
        if ($n==1){$cell_use=1;$cell_id=$db->result($r,0,"id");}
        if ($n==0){
            $r1=$db->query("SELECT `id` FROM `STORAGE_CELLS` WHERE `storage_id`='$storage_id' AND `default`='0' AND `status`='1' LIMIT 1");$n1=$db->num_rows($r1);
    ...
    function getSelectFromJmoving($doc_nom) { $db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `J_SELECT` WHERE `parrent_doc_type_id`=1 AND `parrent_doc_id`='$doc_nom' LIMIT 1;"); $n=$db->num_rows($r);
        $n>0 ? $select_id = $db->result($r,0,"id") : $select_id="";
        return $select_id;
    }
    ...
    function loadDpJmoving($dp_id) { $db=DbSingleton::getDb();
        ...
        ORDER BY j.status_jmoving ASC, j.data DESC, j.doc_nom DESC, j.id DESC;");$n=$db->num_rows($r);$list="";
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
            $select_id=$this->getSelectFromJmoving($id);
            $type_id=$db->result($r,$i-1,"type_id");
            $type_name="<i class='fa fa-inbox'></i> Внутрішнє переміщення";if ($type_id==1){$type_name="<i class='fa fa-truck'></i> Між складами";}
            $prefix=$db->result($r,$i-1,"prefix");
            $doc_nom=$db->result($r,$i-1,"doc_nom");
            ...
            if ($doc_type_id==63) $print_id=$select_id; else $print_id=$id;
            if ($print_id=="") $print_id=$id;
        ...
        if ($n==0){$list="<tr><td colspan=7 align='center'>Віддалені переміщення відсутні</td></tr>";}
    ...
    function loadDpStorsel($dp_id) { $db=DbSingleton::getDb();
        ...
        if ($n==0){$list="<tr><td colspan=12 align='center'>Локальні відбори відсутні</td></tr>";}
    ...
    function viewDpStorageSelect($dp_id,$select_id,$select_status){$db=DbSingleton::getDb();
        ...
        $r=$db->query("SELECT * FROM `J_SELECT_STR` WHERE `select_id`='$select_id' ORDER BY `id` ASC;");$n=$db->num_rows($r);$list="";
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
            $art_id=$db->result($r,$i-1,"art_id");
            $article_nr_displ=$db->result($r,$i-1,"article_nr_displ");
    ...
    function getStorageSelectBugList($select_id,$art_id,$str_id){$db=DbSingleton::getDb();
        $manual=new manual;
        $r=$db->query("SELECT * FROM `J_SELECT_STR_BUG` WHERE `select_id`='$select_id' AND `art_id`='$art_id' AND `str_id`='$str_id' ORDER BY `id` ASC;");$n=$db->num_rows($r);$list="";
        for ($i=1;$i<=$n;$i++){
    ...
    function loadDpSaleInvoice($dp_id){$db=DbSingleton::getDb();
        ...
        WHERE sv.status=1 AND sv.dp_id='$dp_id' ORDER BY sv.status_invoice ASC, sv.data_create ASC, sv.id ASC;");$n=$db->num_rows($r);$list="";
    ...
    function showDpStorselForSaleInvoice($dp_id){$db=DbSingleton::getDb();
        $gmanual=new gmanual;$list="";$loc_type_name="";
    ...
    function showDpStorselForWriteOff($dp_id){$db=DbSingleton::getDb();
        $gmanual=new gmanual;$list="";$loc_type_name="";
        $form="";$form_htm=RD."/tpl/dp_storsel_list_for_write_off.htm";	if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("SELECT `select_id` FROM `J_WRITE_OFF_STORSEL` WHERE `dp_id`='$dp_id';");$n=$db->num_rows($r);$sel_use="0";
        for ($i=1;$i<=$n;$i++){	$sel_use.=",".$db->result($r,$i-1,"select_id");	}
        ...
        $r=$db->query("SELECT * FROM `manual` WHERE `key`='write_off';"); $n=$db->num_rows($r); $status_write_off_select="";
         for ($i=1;$i<=$n;$i++) {
             $id = $db->result($r, $i - 1, "id");
             $name = $db->result($r, $i - 1, "mcaption");
    ...
    function checkClientSaleInvoiceDataPayLimit($client_id){$db=DbSingleton::getDb();
        ...
        $kol=$db->result($r,0,"kol");$doc_ex=0;
        if ($kol>0){$doc_ex=1;}
        return $doc_ex;
    }
    ...
    function checkClientCreditLimitBeforeSaleInvoice($dp_id,$kol_storsel,$ar_storsel){$db=DbSingleton::getDb();
        ...
        if ($n==1){
            ...
            for ($i2=1;$i2<=$n2;$i2++){
                ...
                $sel_dp_id=$db->result($r2,$i2-1,"dp_id"); $client_id=$this->getDpClient($sel_dp_id);
...
//                $summ_all+=$summ2;
    ...
    function getClientCreditLimit($client_id){$db=DbSingleton::getDb();$credit_limit=0;
        $r=$db->query("SELECT `credit_limit` FROM `A_CLIENTS_CONDITIONS` WHERE `client_id`='$client_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
    ...
    function checkLimitBeforeSaleInvoice($dp_id,$kol_storsel,$ar_storsel) { $db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;");$n=$db->num_rows($r);$summ_all=0;
        if ($n==1){
            $cash_id=$db->result($r,0,"cash_id");
            $usd_to_uah=$db->result($r,0,"usd_to_uah");
            $eur_to_uah=$db->result($r,0,"eur_to_uah");
            ...
            GROUP BY jds.art_id, jss.storage_id_from, jss.cell_id_from;");$n2=$db->num_rows($r2);$summ_all=0;
    ...
    function sendDpStorselToWriteOff($dp_id,$kol_storsel,$ar_storsel,$status_write_off) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        ...
        $write_off_nom=0;$dp_id=$slave->qq($dp_id);$kol_storsel=$slave->qq($kol_storsel);
        ...
        if ($dp_id>0){
            if ($kol_storsel>0){
                ...
                $r=$db->query("SELECT MAX(`id`) as mid FROM `J_WRITE_OFF`;");$write_off_id=0+$db->result($r,0,"mid")+1; $write_off_nom=$write_off_id;
                $db->query("INSERT INTO `J_WRITE_OFF` (`id`,`dp_id`,`status_write_off`) VALUES ('$write_off_id','$dp_id','$status_write_off');");$ai=0;
                for ($i=1;$i<=$kol_storsel;$i++){
                    if ($ar_storsel[$i]!="" && $ar_storsel[$i]>0){$ai+=1;
                ...
                if ($n==1){
                    ...
                    list(,,$data_pay)=$this->getClientPaymentDelay($client_conto_id);
                    $data_create=date("Y-m-d"); $seller_prefix="СП"; $seller_doc_nom=$write_off_id;
                    ...
                    `carrier_id`='$carrier_id', `delivery_address`='$delivery_address', `user_id`='$user_id' WHERE `id`='$write_off_id' LIMIT 1;");

                    $r3=$db->query("SELECT * FROM `J_WRITE_OFF_STORSEL` 
                    WHERE `write_off_id`='$write_off_id' AND `dp_id`='$dp_id' AND `status`='1' ORDER BY `id` ASC;"); $n3=$db->num_rows($r3);$select_str="0";
                    for ($i3=1;$i3<=$n3;$i3++){
...
//                            if ($cash_id==1){
//                                $price2_cash=round($price2*$usd_to_uah,2);          $price2_cash=$this->getClientPriceRounding($client_conto_id, $price2_cash);
//                                $price_end2_cash=round($price_end2*$usd_to_uah,2);  $price_end2_cash=$this->getClientPriceRounding($client_conto_id, $price_end2_cash);
...
//                            }
                            ...
                            $rsi=$db->query("SELECT MAX(`id`) as mid FROM `J_WRITE_OFF_STR`;");$write_off_str_id=0+$db->result($rsi,0,"mid")+1;
                            ...
                            $rr=$dbt->query("SELECT * FROM `T2_ARTICLES_STRORAGE` WHERE `ART_ID`='$art_id2' AND `STORAGE_ID`='$storage_id_from2' LIMIT 1;");$nr=$dbt->num_rows($rr);
                            if ($nr==1){
                            ...
                            $rr=$dbt->query("SELECT * FROM `T2_ARTICLES_STRORAGE_CELLS` WHERE `ART_ID`='$art_id2' AND `STORAGE_ID`='$storage_id_from2' AND `STORAGE_CELLS_ID`='$cell_id_from2' LIMIT 1;");$nr=$dbt->num_rows($rr);
                            if ($nr==1){
                                $rr_reserv=$dbt->result($rr,0,"RESERV_AMOUNT"); $rr_reserv-=$amount2;
        ...
        $rsel=$db->query("SELECT `prefix`, `doc_nom` FROM `J_WRITE_OFF` WHERE `id`='$write_off_nom' LIMIT 1;"); $nsel=$db->num_rows($rsel); $write_off_prefix=0;
        if ($nsel>0) {
                    ...
                    if ($client_saldo==0 || ($client_saldo>0 && $sale_storsell_summ<$client_saldo+$client_credit_limit) || ($client_saldo<0 && $sale_storsell_summ<$client_credit_limit-abs($client_saldo))) {
                        ...
                        if ($doc_type_id_check>0) {
                            ...
                            if ($n==1){
                                ...
                                `carrier_id`='$carrier_id', `delivery_address`='$delivery_address', `user_id`='$user_id' WHERE `id`='$invoice_id' LIMIT 1;");

                                $r3=$db->query("SELECT * FROM `J_SALE_INVOICE_STORSEL`
                                WHERE `invoice_id`='$invoice_id' AND `dp_id`='$dp_id' AND `status`='1' ORDER BY `id` ASC;"); $n3=$db->num_rows($r3);$select_str="0";
                                for ($i3=1;$i3<=$n3;$i3++){
                                    $select_str.=",".$db->result($r3,$i3-1,"select_id");
                                }
                                ...
                                WHERE jss.select_id IN ($select_str) AND jds.art_id=jss.art_id AND jds.status_dps!=97
                                GROUP BY jds.art_id, jss.storage_id_from, jss.cell_id_from, jds.reserv_type_id;"); $n2=$db->num_rows($r2);$summ_all=0;
                                ...
                                for ($i2=1;$i2<=$n2;$i2++){
                                    ...
                                    $rt=$db->query("SELECT `id` FROM `J_DP_STR` WHERE `dp_id`='$dp_id' AND `art_id`='$art_id2' $where_storage LIMIT 1;"); // по старому
...
//                                        }
        ...
        $rsel=$db->query("SELECT `prefix`, `doc_nom`, `doc_type_id` FROM `J_SALE_INVOICE` WHERE `id`='$sale_invoice_nom' LIMIT 1;"); $nsel=$db->num_rows($rsel); $sale_invoice_prefix=0; $sale_invoice_doc_type_id=0;
        if ($nsel>0) {
            $prefix=$db->result($rsel,0,"prefix");
            $doc_nom=$db->result($rsel,0,"doc_nom");
    ...
    function setWriteOffPartitions($write_off_id,$write_off_str_id,$art_id,$article_nr_displ,$brand_id,$amount_invoice,$price_invoice){$db=DbSingleton::getDb();
        $cat=new catalogue;
    ...
    function writeOffPartitions($invoice_id,$invoice_str_id,$art_id,$article_nr_displ,$brand_id,$amount_invoice,$price_invoice){$db=DbSingleton::getDb();
        $cat=new catalogue; //price_invoice in USD
        $r=$db->query("SELECT * FROM `T2_ARTICLES_PARTITIONS` WHERE `art_id`='$art_id' AND `rest`>0 AND `status`='1' ORDER BY `id` ASC;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
            $rest=$db->result($r,$i-1,"rest");
    ...
    function getSellerId($tpoint_id,$doc_type_id) { $db=DbSingleton::getDb();
        $r=$db->query("SELECT `client_id` FROM `T_POINT_CLIENTS` WHERE `tpoint_id`='$tpoint_id' AND `sale_type`='$doc_type_id' AND `in_use`='1' AND `status`='1' ORDER BY `id` ASC LIMIT 1;");
        $n=$db->num_rows($r);$seller_id=0;
        if ($n==1){
    ...
    function getSellerPrefixDocNom($seller_id,$doc_type_id){$db=DbSingleton::getDb();
        $sale_type=array(61=>86, 62=>87, 63=>87, 64=>88);$sale_type_id=$sale_type[$doc_type_id];$year_today=date("Y");$prefix="";
        $r=$db->query("SELECT `prefix` FROM `A_CLIENTS_DOCUMENT_PREFIX` WHERE `client_id`='$seller_id' AND `doc_type_id`='$sale_type_id' AND `status`='1' ORDER BY `id` ASC LIMIT 1;");$n=$db->num_rows($r);
    ...
    function updateClientBalans($client_conto_id,$cash_id,$summ){$db=DbSingleton::getDb();
        $r=$db->query("SELECT * FROM `B_CLIENT_BALANS` WHERE `client_id`='$client_conto_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==0){
    ...
    function viewDpSaleInvoice($dp_id,$invoice_id){$db=DbSingleton::getDb();
        $gmanual=new gmanual; $prefix=$doc_type_name=$status_invoice_cap="";$doc_nom=0;$volume=0;$list="";
        ...
        FROM `J_SALE_INVOICE` sv
            ...
            LEFT OUTER JOIN `A_CLIENTS` cl on cl.id=sv.client_conto_id
            LEFT OUTER JOIN `manual` dt on dt.key='client_sale_type' and dt.id=sv.doc_type_id
        WHERE sv.status=1 AND sv.dp_id='$dp_id' AND sv.id='$invoice_id' LIMIT 1;");$n=$db->num_rows($r);
    ...
    function printDpJournal($dp_id) { $db=DbSingleton::getDb();
        $money=new toMoney;$slave=new slave;$invoice_summ=0; $list="";
    ...
    function printDpSaleInvoice($dp_id) {$db=DbSingleton::getDb();
        ...
        $prefix=$db->result($r,0,"prefix");
        $doc_nom=$db->result($r,0,"doc_nom");
        $data_create=$db->result($r,0,"data_create"); $data_create = date("d.m.Y", strtotime($data_create));
        $client_name=$db->result($r,0,"client_name");
    ...
    function printSaleInvoice($invoice_id) { $db=DbSingleton::getDb();
        $slave=new slave; $money=new toMoney;
        $sel_ar=[]; $list=""; $form=""; $address_send=""; $volume=0; $invoice_summ=0;
        ...
        if ($n==1) {
            ...
            $cash_abr=$db->result($r,0,"cash_abr");
            $data_pay=$db->result($r,0,"data_pay");
            $dp_delivery_address=$db->result($r,0,"delivery_address");
            $dp_id=$db->result($r,0,"dp_id");
            $delivery_address = $this->getDPAddress($dp_id);
            if ($delivery_address=="") $delivery_address=$dp_delivery_address;
    ...
    function printWriteOff($write_off_id){$db=DbSingleton::getDb();
        $slave=new slave;$money=new toMoney;
        $list="";$sel_ar=[];$form="";$address_send="";$volume=0;$invoice_summ=0;
        $r=$db->query("SELECT sv.*, t.name as tpoint_name, sl.name as seller_name, sld.edrpou, ot.name as org_type_abr, 
        cl.name as client_name, dt.mcaption as doc_type_name, dt.mvalue as doc_type_abr, ch.abr2 as cash_abr 
        FROM `J_WRITE_OFF` sv
            LEFT OUTER JOIN `CASH` ch ON ch.id=sv.cash_id
            ...
            LEFT OUTER JOIN `A_CLIENTS` cl ON cl.id=sv.client_conto_id
            LEFT OUTER JOIN `manual` dt ON (dt.key='client_sale_type' AND dt.id=sv.doc_type_id)
        WHERE sv.status=1 AND sv.id='$write_off_id' LIMIT 1;");$n=$db->num_rows($r);
        ...
        if ($n==1){
            ...
            FROM `J_WRITE_OFF_STR` sis 
                ...
                LEFT OUTER JOIN J_SELECT_STR ss ON ss.select_id=iss.select_id
            WHERE sis.write_off_id='$write_off_id' GROUP BY sis.id ORDER BY sis.id ASC;");$n=$db->num_rows($r);
    ...
    function updateStockToStorage($art_id,$storage_id_to,$cell_id_to,$cell_use,$amount) { $dbt=DbSingleton::getTokoDb();
        ...
        if ($n==0){
            ...
            if ($cell_use==1){
                ...
                VALUES ('$art_id','$amount','0','$storage_id_to','$cell_id_to');");
            }
            $er=0;
        }
        if ($n==1){
    ...
    function updateStockFromStorageLocal($art_id,$storage_id_from,$cell_id_from,$cell_id_to,$amount) { $dbt=DbSingleton::getTokoDb();
        ...
        if ($n==1){
            ...
            if ($amount<=$t2s_reserv_amount){
                ...
                if ($n2==0){
                    $dbt->query("INSERT INTO `T2_ARTICLES_STRORAGE_CELLS` (`ART_ID`,`AMOUNT`,`RESERV_AMOUNT`,`STORAGE_ID`,`STORAGE_CELLS_ID`) 
                    VALUES ('$art_id','$amount','0','$storage_id_from','$cell_id_to');");
                }
                if ($n2==1){
    ...
    /*==== MONEY PAY ====*/
    function loadDpMoneyPay($dp_id) { $db=DbSingleton::getDb();
        $form="";$form_htm=RD."/tpl/dp_money_pay_list.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("SELECT `id` FROM `J_SALE_INVOICE` WHERE `status`=1 AND `dp_id`='$dp_id';");$n=$db->num_rows($r);$invoice_str="0";$list="";
        ...
        GROUP BY pay.id ORDER BY pay.data_time DESC, pay.id DESC;"); $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $id=$db->result($r,$i-1,"id");
    ...
    /*==== Orders From Site ====*/
    function countT2Requests() { $db=DbSingleton::getTokoDb();
        $r = $db->query("SELECT COUNT(`ID`) as kilk FROM `T2_QUESTIONS` WHERE `STATUS`=1;");
        $count = $db->result($r, 0, "kilk");
        if ($count>0) $back = "style='background: red;'"; else $back = "";
    ...
    function countReportOverdrafts() { $db=DbSingleton::getDb();
        session_start();$media_role_id=$_SESSION["media_role_id"];
        $date_cur=date("Y-m-d"); $where=" AND `data_pay`<'$date_cur'"; $tpoint=$this->getTpointbyUser();
        $where_tpoint=" AND `tpoint_id`=$tpoint "; if ($media_role_id=="1") $where_tpoint="";
        $r=$db->query("SELECT * FROM `J_SALE_INVOICE` WHERE `status`=1 AND `summ_debit`>0 $where $where_tpoint;"); $count=$db->num_rows($r);
        if ($count>0) $back="style='background: #f8ac59;'"; else $back="";
        return array($count, $back);
    }

    function getTpointbyUser() { $db=DbSingleton::getDb();
        $tpoint=1;session_start();$user_id=$_SESSION["media_user_id"];
        $r=$db->query("SELECT `tpoint_id` FROM `media_users` WHERE `id`='$user_id';"); $n=$db->num_rows($r);
        if ($n>0) $tpoint=$db->result($r,0,"tpoint_id");
        return $tpoint;
    }

    function countOrdersSite() { $db=DbSingleton::getDb();
    ...
    function countUsersSite() { $db=DbSingleton::getDb();
        ...
        if ($kilk_mag>0) $back="style='background: red;'"; else $back="";
        return array($count,$back);
    }
    ...
    function showOrderSiteRange($press, $data_start, $data_end) { $db=DbSingleton::getDb();
        session_start();$user_id=$_SESSION["media_user_id"];$ses_tpoint_id=$_SESSION["media_tpoint_id"];$data_cur=date("Y-m-d");
        $where=" AND o.tpoint_id='$ses_tpoint_id'";if ($user_id==1 || $user_id==2 || $user_id==7){$where="";} $where_status=""; $list="";
        if (!$press) $where_status="AND o.status=1 AND o.dp_id=''";
            else if ($data_start!="" && $data_end!="") $where="AND o.data>='$data_start 00:00:00' AND o.data<='$data_end 23:59:59'";
        ...
        if ($n>0) {
            for ($i=1;$i<=$n;$i++) {
                ...
                $tpoint_name=$db->result($r,$i-1,"tpoint_name");
                $cash_id=$db->result($r,$i-1,"cash_id");$cash_abr=$db->result($r,$i-1,"cash_abr");
                $name=$db->result($r,$i-1,"name");
                $email=$db->result($r,$i-1,"email");
                $phone=$db->result($r,$i-1,"phone");
    ...
    function showOrdersSiteCard($order_id) { $db=DbSingleton::getDb();
        ...
        if ($n==0){$form="<h2 align='center'>Замовлення відсутнє або передано в роботу</h2>";}
        if ($n==1){
            $id=$db->result($r,0,"id");
            $data=$db->result($r,0,"data");
            ...
            for ($i=1;$i<=$n1;$i++){
                ...
                if ($status_action>0) $action_cap="<i class='fa fa-box-open' title='Акційна ціна'></i>"; else $action_cap="";
                $list.="<tr>
                    <td>$i</td>
                    <td>$art_id</td>
    ...
    function getDPAddress($dp_id) { $db = DbSingleton::getDb(); $dbt = DbSingleton::getTokoDb();
        $dp = new dp;
        $text = "";
        $r = $db->query("SELECT * FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;"); $n = $db->num_rows($r);
        if ($n>0) {
            ...
            $department = $db->result($r, 0, "DEL_DEPARTMENT_TEXT");
            $express = $db->result($r, 0, "DEL_EXPRESS");
            $express_info = $db->result($r, 0, "DEL_EXPRESS_INFO");

            if ($delivery_id>0) {
                $r2 = $dbt->query("SELECT * FROM `T2_DELIVERY` WHERE `ID`='$delivery_id' LIMIT 1;");
                $text = $dbt->result($r2, 0, "DESCRIPTION");
            }

            // kurer
            if ($delivery_id==2) {
                $text.=" - вул. $street, буд. $house, п. $porch";
            }
            // kurer STO
            if ($delivery_id==3) {
                $text.=" - вул. $street, буд. $house";
            }
            // NP
            if ($delivery_id==4) {
                $text.=" - $department";
            ...
            // Express
            if ($delivery_id==7) {
                $express_name = $dp->getExpressInfoName($express);
    ...
    function getOrderInfoForm($form, $order_info_id) { $db = DbSingleton::getDb();
        $client = new clients;

        $r = $db->query("SELECT * FROM `ORDERS_CLIENT_INFO` WHERE `ID`='$order_info_id' LIMIT 1;");
        $city_id = $db->result($r, 0, "CITY_ID"); $city_name = $client->getCityName($city_id);
        $delivery_id = $db->result($r, 0, "DELIVERY_ID"); $delivery_text = $client->getDeliveryText($delivery_id);
        $payment_id = $db->result($r, 0, "PAYMENT_ID"); $payment_text = $client->getPaymentText($payment_id);
        $street = $db->result($r, 0, "DEL_STREET");
        $house = $db->result($r, 0, "DEL_HOUSE");
        $porch = $db->result($r, 0, "DEL_PORCH");
        $department_text = $db->result($r, 0, "DEL_DEPARTMENT_TEXT");
        $express = $db->result($r, 0, "DEL_EXPRESS"); $express_text = $client->getExpressText($express);
        $express_info = $db->result($r, 0, "DEL_EXPRESS_INFO");

        $user_id = $db->result($r, 0, "USER_ID"); $user_name = $client->getUserNameById($user_id);
        $recipient_name = $db->result($r, 0, "DEL_NAME");
        $recipient_phone = $db->result($r, 0, "DEL_PHONE");

        $form = str_replace("{order_user}", $user_name, $form);
        $form = str_replace("{order_del_name}", $recipient_name, $form);
        $form = str_replace("{order_del_phone}", $recipient_phone, $form);

        $form = str_replace("{order_city}", $city_name, $form);
        $form = str_replace("{order_delivery}", $delivery_text, $form);
        $form = str_replace("{order_payment}", $payment_text, $form);

        $form = str_replace("{order_del_street}", $street, $form);
        $form = str_replace("{order_del_house}", $house, $form);
        $form = str_replace("{order_del_porch}", $porch, $form);

        $form = str_replace("{order_del_department}", $department_text, $form);
        $form = str_replace("{order_del_express}", $express_text, $form);
        $form = str_replace("{order_del_express_info}", $express_info, $form);

        return $form;
    }

    function getOrderDeliveryInfo($order_info_id) { $db = DbSingleton::getDb(); $dbt = DbSingleton::getTokoDb();
        $text = "";
        $r = $db->query("SELECT * FROM `ORDERS_CLIENT_INFO` WHERE `ID`='$order_info_id' LIMIT 1;");
        $delivery_id = $db->result($r, 0, "DELIVERY_ID");
        if ($delivery_id>0) {
        ...
        if ($delivery_id==7) {
            $express = $db->result($r, 0, "DEL_EXPRESS"); $express_name = $this->getExpressInfoName($express);
            $express_info = $db->result($r, 0, "DEL_EXPRESS_INFO");
            $text.=" - $express_name: $express_info";
        }
        return $text;
    ...
    function getExpressInfoName($express) { $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT * FROM `T2_DELIVERY_EXPRESS` WHERE `ID`='$express' LIMIT 1;");
        $express_name = $db->result($r, 0, "DESCRIPTION");
        return $express_name;
    }

    function getOrderInfoBlock($form, $order_info_id, $client_conto_id) { $db = DbSingleton::getDb();
        $client = new clients;
        ...
        if ($order_info_id==0) $client_id = $client_conto_id;
        $user_list = $client->getUserList($client_id, $user_id);
        $city_list = $client->getCityList($city_id);
        $delivery_list = $client->getDeliveryList($delivery_id);
        $payment_list = $client->getPaymentList($payment_id);
        $department_list = $client->getDepartmentList($department, $department_text);
        $express_list = $client->getExpressList($express);

        $form = str_replace("{order_info_id}", $order_info_id, $form);
        $form = str_replace("{order_user_list}", $user_list, $form);
        $form = str_replace("{order_city_list}", $city_list, $form);
        $form = str_replace("{order_delivery_list}", $delivery_list, $form);
        $form = str_replace("{order_payment_list}", $payment_list, $form);

        $form = str_replace("{order_del_street}", $street, $form);
        $form = str_replace("{order_del_house}", $house, $form);
        $form = str_replace("{order_del_porch}", $porch, $form);

        $form = str_replace("{department_ref}", $department, $form);
        $form = str_replace("{order_del_department_list}", $department_list, $form);
        $form = str_replace("{order_del_express_list}", $express_list, $form);
        $form = str_replace("{order_del_express_info}", $express_info, $form);
    ...
    function saveDpOrderInfo($dp_id, $order_info_id, $client_id, $user_id, $city_id, $delivery_id, $payment_id, $street, $house, $porch, $department, $department_text, $express, $express_info) { $db = DbSingleton::getDb();
        $r = $db->query("SELECT * FROM `ORDERS_CLIENT_INFO` WHERE `ID`='$order_info_id' LIMIT 1;"); $n = $db->num_rows($r);
        if ($n==0) {
            $r2 = $db->query("SELECT MAX(`ID`) as mid FROM `ORDERS_CLIENT_INFO`;"); $max = 0 + $db->result($r2, 0, "mid") + 1;
            ...
            VALUES ($max, $client_id, $user_id, $city_id, $delivery_id, $payment_id, '$street', '$house', '$porch', '$department', '$department_text', $express, '$express_info');");
            $db->query("UPDATE `J_DP` SET `order_info_id`=$max WHERE `id`=$dp_id LIMIT 1;");
            $answer = "Додано нову інформацію про доставку!";
        } else {
            $db->query("UPDATE `ORDERS_CLIENT_INFO` SET `CITY_ID`=$city_id, `DELIVERY_ID`=$delivery_id, `PAYMENT_ID`=$payment_id, `DEL_STREET`='$street', `DEL_HOUSE`='$house', `DEL_PORCH`='$porch', `DEL_DEPARTMENT`='$department', `DEL_DEPARTMENT_TEXT`='$department_text', `DEL_EXPRESS`=$express, `DEL_EXPRESS_INFO`='$express_info' WHERE `ID`='$order_info_id' LIMIT 1;");
    ...
    function getNovaPoshtaWarehousesSelect($ref, $department_ref) {
        ...
        foreach ($arr as $val) {
            ...
            $war_ref = $val["Ref"];
            if ($war_ref==$department_ref) $sel="selected"; else $sel="";
            $list.="<option value='$war_ref' $sel>$name</option>";
        }
        return $list;
    }

    function deleteOrderSite($order_id) { $db=DbSingleton::getDb();
    ...
    function createDpFromOrder($order_id) { $db=DbSingleton::getDb();
        $slave = new slave;
        $answer = 0; $err = "Помилка!"; $dp_id = 0;
        ...
        if ($order_id>0) {
            ...
            FROM `orders_new` o
                ...
                LEFT OUTER JOIN `CASH` csh ON (csh.id=o.cash_id)
                LEFT OUTER JOIN `manual` dt ON (dt.id=o.delivery AND dt.`key`='delivery_type')
            WHERE o.status=1 AND o.id='$order_id' LIMIT 1;"); $n = $db->num_rows($r);
            if ($n==1) {
                $client_id=$db->result($r,0,"client_id");
                $tpoint_id=$db->result($r,0,"tpoint_id");
                $cash_id=$db->result($r,0,"cash_id");
                ...
                if ($dp_id>0) {
                    ...
                    if ($delivery_info!="" && $dp_note=="") $this->setDpNote($dp_id, $del_note);
                    ...
                    for ($i=1; $i<=$n1; $i++) {
                        ...
                        $return_delay=$db->result($r1,$i-1,"return_delay");
                        ...
                        $dp_str_id=0; $ans=0;
                        if ($suppl_id==0) {
                            list($ans,,$dp_str_id) = $this->setArticleToDp($dp_id,$tpoint_id,$art_id,$article_nr_displ,$brand_id,$storage_id,$amount,$status_action,$order_price,$return_delay);
                        }
                        if ($suppl_id!=0) {
                            list($ans,,$dp_str_id) = $this->setArticleSupplToDp($dp_id,$art_id,$article_nr_displ,$brand_id,$suppl_id,$storage_id,$amount,$status_action,$order_price,$return_delay);
                        }
    ...
    function getDpArticleAmount($str_id, $art_id) { $db=DbSingleton::getDb();
        $r=$db->query("SELECT `amount` FROM `J_DP_STR` WHERE `id`='$str_id' AND `art_id`='$art_id';"); $n=$db->num_rows($r); $amount=0;
        if ($n==1){$amount=$db->result($r,0,"amount");}
        return $amount;
    }
    ...
    function loadDpSiteOrder($dp_id) { $db=DbSingleton::getDb();
        ...
        if ($dp_id==0){$form="<h2 align='center'>Замовлення відсутнє або ще не передано в роботу</h2>";}
        if ($dp_id>0){
            ...
            FROM `orders_new` o
                ...
                LEFT OUTER JOIN `CASH` csh ON (csh.id=o.cash_id)
                LEFT OUTER JOIN `manual` dt ON (dt.id=o.delivery AND dt.`key`='delivery_type')
            WHERE o.status=1 AND o.dp_id='$dp_id' LIMIT 1;");$n=$db->num_rows($r);

            if ($n==0){$form="<h2 align='center'>Замовлення відсутнє або ще не передано в роботу</h2>";}
            if ($n==1){
    ...
    function getDistinctOrdersItems($dp_id) { $db=DbSingleton::getDb();
        $slave=new slave; $amount=""; $dp_id=$slave->qq($dp_id);
        if ($dp_id>0){
    ...
    function showSupplToLocalChangeForm($art_id, $dp_id, $dp_str_id) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        $slave=new slave;$art_id=$slave->qq($art_id);$dp_id=$slave->qq($dp_id);
    ...
    function saveDpSupplToLocalChangeForm($dp_id,$dp_str_id,$art_id,$amount,$price,$storage_id) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        $slave=new slave; $answer=0; $err="Помилка!"; $rr_amount=$rr_reserv=0;
        ...
        if ($art_id>0 && $amount>0 && $dp_str_id>0 && $storage_id>0){
            ...
            $r1=$dbt->query("SELECT `AMOUNT`, `RESERV_AMOUNT` FROM `T2_ARTICLES_STRORAGE` WHERE `ART_ID`='$art_id' AND `STORAGE_ID`='$storage_id' AND `AMOUNT`>='$amount' LIMIT 1;"); $n1=$dbt->num_rows($r1); $stor_ex=0;print "n1=$n1\n;";
            if ($n1==1){
    ...
    function loadDpImport($dp_id) {
        $form="";$form_htm=RD."/tpl/dp_import_str_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $header="";$header_htm=RD."/tpl/dp_import_str_header.htm";if (file_exists("$header_htm")){ $header = file_get_contents($header_htm);}
        list(, $csv_file_name, $pre_table)=$this->showCsvPreview($dp_id);
    ...
    function getCsvIndexBrands($dp_id) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        $brands=[]; $list="";
        $r=$db->query("SELECT * FROM `J_DP_IMPORT` WHERE `dp_id`='$dp_id' AND `status`=0;"); $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
            $article_nr_displ = $db->result($r, $i - 1, "article_nr_displ");
            $r2=$dbt->query("SELECT * FROM `T2_ARTICLES` WHERE `ARTICLE_NR_DISPL`='$article_nr_displ' OR `ARTICLE_NR_SEARCH`='$article_nr_displ';"); $n2=$dbt->num_rows($r2);
            for ($j=1;$j<=$n2;$j++) {
    ...
    function showTablePreview($dp_id) {
        $form="";$form_htm=RD."/tpl/csv_str_dp_import.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $form=str_replace("{dp_id}",$dp_id,$form);
    ...
    function getDpStockInfo($art_id) { $db=DbSingleton::getTokoDb();
        ...
        if ($art_id>0) {
            ...
            for ($i=1;$i<=$n;$i++) {
                ...
                if ($amount>0) $list.="<span style='font-weight: bold;'>$storage_name</span>($amount); ";
                $full_amount+=$amount;
            }
        }
        $list=trim($list,"; ");
    ...
    function moveArticlesStorage($dp_id, $art_id, $article_nr_displ, $brand_id, $amount, $storages) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        $storage_str=[]; $result=false;
        usort($storages,"cmpStorages");
        foreach ($storages as $value) { array_push($storage_str,$value["storage_id"]); }
        $storage_str=implode(",",$storage_str); if ($storage_str!="") $where_storages="AND `STORAGE_ID` IN ($storage_str)"; else $where_storages="";
        ...
        $r2=$db->query("SELECT * FROM `J_DP` WHERE `id`='$dp_id' LIMIT 1;"); $tpoint_id=$db->result($r2,0,"tpoint_id");
        ...
        if ($summ<$amount && $summ>0) {
            ...
            $amount=$summ;
            $r2=$db->query("SELECT MAX(`id`) as mid FROM `J_DP_STR_UNKNOWN`;");$unknown_id=0+$db->result($r2,0,"mid")+1;
        ...
        if ($summ>0) {
            if ($article_price>0) {
                ...
                foreach ($storages as $val) {
                    ...
                    if ($cur_amount>0) {
                        if ($cur_amount<=$full_amount) $cut_amount=$cur_amount; else $cut_amount=$full_amount;

                        $price_end=round($article_price,2);
                        $summ=round($price_end*$cut_amount,2);
                        $reserv_type_id=$this->getArticleReservType($tpoint_id,$storage_id);

                        $rs=$db->query("SELECT * FROM `J_DP_STR` WHERE `dp_id`='$dp_id' AND `art_id`='$art_id' AND `storage_id_from`='$storage_id' AND `status_dps`=93;"); $ns=$db->num_rows($rs);
                        if ($ns>0) {
    ...
    function loadTablePreview($dp_id, $brands=0) { $db=DbSingleton::getDb(); $dbt=DbSingleton::getTokoDb();
        ...
        if ($brands!=0) {$arts=[];
            ...
            $arts=array_unique($arts); $arts=implode(",",$arts);

            if ($arts!="") {$new_arts=[];
                $r=$dbt->query("SELECT * FROM `T2_CROSS` WHERE (`DISPLAY_NR` IN ($arts) OR `SEARCH_NUMBER` IN ($arts)) AND `BRAND_ID`='$brands';");$n=$db->num_rows($r);
                for ($i=1;$i<=$n;$i++) {
        ...
        $r=$db->query("SELECT * FROM `J_DP_IMPORT` WHERE `dp_id`='$dp_id' $where_arts ORDER BY `status` ASC, `id` ASC;"); $n=$db->num_rows($r); $mas=[];
        for ($i=1;$i<=$n;$i++) {
            $id = $db->result($r, $i - 1, "id");
            $article_nr_displ = $db->result($r, $i - 1, "article_nr_displ");
            $amount = $db->result($r, $i - 1, "amount");
        ...
        usort($mas,"sortStyle"); $i=0;
        foreach ($mas as $val) {$i++;
            $style=$val["style"];$st="";
            ...
            $amount=$val["amount"];
            $art_id=$val["art_id"]; $art_id!=0 ?: $art_id="Невідомо";
            $brand_id=$val["brand_id"];
            $brand_name=$val["brand_name"]; $brand_name!="" ?: $brand_name="Не визначено";
            $storage_info=$val["storage_info"]; $storage_info!="" ?: $storage_info="Пусто";
            $table.="<tr $st>
                <td>$i</td>
                <td>$article_nr_displ</td>
                <td>$amount</td>
            ...
            if ($brands!=0) $db->query("UPDATE `J_DP_IMPORT` SET `selected`=1 WHERE `id`='$id';");
        }
        return $table;
    }
    ...
    function showStorageFieldsTitle() { $db=DbSingleton::getDb();
        ...
        $r=$db->query("SELECT * FROM `CFN_USERS_STORAGE_CONFIG` WHERE `user_id`=$user_id;"); $n=$db->num_rows($r); if ($n==0) $user_id=0;
        $r=$db->query("SELECT * FROM `CFN_USERS_STORAGE_CONFIG` WHERE `user_id`=$user_id AND `field_active`=1 ORDER BY `field_pos` ASC;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $storage_id=$db->result($r,$i-1,"storage_id");
            $storage_name=$this->getStorageName($storage_id);
            $list.="$i - $storage_name \n";
    ...
    function showStorageFieldsViewForm() { $db=DbSingleton::getDb();
        session_start();$user_id=$_SESSION["media_user_id"];
        $form="";$form_htm=RD."/tpl/storage_fields_view_form.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $r=$db->query("SELECT * FROM `CFN_USERS_STORAGE_CONFIG` WHERE `user_id`=$user_id;"); $n=$db->num_rows($r); if ($n==0) $user_id=0;
        $r=$db->query("SELECT * FROM `CFN_USERS_STORAGE_CONFIG` WHERE `user_id`='$user_id' ORDER BY `id` ASC;");$n=$db->num_rows($r);$list="";$lst=array();
        for ($i=1;$i<=$n;$i++){
            $storage_id=$db->result($r,$i-1,"storage_id");
            $field_active=$db->result($r,$i-1,"field_active");if ($field_active==0) $checked=""; else $checked="checked";
    ...
    function saveTablePreview($dp_id, $brands=0) { $db=DbSingleton::getDb();
        $answer=0;$err="Помилка збереження даних!";
        if ($dp_id>0 && $brands!=0) {
            $r=$db->query("SELECT * FROM `J_DP_IMPORT` WHERE `dp_id`='$dp_id' AND `status`=0 AND `selected`=1;");$n=$db->num_rows($r);
            for ($i=1;$i<=$n;$i++) {
                $id = $db->result($r, $i - 1, "id");
                $article_nr_displ = $db->result($r, $i - 1, "article_nr_displ");
                $brand_id=$brands;
                $art_id=$this->getArtId($article_nr_displ,$brand_id);
                if ($art_id!=0 && $brand_id!=0) $db->query("UPDATE `J_DP_IMPORT` SET `brand_id`='$brand_id', `art_id`='$art_id', `status`=1 WHERE `id`='$id';");
            }
            $answer=1;$err="";
        }
        return array($answer,$err);
    ...
    function showCsvPreview($dp_id) { $db=DbSingleton::getDb();
        $csv_exist=0;$csv_file_name="Оберіть файл";$pre_table="<h3>Файл не знайдено</h3>";$kol_cols=0;$fn=0;
        $r=$db->query("SELECT * FROM `J_DP_CSV` WHERE `dp_id`='$dp_id' LIMIT 1;");$n=$db->num_rows($r);
        if ($n==1){
            $file_name=$db->result($r,0,"file_name");
            $file_path=RD."/cdn/dp_files/csv/$dp_id/$file_name";
            if (file_exists($file_path)){
                $form="";$form_htm=RD."/tpl/csv_str_dp_file.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
    ...
    function saveCsvDpImport($dp_id, $start_row, $kol_cols, $cols) { $db=DbSingleton::getDb();
        $slave=new slave; $answer=0; $err="Помилка збереження даних!"; $fn=0;
                        ...
                        while (($buffer = fgets($handle, 4096)) !== false) {$fn+=1;
                            ...
                            if ($buffer!=""){
                                if ($fn>=$start_row){
                                    ...
                                    $r2=$db->query("SELECT MAX(`id`) as mid FROM `J_DP_IMPORT`;");$import_id=0+$db->result($r2,0,"mid")+1;
                                    $rs=$db->query("SELECT * FROM `J_DP_IMPORT` WHERE `dp_id`='$dp_id' AND `art_id`='$art_id' LIMIT 1;"); $ns=$db->num_rows($rs);
                                    if ($ns>0 && $art_id>0) {
                                        $db->query("UPDATE `J_DP_IMPORT` SET `amount`=`amount`+$amnt WHERE `dp_id`='$dp_id' AND `art_id`='$art_id' LIMIT 1;");
                                    } else {
                                        $db->query("INSERT INTO `J_DP_IMPORT` (`id`,`dp_id`,`art_id`,`article_nr_displ`,`brand_id`,`amount`,`status`) 
                                        VALUES ('$import_id','$dp_id','$art_id','$ind','$brand_id','$amnt','$status');");
                                    }
                                }
                            }
                        }
                        fclose($handle);
                        if (file_exists(RD."/cdn/dp_files/csv/$dp_id/$file_name")){unlink(RD."/cdn/dp_files/csv/$dp_id/$file_name");}
    ...
    function getUserStorages() { $db=DbSingleton::getDb();
        session_start(); $user_id=$_SESSION["media_user_id"]; $storages=[];
        $r=$db->query("SELECT * FROM `CFN_USERS_STORAGE_CONFIG` WHERE `user_id`=$user_id;"); $n=$db->num_rows($r); if ($n==0) $user_id=0;
        $r=$db->query("SELECT * FROM `CFN_USERS_STORAGE_CONFIG` WHERE `user_id`=$user_id AND `field_active`=1 ORDER BY `field_pos` ASC;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $storage_id=$db->result($r,$i-1,"storage_id");
    ...
    function finishDpImport($dp_id) { $db=DbSingleton::getDb();
        $slave=new slave;$answer=0;$err="Помилка збереження даних!";$dp_id=$slave->qq($dp_id);
        if ($dp_id>0){
            $db->query("DELETE FROM `J_DP_STR_UNKNOWN` WHERE `dp_id`='$dp_id';");
            $r=$db->query("SELECT * FROM `J_DP_IMPORT` WHERE `dp_id`='$dp_id';");$n=$db->num_rows($r);
            for ($i=1;$i<=$n;$i++) {
                $id = $db->result($r, $i - 1, "id");
                $article_nr_displ = $db->result($r, $i - 1, "article_nr_displ");
                $amount = $db->result($r, $i - 1, "amount");
                ...
                if ($status) {
                    ...
                    $this->moveArticlesStorage($dp_id,$art_id,$article_nr_displ,$brand_id,$amount,$storages);
                } else {
                    $r2=$db->query("SELECT MAX(`id`) as mid FROM `J_DP_STR_UNKNOWN`;");$unknown_id=0+$db->result($r2,0,"mid")+1;
                    $db->query("INSERT INTO `J_DP_STR_UNKNOWN` (`id`,`dp_id`,`art_id`,`article_nr_displ`,`brand_id`,`amount`,`bug_amount`,`caption`)
                    VALUES ('$unknown_id','$dp_id','$art_id','$article_nr_displ','$brand_id','0','$amount','Не було ART_ID чи BRAND_ID');");
                }
    ...
    function showDpUnknownStrList($dp_id) { $db=DbSingleton::getDb();
        $empty_kol=0; $list="";
        $r=$db->query("SELECT * FROM `J_DP_STR_UNKNOWN` WHERE `dp_id`='$dp_id' ORDER BY `id` ASC;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $art_id=$db->result($r,$i-1,"art_id");
            $article_nr_displ=$db->result($r,$i-1,"article_nr_displ");
            $brand_id=$db->result($r,$i-1,"brand_id");$brand_name=$this->getBrandName($brand_id);
    ...
    function exportDpCard($dp_id, $type_id) { $db=DbSingleton::getDb();
        ...
        for ($i=1;$i<=$kl_rw;$i++){
            ...
            if ($type_id==2) $price=str_replace(".",",","$price");
            if ($type_id==2) $summ=str_replace(".",",","$summ");
            $list[$i]=array($article_nr_displ, $brand_name, $text, $amount, $price, $summ);
        }
        return $list;
    }
