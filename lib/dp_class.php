...
class dp {
    ...
    function show_dp_list_filter($status,$filstatus,$filauthor,$filtpoint){$db=DbSingleton::getDb();$gmanual=new gmanual;session_start();$ses_tpoint_id=$_SESSION["media_tpoint_id"];$media_user_id=$_SESSION["media_user_id"];$media_role_id=$_SESSION["media_role_id"];
        ...
        if (!$status) $where_status=" and j.status_dp!=81 "; else $where_status="";
        ...
        for ($i=1;$i<=$n;$i++){
            ...
            $user_name=$this->getMediaUserName($db->result($r,$i-1,"user_id"));
            $status_dp=$db->result($r,$i-1,"status_dp");
            if ($status_dp!=80){$status_dp_name=$gmanual->get_gmanual_caption($status_dp);}
            else {$status_dp_name=$this->getDpStrStatus($id);}
    ...
    function showDpCard($dp_id){$db=DbSingleton::getDb();$gmanual=new gmanual;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$client=new clients;
        $form="";$form_htm=RD."/tpl/dp_card.htm";if (file_exists("$form_htm")){ $form = file_get_contents($form_htm);}
        $prefix=""; $doc_nom=0; $comment=""; $cells_show=""; $type_name=""; $label_art_unknown="";
        $r=$db->query("select j.* from J_DP j where j.id='$dp_id' limit 0,1;");$n=$db->num_rows($r);
    ...
    function showDpTpointList($sel_id){$db=DbSingleton::getDb();
        ...
        $r=$db->query("select t.*, t2cn.COUNTRY_NAME, t2st.STATE_NAME, t2rg.REGION_NAME, t2ct.CITY_NAME from T_POINT t 
    ...
    function getClientArticleMaxDiscount($art_id,$client_id,$price){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();session_start();$max_manager_discount=$_SESSION["user_discount"];
    $max_discount_persent=0;$max_discount_price=0;
        ...
        where t2aps.ART_ID='$art_id' and t2apr.in_use='1' limit 0,1;");$n=$dbt->num_rows($r);
        if ($n==1){
    ...
    function getDpStrStatus($dp_id){$db=DbSingleton::getDb();$status_name="";
        $r=$db->query("select j.*, dps.mcaption as status_dps_name from J_DP_STR j 
    ...
    function setArticleToDp($dp_id,$tpoint_id,$artIdStr,$article_nr_displStr,$brandIdStr,$storageIdStr,$amountStr){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;
        ...
        if ($dp_id>0){
            ...
            where dp_id='$dp_id' and art_id='$artIdS' and `storage_id_from`='$storageIdS' and status_dps='93' limit 0,1;");$n=$db->num_rows($r);
            if ($n==1){
    ...
    function setArticleSupplToDp($dp_id,$tpoint_id,$artIdStr,$article_nr_displStr,$brandIdStr,$supplIdStr,$supplStorageIdStr,$amountStr){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
        ...
        if ($dp_id>0){
            ...
            $amountEx=0;
            $r=$db->query("select id, amount, discount from J_DP_STR 
            where dp_id='$dp_id' and art_id='$artIdS' and `suppl_id`='$supplIdS' and `suppl_storage_id`='$supplStorageIdS' and status_dps='93' limit 0,1;");$n=$db->num_rows($r);
            if ($n==1){
                $idS=$db->result($r,0,"id");
                $amountEx=$db->result($r,0,"amount");
                $discountEx=$db->result($r,0,"discount");
...
//    }
    ...
    function showArticlesSearchDocumentList($art,$brand_id,$search_type,$dp_id,$tpoint_id){$db=DbSingleton::getTokoDb();$cat=new catalogue;$r="";$query="";
...
//                    $dp_amount=$this->getArticleInDp($art_id,$dp_id);
    ...
    function getTpointSupplFm($tpoint_id,$suppl_id,$suppl_storage_id,$price_suppl,$price_suppl_lvl){$dbt=DbSingleton::getTokoDb();$margin=0;$delivery=0;$margin2=0;
        ...
        where tpoint_id='$tpoint_id' and suppl_id='$suppl_id' and suppl_storage_id='$suppl_storage_id' and price_from<='$price_suppl' and price_to>='$price_suppl' and price_rating_id='$price_suppl_lvl' limit 0,1;";
        $r=$dbt->query($query);$n=$dbt->num_rows($r);
        if ($n==1){
    ...
    function getArticleStorageAmountDp($art_id,$dp_id,$storage_id){$db=DbSingleton::getDb();$amount=0;
        ...
        where dp_id='$dp_id' and status_dps='93' and storage_id_from='$storage_id' and art_id='$art_id' limit 0,1");$n=$db->num_rows($r);
    ...
    function getArticleSupplStorageAmountDp($art_id,$dp_id,$suppl_id,$suppl_storage_id){$db=DbSingleton::getDb();$amount=0;
        $r=$db->query("select amount from J_DP_STR 
        where dp_id='$dp_id' and status_dps='93' and suppl_id='$suppl_id' and suppl_storage_id='$suppl_storage_id' and art_id='$art_id' limit 0,1");$n=$db->num_rows($r);
        if ($n==1){ $amount=$db->result($r,0,"amount"); }
        return $amount;
    }
    ...
    function getArticleReservType($tpoint_id,$storage_id){$db=DbSingleton::getTokoDb();$reserv_type_id=68;
        $r=$db->query("select * from T_POINT_STORAGE 
        where `tpoint_id`='$tpoint_id' and status='1' and `storage_id`='$storage_id' limit 0,1;");$n=$db->num_rows($r);
        if ($n==1){
    ...
    function saveDpComment($dp_id,$comment){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="Помилка збереження даних!";
...
//    }
    ...
    function checkRemoteStorage($dp_id,$tpoint_id){$db=DbSingleton::getDb();$suppl_ex=0;
        $r=$db->query("select COUNT(ds.id) as kol from J_DP_STR ds 
    ...
    function makeDpJmovingStorselPreorder($dp_id,$local){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$jmoving=new jmoving;session_start();$user_id=$_SESSION["media_user_id"];
        ...
        $r=$db->query("select ds.storage_id_from from J_DP_STR ds where ds.dp_id='$dp_id' and ds.status_dps='93' group by ds.storage_id_from;");$n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++){
            $storage_id_from=$db->result($r,$i-1,"storage_id_from");
            if ($storage_id_from>0){
                $storage_local=$this->getStorageToTpointLocal($tpoint_id,$storage_id_from);
                if ($storage_local==41){
                    $r1=$db->query("select ds.* from J_DP_STR ds where ds.dp_id='$dp_id' and ds.status_dps='93' and ds.storage_id_from='$storage_id_from';");$n1=$db->num_rows($r1);
                    if ($n1>0){//create storsel;
                ...
                if ($storage_local==42 && $local==42){
                    ...
                    if ($n1>0){
                        ...
                        $r2=$db->query("select ds.* from J_DP_STR ds where ds.dp_id='$storage_dp_id' and ds.status_dps='93' and ds.storage_id_from='$storage_id_from';");$n2=$db->num_rows($r2);
                        if ($n2>0){
            ...
            if($storage_id_from==0){ $dp_cash_id=$this->getDpCashId($dp_id);
                $r1=$db->query("select ds.* from J_DP_STR ds where ds.dp_id='$dp_id' and ds.status_dps='93' and ds.storage_id_from='$storage_id_from' and ds.suppl_id>0;");$n1=$db->num_rows($r1);
                if ($n1>0){
                    for ($i1=1;$i1<=$n1;$i1++){
                        $id=$db->result($r1,$i1-1,"id");
    ...
    function showDpStorselForSaleInvoice($dp_id){$db=DbSingleton::getDb();$gmanual=new gmanual;$list="";$loc_type_name="";
        ...
        $r=$db->query("select sel.*, s.name as storage_name, t.name as tpoint_name from J_SELECT sel
            left outer join T_POINT t on t.id=sel.tpoint_id
            left outer join STORAGE s on s.id=sel.storage_id
        where sel.status=1 and sel.parrent_doc_type_id='2' and sel.parrent_doc_id='$dp_id' and status_select='85' and sel.id not in ($sel_use) 
...
//        $n=$db->num_rows($r);$sel_use="0";
    ...
    function sendDpStorselToSaleInvoice($dp_id,$kol_storsel,$ar_storsel){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();session_start();$user_id=$_SESSION["media_user_id"];
        $slave=new slave;$answer=0;$err="Помилка індексу!";$sale_invoice_nom=0;$dp_id=$slave->qq($dp_id);$kol_storsel=$slave->qq($kol_storsel);$price2_cash=$price_end2_cash=$discount2_cash=$summ2_cash=0;
        if ($dp_id>0){
            if ($kol_storsel>0){
                            ...
                            if ($n==1){
                                ...
                                for ($i2=1;$i2<=$n2;$i2++){
                                    ...
                                    if ($amount2>0){
                                        ...
                                        if ($tax_id>0){
                                            $zed=$this->getArticleZED($art_id2);$art_name=$this->getArticleNameLang($art_id2);
...
//            if($eur_to_uah!=$eur_to_uah_new){$eur_to_uah=$eur_to_uah_new;}
    ...
    function getClientInfo($client_id) {$db=DbSingleton::getDb();
        $r=$db->query("select sl.full_name as seller_name, sl.phone, sld.edrpou, sld.account, sld.bank, sld.mfo, sld.vytjag, sld.address_fakt as seller_address from A_CLIENTS sl 
    ...
    function printDpJournal($dp_id) {$db=DbSingleton::getDb();$money=new toMoney;$slave=new slave;$invoice_summ=0; $list="";
    ...
    function printDpSaleInvoice($dp_id) {$db=DbSingleton::getDb();$money=new toMoney;$slave=new slave;$invoice_summ=0;$list="";
    ...
    function printSaleInvoice($invoice_id){$db=DbSingleton::getDb();$slave=new slave;$money=new toMoney;
...
//            if($eur_to_uah!=$eur_to_uah_new){$eur_to_uah=$eur_to_uah_new;}
    ...
    function countReportOverdrafts() { $db=DbSingleton::getDb();session_start();$media_role_id=$_SESSION["media_role_id"];
        $date_cur=date("Y-m-d"); $where=" and sv.data_pay<'$date_cur'"; $tpoint=$this->getTpointbyUser();
        $where_tpoint=" and sv.tpoint_id=$tpoint "; if ($media_role_id=="1") $where_tpoint="";
        $r=$db->query("select sv.* from J_SALE_INVOICE sv where sv.status=1 and sv.summ_debit>0 $where $where_tpoint;"); $count=$db->num_rows($r);
        if ($count>0) $back="background: red;"; else $back="";
        return array($count,$back);
    }

    function getTpointbyUser() { $db=DbSingleton::getDb();$tpoint=1;session_start();$user_id=$_SESSION["media_user_id"];
        $r=$db->query("select tpoint_id from media_users where id='$user_id';"); $n=$db->num_rows($r);
        if ($n>0) $tpoint=$db->result($r,0,"tpoint_id");
        return $tpoint;
    }

    function countOrdersSite() { $db=DbSingleton::getDb();session_start();$user_id=$_SESSION["media_user_id"];$ses_tpoint_id=$_SESSION["media_tpoint_id"];
    ...
    function createDpFromOrder($order_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка!";$dp_id=0;
    ...
    function loadDpSiteOrder($dp_id){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;
    ...
    function getDistinctOrdersItems($dp_id){$db=DbSingleton::getDb();$slave=new slave;$amount="";$dp_id=$slave->qq($dp_id);
        if ($dp_id>0){
    ...
    function showSupplToLocalChangeForm($art_id,$brand_id,$dp_id,$dp_str_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;
    ...
    function saveDpSupplToLocalChangeForm($dp_id,$dp_str_id,$art_id,$amount,$price,$storage_id,$cell_id){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="Помилка!";
