...
function show_dp_list(){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;$income=new income;  session_start(); $ses_tpoint_id=$_SESSION["media_tpoint_id"]; $where=" and j.tpoint_id='$ses_tpoint_id' and j.status_dp!=0"; 
	...
	where j.status=1 $where order by j.id desc limit 0,500;");$n=$db->num_rows($r);$list="";
	for ($i=1;$i<=$n;$i++){
...
function showDpClientList($sel_id){$db=DbSingleton::getDb();$slave=new slave;
	...
	for ($i=1;$i<=$n;$i++){
		...
		$tpoint_id=$db->result($r,$i-1,"tpoint_id");
		$tpoint_name=$db->result($r,$i-1,"tpoint_name");
		$cur="";$fn=" onClick='setDpClient(\"$id\", \"$name\",\"$tpoint_id\",\"tpoint_name\")'";
...
function getDpStrStatus($dp_id){$db=DbSingleton::getDb();$status_name="";
	...
	for ($i=1;$i<=$n;$i++){
		$status_name.=$db->result($r,$i-1,"status_dps_name");
...
function showDpStrList($dp_id,$status_dp,$client_id,$cash_id,$usd_to_uah,$euro_to_uah){$db=DbSingleton::getDb();$slave=new slave;$cat=new catalogue;$manual=new manual;$gmanual=new gmanual;$income=new income;$list="";if ($status_dp==""){$status_dp=79;} if ($client_id==""){$client_id=$this->getDpClient($dp_id);}if ($cash_id==""){$cash_id=$this->getDpCashId($dp_id);} if ($usd_to_uah=="" || $euro_to_uah==""){list($usd_to_uah,$euro_to_uah)=$this->getKoursData('');} $tpoint_id=$this->getDpTpoint($dp_id);
	...
	for ($i=1;$i<=$kl_rw;$i++){
		...
		$status_dps_name=$db->result($r,$i-1,"status_dps_name");
	...
	if ($dp_id>0 && $str_id>0 && $discount_new!=""){
		...
		if ($n==1){
			...
			if ($discount!=$discount_new){
				...
				$price_end=round($price-$price*$discount/100,2); //print "price_end=$price_end\n";
...
function getCarrierSelectList($sel_id){$db=DbSingleton::getDb();$list="<option value=0>������ � ������</option>";$cells_show=1;
...
function saveDpStorage($dp_id,$storage_id,$storage_cells_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$media_user_id=$_SESSION["media_user_id"];$media_user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function saveDpComment($dp_id,$comment){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ���������� �����!";
...
function dpCDNDropFile($dp_id,$file_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];$answer=0;$err="������� ��������� �����!";
...
function makeDpJmovingStorselPreorder($dp_id,$local){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$jmoving=new jmoving;$answer=0;$err="������� �������!";
...
function loadDpJmoving($dp_id){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;
	...
	for ($i=1;$i<=$n;$i++){
		...
		$list.="<tr style='cursor:pointer' onClick='$function'>
				...
				<td>$user_name</td>
				<td align='center'><button class='btn btn-xs btn-primary' onClick='viewDpJmoving(\"$dp_id\",\"$id\",\"$status_jmoving\");'><i class='fa fa-eye'></i></button></td>
...
function showDpStorselForSaleInvoice($dp_id){$db=DbSingleton::getDb();$cat=new catalogue;$slave=new slave;$manual=new manual;session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"]; $storsel=new storsel; $gmanual=new gmanual; 
	...
	$r=$db->query("select sel.*, s.name as storage_name, t.name as tpoint_name from J_SELECT sel
					left outer join T_POINT t on t.id=sel.tpoint_id
					left outer join STORAGE s on s.id=sel.storage_id
					where sel.status=1 and sel.parrent_doc_type_id='2' and sel.parrent_doc_id='$dp_id' and status_select='85' 
					and sel.id not in ($sel_use) order by sel.status_select asc, sel.data_create desc, sel.id desc;");$n=$db->num_rows($r);
	for ($i=1;$i<=$n;$i++){
		$id=$db->result($r,$i-1,"id");
		$data_create=$db->result($r,$i-1,"data_create");
		$tpoint_id=$db->result($r,$i-1,"tpoint_id");
...
function checkClientSaleInvoiceDataPayLimit($client_id){
...
function sendDpStorselToSaleInvoice($dp_id,$kol_storsel,$ar_storsel){$db=DbSingleton::getDb();$dbt=DbSingleton::getTokoDb();$slave=new slave;$answer=0;$err="������� �������!";$sale_invoice_nom=0; session_start();$user_id=$_SESSION["media_user_id"];
	$dp_id=$slave->qq($dp_id);
	if ($dp_id>0){
		$kol_storsel=$slave->qq($kol_storsel);
		if ($kol_storsel>0){ 
			list($client_saldo,$client_credit_limit,$sale_storsell_summ)=$this->checkClientCreditLimitBeforeSaleInvoice($dp_id,$kol_storsel,$ar_storsel);
			...
			if ($client_saldo==0 || (($sale_storsell_summ+abs($client_saldo)<$client_credit_limit))){ //////CHECK NOW
				...
				if ($n==1){
					...
					for ($i2=1;$i2<=$n2;$i2++){
						...
						$db->query("insert into J_SALE_INVOICE_STR (`invoice_id`,`art_id`,`article_nr_displ`,`brand_id`,`amount`,`price`,`price_end`,`discount`,`summ`) values ('$invoice_id','$art_id2','$article_nr_displ2','$brand_id2','$amount2','$price2','$price_end2','$discount2','$summ2');");

						$this->writeOffPartitions($invoice_id,$art_id2,$article_nr_displ2,$brand_id2,$amount2,$price_end2);
				...
				$answer=0;$err="� �볺��� � ������������� � ���: $client_saldo(".$this->getCashAbr($cash_id).")";
			}
		}
	}
...
function writeOffPartitions($invoice_id,$art_id,$article_nr_displ,$brand_id,$amount_invoice,$price_invoice){$db=DbSingleton::getDb();$cat=new catalogue;
	...
	for ($i=1;$i<=$n;$i++){
		...
		if ($amount_invoice<=$rest){
			...
			$db->query("insert into J_SALE_INVOICE_PARTITION_STR (`partition_id`,`invoice_id`,`art_id`,`article_nr_displ`,`brand_id`,`partition_amount`,`oper_price_partition`,`price_partition`,`price_buh_uah`,`price_man_uah`,`price_invoice`) values ('$id','$invoice_id','$art_id','$article_nr_displ','$brand_id','$amount_invoice','$oper_price','$price','$price_buh_uah','$price_man_uah','$price_invoice');");
			$i=$n+1;
		}
		if ($amount_invoice>$rest){
			$new_rest=0;
			...
			$db->query("update T2_ARTICLES_PARTITIONS set rest='$new_rest' where id='$id' limit 1;");
			$db->query("insert into J_SALE_INVOICE_PARTITION_STR (`partition_id`,`invoice_id`,`art_id`,`article_nr_displ`,`brand_id`,`partition_amount`,`oper_price_partition`,`price_partition`,`price_buh_uah`,`price_man_uah`,`price_invoice`) values ('$id','$invoice_id','$art_id','$article_nr_displ','$brand_id','$rest','$oper_price','$price','$price_buh_uah','$price_man_uah','$price_invoice');");
		}
	}
	return;
}
...
function loadDpMoneyPay($dp_id){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;
	...
	$r=$db->query("select pay.*, pt.mcaption as pay_type_caption, pb.name as paybox_name, c.abr, pst.summ_pay, pst.doc_cash_id, cd.abr as d_abr,pst.pay_cash_kours from J_PAY pay
