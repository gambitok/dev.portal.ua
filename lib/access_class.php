...
class access {
    ...
    public function check_user_access($file): array
    {
        ...
        if ($media_user_id !== 1){
            ...
            WHERE rs.user_id='$media_user_id' and mf.file='$file' LIMIT 1;");
            $n = (int)$db->num_rows($r);

            if ($n === 1) {
                $access = "1";
                $acc_lvl = $db->result($r, 0, "lvl");
            }
        }
    ...
    public function checkTrustedIp($media_user_id): bool
    {
        $db = DbSingleton::getDb();
	  	$ip_address = $_SERVER['REMOTE_ADDR'];

	  	$r = $db->query("SELECT * FROM `trusted_ip` WHERE `ip`='$ip_address' and `status`=1 LIMIT 1;");
	  	$n = $db->num_rows($r);
		$n > 0 ? $result = true : $result = false;
        (int)$media_user_id !== 1 ?: $result = true;

		return $result;
	}
	
	public function checkAccessTime($media_user_id): bool
    {
	    $db = DbSingleton::getDb();
		$result = false;
		$cur_time = date("h:i:s");
		...
		if ($n > 0) {
			$access         = (int)$db->result($r,0,"access");
			$access_time    = (int)$db->result($r,0,"access_time");
	...
	public function getMediaUserRole()
    {
	    $db = DbSingleton::getDb();
