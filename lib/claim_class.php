...
function saveClaimCard($claim_id,$art,$brand,$count,$date,$supplier,$manufacturer,$client,$client_invoice,$comment,$receipt_doc,$kilometers,$state,$text_ukr,$text_eng) {
	$r=$db->query("update clients_claim SET art='$art', brand='$brand', count=$count, date=$date, supplier='$supplier', manufacturer='$manufacturer', client='$client', client_invoice='$client_invoice', comment='$comment', receipt_doc='$receipt_doc', kilometers=$kilometers, state=$state, text_ukr='$text_ukr', text_eng='$text_eng' 
	where id='$claim_id';");
	return true;
}
