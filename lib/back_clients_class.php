...
class back_clients {
    ...
    function get_df_doc_nom_new($sale_invoice_id){ $db=DbSingleton::getDb();$doc_nom=0;$doc_type_id=0;
    ...
    function getBackClientsName($back_id){$db=DbSingleton::getDb();$prefix="";$doc_nom=0;
    ...
    function newBackClientsCard(){$db=DbSingleton::getDb();session_start();$user_id=$_SESSION["media_user_id"];$ses_tpoint_id=$_SESSION["media_tpoint_id"];$storage_id=$this->getTpointLocalStorage($ses_tpoint_id);
    ...
    function show_back_clients_list(){$db=DbSingleton::getDb();$gmanual=new gmanual;session_start();$ses_tpoint_id=$_SESSION["media_tpoint_id"];
    ...
    function show_back_clients_list_filter($data_start,$data_end){$db=DbSingleton::getDb();$gmanual=new gmanual;session_start();$ses_tpoint_id=$_SESSION["media_tpoint_id"];
        $where=" and j.tpoint_id='$ses_tpoint_id' and j.status_back!=0";
    ...
    function showBackClientsCard($back_id){$db=DbSingleton::getDb();session_start();$user_id=$_SESSION["media_user_id"];$user_name=$_SESSION["user_name"];
        $prefix="";$doc_nom=0;
        ...
        if ($n==1){ $this->updateBackClientsSumm($back_id);
            ...
            if ($user_id==$user_use || $user_use==0){
                ...
                $form=str_replace("{eur_to_uah}",$eur_to_uah,$form);$comment="";//???
    ...
    function unlockBackClientsCard($back_id){session_start();$user_id=$_SESSION["media_user_id"];$answer=0;
    ...
    function closeBackClientsCard($back_id){session_start();$user_id=$_SESSION["media_user_id"];
    ...
    function clearBackClientsStr($back_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="";
    ...
    function setBackClientsClient($back_id,$client_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";$client_name="";
    ...
    function getSaleInvoiceCashData($id){$db=DbSingleton::getDb();$cash_id=1;$usd_to_uah=$eur_to_uah=1;
    ...
    function showBackClientsSaleInvoiceList($client_id,$si_id){$db=DbSingleton::getDb();$list="";
    ...
    function saveBackClientsCard($back_id,$data_pay,$cash_id,$back_clients_summ,$doc_type_id,$tpoint_id,$client_id,$client_conto_id,$delivery_type_id,$carrier_id,$delivery_address){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function setArticleToBackClients($back_id,$si_id,$sis_id,$art_id,$article_nr_displ,$amount_back){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function changeBackClientsCash($back_id,$cash_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function updateBackClientsPriceCash($back_id){$db=DbSingleton::getDb();$answer=0;
        $r=$db->query("select * from J_BACK_CLIENTS where id='$back_id' limit 0,1;");$n=$db->num_rows($r);
        if ($n==1){
            $oper_status=$db->result($r,0,"oper_status");
            if ($oper_status==30) {
    ...
    function getBackClientsCashId($back_id){$db=DbSingleton::getDb();$cash_id=2;
    ...
    function getBackClientsClient($back_id){$db=DbSingleton::getDb();$client_conto_id=0;
    ...
    function getArticleStorageAmountBackClients($art_id,$back_id,$storage_id){$db=DbSingleton::getDb();$amount=0;
        $r=$db->query("select amount from J_BACK_CLIENTS_STR 
        where back_id='$back_id' and status_back='93' and storage_id_from='$storage_id' and art_id='$art_id' limit 0,1");$n=$db->num_rows($r);
        if ($n==1){ $amount=$db->result($r,0,"amount"); }
        return $amount;
    }

    function getArticleSupplStorageAmountBackClients($art_id,$back_id,$suppl_id,$suppl_storage_id){$db=DbSingleton::getDb();$amount=0;
        $r=$db->query("select amount from J_BACK_CLIENTS_STR 
        where back_id='$back_id' and status_back='93' and suppl_id='$suppl_id' and suppl_storage_id='$suppl_storage_id' and art_id='$art_id' limit 0,1");$n=$db->num_rows($r);
        if ($n==1){ $amount=$db->result($r,0,"amount"); }
        return $amount;
    }
    ...
    function loadback_clientsStorage($back_id){$db=DbSingleton::getDb();
        ...
        $r=$db->query("select storage_id,storage_cells_id from J_INCOME where `id`='$back_id' limit 0,1;");
        $storage_id=$db->result($r,0,"storage_id");
        $storage_cells_id=$db->result($r,0,"storage_cells_id");
        $form=str_replace("{back_id}",$back_id,$form);
    ...
    function saveBackClientsStorage($back_id,$storage_id,$storage_cells_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка збереження даних!";
    ...
    function saveBackClientsComment($back_id,$comment){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$answer=0;$err="Помилка збереження даних!";
    ...
    function dropBackClientsComment($back_id,$comment_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка видалення запису!";
    ...
    function BackClientsCDNDropFile($back_id,$file_id){$db=DbSingleton::getDb();$slave=new slave;$answer=0;$err="Помилка видалення файлу!";
    ...
    function findTaxBySaleInvoiceId($sale_invoice_id){$db=DbSingleton::getDb(); //добавив умову на tax_type_id=160, т.к. має брати документ-основу
        $r=$db->query("select id, seller_id, client_id, tpoint_id from J_TAX_INVOICE where sale_invoice_id='$sale_invoice_id' and tax_type_id='160' order by id desc limit 1;");
    ...
    function createBackClientsTax($back_id){$db=DbSingleton::getDb();$slave=new slave;session_start();$user_id=$_SESSION["media_user_id"];$cat=new catalogue;
        ...
        if ($n==1){
            ...
            if ($sale_doc_type_id==61){
                ...
                $r1=$db->query("select max(id) as mid from J_TAX_INVOICE;");$tax_id=$db->result($r1,0,"mid")+1;
                $r1=$db->query("select max(doc_nom) as doc_nom from J_TAX_INVOICE where seller_id='$seller_id' and tax_type_id='$tax_type_id';");
                ...
                for ($i=1;$i<=$n;$i++){
                    ...
                    $db->query("insert into J_TAX_INVOICE_STR (`tax_id`,`zed`,`art_id`,`goods_name`,`amount`,`price`,`summ`,`tax_str_id`,`tax_str_nom`,`sale_invoice_str_id`) values ('$tax_id','$zed','$art_id','$article_name','$amount','$price','$summ','$sale_invoice_str_id','$tax_nom','0');");
                }
                $answer=1;$err="";
            }
        }
    ...
    function printBackClientsN1($back_id){$db=DbSingleton::getDb();$slave=new slave;$gmanual=new gmanual;$money=new toMoney;$cat=new catalogue;$form=$form_htm=$list="";
    ...
    function loadBackClientsPartition($back_id) {$db=DbSingleton::getDb(); $cat=new catalogue; $income=new income; $list=""; $doc_name=0; $prev_doc_id=0;//???
