...
if ($_REQUEST["w"]=="saveClientConditions"){ list($answer,$err)=$cl->saveClientConditions($_REQUEST["client_id"],$_REQUEST["cash_id"],$_REQUEST["country_cash_id"],$_REQUEST["price_lvl"],$_REQUEST["margin_price_lvl"],$_REQUEST["price_suppl_lvl"],$_REQUEST["margin_price_suppl_lvl"],$_REQUEST["tpoint_id"],$_REQUEST["client_vat"],$_REQUEST["payment_delay"],$_REQUEST["credit_limit"],$_REQUEST["credit_cash_id"],$_REQUEST["credit_return"],$_REQUEST["doc_type_id"]); $GLOBALS['_RESULT'] = array("answer"=>$answer,"error"=>$err);}
