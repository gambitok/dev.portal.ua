...
if ($media_user_id == 31) {
    ...
    $content = str_replace("{ModuleMenu}", $form, $content);
    $content = str_replace("{nav_top_menu}", "", $content);
    $content = str_replace("{user_visible}", "style='display:none;'", $content);
}
...
$file = $_REQUEST["file"];
$alerts = new alerts;
list($alerts_list, $alerts_kol) = $alerts->list_alerts_navigation();
...
$config = new config;
list($title, $title_short, $keywords, $descr, $site_address) = $config->get_meta_head();
